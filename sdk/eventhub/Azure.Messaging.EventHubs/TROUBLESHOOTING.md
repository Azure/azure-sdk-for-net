...
This warning indicates that the processor was configured such that the interval that controls when load balancing runs is close to the length of time that a partition is considered owned.  When these intervals are too close together, ownership may expire before it is renewed during load balancing which will cause partitions to migrate. 
