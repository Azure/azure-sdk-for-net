...
namespace Azure.Messaging.EventHubs.Processor.Diagnostics
{
    ...
    internal class EventProcessorClientEventSource : AzureEventSource
    {
        ...
        /// <param name="globalOffset">The global offset associated with the checkpoint being written.</param>
        ///
        [Event(23, Level = EventLevel.Verbose, Message = "Starting to perform a checkpoint update for partition '{0}' by processor instance with identifier '{1}' for Event Hub: {2} and Consumer Group: {3} with Sequence Number: {4} Offset: {5} Global Offset: {6}.")]
                                                  ...
                                                  string globalOffset)
        {
            if (IsEnabled())
            {
                WriteEvent(23, partitionId ?? string.Empty, identifier ?? string.Empty, eventHubName ?? string.Empty, consumerGroup ?? string.Empty, sequenceNumber ?? string.Empty, offset ?? string.Empty, globalOffset ?? string.Empty);
            }
        }
        ...
        /// <param name="globalOffset">The global offset associated with the checkpoint being written.</param>
        ///
        [Event(24, Level = EventLevel.Verbose, Message = "Completed performing a checkpoint update for partition '{0}' by processor instance with identifier '{1}' for Event Hub: {2} and Consumer Group: {3} with Sequence Number: {4}, Offset: {5} Global Offset: {6}.")]
                                                     ...
                                                     string globalOffset)
        {
            if (IsEnabled())
            {
                WriteEvent(24, partitionId ?? string.Empty, identifier ?? string.Empty, eventHubName ?? string.Empty, consumerGroup ?? string.Empty, sequenceNumber ?? string.Empty, offset ?? string.Empty, globalOffset ?? string.Empty);
            }
        }
        ...
        /// <param name="globalOffset">The global offset associated with the checkpoint being written.</param>
        ///
        [Event(25, Level = EventLevel.Error, Message = "An exception occurred while attempting to perform a checkpoint update for partition '{0}' by processor instance with identifier '{1}' for Event Hub: {2} and Consumer Group: {3}  Error Message: '{4}' Sequence Number: {5} Offset: {6} Global Offset: {7}.")]
                                                  ...
                                                  string globalOffset)
        {
            if (IsEnabled())
            {
                WriteEvent(25, partitionId ?? string.Empty, identifier ?? string.Empty, eventHubName ?? string.Empty, consumerGroup ?? string.Empty, errorMessage ?? string.Empty, globalOffset ?? string.Empty);
            }
        }
