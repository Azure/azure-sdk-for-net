...
namespace Microsoft.Azure.WebJobs.Host.EndToEndTests
{
    ...
    public class EventHubEndToEndTests : WebJobsEventHubTestBase
    {
        ...
        public async Task EventHub_SingleDispatch_Dispose()
        {
            ...
            Assert.That(_eventWait1.WaitOne(Timeout), Is.True);
            jobHost.Dispose();
            Assert.That(_eventWait2.WaitOne(Timeout), Is.True);
        }
        ...
        public async Task EventHub_InitialOffsetFromEnd()
        {
            ...
            using (jobHost)
            {
                ...
                Assert.That(result, Is.False, "An event was received while none were expected.");
        ...
        private static void AssertSingleDispatchLogs(IHost host)
        {
            ...
            Assert.That(1, Is.EqualTo(logMessages.Where(x => !string.IsNullOrEmpty(x.FormattedMessage)
                && x.FormattedMessage.Contains("Trigger Details:")
                && x.FormattedMessage.Contains("Offset:")).Count()));

            Assert.That(logMessages.Where(x => !string.IsNullOrEmpty(x.FormattedMessage)
                && x.FormattedMessage.Contains("OpenAsync")).Any(), Is.True);

            Assert.That(logMessages.Where(x => !string.IsNullOrEmpty(x.FormattedMessage)
                && x.FormattedMessage.Contains("CheckpointAsync")
                && x.FormattedMessage.Contains("lease")
                && x.FormattedMessage.Contains("offset")
                && x.FormattedMessage.Contains("sequenceNumber")).Any(), Is.True);

            Assert.That(logMessages.Where(x => !string.IsNullOrEmpty(x.FormattedMessage)
                && x.FormattedMessage.Contains("Sending events to EventHub")).Any(), Is.True);
        ...
        private static void AssertMultipleDispatchLogsMinBatch(IHost host)
        {
            ...
            Assert.That(logMessages.Where(x => !string.IsNullOrEmpty(x.FormattedMessage)
                && x.FormattedMessage.Contains("Trigger Details:")
                && x.FormattedMessage.Contains("Offset:")).Any(), Is.True);
        ...
        {
            Assert.That(logMessages.Any(x => x.Category.StartsWith("Azure.")), Is.True);
        }
        ...
        public class EventHubTestCollectorDispatch
        {
            ...
            {
                Assert.That("Event 1", Is.EqualTo(eventData.EventBody.ToString()));

                if (!string.IsNullOrEmpty(s_partitionKey))
                {
                    Assert.That(s_partitionKey, Is.EqualTo(eventData.PartitionKey));
                }
        ...
        public class EventHubTestSingleDispatchWithConsumerGroupJobs
        {
            ...
            {
                Assert.That(nameof(EventHubTestSingleDispatchWithConsumerGroupJobs), Is.EqualTo(evt));
        ...
        public class EventHubTestSingleDispatchJobsBinaryData
        {
            ...
            public static void ProcessSingleEvent([EventHubTrigger(TestHubName, Connection = TestHubName)] BinaryData evt,
                       ...
                       IDictionary<string, object> systemProperties)
            {
                Assert.That(evt.ToString(), Is.EqualTo("data"));
                _eventWait1.Set();
            }
        }
        ...
        public class EventHubTestBindToPocoJobs
        {
            ...
            {
                Assert.That("data", Is.EqualTo(input.Value));
                Assert.That("foo", Is.EqualTo(input.Name));
        ...
        public class EventHubTestMultipleDispatchJobs
        {
            ...
            {
                Assert.That(partitionKeyArray.Length, Is.EqualTo(events.Length));
                Assert.That(enqueuedTimeUtcArray.Length, Is.EqualTo(events.Length));
                Assert.That(propertiesArray.Length, Is.EqualTo(events.Length));
                Assert.That(systemPropertiesArray.Length, Is.EqualTo(events.Length));
        ...
        public class EventHubTestMultipleDispatchMinBatchSizeJobs
        {
                ...
                IDictionary<string, object>[] systemPropertiesArray, PartitionContext partitionContext, TriggerPartitionContext triggerPartitionContext)
            {
                ...
                Assert.That(events.Length, Is.EqualTo(s_eventCount));
        ...
        public class EventHubPartitionKeyTestJobs
        {
            ...
            public static void ProcessMultiplePartitionEvents([EventHubTrigger(TestHubName, Connection = TestHubName)] EventData[] events)
            {
                foreach (EventData eventData in events)
                {
                    ...
                    Assert.That(eventData.PartitionKey.StartsWith(PartitionKeyPrefix), Is.True);
                }
