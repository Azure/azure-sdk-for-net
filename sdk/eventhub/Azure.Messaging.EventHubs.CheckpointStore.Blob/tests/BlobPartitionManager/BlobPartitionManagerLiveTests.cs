    ...
    public class BlobPartitionManagerLiveTests
    {
        ...
        public async Task BlobPartitionManagerCanListOwnership()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                Assert.That(async () => await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName", "consumerGroup"), Throws.Nothing);
            }
        }
        ...
        public async Task BlobPartitionManagerCanUpdateCheckpoint()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var checkpoint = new MockCheckpoint("eventHubsHostName", "eventHubName", "consumerGroup", "ownerIdentifier", "partitionId", 10, 20);
        ...
        public async Task FirstOwnershipClaimSucceeds()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var storedOwnershipList = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName", "consumerGroup");
        ...
        public async Task ClaimOwnershipAsyncCanClaimMultipleOwnership()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                foreach (var ownership in storedOwnershipList.OrderBy(ownership => ownership.PartitionId))
                {
        ...
        public async Task ClaimOwnershipAsyncReturnsOnlyTheSuccessfullyClaimedOwnership()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var claimedOwnershipList = await partitionManager.ClaimOwnershipAsync(ownershipList);
                var expectedOwnership = ownershipList.Where(ownership => ownership.Offset % 2 == 1);
                ...
                foreach (var ownership in claimedOwnershipList.OrderBy(ownership => ownership.PartitionId))
                {
        ...
        public async Task OwnershipClaimDoesNotInterfereWithOtherConsumerGroups()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var storedOwnershipList = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName", "consumerGroup1");
        ...
        public async Task OwnershipClaimDoesNotInterfereWithOtherEventHubs()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var storedOwnershipList = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName1", "consumerGroup");
        ...
        public async Task OwnershipClaimDoesNotInterfereWithOtherNamespaces()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var storedOwnershipList = await partitionManager.ListOwnershipAsync("eventHubsHostName1", "eventHubName", "consumerGroup");
        ...
        public async Task CheckpointUpdateUpdatesOwnershipInformation()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                var partitionManager = new BlobPartitionManager(containerClient);
                var originalOwnership = new MockPartitionOwnership
                    ("eventHubsHostName", "eventHubName", "consumerGroup", "ownerIdentifier", "partitionId", offset: 1, sequenceNumber: 2);
                ...
                var storedOwnership = storedOwnershipList.Single();
        ...
        public async Task CheckpointUpdateDoesNotInterfereWithOtherConsumerGroups()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                await partitionManager.ClaimOwnershipAsync(new List<PartitionOwnership>()
                {
                    new MockPartitionOwnership("eventHubsHostName","eventHubName", "consumerGroup1", "ownerIdentifier", "partitionId", offset: 1),
                    new MockPartitionOwnership("eventHubsHostName","eventHubName", "consumerGroup2", "ownerIdentifier", "partitionId", offset: 1)
                });

                await partitionManager.UpdateCheckpointAsync(new MockCheckpoint
                    ("eventHubsHostName", "eventHubName", "consumerGroup1", "ownerIdentifier", "partitionId", 10, 20));

                var storedOwnershipList1 = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName", "consumerGroup1");
                var storedOwnershipList2 = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName", "consumerGroup2");
        ...
        public async Task CheckpointUpdateDoesNotInterfereWithOtherEventHubs()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                await partitionManager.ClaimOwnershipAsync(new List<PartitionOwnership>()
                {
                    new MockPartitionOwnership("eventHubsHostName","eventHubName1", "consumerGroup", "ownerIdentifier", "partitionId", offset: 1),
                    new MockPartitionOwnership("eventHubsHostName","eventHubName2", "consumerGroup", "ownerIdentifier", "partitionId", offset: 1)
                });

                await partitionManager.UpdateCheckpointAsync(new MockCheckpoint
                    ("eventHubsHostName", "eventHubName1", "consumerGroup", "ownerIdentifier", "partitionId", 10, 20));

                var storedOwnershipList1 = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName1", "consumerGroup");
                var storedOwnershipList2 = await partitionManager.ListOwnershipAsync("eventHubsHostName", "eventHubName2", "consumerGroup");
        ...
        public async Task CheckpointUpdateDoesNotInterfereWithOtherNamespaces()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                await partitionManager.ClaimOwnershipAsync(new List<PartitionOwnership>()
                {
                    new MockPartitionOwnership("eventHubsHostName1","eventHubName", "consumerGroup", "ownerIdentifier", "partitionId", offset: 1),
                    new MockPartitionOwnership("eventHubsHostName2","eventHubName", "consumerGroup", "ownerIdentifier", "partitionId", offset: 1)
                });

                await partitionManager.UpdateCheckpointAsync(new MockCheckpoint
                    ("eventHubsHostName1", "eventHubName", "consumerGroup", "ownerIdentifier", "partitionId", 10, 20));

                var storedOwnershipList1 = await partitionManager.ListOwnershipAsync("eventHubsHostName1", "eventHubName", "consumerGroup");
                var storedOwnershipList2 = await partitionManager.ListOwnershipAsync("eventHubsHostName2", "eventHubName", "consumerGroup");
        ...
        public async Task CheckpointUpdateDoesNotInterfereWithOtherPartitions()
        {
            await using (var storageScope = await StorageScope.CreateAsync())
            {
                ...
                await partitionManager.ClaimOwnershipAsync(new List<PartitionOwnership>()
                {
                    new MockPartitionOwnership("eventHubsHostName","eventHubName", "consumerGroup", "ownerIdentifier", "partitionId1", offset: 1),
                    new MockPartitionOwnership("eventHubsHostName","eventHubName", "consumerGroup", "ownerIdentifier", "partitionId2", offset: 1)
                });
        ...
        private class MockCheckpoint : Checkpoint
        {
            ...
            /// <param name="sequenceNumber">The sequence number of the last <see cref="EventData" /> received by the associated partition processor.</param>
            ///
            public MockCheckpoint(string eventHubsHostName,
                                  string eventHubName,
                                  string consumerGroup,
                                  string ownerIdentifier,
                                  string partitionId,
                                  long offset,
                                  long sequenceNumber) : base(eventHubsHostName, eventHubName, consumerGroup, ownerIdentifier, partitionId, offset, sequenceNumber)
            {
            }
        }
    }
