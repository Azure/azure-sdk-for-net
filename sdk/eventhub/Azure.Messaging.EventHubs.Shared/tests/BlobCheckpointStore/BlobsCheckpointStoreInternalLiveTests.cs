    ...
    public class BlobsCheckpointStoreInternalLiveTests
    {
        ...
        public async Task CheckpointUpdateCreatesTheBlobOnFirstCall()
        {
            await using (StorageScope storageScope = await StorageScope.CreateAsync())
            {
                ...
                await checkpointStore.UpdateCheckpointAsync(checkpoint.FullyQualifiedNamespace, checkpoint.EventHubName, checkpoint.ConsumerGroup, checkpoint.PartitionId, checkpoint.ClientIdentifier, new CheckpointPosition(mockEvent.GlobalOffset, mockEvent.SequenceNumber), default);
                ...
                Assert.That(storedCheckpoint.StartingPosition, Is.EqualTo(EventPosition.FromGlobalOffset(mockEvent.GlobalOffset, false)));
        ...
        public async Task CheckpointUpdatesAnExistingBlob()
        {
            await using (StorageScope storageScope = await StorageScope.CreateAsync())
            {
                ...
                await checkpointStore.UpdateCheckpointAsync(checkpoint.FullyQualifiedNamespace, checkpoint.EventHubName, checkpoint.ConsumerGroup, checkpoint.PartitionId, checkpoint.ClientIdentifier, new CheckpointPosition(mockEvent.GlobalOffset), default);
