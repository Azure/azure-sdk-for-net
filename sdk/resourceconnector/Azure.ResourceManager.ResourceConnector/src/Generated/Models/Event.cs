// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.ResourceConnector.Models
{
    /// <summary> Event contains information about customer driven, platform driven, or unplanned events that occurred on the Appliance. </summary>
    public partial class Event
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="Event"/>. </summary>
        internal Event()
        {
        }

        /// <summary> Initializes a new instance of <see cref="Event"/>. </summary>
        /// <param name="type"> The type of event is used to classify how the event was initiated. </param>
        /// <param name="code"> Code is used to break down the event further to identify why it occurred. </param>
        /// <param name="status"> Status is used to represent the outcome of the event. </param>
        /// <param name="message"> Message is intended to be actionable and should be used to inform the user of the event. </param>
        /// <param name="severity"> Severity is the classification of the event to relay the importance of the event. </param>
        /// <param name="timestamp"> Timestamp is the time the event occurred. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal Event(string @type, string code, string status, string message, string severity, DateTimeOffset? timestamp, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Type = @type;
            Code = code;
            Status = status;
            Message = message;
            Severity = severity;
            Timestamp = timestamp;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The type of event is used to classify how the event was initiated. </summary>
        public string Type { get; }

        /// <summary> Code is used to break down the event further to identify why it occurred. </summary>
        public string Code { get; }

        /// <summary> Status is used to represent the outcome of the event. </summary>
        public string Status { get; }

        /// <summary> Message is intended to be actionable and should be used to inform the user of the event. </summary>
        public string Message { get; }

        /// <summary> Severity is the classification of the event to relay the importance of the event. </summary>
        public string Severity { get; }

        /// <summary> Timestamp is the time the event occurred. </summary>
        public DateTimeOffset? Timestamp { get; }
    }
}
