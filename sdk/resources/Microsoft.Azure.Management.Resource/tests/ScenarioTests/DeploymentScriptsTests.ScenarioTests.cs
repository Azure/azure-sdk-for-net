...
public class LiveDeploymentScriptsTests : TestBase
{
    ...
    private const string ScriptContent = "param([string] $helloWorld) Write-Output $helloWorld $DeploymentScriptOutputs['output'] = $helloWorld";
    private const string ScriptArguments = "'Hello World'";

    [Fact]
    public void CanCrudSimpleDeploymentScriptThruSdk()
    {
        using (var context = MockContext.Start(this.GetType()))
        {
            ...
            Assert.Throws<CloudException>(() => client.DeploymentScripts.Get(ResourceGroupName, deploymentScript.Name));
