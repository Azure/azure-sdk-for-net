// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Text.Json;

namespace Azure.ResourceManager.Resources._Deployments.Models
{
    /// <summary> Deployment parameter for the template. </summary>
    public partial class DeploymentParameterValue
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="DeploymentParameterValue"/>. </summary>
        public DeploymentParameterValue()
        {
        }

        /// <summary> Initializes a new instance of <see cref="DeploymentParameterValue"/>. </summary>
        /// <param name="value"> Input value to the parameter . </param>
        /// <param name="reference"> Azure Key Vault parameter reference. </param>
        /// <param name="expression"> Input expression to the parameter. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal DeploymentParameterValue(BinaryData value, KeyVaultParameterReference reference, string expression, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Value = value;
            Reference = reference;
            Expression = expression;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary>
        /// Input value to the parameter .
        /// <para> To assign an object to this property use <see cref="BinaryData.FromObjectAsJson{T}(T, JsonSerializerOptions?)"/>. </para>
        /// <para> To assign an already formatted json string to this property use <see cref="BinaryData.FromString(string)"/>. </para>
        /// <para>
        /// Examples:
        /// <list type="bullet">
        /// <item>
        /// <term> BinaryData.FromObjectAsJson("foo"). </term>
        /// <description> Creates a payload of "foo". </description>
        /// </item>
        /// <item>
        /// <term> BinaryData.FromString("\"foo\""). </term>
        /// <description> Creates a payload of "foo". </description>
        /// </item>
        /// <item>
        /// <term> BinaryData.FromObjectAsJson(new { key = "value" }). </term>
        /// <description> Creates a payload of { "key": "value" }. </description>
        /// </item>
        /// <item>
        /// <term> BinaryData.FromString("{\"key\": \"value\"}"). </term>
        /// <description> Creates a payload of { "key": "value" }. </description>
        /// </item>
        /// </list>
        /// </para>
        /// </summary>
        public BinaryData Value { get; set; }

        /// <summary> Azure Key Vault parameter reference. </summary>
        public KeyVaultParameterReference Reference { get; set; }

        /// <summary> Input expression to the parameter. </summary>
        public string Expression { get; set; }
    }
}
