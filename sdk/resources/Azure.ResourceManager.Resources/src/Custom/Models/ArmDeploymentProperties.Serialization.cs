...
#if NET6_0_OR_GREATER
                // TODO: We also need to filter the parameters once our SDK's target framework is greater than or equal to NET6.0
				writer.WriteRawValue(Parameters);
#else
                var rootElement = JsonDocument.Parse(Parameters.ToString()).RootElement;
                if (rootElement.TryGetProperty("parameters", out var targetElement))
                {
                    JsonSerializer.Serialize(writer, targetElement);
                }
                else
                {
                    JsonSerializer.Serialize(writer, rootElement);
                }
#endif
            }
