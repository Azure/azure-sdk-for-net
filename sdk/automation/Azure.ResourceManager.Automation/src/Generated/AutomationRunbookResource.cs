...
namespace Azure.ResourceManager.Automation
{
    ...
    public partial class AutomationRunbookResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation> ReplaceContentRunbookDraftAsync(WaitUntil waitUntil, Stream runbookContent, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _runbookDraftRestClient.CreateReplaceContentRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, runbookContent);
                var response = await _runbookDraftRestClient.ReplaceContentAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new AutomationArmOperation(_runbookDraftClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionResponseAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        public virtual ArmOperation ReplaceContentRunbookDraft(WaitUntil waitUntil, Stream runbookContent, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _runbookDraftRestClient.CreateReplaceContentRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, runbookContent);
                var response = _runbookDraftRestClient.ReplaceContent(message, cancellationToken);
                var operation = new AutomationArmOperation(_runbookDraftClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletionResponse(cancellationToken);
                return operation;
