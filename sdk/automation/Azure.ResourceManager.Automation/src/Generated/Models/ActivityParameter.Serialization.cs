...
namespace Azure.ResourceManager.Automation.Models
{
    ...
    {
        internal static ActivityParameter DeserializeActivityParameter(JsonElement element)
        {
            ...
            Optional<IReadOnlyList<ActivityParameterValidationSet>> validationSet = default;
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("validationSet"))
                {
                    ...
                    List<ActivityParameterValidationSet> array = new List<ActivityParameterValidationSet>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(ActivityParameterValidationSet.DeserializeActivityParameterValidationSet(item));
                    }
                    validationSet = array;
                    continue;
                }
            }
            return new ActivityParameter(name.Value, type.Value, Optional.ToNullable(isMandatory), Optional.ToNullable(isDynamic), Optional.ToNullable(position), Optional.ToNullable(valueFromPipeline), Optional.ToNullable(valueFromPipelineByPropertyName), Optional.ToNullable(valueFromRemainingArguments), description.Value, Optional.ToList(validationSet));
        }
    }
}
