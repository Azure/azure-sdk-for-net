    ...
    internal partial class BlobTriggerExecutor
    {
        private static class Logger
        {
            ...
            private static readonly Action<ILogger<BlobListener>, string, string, string, string, BlobTriggerScanSource, Exception> _blobProcessingStarted =
                LoggerMessage.Define<string, string, string, string, BlobTriggerScanSource>(LogLevel.Debug, new EventId(104, nameof(BlobProcessingStarted)),
                    "Blob '{blobName}' is being processed for function '{functionName}'. PollId: '{pollId}'. ETag: '{eTag}'. Source: '{triggerSource}'.");
            ...
            public static void BlobProcessingStarted(ILogger<BlobListener> logger, string functionName, string blobName, string pollId, string eTag, BlobTriggerScanSource triggerSource) =>
                _blobProcessingStarted(logger, blobName, functionName, pollId, eTag, triggerSource, null);
