...
namespace Azure.Storage.Blobs.Tests.ManagedDisk
{
    ...
    public class ManagedDiskTests : BlobTestBase
    {
        ...
        public void Setup()
        {
            snapshot1SASUri = new Uri(Recording.GetVariable(nameof(snapshot1SASUri), ManagedDiskFixture.Instance.Snapshot1SASUri?.AbsoluteUri, v => ((StorageRecordedTestSanitizer) Sanitizer).SanitizeUri(v)));
            snapshot2SASUri = new Uri(Recording.GetVariable(nameof(snapshot2SASUri), ManagedDiskFixture.Instance.Snapshot2SASUri?.AbsoluteUri, v => ((StorageRecordedTestSanitizer) Sanitizer).SanitizeUri(v)));
