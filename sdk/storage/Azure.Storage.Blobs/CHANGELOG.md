...
### Bugs Fixed
- Fixed an issue where the `GenerateSasUri` and `GenerateUserDelegationSasUri` convenience methods on blob clients did not honor the `DelegatedUserObjectId`, `RequestHeaders`, and `RequestQueryParameters` properties set on the supplied `BlobSasBuilder`.
