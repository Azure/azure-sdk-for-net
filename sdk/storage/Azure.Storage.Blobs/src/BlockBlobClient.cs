            ...
            return new PartitionedUploader<BlobUploadOptions, BlobContentInfo>.Behaviors
            {
                ...
                UploadPartition = async (stream, offset, args, progressHandler, async, cancellationToken)
                    => await client.StageBlockInternal(
                        ...
                        async,
                        cancellationToken).ConfigureAwait(false),
                CommitPartitionedUpload = async (partitions, initalialPosition, args, async, cancellationToken)
                    => await client.CommitBlockListInternal(
                        partitions.Select(partition => StorageExtensions.GenerateBlockId(partition.Offset - initalialPosition)),
                        args.HttpHeaders,
                        args.Metadata,
                        args.Tags,
                        args.Conditions,
