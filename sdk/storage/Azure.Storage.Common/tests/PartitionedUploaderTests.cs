...
namespace Azure.Storage.Tests
{
    ...
    public class PartitionedUploaderTests
    {
        ...
        private readonly UploadTransferValidationOptions s_hashingOptions = new UploadTransferValidationOptions();
        ...
        private Mock<PartitionedUploader<object, object>.SingleUploadInternal> GetMockSingleUploadInternal(int expectedSize)
        {
            ...
            mock.Setup(del => del(It.IsNotNull<Stream>(), s_objectArgs, It.IsAny<IProgress<long>>(), s_hashingOptions, s_operationName, IsAsync, s_cancellation))
                .Returns<Stream, object, IProgress<long>, UploadTransferValidationOptions, string, bool, CancellationToken>((stream, obj, progress, s_hashingOptions, operation, async, cancellation) =>
                {
                    if (!stream.CanSeek)
                    {
        ...
        private Mock<PartitionedUploader<object, object>.UploadPartitionInternal> GetMockUploadPartitionInternal(int maxSize)
        {
            ...
            mock.Setup(del => del(It.IsNotNull<Stream>(), It.IsAny<long>(), s_objectArgs, It.IsAny<IProgress<long>>(), It.IsNotNull<UploadTransferValidationOptions>(), IsAsync, s_cancellation))
                .Returns<Stream, long, object, IProgress<long>, UploadTransferValidationOptions, bool, CancellationToken>((stream, offset, obj, progress, s_hashingOptions, async, cancellation) =>
                {
                    if (!stream.CanSeek)
                    {
                        throw new Exception("PartitionedUploader sent non-seekable stream to the REST client");
