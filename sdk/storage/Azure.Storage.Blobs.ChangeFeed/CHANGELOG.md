...
### Breaking Changes
- `BlobChangeFeedClient.GetChanges(start, end)` and `GetChangesAsync(start, end)` now treat `end` as **inclusive**. Events whose `EventTime` equals `end` are returned. Previously the upper bound was exclusive.
