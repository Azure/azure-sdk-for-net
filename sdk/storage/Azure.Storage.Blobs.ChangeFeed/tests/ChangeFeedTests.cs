...
namespace Azure.Storage.Blobs.ChangeFeed.Tests
{
    public class ChangeFeedTests : ChangeFeedTestBase
    {
        ...
        public async Task GetCursor()
        {
            ...
            if (IsAsync)
            {
                blobClient.Setup(r => r.DownloadStreamingAsync(default)).ReturnsAsync(downloadResponse);
            }
            else
            {
                blobClient.Setup(r => r.DownloadStreaming(default)).Returns(downloadResponse);
            }
            ...
            if (IsAsync)
            {
                blobClient.Verify(r => r.DownloadStreamingAsync(default));
            }
            else
            {
                blobClient.Verify(r => r.DownloadStreaming(default));
            }
