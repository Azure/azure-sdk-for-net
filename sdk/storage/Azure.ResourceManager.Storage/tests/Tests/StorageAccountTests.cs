...
namespace Azure.ResourceManager.Storage.Tests.Tests
{
    public class StorageAccountTests : StorageTestBase
    {
        ...
        public async Task sample()
        {
            ...
            ResourceGroupContainer groupContainer = DefaultSubscription.GetResourceGroups();
            ResourceGroup resourceGroup = (await groupContainer.CreateOrUpdateAsync(resourceGroupName, new ResourceGroupData(Location.EastUS2))).Value;

            //create a storage account
            string accountName = "myaccount";
            StorageAccountCreateParameters parameter = new StorageAccountCreateParameters(new Sku(SkuName.StandardGRS), Kind.StorageV2, Location.EastUS2);
            StorageAccountContainer accountContainer = resourceGroup.GetStorageAccounts();
            StorageAccount account = (await accountContainer.CreateOrUpdateAsync(accountName, parameter)).Value;

            //get the storage account
            account = await accountContainer.GetAsync(accountName);

            //update the account's tag
            StorageAccountUpdateParameters updateParameter = new StorageAccountUpdateParameters();
            updateParameter.Tags.Add("key", "value");
            account = await account.UpdateAsync(updateParameter);
            ...
            BlobService blobService = await account.GetBlobServices().GetAsync("default");
            BlobContainerContainer blobContainerContainer = blobService.GetBlobContainers();
            BlobContainer blobContainer = (await blobContainerContainer.CreateOrUpdateAsync(containerName, new BlobContainerData())).Value;

            //get the blob container
            blobContainer = await blobContainerContainer.GetAsync(containerName);

            //update the blob container's metadata
            blobContainer.Data.Metadata.Add("key", "value");
            blobContainer = await blobContainer.UpdateAsync(blobContainer.Data);
            ...
            FileService fileService = await account.GetFileServices().GetAsync("default");
            FileShareContainer fileShareContainer = fileService.GetFileShares();
            FileShare fileShare = (await fileShareContainer.CreateOrUpdateAsync(shareName, new FileShareData())).Value;

            //get the file share
            fileShare = await fileShareContainer.GetAsync(shareName);

            //update the file share's metadata
            fileShare.Data.Metadata.Add("key", "value");
            fileShare = await fileShare.UpdateAsync(fileShare.Data);
            ...
            QueueService queueService = await account.GetQueueServices().GetAsync("default");
            StorageQueueContainer queueContainer = queueService.GetStorageQueues();
            StorageQueue queue = (await queueContainer.CreateOrUpdateAsync(queueName, new StorageQueueData())).Value;

            //get the storage queue
            queue = await queueContainer.GetAsync(queueName);

            //update the storage queue's metadata
            queue.Data.Metadata.Add("key", "value");
            queue = await queue.UpdateAsync(queue.Data);
            ...
            TableService tableService = await account.GetTableServices().GetAsync("default");
            TableContainer tableContainer = tableService.GetTables();
            Table table = (await tableContainer.CreateOrUpdateAsync(tableName)).Value;

            //get the table
            table = await tableContainer.GetAsync(tableName);

            //delete the table
            await table.DeleteAsync();
            string subscriptionId = DefaultSubscription.Id;

            //get the blob container by id
            ArmClient armClient = new ArmClient(new DefaultAzureCredential());
            string containerId = $"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}" +
                $"/providers/Microsoft.Storage/storageAccounts/{accountName}/blobServices/default/containers/{containerName}";
            BlobContainer container = armClient.GetBlobContainer(containerId);
            //If we want to retrieve the objects data we need to call Get
            container = await container.GetAsync();

            //get the file share by id
            string fileShareId = $"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}" +
                $"/providers/Microsoft.Storage/storageAccounts/{accountName}/fileServices/default/shares/{shareName}";
            FileShare share = armClient.GetFileShare(fileShareId);
            share = await share.GetAsync();

            //get the management policy by id
            string managementPolicyId = $"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}" +
                $"/providers/Microsoft.Storage/storageAccounts/{accountName}/managementpolicies/default";
            ManagementPolicy managementPolicy = armClient.GetManagementPolicy(managementPolicyId);
            managementPolicy = await managementPolicy.GetAsync();

            //get the blob inventory policy by id
            string inventoryPolicyId = $"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}" +
                $"/providers/Microsoft.Storage/storageAccounts/{accountName}/inventoryPolicies/default";
            BlobInventoryPolicy inventoryPolicy = armClient.GetBlobInventoryPolicy(inventoryPolicyId);
            inventoryPolicy = await inventoryPolicy.GetAsync();
        }
    }
