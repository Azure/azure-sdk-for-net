...
namespace Azure.ResourceManager.Storage.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<DeletedShare>)this).Write(writer, ModelSerializationExtensions.WireOptions);
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(DeletedShareName), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("  deletedShareName: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsDefined(DeletedShareName))
                {
                    builder.Append("  deletedShareName: ");
                    if (DeletedShareName.Contains(Environment.NewLine))
                    {
                        builder.AppendLine("'''");
                        builder.AppendLine($"{DeletedShareName}'''");
                    }
                    else
                    {
                        builder.AppendLine($"'{DeletedShareName}'");
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(DeletedShareVersion), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("  deletedShareVersion: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsDefined(DeletedShareVersion))
                {
                    builder.Append("  deletedShareVersion: ");
                    if (DeletedShareVersion.Contains(Environment.NewLine))
                    {
                        builder.AppendLine("'''");
                        builder.AppendLine($"{DeletedShareVersion}'''");
                    }
                    else
                    {
                        builder.AppendLine($"'{DeletedShareVersion}'");
