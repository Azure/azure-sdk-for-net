...
namespace Azure.Storage.Blobs.Experimental
{
    ...
    public class BlobTransferManager
    {
        ...
        [ThreadStatic]
        private static bool _currentThreadIsProcessingJob;

        // Indicates whether the current thread is processing Jobs Parts.
        [ThreadStatic]
        private static bool _currentThreadIsProcessingJobPart;
        ...
        public BlobTransferManager(StorageTransferManagerOptions options)
        {
            ...
            _currentThreadIsProcessingJobPart = false;
            _currentThreadIsProcessingJobChunk = false;
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        private async Task QueueJobAsync(BlobTransferJobInternal job)
        {
            ...
            if (!_currentThreadIsProcessingJob)
            {
                await NotifyOfPendingJobProcessing().ConfigureAwait(false);
            }
        }
        ...
        private async Task QueueJobPartAsync(BlobJobPartInternal part)
        {
            ...
            if (!_currentThreadIsProcessingJobPart)
            {
                NotifyOfPendingJobPartProcessing();
            }
        }

        // Inform the Reader that there's work to be executed for this Channel.
        private void NotifyOfPendingJobPartProcessing()
        {
            // Note that the current thread is now processing work items.
            // This is necessary to enable inlining of tasks into this thread.
            _currentThreadIsProcessingJobPart = true;
            try
            {
                // Process all available items in the queue.
                ...
                while (_partsToProcessChannel.Reader.TryRead(out BlobJobPartInternal item))
                {
        ...
        private async Task QueueJobChunkAsync(Func<Task> item)
        {
            ...
            if (!_currentThreadIsProcessingJobChunk)
            {
                await NotifyOfPendingJobChunkProcessing().ConfigureAwait(false);
            }
        }
        ...
        private async Task NotifyOfPendingJobChunkProcessing()
        {
            try
            {
                ...
                // We could endlessly wait to read, but we would rather let go of the
                // thread if there are no more jobs to process
                while (_chunksToProcessChannel.Reader.TryRead(out Func<Task> item))
                {
