// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;

namespace Azure.ContainerApps.Sandbox
{
    /// <summary> Connection-list response body. </summary>
    public partial class ConnectionsListResponse
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ConnectionsListResponse"/>. </summary>
        /// <param name="connectionIds"> connectionIds value. </param>
        internal ConnectionsListResponse(IEnumerable<string> connectionIds)
        {
            ConnectionIds = connectionIds.ToList();
        }

        /// <summary> Initializes a new instance of <see cref="ConnectionsListResponse"/>. </summary>
        /// <param name="connectionIds"> connectionIds value. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ConnectionsListResponse(IList<string> connectionIds, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            ConnectionIds = connectionIds;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> connectionIds value. </summary>
        public IList<string> ConnectionIds { get; }
    }
}
