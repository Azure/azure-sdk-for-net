// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.StreamAnalytics.Models
{
    public partial class QueryCompilationError
    {
        internal static QueryCompilationError DeserializeQueryCompilationError(JsonElement element)
        {
            Optional<string> message = default;
            Optional<int> startLine = default;
            Optional<int> startColumn = default;
            Optional<int> endLine = default;
            Optional<int> endColumn = default;
            Optional<bool> isGlobal = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("message"))
                {
                    message = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("startLine"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    startLine = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("startColumn"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    startColumn = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("endLine"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    endLine = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("endColumn"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    endColumn = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("isGlobal"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    isGlobal = property.Value.GetBoolean();
                    continue;
                }
            }
            return new QueryCompilationError(message.Value, Optional.ToNullable(startLine), Optional.ToNullable(startColumn), Optional.ToNullable(endLine), Optional.ToNullable(endColumn), Optional.ToNullable(isGlobal));
        }
    }
}
