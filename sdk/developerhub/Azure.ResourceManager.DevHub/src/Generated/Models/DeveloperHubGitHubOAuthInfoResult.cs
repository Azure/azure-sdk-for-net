// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.DevHub.Models
{
    /// <summary> URL used to authorize the Developer Hub GitHub App. </summary>
    public partial class DeveloperHubGitHubOAuthInfoResult
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="DeveloperHubGitHubOAuthInfoResult"/>. </summary>
        internal DeveloperHubGitHubOAuthInfoResult()
        {
        }

        /// <summary> Initializes a new instance of <see cref="DeveloperHubGitHubOAuthInfoResult"/>. </summary>
        /// <param name="authUri"> URL for authorizing the Developer Hub GitHub App. </param>
        /// <param name="token"> OAuth token used to make calls to GitHub. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal DeveloperHubGitHubOAuthInfoResult(string authUri, string token, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            AuthUri = authUri;
            Token = token;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> URL for authorizing the Developer Hub GitHub App. </summary>
        public string AuthUri { get; }

        /// <summary> OAuth token used to make calls to GitHub. </summary>
        public string Token { get; }
    }
}
