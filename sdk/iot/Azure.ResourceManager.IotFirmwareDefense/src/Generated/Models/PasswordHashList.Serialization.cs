...
namespace Azure.ResourceManager.IotFirmwareDefense.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<PasswordHashList>)this).Write(writer, new ModelReaderWriterOptions("W"));

        void IJsonModel<PasswordHashList>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<PasswordHashList>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(PasswordHashList)} does not support '{format}' format.");
            }

            writer.WriteStartObject();
            if (options.Format != "W" && !(Value is ChangeTrackingList<PasswordHash> collection && collection.IsUndefined))
            {
        ...
        internal static PasswordHashList DeserializePasswordHashList(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            return new PasswordHashList(value ?? new ChangeTrackingList<PasswordHash>(), nextLink.Value, serializedAdditionalRawData);
        }
        ...
        string IPersistableModel<PasswordHashList>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
