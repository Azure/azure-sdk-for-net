...
namespace Azure.ResourceManager.Grafana.Models
{
    public partial class ManagedGrafanaProperties : IUtf8JsonSerializable, IJsonModel<ManagedGrafanaProperties>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            {
                builder.Append("  grafanaVersion:");
                if (GrafanaVersion.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{GrafanaVersion}'''");
                }
                else
                {
                    builder.AppendLine($" '{GrafanaVersion}'");
            ...
            if (Optional.IsDefined(ApiKey))
            {
                ...
                builder.AppendLine($" '{ApiKey.Value.ToString()}'");
            }

            if (Optional.IsDefined(DeterministicOutboundIP))
            {
                builder.Append("  deterministicOutboundIP:");
                builder.AppendLine($" '{DeterministicOutboundIP.Value.ToString()}'");
            }
            ...
            if (Optional.IsDefined(GrafanaIntegrations))
            {
                builder.Append("  grafanaIntegrations:");
                AppendChildObject(builder, GrafanaIntegrations, options, 2, false);
            }

            if (Optional.IsDefined(EnterpriseConfigurations))
            {
                builder.Append("  enterpriseConfigurations:");
                AppendChildObject(builder, EnterpriseConfigurations, options, 2, false);
            }

            if (Optional.IsDefined(GrafanaConfigurations))
            {
                builder.Append("  grafanaConfigurations:");
                AppendChildObject(builder, GrafanaConfigurations, options, 2, false);
            }

            if (Optional.IsCollectionDefined(GrafanaPlugins))
            {
                if (GrafanaPlugins.Any())
                {
                    builder.Append("  grafanaPlugins:");
                    builder.AppendLine(" {");
                    foreach (var item in GrafanaPlugins)
                    {
            ...
            if (Optional.IsDefined(GrafanaMajorVersion))
            {
                builder.Append("  grafanaMajorVersion:");
                if (GrafanaMajorVersion.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{GrafanaMajorVersion}'''");
                }
                else
                {
                    builder.AppendLine($" '{GrafanaMajorVersion}'");
