// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.ContainerService;

namespace Azure.ResourceManager.ContainerService.Models
{
    /// <summary> A label selector requirement is a selector that contains values, a key, and an operator that relates the key and values. </summary>
    public partial class ManagedClusterLoadBalancerLabelSelectorRequirement
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ManagedClusterLoadBalancerLabelSelectorRequirement"/>. </summary>
        public ManagedClusterLoadBalancerLabelSelectorRequirement()
        {
            Values = new ChangeTrackingList<string>();
        }

        /// <summary> Initializes a new instance of <see cref="ManagedClusterLoadBalancerLabelSelectorRequirement"/>. </summary>
        /// <param name="key"> key is the label key that the selector applies to. </param>
        /// <param name="operator"> operator represents a key's relationship to a set of values. Valid operators are In and NotIn. </param>
        /// <param name="values"> values is an array of string values, the values array must be non-empty. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ManagedClusterLoadBalancerLabelSelectorRequirement(string key, ManagedClusterLabelSelectorOperator? @operator, IList<string> values, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Key = key;
            Operator = @operator;
            Values = values;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> key is the label key that the selector applies to. </summary>
        [WirePath("key")]
        public string Key { get; set; }

        /// <summary> operator represents a key's relationship to a set of values. Valid operators are In and NotIn. </summary>
        [WirePath("operator")]
        public ManagedClusterLabelSelectorOperator? Operator { get; set; }

        /// <summary> values is an array of string values, the values array must be non-empty. </summary>
        [WirePath("values")]
        public IList<string> Values { get; }
    }
}
