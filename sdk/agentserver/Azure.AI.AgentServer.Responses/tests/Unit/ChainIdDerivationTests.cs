...
public class ChainIdDerivationTests
{
    ...
    public void Derive_DiscriminatorNamespacesIdenticalPartitionValues()
    {
        ...
        // and the other as a non-steerable response fork ("resp"). Neither value is in canonical
        // ID format, so the partition is the raw value in both cases; only the discriminator differs.
        var asConversation = ChainIdDerivation.Derive("shared", null, "ignored", new AgentReference("my-agent"), "sess-1");
        var asResponse = ChainIdDerivation.Derive(null, null, "shared", new AgentReference("my-agent"), "sess-1", steerable: false);

        Assert.That(asConversation, Is.Not.EqualTo(asResponse),
            "Identical partition values from different sources must not collide");
    }

    // ── Non-steerable forking ──

    [Test]
    public void Derive_NonSteerable_UsesFullResponseIdAndForkDiscriminator()
    {
        // Non-steerable with a previous response => "fork" discriminator and the FULL response id.
        var result = ChainIdDerivation.Derive(null, ParentId, OtherPartitionId, new AgentReference("my-agent"), "sess-1", steerable: false);

        var composite = $"my-agent:sess-1:fork:{OtherPartitionId}";
        var expectedBytes = SHA256.HashData(Encoding.UTF8.GetBytes(composite));
        var expected = Convert.ToHexString(expectedBytes).ToLowerInvariant()[..ExpectedLength];
    ...
    [Test]
    public void Derive_NonSteerable_NoPreviousResponse_UsesRespDiscriminator()
    {
        // Non-steerable without a previous response => "resp" discriminator and the FULL response id.
        var result = ChainIdDerivation.Derive(null, null, ParentId, new AgentReference("my-agent"), "sess-1", steerable: false);

        var composite = $"my-agent:sess-1:resp:{ParentId}";
        var expectedBytes = SHA256.HashData(Encoding.UTF8.GetBytes(composite));
        var expected = Convert.ToHexString(expectedBytes).ToLowerInvariant()[..ExpectedLength];
    ...
    [Test]
    public void Derive_NonSteerable_DistinctResponseIds_DoNotShareChainId()
    {
        // Two concurrent forks sharing a partition key but with different entropy must stay distinct.
        var fork1 = ChainIdDerivation.Derive(null, ParentId, ParentId, new AgentReference("my-agent"), "sess-1", steerable: false);
        var fork2 = ChainIdDerivation.Derive(null, ParentId, SamePartitionId, new AgentReference("my-agent"), "sess-1", steerable: false);

        Assert.That(fork1, Is.Not.EqualTo(fork2),
            "Non-steerable forks keyed on the full response id must not collide");
    }
}
