...
PROJ_DIR="$SAMPLE_ROOT/src/resilient-responses-agent-demo"

echo "==> Staging the NuGet package drop"
"$SAMPLE_ROOT/build.sh"

echo "==> Restoring + building (Release)"
dotnet build -c Release "$PROJ_DIR/ResilientResponsesAgentDemo.csproj"

echo ""
echo "Done. Next:"
echo "  ./run.sh                        # automated crash -> recover demo (credential-free 'echo' mode by default)"
...
echo "  DEMO_ROUTE=research ./run.sh"
