...
# copies them into the per-sample staging dir (src/resilient-responses-agent-demo/packages/,
...
# sample root -> Responses -> agentserver
REPO_AGENTSERVER="$(cd "$SCRIPT_DIR/../../.." && pwd)"
CENTRAL_PACKAGES="$REPO_AGENTSERVER/packages"
STAGING_DIR="$SCRIPT_DIR/src/resilient-responses-agent-demo/packages"
...
echo ""
echo "Done. Now run: azd up   (or docker build, or dotnet run from src/resilient-responses-agent-demo)."
