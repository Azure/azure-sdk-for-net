...
/// </summary>
internal static partial class OpenApiToolValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        // Required: openapi
        if (!element.TryGetProperty("openapi", out var openapiProp))
            errors.Add(new ValidationError("$.openapi", "Required property 'openapi' is missing"));
        else
        {
            var openapiResult = OpenApiFunctionDefinitionValidator.Validate(openapiProp);
            if (!openapiResult.IsValid)
            {
                foreach (var e in openapiResult.Errors)
                    errors.Add(new ValidationError("$.openapi" + e.Path.Substring(1), e.Message));
            }
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("openapi"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: openapi"));
        }
