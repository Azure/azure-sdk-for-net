...
/// </summary>
internal static partial class CompoundFilterValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        // Required: filters
        if (!element.TryGetProperty("filters", out var filtersProp))
            errors.Add(new ValidationError("$.filters", "Required property 'filters' is missing"));
        else
        {
            if (filtersProp.ValueKind != JsonValueKind.Array)
                errors.Add(new ValidationError("$.filters", $"Expected array, got {filtersProp.ValueKind}"));
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("and" or "or"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: and, or"));
        }
