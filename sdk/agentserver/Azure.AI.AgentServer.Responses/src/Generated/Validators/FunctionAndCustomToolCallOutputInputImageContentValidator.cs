// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

#nullable enable
// <auto-generated>
// This file was generated by scripts/generate-validators.py from the OpenAPI spec.
// Do not edit manually. Re-run scripts/Generate-Contracts.ps1 to regenerate.
// </auto-generated>

using System.Text.Json;
using Azure.AI.AgentServer.Responses;

namespace Azure.AI.AgentServer.Responses.Validators;

/// <summary>
/// Validates JSON payloads against the API schema.
/// Extend via partial class in Custom/Validators/ to add custom rules.
/// </summary>
internal static partial class FunctionAndCustomToolCallOutputInputImageContentValidator
{
    /// <summary>
    /// Validates a parsed JSON element.
    /// </summary>
    public static ValidationResult Validate(JsonElement element)
    {
        var errors = new List<ValidationError>();

        if (element.ValueKind != JsonValueKind.Object)
        {
            errors.Add(new ValidationError("$", $"Expected object, got {element.ValueKind}"));
            return ValidationResult.Failure(errors);
        }

        // Required: detail
        if (!element.TryGetProperty("detail", out var detailProp))
            errors.Add(new ValidationError("$.detail", "Required property 'detail' is missing"));
        else
        {
            if (detailProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.detail", $"Expected string, got {detailProp.ValueKind}"));
            else if (detailProp.GetString() is not ("low" or "high" or "auto" or "original"))
                errors.Add(new ValidationError("$.detail", $"Value '{detailProp.GetString()}' is not valid. Allowed: low, high, auto, original"));
        }

        // Optional: file_id
        if (element.TryGetProperty("file_id", out var fileIdProp) && fileIdProp.ValueKind != JsonValueKind.Null)
        {
            if (fileIdProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.file_id", $"Expected string, got {fileIdProp.ValueKind}"));
        }

        // Optional: image_url
        if (element.TryGetProperty("image_url", out var imageUrlProp) && imageUrlProp.ValueKind != JsonValueKind.Null)
        {
            if (imageUrlProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.image_url", $"Expected string, got {imageUrlProp.ValueKind}"));
        }

        // Required: type
        if (!element.TryGetProperty("type", out var typeValProp))
            errors.Add(new ValidationError("$.type", "Required property 'type' is missing"));
        else
        {
            if (typeValProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.type", $"Expected string, got {typeValProp.ValueKind}"));
            else if (typeValProp.GetString() is not ("input_image"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: input_image"));
        }

        ValidateCustom(element, errors);

        return errors.Count == 0 ? ValidationResult.Success : ValidationResult.Failure(errors);
    }

    /// <summary>
    /// Extension point for custom validation rules.
    /// </summary>
    static partial void ValidateCustom(JsonElement element, List<ValidationError> errors);
}
