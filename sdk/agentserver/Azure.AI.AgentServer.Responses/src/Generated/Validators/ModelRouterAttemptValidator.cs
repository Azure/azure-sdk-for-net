...
/// </summary>
internal static partial class ModelRouterAttemptValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        if (!element.TryGetProperty("model", out var modelProp))
            errors.Add(new ValidationError("$.model", "Required property 'model' is missing"));
        else
        {
        ...
        {
            var resultResult = ModelRouterAttemptResultValidator.Validate(resultProp);
            if (!resultResult.IsValid)
            {
                foreach (var e in resultResult.Errors)
                    errors.Add(new ValidationError("$.result" + e.Path.Substring(1), e.Message));
            }
        }
