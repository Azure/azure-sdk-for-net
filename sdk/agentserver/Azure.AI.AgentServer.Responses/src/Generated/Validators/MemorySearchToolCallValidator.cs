...
/// </summary>
internal static partial class MemorySearchToolCallValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        // Optional: memories
        if (element.TryGetProperty("memories", out var memoriesProp) && memoriesProp.ValueKind != JsonValueKind.Null)
        {
            if (memoriesProp.ValueKind != JsonValueKind.Array)
                errors.Add(new ValidationError("$.memories", $"Expected array, got {memoriesProp.ValueKind}"));
            else
            {
                var memoriesIdx = 0;
                foreach (var item in memoriesProp.EnumerateArray())
                {
                    var itemResult = MemoryItemValidator.Validate(item);
                    if (!itemResult.IsValid)
                    {
                        foreach (var e in itemResult.Errors)
                            errors.Add(new ValidationError($"$.memories[{memoriesIdx}]" + e.Path.Substring(1), e.Message));
                    }
                    memoriesIdx++;
                }
            }
