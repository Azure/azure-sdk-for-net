...
/// </summary>
internal static partial class AzureAISearchToolValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        if (!element.TryGetProperty("azure_ai_search", out var azureAiSearchProp))
            errors.Add(new ValidationError("$.azure_ai_search", "Required property 'azure_ai_search' is missing"));
        else
        {
            var azureAiSearchResult = AzureAISearchToolResourceValidator.Validate(azureAiSearchProp);
            if (!azureAiSearchResult.IsValid)
            {
                foreach (var e in azureAiSearchResult.Errors)
                    errors.Add(new ValidationError("$.azure_ai_search" + e.Path.Substring(1), e.Message));
            }
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("azure_ai_search"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: azure_ai_search"));
        }
