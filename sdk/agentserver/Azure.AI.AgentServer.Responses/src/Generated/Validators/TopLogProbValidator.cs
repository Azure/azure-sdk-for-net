// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

#nullable enable
// <auto-generated>
// This file was generated by scripts/generate-validators.py from the OpenAPI spec.
// Do not edit manually. Re-run scripts/Generate-Contracts.ps1 to regenerate.
// </auto-generated>

using System.Text.Json;
using Azure.AI.AgentServer.Responses;

namespace Azure.AI.AgentServer.Responses.Validators;

/// <summary>
/// Validates JSON payloads against the API schema.
/// Extend via partial class in Custom/Validators/ to add custom rules.
/// </summary>
internal static partial class TopLogProbValidator
{
    /// <summary>
    /// Validates a parsed JSON element.
    /// </summary>
    public static ValidationResult Validate(JsonElement element)
    {
        var errors = new List<ValidationError>();

        if (element.ValueKind != JsonValueKind.Object)
        {
            errors.Add(new ValidationError("$", $"Expected object, got {element.ValueKind}"));
            return ValidationResult.Failure(errors);
        }

        // Required: bytes
        if (!element.TryGetProperty("bytes", out var bytesProp))
            errors.Add(new ValidationError("$.bytes", "Required property 'bytes' is missing"));
        else
        {
            if (bytesProp.ValueKind != JsonValueKind.Array)
                errors.Add(new ValidationError("$.bytes", $"Expected array, got {bytesProp.ValueKind}"));
        }

        // Required: logprob
        if (!element.TryGetProperty("logprob", out var logprobProp))
            errors.Add(new ValidationError("$.logprob", "Required property 'logprob' is missing"));
        else
        {
            if (logprobProp.ValueKind != JsonValueKind.Number)
                errors.Add(new ValidationError("$.logprob", $"Expected number, got {logprobProp.ValueKind}"));
        }

        // Required: token
        if (!element.TryGetProperty("token", out var tokenProp))
            errors.Add(new ValidationError("$.token", "Required property 'token' is missing"));
        else
        {
            if (tokenProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.token", $"Expected string, got {tokenProp.ValueKind}"));
        }

        ValidateCustom(element, errors);

        return errors.Count == 0 ? ValidationResult.Success : ValidationResult.Failure(errors);
    }

    /// <summary>
    /// Extension point for custom validation rules.
    /// </summary>
    static partial void ValidateCustom(JsonElement element, List<ValidationError> errors);
}
