// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.AI.Agents.Contracts.V2.Models
{
    /// <summary> Local shell call output. </summary>
    public partial class OutputItemLocalShellToolCallOutput : OutputItem
    {
        /// <summary> Initializes a new instance of <see cref="OutputItemLocalShellToolCallOutput"/>. </summary>
        /// <param name="id"> The unique ID of the local shell tool call generated by the model. </param>
        /// <param name="output"> A JSON string of the output of the local shell tool call. </param>
        internal OutputItemLocalShellToolCallOutput(string id, string output) : base(OutputItemType.LocalShellCallOutput)
        {
            _id = id;
            Output = output;
        }

        /// <summary> Initializes a new instance of <see cref="OutputItemLocalShellToolCallOutput"/>. </summary>
        /// <param name="type"></param>
        /// <param name="createdBy"> The information about the creator of the item. </param>
        /// <param name="agentReference"> The agent that created the item. </param>
        /// <param name="responseId"> The response on which the item is created. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="id"> The unique ID of the local shell tool call generated by the model. </param>
        /// <param name="output"> A JSON string of the output of the local shell tool call. </param>
        /// <param name="status"></param>
        internal OutputItemLocalShellToolCallOutput(OutputItemType @type, BinaryData createdBy, AgentReference agentReference, string responseId, IDictionary<string, BinaryData> additionalBinaryDataProperties, string id, string output, ItemLocalShellToolCallOutputStatus? status) : base(@type, id, createdBy, agentReference, responseId, additionalBinaryDataProperties)
        {
            Output = output;
            Status = status;
        }

        /// <summary> The unique ID of the local shell tool call generated by the model. </summary>
        public new string Id => _id ?? default;

        /// <summary> A JSON string of the output of the local shell tool call. </summary>
        public string Output { get; }

        /// <summary> Gets the Status. </summary>
        public ItemLocalShellToolCallOutputStatus? Status { get; }
    }
}
