// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.AI.Agents.Contracts.V2.Models;

namespace Azure.AI.Agents.Contracts.V2
{
    /// <summary> The UpdateMemoriesRequest. </summary>
    public partial class UpdateMemoriesRequest
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="UpdateMemoriesRequest"/>. </summary>
        /// <param name="scope"> The namespace that logically groups and isolates memories, such as a user ID. </param>
        internal UpdateMemoriesRequest(string scope)
        {
            Scope = scope;
            Items = new ChangeTrackingList<Item>();
        }

        /// <summary> Initializes a new instance of <see cref="UpdateMemoriesRequest"/>. </summary>
        /// <param name="scope"> The namespace that logically groups and isolates memories, such as a user ID. </param>
        /// <param name="items"> Conversation items to be stored in memory. </param>
        /// <param name="previousUpdateId"> The unique ID of the previous update request, enabling incremental memory updates from where the last operation left off. </param>
        /// <param name="updateDelay">
        /// Timeout period before processing the memory update in seconds.
        /// If a new update request is received during this period, it will cancel the current request and reset the timeout.
        /// Set to 0 to immediately trigger the update without delay.
        /// Defaults to 300 (5 minutes).
        /// </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal UpdateMemoriesRequest(string scope, IList<Item> items, string previousUpdateId, int? updateDelay, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Scope = scope;
            Items = items;
            PreviousUpdateId = previousUpdateId;
            UpdateDelay = updateDelay;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The namespace that logically groups and isolates memories, such as a user ID. </summary>
        public string Scope { get; }

        /// <summary> Conversation items to be stored in memory. </summary>
        public IList<Item> Items { get; }

        /// <summary> The unique ID of the previous update request, enabling incremental memory updates from where the last operation left off. </summary>
        public string PreviousUpdateId { get; }

        /// <summary>
        /// Timeout period before processing the memory update in seconds.
        /// If a new update request is received during this period, it will cancel the current request and reset the timeout.
        /// Set to 0 to immediately trigger the update without delay.
        /// Defaults to 300 (5 minutes).
        /// </summary>
        public int? UpdateDelay { get; }
    }
}
