...
public class VoiceProtocolCodecTests
{
    ...
    [Test]
    public void ParseDtmfDistinguishesRawKeyAndCollectedTurn()
    {
        var raw = VoiceProtocolCodec.DecodeFrame(
            """{"type":"dtmf","id":"m_4","ts":"2026-07-29T00:00:00.000Z","digits":"#"}""");
        var collected = VoiceProtocolCodec.DecodeFrame(
            """{"type":"dtmf","id":"m_5","ts":"2026-07-29T00:00:00.000Z","digits":"12","collection_id":"dc_1","item_id":"in_3","completion_reason":"max_digits"}""");

        Assert.Multiple(() =>
        {
            Assert.That(VoiceProtocolCodec.ParseDtmf(raw), Is.TypeOf<DtmfKeyEvent>());
            Assert.That(VoiceProtocolCodec.ParseDtmf(collected), Is.TypeOf<DtmfCollectedEvent>());
        });
    }
    ...
    {
        var root = VoiceProtocolCodec.DecodeFrame(
            """{"type":"conversation.item.create","id":"m_6","ts":"2026-07-29T00:00:00.000Z","item":{"id":"hi_1","role":"assistant","content":[{"type":"input_text","text":"x"}]}}""");

        var exception = Assert.Throws<VoiceBridgeProtocolException>(() => VoiceProtocolCodec.ParseConversationItemCreate(root));
        Assert.That(exception!.CloseCode, Is.EqualTo(1008));
    }
    ...
    {
        var root = VoiceProtocolCodec.DecodeFrame(JsonSerializer.Serialize(new
        {
            ...
            item = new
            {
                id = "hi_1",
                role = "user",
                content = new[] { new { type = "input_text", text = "x" } },
            },
        }));

        var create = VoiceProtocolCodec.ParseConversationItemCreate(root);

        Assert.That(create.PreviousItemId, Is.EqualTo(previousItemId));
    }
    ...
    [TestCase("ROOT")]
    [TestCase("arbitrary")]
    public void ParseConversationItemCreateRejectsInvalidPreviousItemId(string previousItemId)
    {
        var root = VoiceProtocolCodec.DecodeFrame(JsonSerializer.Serialize(new
        {
            type = "conversation.item.create",
        ...
        var exception = Assert.Throws<VoiceBridgeProtocolException>(() =>
            VoiceProtocolCodec.ParseConversationItemCreate(root));
