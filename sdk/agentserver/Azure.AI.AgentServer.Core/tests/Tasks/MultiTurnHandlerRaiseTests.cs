...
public sealed class MultiTurnHandlerRaiseTests
{
    ...
    public async Task PerTurnRaiseSurfacesTypedFailureAndChainStaysSuspendedWithoutError()
    {
        ...
        Assert.ThrowsAsync<ResilientTaskException>(async () => await run.Completion);
    ...
    public async Task SingleAttemptRaiseIsHandlerErrorWithoutAttemptCount()
    {
        ...
        var ex = Assert.ThrowsAsync<ResilientTaskException>(async () => await run.Completion);
        ...
        Assert.That(ex.Failure!.ErrorType, Is.EqualTo(nameof(InvalidOperationException)));
        Assert.That(ex.Failure!.Attempts, Is.Null);
    }
    ...
    public async Task HandlerErrorPopulatesTracebackForParity()
    {
        ...
        var ex = Assert.ThrowsAsync<ResilientTaskException>(async () => await run.Completion);
        Assert.That(ex!.Failure!.Traceback, Is.Not.Null.And.Contains(nameof(InvalidOperationException)));
    }
    ...
    public async Task ExhaustedRetriesReportsAttemptCount()
    {
        ...
        Assert.That(ex.Failure!.LastErrorType, Is.EqualTo(nameof(InvalidOperationException)));
    }
}
