...
/// </summary>
public static partial class FabricDataAgentToolCallOutputValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        {
            var outputResult = ToolCallOutputContentValidator.Validate(outputProp);
            if (!outputResult.IsValid)
            {
                foreach (var e in outputResult.Errors)
                    errors.Add(e with { Path = "$.output" + e.Path[1..] });
            }
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("fabric_dataagent_preview_call_output"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: fabric_dataagent_preview_call_output"));
        }
