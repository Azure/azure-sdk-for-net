...
/// </summary>
public static partial class ApplyPatchToolCallOutputItemParamValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        else
        {
            ...
            {
                var strVal = callIdProp.GetString()!;
                if (strVal.Length < 1)
                    errors.Add(new ValidationError("$.call_id", $"String length {strVal.Length} is below minimum 1"));
                if (strVal.Length > 64)
                    errors.Add(new ValidationError("$.call_id", $"String length {strVal.Length} exceeds maximum 64"));
            }
        }
        ...
        else
        {
            ...
            else if (statusProp.GetString() is not ("completed" or "failed"))
                errors.Add(new ValidationError("$.status", $"Value '{statusProp.GetString()}' is not valid. Allowed: completed, failed"));
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("apply_patch_call_output"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: apply_patch_call_output"));
        }
