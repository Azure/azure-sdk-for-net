...
/// </summary>
public static partial class WebSearchActionFindValidator
{
    /// <summary>
    ...
    public static ValidationResult Validate(JsonElement element)
    {
        ...
        if (!element.TryGetProperty("pattern", out var patternProp))
            errors.Add(new ValidationError("$.pattern", "Required property 'pattern' is missing"));
        else
        {
            if (patternProp.ValueKind != JsonValueKind.String)
                errors.Add(new ValidationError("$.pattern", $"Expected string, got {patternProp.ValueKind}"));
        }
        ...
        else
        {
            ...
            else if (typeValProp.GetString() is not ("find_in_page"))
                errors.Add(new ValidationError("$.type", $"Value '{typeValProp.GetString()}' is not valid. Allowed: find_in_page"));
        }
