...
- Library-owned hosting foundation via `AgentServer.Run<THandler>()` (Tier 1) and `AgentServerBuilder` (Tier 2).
...
- Multi-protocol composition via `AgentServerBuilder.AddResponses<T>()` and `AddInvocations<T>()`.
