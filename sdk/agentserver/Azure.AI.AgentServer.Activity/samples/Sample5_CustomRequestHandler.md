...
For full control over the request pipeline, pass a `RequestDelegate` to `ActivityServer.Run(...)`. The Microsoft 365 Agents SDK is **not** initialized in this mode — no `AgentApplication` is created, and your delegate receives every inbound `POST /activity/messages` request directly.
