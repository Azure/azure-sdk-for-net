...
`ConfigureServices` runs after the Microsoft 365 Agents SDK registers its defaults, so normal Microsoft DI last-registration semantics make your adapter, authorization, or channel-service factory override win. The one exception is `IConnections`: the outbound-auth provider is always substituted after this callback runs, so register a custom connection provider through `options.Connections` instead.
