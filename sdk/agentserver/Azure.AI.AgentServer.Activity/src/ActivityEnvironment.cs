...
/// <summary>
/// Resolves the Microsoft 365 Agents SDK connection settings (the <c>Connections:*</c> /
/// <c>ConnectionsMap:*</c> keys) from the Foundry-native identity and returns them as a plain
/// configuration map.
/// </summary>
/// <remarks>
/// <para>
/// <see cref="GetHostedAgentConfiguration(bool)"/> is a <b>pure function</b>: it reads the current
/// environment at call time and <b>never mutates it</b>. Feed the returned map into a configuration
/// builder (for example <c>new ConfigurationBuilder().AddEnvironmentVariables().AddInMemoryCollection(config)</c>)
/// so the M365 Agents SDK can read the settings.
/// </para>
...
/// <list type="bullet">
///   <item><b>Simple</b> (<c>digitalWorker: false</c>, default): the <em>instance</em> identity
///     (<c>FOUNDRY_AGENT_INSTANCE_CLIENT_ID</c>) mints the Bot Connector token directly, scoped to
///     <c>https://api.botframework.com/.default</c>.</item>
///   <item><b>Digital worker</b> (<c>digitalWorker: true</c>): the <em>blueprint</em> identity
///     (<c>FOUNDRY_AGENT_BLUEPRINT_CLIENT_ID</c>) is used with the federated-identity exchange,
///     scoped to the agentic resource.</item>
/// </list>
/// </para>
/// </remarks>
...
{
    /// <summary>
    /// Resolves the M365 SDK connection settings for the selected outbound-auth model as a
    /// configuration map. Does not mutate the process environment.
    /// </summary>
    ...
    /// </param>
    /// <returns>
    /// The effective connection settings (existing environment value where present, otherwise the
    /// derived value) keyed by their M365 configuration names.
    /// </returns>
    public static IReadOnlyDictionary<string, string?> GetHostedAgentConfiguration(bool digitalWorker = false)
    {
        string scope;
        ...
        if (digitalWorker)
        {
            // Digital worker: blueprint identity + native FMI token exchange. The
            // IdentityProxyManager auth type makes the SDK's MsalAuth act as an
            // IAgenticTokenProvider, which the connector's agentic reply path requires.
            scope = ConnectionEnvironment.DigitalWorkerScope;
            clientIdEnvVar = ConnectionEnvironment.FoundryBlueprintClientId;
            authType = ConnectionEnvironment.DigitalWorkerAuthType;
        }
        else
        {
            ...
            authType = ConnectionEnvironment.DefaultAuthType;
        }

        var settings = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase);

        // Records the effective value for a setting: an existing explicit environment value takes
        // precedence; otherwise the derived value is used. Empty values are omitted. The process
