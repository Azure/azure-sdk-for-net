// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.ComputeBulkActions;

namespace Azure.ResourceManager.ComputeBulkActions.Models
{
    /// <summary> Zone preferences for LaunchBulkInstancesOperation zone allocation policy. </summary>
    public partial class ComputeBulkActionsZonePreference
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ComputeBulkActionsZonePreference"/>. </summary>
        /// <param name="zone"> Name of the zone. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="zone"/> is null. </exception>
        public ComputeBulkActionsZonePreference(string zone)
        {
            Argument.AssertNotNull(zone, nameof(zone));

            Zone = zone;
        }

        /// <summary> Initializes a new instance of <see cref="ComputeBulkActionsZonePreference"/>. </summary>
        /// <param name="zone"> Name of the zone. </param>
        /// <param name="rank">
        ///     The rank of the zone. This is used with 'Prioritized' ZoneDistributionStrategy.
        ///     The lower the number, the higher the priority, starting with 0.
        ///     0 is the highest rank. If not specified, defaults to lowest rank.
        /// </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ComputeBulkActionsZonePreference(string zone, int? rank, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Zone = zone;
            Rank = rank;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Name of the zone. </summary>
        public string Zone { get; set; }

        /// <summary>
        ///     The rank of the zone. This is used with 'Prioritized' ZoneDistributionStrategy.
        ///     The lower the number, the higher the priority, starting with 0.
        ///     0 is the highest rank. If not specified, defaults to lowest rank.
        /// </summary>
        public int? Rank { get; set; }
    }
}
