...
namespace Microsoft.Azure.Management.Attestation
{
    ...
    /// </summary>
    internal partial class Operations : IServiceOperations<AttestationManagementClient>, IOperations
    {
        /// <summary>
        ...
        public async Task<AzureOperationResponse<IEnumerable<OperationsDefinition>>> ListWithHttpMessagesAsync(Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                var ex = new ErrorResultException(string.Format("Operation returned an invalid status code '{0}'", _statusCode));
                try
                {
                    _responseContent = await _httpResponse.Content.ReadAsStringAsync().ConfigureAwait(false);
                    ErrorResult _errorBody =  Rest.Serialization.SafeJsonConvert.DeserializeObject<ErrorResult>(_responseContent, Client.DeserializationSettings);
                    if (_errorBody != null)
                    {
                        ex.Body = _errorBody;
                    }
