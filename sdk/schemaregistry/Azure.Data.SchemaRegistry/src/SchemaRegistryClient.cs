    ...
    public partial class SchemaRegistryClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (async)
                {
                    response = await RegisterSchemaAsync(groupName, schemaName, new BinaryData(schemaDefinition), new Core.ContentType(format.ToContentType().ToString()), cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    response = RegisterSchema(groupName, schemaName, new BinaryData(schemaDefinition), new Core.ContentType(format.ToContentType().ToString()), cancellationToken);
                }
...
#pragma warning restore AZC0015 // Unexpected client method return type.
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (async)
                {
                    response = await GetSchemaPropertiesByContentAsync(groupName, schemaName, new BinaryData(schemaDefinition), new Core.ContentType(format.ToContentType().ToString()), cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    response = GetSchemaPropertiesByContent(groupName, schemaName, new BinaryData(schemaDefinition), new Core.ContentType(format.ToContentType().ToString()), cancellationToken);
                }
