...
#pragma warning restore AZC0015 // Unexpected client method return type.
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (async)
                {
                    response = await GetSchemaPropertiesByContentAsync(groupName, schemaName, new BinaryData(schemaDefinition), format.ToContentType(), cancellationToken).ConfigureAwait(false);
                }
                else
                {
