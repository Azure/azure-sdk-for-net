    ...
    public class SchemaRegistryClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (async)
                {
                    response = await RestClient.RegisterAsync(groupName, schemaName, ConstructContentType(format), new BinaryData(schemaDefinition).ToStream(), cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    response = RestClient.Register(groupName, schemaName, ConstructContentType(format) , new BinaryData(schemaDefinition).ToStream(), cancellationToken);
                }
