// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.Analytics.PlanetaryComputer
{
    internal partial class UnknownIngestionSource : IngestionSource
    {
        /// <summary> Initializes a new instance of <see cref="UnknownIngestionSource"/>. </summary>
        /// <param name="id"> Ingestion source id. </param>
        /// <param name="createdOn"> Created time in UTC format. </param>
        /// <param name="kind"> Discriminator for the ingestion source. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal UnknownIngestionSource(Guid id, DateTimeOffset? createdOn, IngestionSourceKind kind, IDictionary<string, BinaryData> additionalBinaryDataProperties) : base(id, createdOn, kind != default ? kind : "unknown", additionalBinaryDataProperties)
        {
        }
    }
}
