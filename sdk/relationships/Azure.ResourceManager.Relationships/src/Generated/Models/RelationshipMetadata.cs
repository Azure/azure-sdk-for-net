// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.Relationships.Models
{
    /// <summary> Provides information about the relationship properties. </summary>
    public partial class RelationshipMetadata
    {
        /// <summary>
        /// Keeps track of any properties unknown to the library.
        /// <para>
        /// To assign an object to the value of this property use <see cref="BinaryData.FromObjectAsJson{T}(T, System.Text.Json.JsonSerializerOptions?)"/>.
        /// </para>
        /// <para>
        /// To assign an already formatted json string to this property use <see cref="BinaryData.FromString(string)"/>.
        /// </para>
        /// <para>
        /// Examples:
        /// <list type="bullet">
        /// <item>
        /// <term>BinaryData.FromObjectAsJson("foo")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("\"foo\"")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromObjectAsJson(new { key = "value" })</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("{\"key\": \"value\"}")</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// </list>
        /// </para>
        /// </summary>
        private IDictionary<string, BinaryData> _serializedAdditionalRawData;

        /// <summary> Initializes a new instance of <see cref="RelationshipMetadata"/>. </summary>
        /// <param name="sourceType"> The type of the relationship source resource. </param>
        /// <param name="targetType"> The type of the relationship target resource. </param>
        internal RelationshipMetadata(string sourceType, string targetType)
        {
            SourceType = sourceType;
            TargetType = targetType;
        }

        /// <summary> Initializes a new instance of <see cref="RelationshipMetadata"/>. </summary>
        /// <param name="sourceType"> The type of the relationship source resource. </param>
        /// <param name="targetType"> The type of the relationship target resource. </param>
        /// <param name="serializedAdditionalRawData"> Keeps track of any properties unknown to the library. </param>
        internal RelationshipMetadata(string sourceType, string targetType, IDictionary<string, BinaryData> serializedAdditionalRawData)
        {
            SourceType = sourceType;
            TargetType = targetType;
            _serializedAdditionalRawData = serializedAdditionalRawData;
        }

        /// <summary> Initializes a new instance of <see cref="RelationshipMetadata"/> for deserialization. </summary>
        internal RelationshipMetadata()
        {
        }

        /// <summary> The type of the relationship source resource. </summary>
        public string SourceType { get; }
        /// <summary> The type of the relationship target resource. </summary>
        public string TargetType { get; }
    }
}
