...
public partial class ExternalEndpointTrafficManagerEndpoint
{
    // The provisioning emitter only flattened shared EndpointProperties onto the first endpoint resource.
    // External endpoints use the same Bicep properties, so expose them here until the emitter handles shared models for all resources.
