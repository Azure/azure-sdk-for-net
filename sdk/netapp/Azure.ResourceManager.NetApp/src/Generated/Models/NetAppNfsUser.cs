// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.NetApp.Models
{
    /// <summary> The effective NFS User ID and Group ID when accessing the volume data. </summary>
    public partial class NetAppNfsUser
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="NetAppNfsUser"/>. </summary>
        public NetAppNfsUser()
        {
        }

        /// <summary> Initializes a new instance of <see cref="NetAppNfsUser"/>. </summary>
        /// <param name="userId"> The NFS user's UID. </param>
        /// <param name="groupId"> The NFS user's GID. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal NetAppNfsUser(long? userId, long? groupId, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            UserId = userId;
            GroupId = groupId;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The NFS user's UID. </summary>
        public long? UserId { get; set; }

        /// <summary> The NFS user's GID. </summary>
        public long? GroupId { get; set; }
    }
}
