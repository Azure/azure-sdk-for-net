...
namespace Azure.Graph.Rbac.Models
{
    public partial class ServicePrincipal
    {
        internal static ServicePrincipal DeserializeServicePrincipal(JsonElement element)
        {
            ...
            return new ServicePrincipal(objectId.HasValue ? objectId.Value : null, objectType.HasValue ? objectType.Value : null, deletionTimestamp.HasValue ? deletionTimestamp.Value : (DateTimeOffset?)null, additionalProperties, accountEnabled.HasValue ? accountEnabled.Value : (bool?)null, new ChangeTrackingList<string>(alternativeNames), appDisplayName.HasValue ? appDisplayName.Value : null, appId.HasValue ? appId.Value : null, appOwnerTenantId.HasValue ? appOwnerTenantId.Value : null, appRoleAssignmentRequired.HasValue ? appRoleAssignmentRequired.Value : (bool?)null, new ChangeTrackingList<AppRole>(appRoles), displayName.HasValue ? displayName.Value : null, errorUrl.HasValue ? errorUrl.Value : null, homepage.HasValue ? homepage.Value : null, new ChangeTrackingList<KeyCredential>(keyCredentials), logoutUrl.HasValue ? logoutUrl.Value : null, new ChangeTrackingList<OAuth2Permission>(oauth2Permissions), new ChangeTrackingList<PasswordCredential>(passwordCredentials), preferredTokenSigningKeyThumbprint.HasValue ? preferredTokenSigningKeyThumbprint.Value : null, publisherName.HasValue ? publisherName.Value : null, new ChangeTrackingList<string>(replyUrls), samlMetadataUrl.HasValue ? samlMetadataUrl.Value : null, new ChangeTrackingList<string>(servicePrincipalNames), servicePrincipalType.HasValue ? servicePrincipalType.Value : null, new ChangeTrackingList<string>(tags));
        }
    }
}
