...
{
    public partial class Sample4_LargePersonGroup : FaceSamplesBase
    {
        [Test]
        public async Task VerifyAndIdentifyFromLargePersonGroupAsync()
        {
            var groupClient = CreateServiceClient().GetLargePersonGroupClientClient();
            ...
            foreach (var person in persons)
            {
                ...
                {
                    await groupClient.AddFaceFromUrlAsync(groupId, personId, new Uri(imageUrl), userData: $"{person.UserData}-{imageUrl}", detectionModel: FaceDetectionModel.Detection03);
                }
            }
            #endregion
