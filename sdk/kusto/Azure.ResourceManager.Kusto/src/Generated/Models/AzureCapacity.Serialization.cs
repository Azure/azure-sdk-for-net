// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.Kusto.Models
{
    public partial class AzureCapacity
    {
        internal static AzureCapacity DeserializeAzureCapacity(JsonElement element)
        {
            AzureScaleType scaleType = default;
            int minimum = default;
            int maximum = default;
            int @default = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("scaleType"))
                {
                    scaleType = new AzureScaleType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("minimum"))
                {
                    minimum = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("maximum"))
                {
                    maximum = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("default"))
                {
                    @default = property.Value.GetInt32();
                    continue;
                }
            }
            return new AzureCapacity(scaleType, minimum, maximum, @default);
        }
    }
}
