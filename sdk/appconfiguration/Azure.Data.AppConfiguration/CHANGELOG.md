...
- Added support for the new feature flag endpoint via `GetFeatureFlag`, `GetFeatureFlags`, `PutFeatureFlag`, and `DeleteFeatureFlag` operations (and their async counterparts) on `ConfigurationClient`, along with the `FeatureFlag` model and related types (`FeatureFlagConditions`, `FeatureFlagAllocation`, `FeatureFlagTelemetryConfiguration`, `FeatureFlagVariantDefinition`, etc.). Requires the `2026-05-01-preview` service API version.
