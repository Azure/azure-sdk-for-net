...
- Added a `Description` property on `ConfigurationSetting` and `ConfigurationSnapshot` to associate descriptive text with settings and snapshots. The `ConfigurationSetting` public constructors now accept an optional `description` parameter so the value can be supplied at construction time.
