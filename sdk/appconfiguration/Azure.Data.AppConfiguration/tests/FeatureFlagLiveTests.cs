    ...
    public class FeatureFlagLiveTests : RecordedTestBase<AppConfigurationTestEnvironment>
    {
        ...
        public async Task SetFeatureFlagWithOnlyIfUnchangedSucceedsWhenEtagMatches()
        {
            ...
            try
            {
                ...
                FeatureFlag updated = new FeatureFlag(name)
                {
                    Enabled = true,
                    Description = "updated"
                };
                ...
                Response<FeatureFlag> response = await service.SetFeatureFlagAsync(withEtag, label: null, onlyIfUnchanged: true);
        ...
        public async Task SetFeatureFlagWithOnlyIfUnchangedThrowsWhenEtagStale()
        {
            ...
            try
            {
                ...
                FeatureFlag stale = CloneWithEtag(new FeatureFlag(name) { Enabled = false }, added.Value.Etag);

                RequestFailedException ex = Assert.ThrowsAsync<RequestFailedException>(
                    async () => await service.SetFeatureFlagAsync(stale, label: null, onlyIfUnchanged: true));
