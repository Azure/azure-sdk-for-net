...
namespace Azure.IoT.TimeSeriesInsights
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<Availability>)this).Write(writer, new ModelReaderWriterOptions("W"));
        ...
        void IJsonModel<Availability>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            ...
            if (format != "J")
            {
                throw new InvalidOperationException($"The model {nameof(Availability)} does not support '{format}' format.");
            }

            writer.WriteStartObject();
            if (options.Format != "W" && Optional.IsDefined(Range))
            {
                writer.WritePropertyName("range"u8);
                writer.WriteObjectValue(Range);
            }
            if (options.Format != "W" && Optional.IsDefined(IntervalSize))
            {
        ...
        BinaryData IPersistableModel<Availability>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new InvalidOperationException($"The model {nameof(Availability)} does not support '{options.Format}' format.");
            }
        }
