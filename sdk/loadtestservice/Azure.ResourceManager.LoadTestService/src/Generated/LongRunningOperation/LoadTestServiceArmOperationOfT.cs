...
#pragma warning restore SA1649 // File name should match first type name
    {
        ...
        internal LoadTestServiceArmOperation(Response<T> response)
        {
            _operation = OperationInternal<T>.Succeeded(response.GetRawResponse(), response.Value);
        }

        internal LoadTestServiceArmOperation(Core.IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            ...
            _operation = new OperationInternal<T>(clientDiagnostics, nextLinkOperation, response, "LoadTestServiceArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }

        internal LoadTestServiceArmOperation(Core.IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, string id)
        {
