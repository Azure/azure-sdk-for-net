...
namespace Azure.ResourceManager.HealthBot.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<HealthBotSku>)this).Write(writer, ModelReaderWriterOptions.Wire);

        void IJsonModel<HealthBotSku>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<HealthBotSku>)this).GetWireFormat(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<HealthBotSku>)} interface");
            }
        ...
        string IPersistableModel<HealthBotSku>.GetWireFormat(ModelReaderWriterOptions options) => "J";
    }
}
