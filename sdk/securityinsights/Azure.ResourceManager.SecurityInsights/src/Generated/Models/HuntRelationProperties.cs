// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.Core;
using Azure.ResourceManager.SecurityInsights;

namespace Azure.ResourceManager.SecurityInsights.Models
{
    /// <summary> Describes hunt relation properties. </summary>
    internal partial class HuntRelationProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="HuntRelationProperties"/>. </summary>
        /// <param name="relatedResourceId"> The id of the related resource. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="relatedResourceId"/> is null. </exception>
        public HuntRelationProperties(ResourceIdentifier relatedResourceId)
        {
            Argument.AssertNotNull(relatedResourceId, nameof(relatedResourceId));

            RelatedResourceId = relatedResourceId;
            Labels = new ChangeTrackingList<string>();
        }

        /// <summary> Initializes a new instance of <see cref="HuntRelationProperties"/>. </summary>
        /// <param name="relatedResourceId"> The id of the related resource. </param>
        /// <param name="relatedResourceName"> The name of the related resource. </param>
        /// <param name="relationType"> The type of the hunt relation. </param>
        /// <param name="relatedResourceKind"> The resource that the relation is related to. </param>
        /// <param name="labels"> List of labels relevant to this hunt. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal HuntRelationProperties(ResourceIdentifier relatedResourceId, string relatedResourceName, string relationType, string relatedResourceKind, IList<string> labels, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            RelatedResourceId = relatedResourceId;
            RelatedResourceName = relatedResourceName;
            RelationType = relationType;
            RelatedResourceKind = relatedResourceKind;
            Labels = labels;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The id of the related resource. </summary>
        [WirePath("relatedResourceId")]
        public ResourceIdentifier RelatedResourceId { get; set; }

        /// <summary> The name of the related resource. </summary>
        [WirePath("relatedResourceName")]
        public string RelatedResourceName { get; }

        /// <summary> The type of the hunt relation. </summary>
        [WirePath("relationType")]
        public string RelationType { get; }

        /// <summary> The resource that the relation is related to. </summary>
        [WirePath("relatedResourceKind")]
        public string RelatedResourceKind { get; }

        /// <summary> List of labels relevant to this hunt. </summary>
        [WirePath("labels")]
        public IList<string> Labels { get; } = new ChangeTrackingList<string>();
    }
}
