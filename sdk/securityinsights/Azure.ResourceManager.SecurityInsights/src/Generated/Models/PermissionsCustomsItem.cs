// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.SecurityInsights.Models
{
    /// <summary> The PermissionsCustomsItem. </summary>
    public partial class PermissionsCustomsItem : ConnectorCustoms
    {
        /// <summary> Initializes a new instance of <see cref="PermissionsCustomsItem"/>. </summary>
        public PermissionsCustomsItem()
        {
        }

        /// <summary> Initializes a new instance of <see cref="PermissionsCustomsItem"/>. </summary>
        /// <param name="name"> Customs permissions name. </param>
        /// <param name="description"> Customs permissions description. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal PermissionsCustomsItem(string name, string description, IDictionary<string, BinaryData> additionalBinaryDataProperties) : base(name, description, additionalBinaryDataProperties)
        {
        }
    }
}
