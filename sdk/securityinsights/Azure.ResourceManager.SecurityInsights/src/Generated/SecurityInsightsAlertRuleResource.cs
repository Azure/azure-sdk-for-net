...
namespace Azure.ResourceManager.SecurityInsights
{
    ...
    /// Otherwise you can get one from its parent resource <see cref="ArmResource"/> using the GetSecurityInsightsAlertRules method.
    /// </summary>
    public partial class SecurityInsightsAlertRuleResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation> TriggerRuleRunAlertRuleAsync(WaitUntil waitUntil, AnalyticsRuleRunTrigger analyticsRuleRunTriggerParameter, CancellationToken cancellationToken = default)
        {
            ...
            using DiagnosticScope scope = _alertRuleClientDiagnostics.CreateScope("SecurityInsightsAlertRuleResource.TriggerRuleRunAlertRule");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext
                ...
                HttpMessage message = _alertRuleRestClient.CreateTriggerRuleRunAlertRuleRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, Id.Parent.Name, Id.Name, AnalyticsRuleRunTrigger.ToRequestContent(analyticsRuleRunTriggerParameter), context);
                Response response = await Pipeline.ProcessMessageAsync(message, context).ConfigureAwait(false);
                SecurityInsightsArmOperation operation = new SecurityInsightsArmOperation(_alertRuleClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                {
                    await operation.WaitForCompletionResponseAsync(cancellationToken).ConfigureAwait(false);
                }
