...
namespace Azure.ResourceManager.NotificationHubs.Models
{
    public partial class NotificationHubCreateOrUpdateContent : IUtf8JsonSerializable, IJsonModel<NotificationHubCreateOrUpdateContent>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            {
                builder.Append("    name:");
                if (NotificationHubName.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
            ...
            if (Optional.IsDefined(RegistrationTtl))
            {
                ...
                var formattedTimeSpan = TypeFormatters.ToString(RegistrationTtl.Value, "P");
                builder.AppendLine($" '{formattedTimeSpan}'");
            }

            if (Optional.IsCollectionDefined(AuthorizationRules))
            {
                if (AuthorizationRules.Any())
                {
            ...
            if (Optional.IsDefined(ApnsCredential))
            {
                ...
                AppendChildObject(builder, ApnsCredential, options, 4, false);
            }

            if (Optional.IsDefined(WnsCredential))
            {
                builder.Append("    wnsCredential:");
                AppendChildObject(builder, WnsCredential, options, 4, false);
            }

            if (Optional.IsDefined(GcmCredential))
            {
                builder.Append("    gcmCredential:");
                AppendChildObject(builder, GcmCredential, options, 4, false);
            }

            if (Optional.IsDefined(MpnsCredential))
            {
                builder.Append("    mpnsCredential:");
                AppendChildObject(builder, MpnsCredential, options, 4, false);
            }

            if (Optional.IsDefined(AdmCredential))
            {
                builder.Append("    admCredential:");
                AppendChildObject(builder, AdmCredential, options, 4, false);
            }

            if (Optional.IsDefined(BaiduCredential))
            {
                builder.Append("    baiduCredential:");
                AppendChildObject(builder, BaiduCredential, options, 4, false);
            }
