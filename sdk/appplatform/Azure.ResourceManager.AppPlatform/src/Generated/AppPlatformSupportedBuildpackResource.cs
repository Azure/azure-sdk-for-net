...
namespace Azure.ResourceManager.AppPlatform
{
    ...
    public partial class AppPlatformSupportedBuildpackResource : ArmResource
    {
        ...
        private readonly AppPlatformSupportedBuildpackData _data;

        /// <summary> Initializes a new instance of the <see cref="AppPlatformSupportedBuildpackResource"/> class for mocking. </summary>
        ...
        internal AppPlatformSupportedBuildpackResource(ArmClient client, ResourceIdentifier id) : base(client, id)
        {
            _appPlatformSupportedBuildpackBuildServiceClientDiagnostics = new ClientDiagnostics("Azure.ResourceManager.AppPlatform", ResourceType.Namespace, Diagnostics);
            TryGetApiVersion(ResourceType, out string appPlatformSupportedBuildpackBuildServiceApiVersion);
        ...
        public virtual AppPlatformSupportedBuildpackData Data
        {
            get
            {
                if (!HasData)
        ...
        public virtual async Task<Response<AppPlatformSupportedBuildpackResource>> GetAsync(CancellationToken cancellationToken = default)
        {
            using var scope = _appPlatformSupportedBuildpackBuildServiceClientDiagnostics.CreateScope("AppPlatformSupportedBuildpackResource.Get");
            scope.Start();
            try
            {
                var response = await _appPlatformSupportedBuildpackBuildServiceRestClient.GetSupportedBuildpackAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Parent.Name, Id.Parent.Name, Id.Name, cancellationToken).ConfigureAwait(false);
        ...
        /// </summary>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        public virtual Response<AppPlatformSupportedBuildpackResource> Get(CancellationToken cancellationToken = default)
        {
            using var scope = _appPlatformSupportedBuildpackBuildServiceClientDiagnostics.CreateScope("AppPlatformSupportedBuildpackResource.Get");
            scope.Start();
            try
            {
                var response = _appPlatformSupportedBuildpackBuildServiceRestClient.GetSupportedBuildpack(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Parent.Name, Id.Parent.Name, Id.Name, cancellationToken);
                if (response.Value == null)
                    throw new RequestFailedException(response.GetRawResponse());
                return Response.FromValue(new AppPlatformSupportedBuildpackResource(Client, response.Value), response.GetRawResponse());
            }
