...
namespace Azure.ResourceManager.AppPlatform
{
    ...
    public partial class AppPlatformBuilderCollection : ArmCollection, IEnumerable<AppPlatformBuilderResource>, IAsyncEnumerable<AppPlatformBuilderResource>
    {
        ...
        {
            async Task<Page<AppPlatformBuilderResource>> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _appPlatformBuilderBuildServiceBuilderClientDiagnostics.CreateScope("AppPlatformBuilderCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _appPlatformBuilderBuildServiceBuilderRestClient.ListAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new AppPlatformBuilderResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
                    scope.Failed(e);
            ...
            async Task<Page<AppPlatformBuilderResource>> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _appPlatformBuilderBuildServiceBuilderClientDiagnostics.CreateScope("AppPlatformBuilderCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _appPlatformBuilderBuildServiceBuilderRestClient.ListNextPageAsync(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new AppPlatformBuilderResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
        ...
        {
            Page<AppPlatformBuilderResource> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _appPlatformBuilderBuildServiceBuilderClientDiagnostics.CreateScope("AppPlatformBuilderCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _appPlatformBuilderBuildServiceBuilderRestClient.List(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new AppPlatformBuilderResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            Page<AppPlatformBuilderResource> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _appPlatformBuilderBuildServiceBuilderClientDiagnostics.CreateScope("AppPlatformBuilderCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _appPlatformBuilderBuildServiceBuilderRestClient.ListNextPage(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new AppPlatformBuilderResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
