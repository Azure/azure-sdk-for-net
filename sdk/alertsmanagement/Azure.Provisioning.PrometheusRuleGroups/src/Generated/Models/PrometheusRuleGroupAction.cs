// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.Core;
using Azure.Provisioning;
using Azure.Provisioning.Primitives;

namespace Azure.Provisioning.PrometheusRuleGroups
{
    /// <summary> An alert action. Only relevant for alerts. </summary>
    public partial class PrometheusRuleGroupAction : ProvisionableConstruct
    {
        private BicepValue<ResourceIdentifier> _actionGroupId;
        private BicepDictionary<string> _actionProperties;

        /// <summary> Creates a new PrometheusRuleGroupAction. </summary>
        public PrometheusRuleGroupAction()
        {
        }

        /// <summary> Gets or sets the ActionGroupId. </summary>
        public BicepValue<ResourceIdentifier> ActionGroupId
        {
            get
            {
                Initialize();
                return _actionGroupId;
            }
            set
            {
                Initialize();
                _actionGroupId.Assign(value);
            }
        }

        /// <summary> Gets or sets the ActionProperties. </summary>
        public BicepDictionary<string> ActionProperties
        {
            get
            {
                Initialize();
                return _actionProperties;
            }
            set
            {
                Initialize();
                _actionProperties.Assign(value);
            }
        }

        /// <summary> Define all the provisionable properties for PrometheusRuleGroupAction. </summary>
        protected override void DefineProvisionableProperties()
        {
            base.DefineProvisionableProperties();
            _actionGroupId = DefineProperty<ResourceIdentifier>(nameof(ActionGroupId), new string[] { "actionGroupId" });
            _actionProperties = DefineDictionaryProperty<string>(nameof(ActionProperties), new string[] { "actionProperties" });
            DefineAdditionalProperties();
        }

        /// <summary> Define additional provisionable properties for PrometheusRuleGroupAction that are not part of the generated code. </summary>
        partial void DefineAdditionalProperties();
    }
}
