# Azure Provisioning PreviewAlertRule client library for .NET

Azure.Provisioning.PreviewAlertRule simplifies declarative resource provisioning in .NET.
...
```dotnetcli
dotnet add package Azure.Provisioning.PreviewAlertRule --prerelease
```
...
The Preview Alert Rule service currently exposes a provider action rather than a declarative ARM resource, so this package does not yet contain provisionable resource types.

## Examples

Provisioning examples will be added when the service exposes a declarative ARM resource.
