...
#pragma warning disable SA1649 // File name should match first type name
    internal class ProviderHubArmOperation<T> : ArmOperation<T>
#pragma warning restore SA1649 // File name should match first type name
    {
        private readonly OperationInternal<T> _operation;

        /// <summary> Initializes a new instance of ProviderHubArmOperation for mocking. </summary>
        protected ProviderHubArmOperation()
        {
        }

        internal ProviderHubArmOperation(Response<T> response)
        {
            _operation = OperationInternal<T>.Succeeded(response.GetRawResponse(), response.Value);
        }

        internal ProviderHubArmOperation(IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia, bool skipApiVersionOverride = false, string apiVersionOverrideValue = null)
        {
            ...
            _operation = new OperationInternal<T>(nextLinkOperation, clientDiagnostics, response, "ProviderHubArmOperation", fallbackStrategy: new SequentialDelayStrategy());
        }
