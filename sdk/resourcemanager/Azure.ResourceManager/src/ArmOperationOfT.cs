...
{
    /// <inheritdoc/>
    public class ArmOperation<T> : Operation<T> where T: IOperationSource<T>
    {
        ...
        {
            IOperationSource<T> source = (IOperationSource<T>)Activator.CreateInstance(typeof(T), BindingFlags.NonPublic | BindingFlags.Instance, null, typeof(ArmResource).IsAssignableFrom(typeof(T)) ? new object[]{client} : null, null);
