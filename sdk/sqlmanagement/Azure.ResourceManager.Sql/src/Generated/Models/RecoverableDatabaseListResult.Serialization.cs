// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;
using System.Text.Json;
using Azure.Core;
using Azure.Core.Serialization;
using Azure.ResourceManager.Sql;

namespace Azure.ResourceManager.Sql.Models
{
    internal partial class RecoverableDatabaseListResult
    {
        internal static RecoverableDatabaseListResult DeserializeRecoverableDatabaseListResult(JsonElement element, Core.Serialization.SerializableOptions options = default)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            IReadOnlyList<RecoverableDatabaseData> value = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("value"u8))
                {
                    List<RecoverableDatabaseData> array = new List<RecoverableDatabaseData>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(RecoverableDatabaseData.DeserializeRecoverableDatabaseData(item));
                    }
                    value = array;
                    continue;
                }
            }
            return new RecoverableDatabaseListResult(value);
        }
    }
}
