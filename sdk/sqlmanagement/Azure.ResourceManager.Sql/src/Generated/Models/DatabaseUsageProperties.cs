// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.Sql;

namespace Azure.ResourceManager.Sql.Models
{
    /// <summary> Properties of a database usage. </summary>
    internal partial class DatabaseUsageProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="DatabaseUsageProperties"/>. </summary>
        internal DatabaseUsageProperties()
        {
        }

        /// <summary> Initializes a new instance of <see cref="DatabaseUsageProperties"/>. </summary>
        /// <param name="displayName"> User-readable name of the metric. </param>
        /// <param name="currentValue"> Current value of the metric. </param>
        /// <param name="limit"> Boundary value of the metric. </param>
        /// <param name="unit"> Unit of the metric. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal DatabaseUsageProperties(string displayName, double? currentValue, double? limit, string unit, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            DisplayName = displayName;
            CurrentValue = currentValue;
            Limit = limit;
            Unit = unit;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> User-readable name of the metric. </summary>
        [WirePath("displayName")]
        public string DisplayName { get; }

        /// <summary> Current value of the metric. </summary>
        [WirePath("currentValue")]
        public double? CurrentValue { get; }

        /// <summary> Boundary value of the metric. </summary>
        [WirePath("limit")]
        public double? Limit { get; }

        /// <summary> Unit of the metric. </summary>
        [WirePath("unit")]
        public string Unit { get; }
    }
}
