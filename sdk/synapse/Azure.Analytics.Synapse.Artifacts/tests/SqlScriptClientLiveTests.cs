    ...
    public class SqlScriptClientLiveTests : RecordedTestBase<SynapseTestEnvironment>
    {
        internal class DisposableSqlScript : IAsyncDisposable
        {
            ...
            public static async ValueTask<SqlScriptResource> CreateResource (SqlScriptClient client, TestRecording recording)
            {
                ...
                SqlConnection connect = new SqlConnection (SqlConnectionType.SqlPool, "connectionName", "tcp:nonexistant.sql.azuresynapse.net,1433", "nonexistant", new Dictionary<string, object> {{ "User ID", "user" }, { "Password", "password" }, { "Encrypt", "rue" }, { "TrustServerCertificate", "False" }, { "Connection Timeout", "30" }});
