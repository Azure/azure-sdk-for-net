...
namespace Microsoft.Azure.Management.Synapse.Tests
{
    public class SqlpoolOperationTests : SynapseManagementTestBase
    {
        ...
        [Fact]
        public void TestSqlPoolV3LifeCycle()
        {
            TestInitialize();
            ...
            // create sqlpool
            string sqlpoolName = TestUtilities.GenerateName("sqlpool");
            var createSqlpoolParams = CommonData.PrepareSqlPoolV3CreateParams();
            var sqlpoolCreate = SynapseSqlV3Client.SqlPoolsV3.CreateOrUpdate(CommonData.ResourceGroupName, workspaceName, sqlpoolName, createSqlpoolParams);
            Assert.Equal(CommonTestFixture.SqlpoolType, sqlpoolCreate.Type);
            Assert.Equal(sqlpoolName, sqlpoolCreate.Name);
            Assert.Equal(CommonData.Location, sqlpoolCreate.Location);
            ...
            for (int i = 0; i < 60; i++)
            {
                var sqlpoolGet = SynapseSqlV3Client.SqlPoolsV3.Get(CommonData.ResourceGroupName, workspaceName, sqlpoolName);
                if (sqlpoolGet.Status.Equals("Online"))
                {
                    Assert.Equal(CommonTestFixture.SqlpoolType, sqlpoolCreate.Type);
                    Assert.Equal(sqlpoolName, sqlpoolCreate.Name);
                    Assert.Equal(CommonData.Location, sqlpoolCreate.Location);
            ...
            // update sqlpool
            Dictionary<string, string> tagsToUpdate = new Dictionary<string, string> { { "TestTag", "TestUpdate" } };
            SqlPoolUpdate sqlPoolV3PatchInfo = new SqlPoolUpdate
            {
                Tags = tagsToUpdate,
                Sku = sqlpoolCreate.Sku
            };

            SynapseSqlV3Client.SqlPoolsV3.Update(CommonData.ResourceGroupName, workspaceName, sqlpoolName, sqlPoolV3PatchInfo);
            if (IsRecordMode)
            {
                Thread.Sleep(30000);
            }

            var sqlpoolUpdate = SynapseSqlV3Client.SqlPoolsV3.Get(CommonData.ResourceGroupName, workspaceName, sqlpoolName);
            Assert.NotNull(sqlpoolUpdate.Tags);
            Assert.Equal("TestUpdate", sqlpoolUpdate.Tags["TestTag"]);

            // list sqlpool from workspace
            var firstPage = SynapseSqlV3Client.SqlPoolsV3.ListByWorkspace(CommonData.ResourceGroupName, workspaceName);
            var sqlpoolFromWorkspace = SynapseManagementTestUtilities.ListResources(firstPage, SynapseSqlV3Client.SqlPoolsV3.ListByWorkspaceNext);
            Assert.True(1 <= sqlpoolFromWorkspace.Count);
            bool isFound = false;
            int sqlpoolCount = sqlpoolFromWorkspace.Count;
            for (int i = 0; i < sqlpoolCount; i++)
            ...
            // delete sqlpool
            SynapseSqlV3Client.SqlPoolsV3.Delete(CommonData.ResourceGroupName, workspaceName, sqlpoolName);
            firstPage = SynapseSqlV3Client.SqlPoolsV3.ListByWorkspace(CommonData.ResourceGroupName, workspaceName);
            var sqlpoolAfterDelete = SynapseManagementTestUtilities.ListResources(firstPage, SynapseSqlV3Client.SqlPoolsV3.ListByWorkspaceNext);
            Assert.True(sqlpoolCount - 1 == sqlpoolAfterDelete.Count);
        }
    }
}
