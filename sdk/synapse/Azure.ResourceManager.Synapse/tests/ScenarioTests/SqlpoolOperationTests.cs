    ...
    public class SqlpoolOperationTests : SynapseManagementTestBase
    {
        ...
        public async Task TestSqlPoolLifeCycle()
        {
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                Thread.Sleep(30000);
                Assert.That(i, Is.LessThan(60), "Synapse SqlPool is not in succeeded state even after 30 min.");
            }
            ...
            Assert.That(sqlpool, Is.Not.EqualTo(null), string.Format("sql pool created earlier is not found when listing all in workspace {0}", workspaceName));

            // delete sqlpool
            await sqlpool.DeleteAsync(WaitUntil.Completed);
            var sqlPoolList = await sqlPoolCollection.GetAllAsync().ToEnumerableAsync();
            Assert.That(sqlPoolList, Has.Count.EqualTo(sqlpoolCount - 1));
        }
    }
}
