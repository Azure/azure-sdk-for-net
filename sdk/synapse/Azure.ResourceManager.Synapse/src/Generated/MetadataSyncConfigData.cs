// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.Core;
using Azure.ResourceManager.Models;

namespace Azure.ResourceManager.Synapse
{
    /// <summary> A class representing the MetadataSyncConfig data model. </summary>
    public partial class MetadataSyncConfigData : ResourceData
    {
        /// <summary> Initializes a new instance of MetadataSyncConfigData. </summary>
        public MetadataSyncConfigData()
        {
        }

        /// <summary> Initializes a new instance of MetadataSyncConfigData. </summary>
        /// <param name="id"> The id. </param>
        /// <param name="name"> The name. </param>
        /// <param name="resourceType"> The resourceType. </param>
        /// <param name="systemData"> The systemData. </param>
        /// <param name="enabled"> Indicates whether the metadata sync is enabled or disabled. </param>
        /// <param name="syncIntervalInMinutes"> The Sync Interval in minutes. </param>
        internal MetadataSyncConfigData(ResourceIdentifier id, string name, ResourceType resourceType, SystemData systemData, bool? enabled, int? syncIntervalInMinutes) : base(id, name, resourceType, systemData)
        {
            Enabled = enabled;
            SyncIntervalInMinutes = syncIntervalInMinutes;
        }

        /// <summary> Indicates whether the metadata sync is enabled or disabled. </summary>
        public bool? Enabled { get; set; }
        /// <summary> The Sync Interval in minutes. </summary>
        public int? SyncIntervalInMinutes { get; set; }
    }
}
