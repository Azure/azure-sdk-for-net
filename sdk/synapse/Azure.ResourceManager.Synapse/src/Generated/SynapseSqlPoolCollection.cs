...
namespace Azure.ResourceManager.Synapse
{
    ...
    public partial class SynapseSqlPoolCollection : ArmCollection, IEnumerable<SynapseSqlPoolResource>, IAsyncEnumerable<SynapseSqlPoolResource>
    {
        ...
        {
            async Task<Page<SynapseSqlPoolResource>> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _synapseSqlPoolSqlPoolsClientDiagnostics.CreateScope("SynapseSqlPoolCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _synapseSqlPoolSqlPoolsRestClient.ListByWorkspaceAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new SynapseSqlPoolResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
                    scope.Failed(e);
            ...
            async Task<Page<SynapseSqlPoolResource>> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _synapseSqlPoolSqlPoolsClientDiagnostics.CreateScope("SynapseSqlPoolCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _synapseSqlPoolSqlPoolsRestClient.ListByWorkspaceNextPageAsync(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new SynapseSqlPoolResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
        ...
        {
            Page<SynapseSqlPoolResource> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _synapseSqlPoolSqlPoolsClientDiagnostics.CreateScope("SynapseSqlPoolCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _synapseSqlPoolSqlPoolsRestClient.ListByWorkspace(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new SynapseSqlPoolResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            Page<SynapseSqlPoolResource> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _synapseSqlPoolSqlPoolsClientDiagnostics.CreateScope("SynapseSqlPoolCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _synapseSqlPoolSqlPoolsRestClient.ListByWorkspaceNextPage(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new SynapseSqlPoolResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
