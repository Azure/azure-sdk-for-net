...
{
    public partial class SparkSession : IUtf8JsonSerializable, IJsonModel<SparkSession>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<SparkSession>)this).Write(writer, new ModelReaderWriterOptions("W"));

        void IJsonModel<SparkSession>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<SparkSession>)this).GetFormatFromOptions(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<SparkSession>)} interface");
            }
        ...
        SparkSession IJsonModel<SparkSession>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            bool isValid = options.Format == "J" || options.Format == "W";
            if (!isValid)
            {
                throw new FormatException($"The model {nameof(SparkSession)} does not support '{options.Format}' format.");
            }

            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeSparkSession(document.RootElement, options);
        }

        internal static SparkSession DeserializeSparkSession(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            return new SparkSession(livyInfo.Value, name.Value, workspaceName.Value, sparkPoolName.Value, submitterName.Value, submitterId.Value, artifactId.Value, Optional.ToNullable(jobType), Optional.ToNullable(result), schedulerInfo.Value, pluginInfo.Value, Optional.ToList(errorInfo), Optional.ToDictionary(tags), id, appId.Value, Optional.ToDictionary(appInfo), Optional.ToNullable(state), Optional.ToList(log), serializedAdditionalRawData);
        }

        BinaryData IPersistableModel<SparkSession>.Write(ModelReaderWriterOptions options)
        {
            bool isValid = options.Format == "J" || options.Format == "W";
            if (!isValid)
            {
        ...
        SparkSession IPersistableModel<SparkSession>.Create(BinaryData data, ModelReaderWriterOptions options)
        {
            bool isValid = options.Format == "J" || options.Format == "W";
            if (!isValid)
            {
        ...
        string IPersistableModel<SparkSession>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
