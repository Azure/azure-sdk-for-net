// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.PureStorageBlock
{
    /// <summary></summary>
    internal partial class PureStoragePoolResourceOperationSource : IOperationSource<PureStoragePoolResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal PureStoragePoolResourceOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        PureStoragePoolResource IOperationSource<PureStoragePoolResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            PureStoragePoolData data = PureStoragePoolData.DeserializePureStoragePoolData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new PureStoragePoolResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<PureStoragePoolResource> IOperationSource<PureStoragePoolResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            PureStoragePoolData data = PureStoragePoolData.DeserializePureStoragePoolData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new PureStoragePoolResource(_client, data);
        }
    }
}
