// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.ContainerServiceFleet
{
    /// <summary></summary>
    internal partial class FleetUpdateStrategyResourceOperationSource : IOperationSource<FleetUpdateStrategyResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal FleetUpdateStrategyResourceOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        FleetUpdateStrategyResource IOperationSource<FleetUpdateStrategyResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            FleetUpdateStrategyData data = FleetUpdateStrategyData.DeserializeFleetUpdateStrategyData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new FleetUpdateStrategyResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<FleetUpdateStrategyResource> IOperationSource<FleetUpdateStrategyResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            FleetUpdateStrategyData data = FleetUpdateStrategyData.DeserializeFleetUpdateStrategyData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new FleetUpdateStrategyResource(_client, data);
        }
    }
}
