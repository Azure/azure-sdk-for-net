...
{
    public partial class FrontDoorTimeSeriesInfo : IUtf8JsonSerializable, IModelJsonSerializable<FrontDoorTimeSeriesInfo>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IModelJsonSerializable<FrontDoorTimeSeriesInfo>)this).Serialize(writer, ModelSerializerOptions.DefaultWireOptions);

        void IModelJsonSerializable<FrontDoorTimeSeriesInfo>.Serialize(Utf8JsonWriter writer, ModelSerializerOptions options)
        {
            Core.ModelSerializerHelper.ValidateFormat<FrontDoorTimeSeriesInfo>(this, options.Format);
            ...
            if (Optional.IsCollectionDefined(TimeSeriesData))
            {
                ...
                foreach (var item in TimeSeriesData)
                {
                    ...
                    {
                        ((IModelJsonSerializable<FrontDoorTimeSeriesDataPoint>)item).Serialize(writer, options);
                    }
                }
        ...
        internal static FrontDoorTimeSeriesInfo DeserializeFrontDoorTimeSeriesInfo(JsonElement element, ModelSerializerOptions options = default)
        {
            ...
            return new FrontDoorTimeSeriesInfo(id, name, type, systemData.Value, Optional.ToDictionary(tags), location, endpoint.Value, Optional.ToNullable(startDateTimeUtc), Optional.ToNullable(endDateTimeUtc), Optional.ToNullable(aggregationInterval), Optional.ToNullable(timeSeriesType), country.Value, Optional.ToList(timeSeriesData), rawData);
        }

        FrontDoorTimeSeriesInfo IModelJsonSerializable<FrontDoorTimeSeriesInfo>.Deserialize(ref Utf8JsonReader reader, ModelSerializerOptions options)
        {
            Core.ModelSerializerHelper.ValidateFormat<FrontDoorTimeSeriesInfo>(this, options.Format);

            using var doc = JsonDocument.ParseValue(ref reader);
            return DeserializeFrontDoorTimeSeriesInfo(doc.RootElement, options);
        }

        BinaryData IModelSerializable<FrontDoorTimeSeriesInfo>.Serialize(ModelSerializerOptions options)
        {
            Core.ModelSerializerHelper.ValidateFormat<FrontDoorTimeSeriesInfo>(this, options.Format);
        ...
        FrontDoorTimeSeriesInfo IModelSerializable<FrontDoorTimeSeriesInfo>.Deserialize(BinaryData data, ModelSerializerOptions options)
        {
            Core.ModelSerializerHelper.ValidateFormat<FrontDoorTimeSeriesInfo>(this, options.Format);
        ...
        /// <summary> Converts a <see cref="FrontDoorTimeSeriesInfo"/> into a <see cref="RequestContent"/>. </summary>
        /// <param name="model"> The <see cref="FrontDoorTimeSeriesInfo"/> to convert. </param>
        public static implicit operator RequestContent(FrontDoorTimeSeriesInfo model)
        {
            if (model is null)
            {
                return null;
        ...
        /// <summary> Converts a <see cref="Response"/> into a <see cref="FrontDoorTimeSeriesInfo"/>. </summary>
        /// <param name="response"> The <see cref="Response"/> to convert. </param>
        public static explicit operator FrontDoorTimeSeriesInfo(Response response)
        {
            if (response is null)
            {
                return null;
            ...
            return DeserializeFrontDoorTimeSeriesInfo(doc.RootElement, ModelSerializerOptions.DefaultWireOptions);
        }
    }
}
