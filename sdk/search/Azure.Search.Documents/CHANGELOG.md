...
### Features Added
- Added support for `Vectorizers`, which contains configuration options for vectorizing text vector queries, and `Profiles`, which define combinations of configurations to use with vector search within `VectorSearch`.
- Added the `VectorQueries` base type, which is used for the query parameters for vector and hybrid search queries. Derived classes include `VectorizableTextQuery` and `RawVectorQuery`. With `RawVectorQuery`, users can pass raw vector values for vector search, while `VectorizableTextQuery` allows the passing of text values to be vectorized for vector search.
