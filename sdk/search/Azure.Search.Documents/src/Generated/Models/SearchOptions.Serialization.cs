...
namespace Azure.Search.Documents
{
    public partial class SearchOptions : IUtf8JsonSerializable
    {
        ...
        internal static SearchOptions DeserializeSearchOptions(JsonElement element)
        {
            ...
            return new SearchOptions(count.HasValue ? count.Value : (bool?)null, new ChangeTrackingList<string>(facets), filter.HasValue ? filter.Value : null, highlight.HasValue ? highlight.Value : null, highlightPostTag.HasValue ? highlightPostTag.Value : null, highlightPreTag.HasValue ? highlightPreTag.Value : null, minimumCoverage.HasValue ? minimumCoverage.Value : (double?)null, orderby.HasValue ? orderby.Value : null, queryType.HasValue ? queryType.Value : (SearchQueryType?)null, new ChangeTrackingList<string>(scoringParameters), scoringProfile.HasValue ? scoringProfile.Value : null, search.HasValue ? search.Value : null, searchFields.HasValue ? searchFields.Value : null, searchMode.HasValue ? searchMode.Value : (SearchMode?)null, select.HasValue ? select.Value : null, skip.HasValue ? skip.Value : (int?)null, top.HasValue ? top.Value : (int?)null);
        }
    }
}
