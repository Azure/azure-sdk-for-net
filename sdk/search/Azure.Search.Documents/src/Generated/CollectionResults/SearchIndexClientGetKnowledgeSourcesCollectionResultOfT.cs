...
namespace Azure.Search.Documents.Indexes
{
    internal partial class SearchIndexClientGetKnowledgeSourcesCollectionResultOfT : Pageable<KnowledgeSource>
    {
        ...
        public SearchIndexClientGetKnowledgeSourcesCollectionResultOfT(SearchIndexClient client, string search, int? pageSize, string searchType, RequestContext context, string diagnosticScope) : base(context?.CancellationToken ?? default)
        {
            _client = client;
            _search = search;
            _pageSize = pageSize;
        ...
        public override IEnumerable<Page<KnowledgeSource>> AsPages(string continuationToken, int? pageSizeHint)
        {
            ...
            while (true)
            {
                ...
                yield return Page<KnowledgeSource>.FromValues((IReadOnlyList<KnowledgeSource>)result.Value, nextPage?.IsAbsoluteUri == true ? nextPage.AbsoluteUri : nextPage?.OriginalString, response);
                if (nextPage == null)
                {
                    yield break;
                }
