...
{
    internal partial class SearchIndexClientGetAliasesAsyncCollectionResultOfT : AsyncPageable<SearchAlias>
    {
        ...
        public SearchIndexClientGetAliasesAsyncCollectionResultOfT(SearchIndexClient client, string search, int? pageSize, string searchType, RequestContext context, string diagnosticScope) : base(context?.CancellationToken ?? default)
        {
            _client = client;
            _search = search;
            _pageSize = pageSize;
        ...
        /// <returns> The pages of SearchIndexClientGetAliasesAsyncCollectionResultOfT as an enumerable collection. </returns>
        public override async IAsyncEnumerable<Page<SearchAlias>> AsPages(string continuationToken, int? pageSizeHint)
        {
            ...
            while (true)
            {
                ...
                yield return Page<SearchAlias>.FromValues(result.Value, nextPage?.IsAbsoluteUri == true ? nextPage.AbsoluteUri : nextPage?.OriginalString, response);
                if (nextPage == null)
                {
                    yield break;
                }
        ...
        {
            HttpMessage message = nextLink != null ? _client.CreateNextGetAliasesRequest(nextLink, _search, _pageSize, _searchType, _context) : _client.CreateGetAliasesRequest(_search, _pageSize, _searchType, _context);
