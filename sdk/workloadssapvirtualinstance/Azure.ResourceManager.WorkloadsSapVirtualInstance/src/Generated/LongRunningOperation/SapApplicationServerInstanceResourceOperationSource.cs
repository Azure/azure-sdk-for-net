// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.WorkloadsSapVirtualInstance
{
    /// <summary></summary>
    internal partial class SapApplicationServerInstanceResourceOperationSource : IOperationSource<SapApplicationServerInstanceResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal SapApplicationServerInstanceResourceOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        SapApplicationServerInstanceResource IOperationSource<SapApplicationServerInstanceResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            SapApplicationServerInstanceData data = SapApplicationServerInstanceData.DeserializeSapApplicationServerInstanceData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new SapApplicationServerInstanceResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<SapApplicationServerInstanceResource> IOperationSource<SapApplicationServerInstanceResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            SapApplicationServerInstanceData data = SapApplicationServerInstanceData.DeserializeSapApplicationServerInstanceData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new SapApplicationServerInstanceResource(_client, data);
        }
    }
}
