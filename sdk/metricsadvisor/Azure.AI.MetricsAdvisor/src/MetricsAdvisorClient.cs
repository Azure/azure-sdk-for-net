    ...
    public partial class MetricsAdvisorClient
    {
        ...
        public virtual async Task<Response<MetricFeedback>> AddFeedbackAsync(MetricFeedback feedback, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var headers = new MicrosoftAzureMetricsAdvisorRestAPIOpenAPIV2CreateMetricFeedbackHeaders(response);
                ResponseWithHeaders<MicrosoftAzureMetricsAdvisorRestAPIOpenAPIV2CreateMetricFeedbackHeaders> responseWithHeaders = ResponseWithHeaders.FromValue(headers, response);

                string feedbackId = ClientCommon.GetFeedbackId(responseWithHeaders.Headers.Location);

                try
                {
                    var addedFeedback = await GetFeedbackAsync(feedbackId, cancellationToken).ConfigureAwait(false);

                    return Response.FromValue(addedFeedback, responseWithHeaders.GetRawResponse());
                }
                catch (Exception ex)
                {
