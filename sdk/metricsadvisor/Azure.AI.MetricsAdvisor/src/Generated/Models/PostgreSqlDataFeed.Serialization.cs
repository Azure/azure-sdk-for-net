...
{
    internal partial class PostgreSqlDataFeed : Core.IUtf8JsonSerializable
    {
        ...
        internal static PostgreSqlDataFeed DeserializePostgreSqlDataFeed(JsonElement element)
        {
            ...
            return new PostgreSqlDataFeed(dataSourceType, dataFeedId.Value, dataFeedName, dataFeedDescription.Value, granularityName, Core.Optional.ToNullable(granularityAmount), metrics, Core.Optional.ToList(dimension), timestampColumn.Value, dataStartFrom, Core.Optional.ToNullable(startOffsetInSeconds), Core.Optional.ToNullable(maxConcurrency), Core.Optional.ToNullable(minRetryIntervalInSeconds), Core.Optional.ToNullable(stopRetryAfterInSeconds), Core.Optional.ToNullable(needRollup), Core.Optional.ToNullable(rollUpMethod), Core.Optional.ToList(rollUpColumns), allUpIdentification.Value, Core.Optional.ToNullable(fillMissingPointType), Core.Optional.ToNullable(fillMissingPointValue), Core.Optional.ToNullable(viewMode), Core.Optional.ToList(admins), Core.Optional.ToList(viewers), Core.Optional.ToNullable(isAdmin), creator.Value, Core.Optional.ToNullable(status), Core.Optional.ToNullable(createdTime), actionLinkTemplate.Value, Core.Optional.ToNullable(authenticationType), credentialId.Value, dataSourceParameter);
        }
    }
}
