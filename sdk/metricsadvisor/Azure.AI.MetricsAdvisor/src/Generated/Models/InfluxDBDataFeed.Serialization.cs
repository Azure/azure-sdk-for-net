...
namespace Azure.AI.MetricsAdvisor.Models
{
    internal partial class InfluxDBDataFeed : IUtf8JsonSerializable
    {
        ...
        internal static InfluxDBDataFeed DeserializeInfluxDBDataFeed(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            InfluxDBParameter dataSourceParameter = default;
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                {
                    dataSourceParameter = InfluxDBParameter.DeserializeInfluxDBParameter(property.Value);
                    continue;
                }
            ...
            return new InfluxDBDataFeed(dataSourceType, dataFeedId.Value, dataFeedName, dataFeedDescription.Value, granularityName, Optional.ToNullable(granularityAmount), metrics, Optional.ToList(dimension), timestampColumn.Value, dataStartFrom, Optional.ToNullable(startOffsetInSeconds), Optional.ToNullable(maxConcurrency), Optional.ToNullable(minRetryIntervalInSeconds), Optional.ToNullable(stopRetryAfterInSeconds), Optional.ToNullable(needRollup), Optional.ToNullable(rollUpMethod), Optional.ToList(rollUpColumns), allUpIdentification.Value, Optional.ToNullable(fillMissingPointType), Optional.ToNullable(fillMissingPointValue), Optional.ToNullable(viewMode), Optional.ToList(admins), Optional.ToList(viewers), Optional.ToNullable(isAdmin), creator.Value, Optional.ToNullable(status), Optional.ToNullable(createdTime), actionLinkTemplate.Value, Optional.ToNullable(authenticationType), credentialId.Value, dataSourceParameter);
        }
    }
}
