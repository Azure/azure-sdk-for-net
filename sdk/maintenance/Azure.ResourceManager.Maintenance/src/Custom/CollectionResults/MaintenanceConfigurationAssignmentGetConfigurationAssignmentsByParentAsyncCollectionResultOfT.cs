...
{
    internal partial class MaintenanceConfigurationAssignmentGetConfigurationAssignmentsByParentAsyncCollectionResultOfT : AsyncPageable<MaintenanceConfigurationAssignmentData>
    {
        ...
        public MaintenanceConfigurationAssignmentGetConfigurationAssignmentsByParentAsyncCollectionResultOfT(MaintenanceConfigurationAssignment client, Guid subscriptionId, string resourceGroupName, string providerName, string resourceParentType, string resourceParentName, string resourceType, string resourceName, RequestContext context, string diagnosticScope) : base(context?.CancellationToken ?? default)
        {
            _client = client;
        ...
        public override async IAsyncEnumerable<Page<MaintenanceConfigurationAssignmentData>> AsPages(string continuationToken, int? pageSizeHint)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    response = await _client.Pipeline.ProcessMessageAsync(message, _context).ConfigureAwait(false);
                }
                catch (Exception e)
                {
                    scope.Failed(e);
