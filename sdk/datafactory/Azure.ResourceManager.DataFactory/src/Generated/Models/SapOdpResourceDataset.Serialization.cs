...
#else
            JsonSerializer.Serialize(writer, JsonDocument.Parse(Context.ToString()).RootElement);
#endif
            writer.WritePropertyName("objectName"u8);
#if NET6_0_OR_GREATER
				writer.WriteRawValue(ObjectName);
#else
            JsonSerializer.Serialize(writer, JsonDocument.Parse(ObjectName.ToString()).RootElement);
#endif
            writer.WriteEndObject();
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("typeProperties"u8))
                {
                    ...
                    foreach (var property0 in property.Value.EnumerateObject())
                    {
                        ...
                        if (property0.NameEquals("objectName"u8))
                        {
                            objectName = BinaryData.FromString(property0.Value.GetRawText());
                            continue;
                        }
                    }
                    continue;
