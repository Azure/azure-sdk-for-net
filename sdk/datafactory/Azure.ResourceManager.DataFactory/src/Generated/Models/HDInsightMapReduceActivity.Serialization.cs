...
namespace Azure.ResourceManager.DataFactory.Models
{
    public partial class HDInsightMapReduceActivity : IUtf8JsonSerializable, IJsonModel<HDInsightMapReduceActivity>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(JarFilePath), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("    jarFilePath: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsDefined(JarFilePath))
                {
                    builder.Append("    jarFilePath: ");
                    builder.AppendLine($"'{JarFilePath.ToString()}'");
                }
            }

            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(JarLinkedService), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("    jarLinkedService: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                ...
                {
                    builder.Append("    jarLinkedService: ");
                    BicepSerializationHelpers.AppendChildObject(builder, JarLinkedService, options, 4, false, "    jarLinkedService: ");
                }
            }

            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(JarLibs), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("    jarLibs: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
