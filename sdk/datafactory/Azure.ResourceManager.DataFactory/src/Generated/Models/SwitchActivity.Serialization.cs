    ...
    public partial class SwitchActivity : IUtf8JsonSerializable, IJsonModel<SwitchActivity>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            else
            {
                ...
                {
                    builder.Append("    on: ");
                    BicepSerializationHelpers.AppendChildObject(builder, On, options, 4, false, "    on: ");
                }
            }

            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(Cases), out propertyOverride);
            if (hasPropertyOverride)
            {
            ...
            else
            {
                if (Optional.IsCollectionDefined(Cases))
                {
                    if (Cases.Any())
                    {
                        ...
                        foreach (var item in Cases)
                        {
                            BicepSerializationHelpers.AppendChildObject(builder, item, options, 6, true, "    cases: ");
                        }
                        builder.AppendLine("    ]");
                    }
                }
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(DefaultActivities), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("    defaultActivities: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsCollectionDefined(DefaultActivities))
                {
                    if (DefaultActivities.Any())
                    {
                        builder.Append("    defaultActivities: ");
                        builder.AppendLine("[");
                        foreach (var item in DefaultActivities)
                        {
                            BicepSerializationHelpers.AppendChildObject(builder, item, options, 6, true, "    defaultActivities: ");
                        }
                        builder.AppendLine("    ]");
                    }
                }
