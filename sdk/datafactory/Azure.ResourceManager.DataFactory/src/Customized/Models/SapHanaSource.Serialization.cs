...
namespace Azure.ResourceManager.DataFactory.Models
{
    ...
    [Microsoft.TypeSpec.Generator.Customizations.CodeGenSuppress("DeserializeSapHanaSource", typeof(JsonElement), typeof(ModelReaderWriterOptions))]
    public partial class SapHanaSource
    {
        internal static SapHanaSource DeserializeSapHanaSource(JsonElement element, ModelReaderWriterOptions options)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            string type = "SapHanaSource";
            ...
            foreach (var prop in element.EnumerateObject())
            {
                ...
                if (prop.NameEquals("partitionSettings"u8))
                {
                    ...
                    partitionSettings = SapHanaPartitionSettings.DeserializeSapHanaPartitionSettings(prop.Value, options);
                    continue;
                }
                if (options.Format != "W")
                {
