...
namespace Azure.ResourceManager.DataFactory.Models
{
    ...
    [Microsoft.TypeSpec.Generator.Customizations.CodeGenSuppress("DeserializeExecutePipelineActivity", typeof(JsonElement), typeof(ModelReaderWriterOptions))]
    public partial class ExecutePipelineActivity
    {
        internal static ExecutePipelineActivity DeserializeExecutePipelineActivity(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            ExecutePipelineActivityTypeProperties typeProperties = default;
            foreach (var prop in element.EnumerateObject())
            {
                ...
                if (prop.NameEquals("policy"u8))
                {
                    ...
                    policy = ExecutePipelineActivityPolicy.DeserializeExecutePipelineActivityPolicy(prop.Value, options);
                    continue;
                }
                if (prop.NameEquals("typeProperties"u8))
                {
                    typeProperties = ExecutePipelineActivityTypeProperties.DeserializeExecutePipelineActivityTypeProperties(prop.Value, options);
                    continue;
                }
                if (options.Format != "W")
                {
