...
namespace Azure.ResourceManager.DataFactory.Models
{
    ...
    [Microsoft.TypeSpec.Generator.Customizations.CodeGenSuppress("DeserializeSapTableSource", typeof(JsonElement), typeof(ModelReaderWriterOptions))]
    public partial class SapTableSource
    {
        internal static SapTableSource DeserializeSapTableSource(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            foreach (var prop in element.EnumerateObject())
            {
                ...
                {
                    ReadRowCount(prop, ref rowCount);
                    continue;
                }
                if (prop.NameEquals("rowSkips"u8))
                {
                    ReadRowSkips(prop, ref rowSkips);
                    continue;
                }
                if (prop.NameEquals("rfcTableFields"u8))
                {
                    ReadRfcTableFields(prop, ref rfcTableFields);
                    continue;
                }
                if (prop.NameEquals("rfcTableOptions"u8))
                {
                    ReadRfcTableOptions(prop, ref rfcTableOptions);
                    continue;
                }
