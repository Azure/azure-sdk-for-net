// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.ContainerServiceAIManager.Models
{
    /// <summary> Platform-resolved specification of a custom model. Extends `ModelSpec` with custom model-specific metadata. All fields are read-only. Reserved so custom-model-specific fields can be added without changing the SDK surface. </summary>
    public partial class CustomAIModelSpec
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="CustomAIModelSpec"/>. </summary>
        internal CustomAIModelSpec()
        {
        }

        /// <summary> Initializes a new instance of <see cref="CustomAIModelSpec"/>. </summary>
        /// <param name="license"> The license of the model, when known. SPDX license identifier, e.g. `mit`, `apache-2.0`. </param>
        /// <param name="isRestricted"> Whether access to the model is restricted and requires credential. </param>
        /// <param name="maxContextLength"> The maximum context length supported by the model, in tokens. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal CustomAIModelSpec(string license, bool isRestricted, int maxContextLength, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            License = license;
            IsRestricted = isRestricted;
            MaxContextLength = maxContextLength;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The license of the model, when known. SPDX license identifier, e.g. `mit`, `apache-2.0`. </summary>
        public string License { get; }

        /// <summary> Whether access to the model is restricted and requires credential. </summary>
        public bool IsRestricted { get; }

        /// <summary> The maximum context length supported by the model, in tokens. </summary>
        public int MaxContextLength { get; }
    }
}
