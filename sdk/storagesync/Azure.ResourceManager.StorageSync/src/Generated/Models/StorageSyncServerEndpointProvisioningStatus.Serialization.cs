    ...
    public partial class StorageSyncServerEndpointProvisioningStatus : IJsonModel<StorageSyncServerEndpointProvisioningStatus>
    {
        ...
        protected virtual void JsonModelWriteCore(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            ...
            if (options.Format != "W" && Optional.IsCollectionDefined(ProvisioningStepStatuses))
            {
                ...
                foreach (ServerEndpointProvisioningStepStatus item in ProvisioningStepStatuses)
                {
        ...
        /// <param name="options"> The client options for reading and writing models. </param>
        StorageSyncServerEndpointProvisioningStatus IJsonModel<StorageSyncServerEndpointProvisioningStatus>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options) => JsonModelCreateCore(ref reader, options);

        /// <param name="reader"> The JSON reader. </param>
        /// <param name="options"> The client options for reading and writing models. </param>
        protected virtual StorageSyncServerEndpointProvisioningStatus JsonModelCreateCore(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<StorageSyncServerEndpointProvisioningStatus>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        /// <param name="options"> The client options for reading and writing models. </param>
        internal static StorageSyncServerEndpointProvisioningStatus DeserializeStorageSyncServerEndpointProvisioningStatus(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            foreach (var prop in element.EnumerateObject())
            {
                if (prop.NameEquals("provisioningStatus"u8))
                {
                    ...
                    provisioningStatus = new StorageSyncServerProvisioningStatus(prop.Value.GetString());
                    continue;
                }
                if (prop.NameEquals("provisioningType"u8))
                {
                    provisioningType = prop.Value.GetString();
                    continue;
                }
                if (prop.NameEquals("provisioningStepStatuses"u8))
                {
                    if (prop.Value.ValueKind == JsonValueKind.Null)
                    {
                        continue;
            ...
            return new StorageSyncServerEndpointProvisioningStatus(provisioningStatus, provisioningType, provisioningStepStatuses ?? new ChangeTrackingList<ServerEndpointProvisioningStepStatus>(), additionalBinaryDataProperties);
        }

        /// <param name="options"> The client options for reading and writing models. </param>
        BinaryData IPersistableModel<StorageSyncServerEndpointProvisioningStatus>.Write(ModelReaderWriterOptions options) => PersistableModelWriteCore(options);
