...
namespace Azure.ResourceManager.StorageSync
{
    public partial class CloudEndpointData : IUtf8JsonSerializable, IJsonModel<CloudEndpointData>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(AzureFileShareName))
            {
                builder.Append("    azureFileShareName:");
                if (AzureFileShareName.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{AzureFileShareName}'''");
                }
                else
                {
                    builder.AppendLine($" '{AzureFileShareName}'");
            ...
            if (Optional.IsDefined(StorageAccountTenantId))
            {
                builder.Append("    storageAccountTenantId:");
                builder.AppendLine($" '{StorageAccountTenantId.Value.ToString()}'");
            }

            if (Optional.IsDefined(PartnershipId))
            {
                builder.Append("    partnershipId:");
                if (PartnershipId.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{PartnershipId}'''");
                }
                else
                {
                    builder.AppendLine($" '{PartnershipId}'");
            ...
            {
                builder.Append("    backupEnabled:");
                if (IsBackupEnabled.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{IsBackupEnabled}'''");
                }
                else
                {
                    builder.AppendLine($" '{IsBackupEnabled}'");
            ...
            {
                builder.Append("    changeEnumerationStatus:");
                AppendChildObject(builder, ChangeEnumerationStatus, options, 4, false);
            }
