...
#pragma warning restore SA1649 // File name should match first type name
    {
        ...
        /// <summary> Initializes a new instance of AnalysisServicesArmOperation for mocking. </summary>
        protected AnalysisServicesArmOperation()
        {
        }

        internal AnalysisServicesArmOperation(Response<T> response)
        {
            _operation = OperationInternal<T>.Succeeded(response.GetRawResponse(), response.Value);
        }

        internal AnalysisServicesArmOperation(Core.IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            ...
            _operation = new OperationInternal<T>(clientDiagnostics, nextLinkOperation, response, "AnalysisServicesArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }

        internal AnalysisServicesArmOperation(Core.IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, string id)
        {
            var nextLinkOperation = NextLinkOperationImplementation.Create(source, pipeline, id, out string finalResponse);
            _operation = OperationInternal<T>.Create(source, clientDiagnostics, nextLinkOperation, finalResponse, "AnalysisServicesArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }
