// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;
using Azure.Core.Serialization;

namespace Azure.ResourceManager.MobileNetwork.Models
{
    public partial class PlmnId : IUtf8JsonSerializable, Core.IModelSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((Core.IModelSerializable)this).Serialize(writer, new Core.Serialization.SerializableOptions());

        void Core.IModelSerializable.Serialize(Utf8JsonWriter writer, Core.Serialization.SerializableOptions options)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("mcc"u8);
            writer.WriteStringValue(Mcc);
            writer.WritePropertyName("mnc"u8);
            writer.WriteStringValue(Mnc);
            writer.WriteEndObject();
        }

        internal static PlmnId DeserializePlmnId(JsonElement element, Core.Serialization.SerializableOptions options = default)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            string mcc = default;
            string mnc = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("mcc"u8))
                {
                    mcc = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("mnc"u8))
                {
                    mnc = property.Value.GetString();
                    continue;
                }
            }
            return new PlmnId(mcc, mnc);
        }
    }
}
