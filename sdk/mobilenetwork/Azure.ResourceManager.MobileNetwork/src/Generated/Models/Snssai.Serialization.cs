// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;
using Azure.Core.Serialization;

namespace Azure.ResourceManager.MobileNetwork.Models
{
    public partial class Snssai : IUtf8JsonSerializable, Core.IModelSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((Core.IModelSerializable)this).Serialize(writer, new Core.Serialization.SerializableOptions());

        void Core.IModelSerializable.Serialize(Utf8JsonWriter writer, Core.Serialization.SerializableOptions options)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("sst"u8);
            writer.WriteNumberValue(Sst);
            if (Optional.IsDefined(Sd))
            {
                writer.WritePropertyName("sd"u8);
                writer.WriteStringValue(Sd);
            }
            writer.WriteEndObject();
        }

        internal static Snssai DeserializeSnssai(JsonElement element, Core.Serialization.SerializableOptions options = default)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            int sst = default;
            Optional<string> sd = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("sst"u8))
                {
                    sst = property.Value.GetInt32();
                    continue;
                }
                if (property.NameEquals("sd"u8))
                {
                    sd = property.Value.GetString();
                    continue;
                }
            }
            return new Snssai(sst, sd.Value);
        }
    }
}
