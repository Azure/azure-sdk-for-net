...
namespace Microsoft.Azure.WebJobs.Host.TestCommon
{
    public static class TestHelpers
    {
        ...
        {
            await Await(() => Task.FromResult(condition()), timeout, pollingInterval, throwWhenDebugging, userMessageCallback).ConfigureAwait(false);
        }
        ...
        {
            await host.CallAsync(typeof(T).GetMethod(methodName), arguments).ConfigureAwait(false);
        }
        public static async Task CallAsync<T>(this JobHost host, string methodName)
        {
            await host.CallAsync(typeof(T).GetMethod(methodName)).ConfigureAwait(false);
        }
