    ...
    internal static class DictionaryTableExtensions
    {
        ...
        internal static void CastAndRemoveAnnotations(this IDictionary<string, object> entity, Dictionary<string, (string TypeAnnotation, string AnnotationKey)>? typeAnnotationsWithKeys = null)
        {
            ...
            foreach (var annotation in typeAnnotationsWithKeys.Keys)
            {
                ...
                {
                    TableConstants.Odata.EdmBinary => Convert.FromBase64String(entity[annotation].ToString() ?? string.Empty),
                    TableConstants.Odata.EdmDateTime => DateTimeOffset.Parse(entity[annotation].ToString() ?? string.Empty, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind),
                    TableConstants.Odata.EdmGuid => Guid.Parse(entity[annotation].ToString() ?? string.Empty),
                    TableConstants.Odata.EdmInt64 => long.Parse(entity[annotation].ToString() ?? string.Empty, CultureInfo.InvariantCulture),
                    TableConstants.Odata.EdmDouble => double.Parse(entity[annotation].ToString() ?? string.Empty, CultureInfo.InvariantCulture),
            ...
            {
                entity[TableConstants.PropertyNames.Timestamp] = DateTimeOffset.Parse(entity[TableConstants.PropertyNames.Timestamp].ToString() ?? string.Empty, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind);
            }
