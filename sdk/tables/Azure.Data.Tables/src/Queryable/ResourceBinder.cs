...
{
    internal class ResourceBinder : DataServiceLinqExpressionVisitor
    {
        ...
        {
            if (expression is ResourceExpression re)
            {
                ...
                if (re.Source != null)
                {
                    if ((re.Source is ResourceSetExpression rse) && !rse.HasKeyPredicate)
                    {
                        return true;
                    }
                }
        ...
        internal static void VerifyNotSelectManyProjection(Expression expression)
        {
            ...
            if (expression is ResourceSetExpression resourceSet)
            {
        ...
        private static Expression AnalyzePredicate(MethodCallExpression mce)
        {
            ...
            if (!TryGetResourceSetMethodArguments(mce, out ResourceSetExpression input, out LambdaExpression le))
            {
        ...
        internal bool AnalyzeProjection(MethodCallExpression mce, SequenceMethod sequenceMethod, out Expression e)
        {
            ...
            if (!(Visit(mce.Arguments[0]) is ResourceExpression source))
            {
                return false;
            }
            ...
            if (sequenceMethod == SequenceMethod.SelectManyResultSelector)
            {
                ...
                if (!PatternRules.MatchDoubleArgumentLambda(resultSelector, out LambdaExpression resultLambda))
                {
                    return false;
                }
                ...
                if (!PatternRules.MatchPropertyProjectionSet(source, collectorReference, out MemberExpression navigationMember))
                {
                    return false;
                }
        ...
        internal static Expression AnalyzeNavigation(MethodCallExpression mce)
        {
            ...
            if (!PatternRules.MatchSingleArgumentLambda(mce.Arguments[1], out LambdaExpression le))
            {
                return mce;
            }
            ...
            else if (IsValidNavigationSource(input, out ResourceExpression navSource) &&
                TryBindToInput(navSource, le, out Expression boundProjection) &&
                PatternRules.MatchPropertyProjectionSingleton(navSource, boundProjection, out MemberExpression navigationMember))
            {
        ...
        private static Expression LimitCardinality(MethodCallExpression mce, int maxCardinality)
        {
            ...
            if (mce.Arguments[0] is ResourceSetExpression rse)
            {
        ...
        {
            if (mce.Arguments[0] is ResourceExpression re)
            {
        ...
        internal static Expression StripConvertToAssignable(Expression e)
        {
            ...
            Expression result;
            if (e is UnaryExpression unary && PatternRules.MatchConvertToAssignable(unary))
            {
        ...
        internal override Expression VisitMemberAccess(MemberExpression m)
        {
            ...
            if (me != null &&
                PatternRules.MatchNonPrivateReadableProperty(me, out PropertyInfo pi) &&
                TypeSystem.TryGetPropertyAsMethod(pi, out MethodInfo mi))
            {
        ...
        internal override Expression VisitMethodCall(MethodCallExpression mce)
        {
            ...
            if (ReflectionUtil.TryIdentifySequenceMethod(mce.Method, out SequenceMethod sequenceMethod))
            {
                ...
                {
                    if (AnalyzeProjection(mce, sequenceMethod, out e))
                    {
                        return e;
                    }
                }
        ...
        internal static class PatternRules
        {
            ...
            internal static bool MatchPropertyAccess(Expression e, out MemberExpression member, out Expression instance, out List<string> propertyPath)
            {
                ...
                while (me != null)
                {
                    if (MatchNonPrivateReadableProperty(me, out PropertyInfo pi))
                    {
            ...
            {
                return e is BinaryExpression be && (be.NodeType == ExpressionType.And || be.NodeType == ExpressionType.AndAlso);
            }
            ...
            internal static bool MatchTransparentIdentitySelector(Expression input, LambdaExpression selector)
            {
                ...
                if (!(input is ResourceSetExpression rse) || rse.TransparentScope == null)
                {
                    return false;
                }
                ...
                if (!MatchPropertyAccess(potentialRef, out _, out Expression paramRef, out List<string> refPath))
                {
                    return false;
                }
            ...
            {
                if (MatchNonSingletonProperty(potentialPropertyRef) == requireSet)
                {
                    if (MatchPropertyAccess(potentialPropertyRef, out navigationMember, out Expression foundInstance, out _))
                    {
            ...
            internal static bool MatchMemberInitExpressionWithDefaultConstructor(Expression source, LambdaExpression e)
            {
                ...
                return MatchResource(source, out _) && (mie != null) && (mie.NewExpression.Arguments.Count == 0);
            }

            internal static bool MatchNewExpression(Expression source, LambdaExpression e)
            {
                return MatchResource(source, out _) && (e.Body is NewExpression);
            }
            ...
            internal static bool MatchStringAddition(Expression e)
            {
                ...
                {
                    return e is BinaryExpression be &&
            ...
            internal static MatchEqualityCheckResult MatchEquality(Expression expression)
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (!(expression is BinaryExpression test))
                        {
                            break;
                        }
        ...
        private static class ValidationRules
        {
            ...
            {
                if (e is ResourceSetExpression resourceSet && resourceSet.HasSequenceQueryOptions)
                {
            ...
            {
                if (e is ResourceExpression re && re.IsSingleton)
                {
        ...
        private sealed class PropertyInfoEqualityComparer : IEqualityComparer<PropertyInfo>
        {
            ...
            public bool Equals(PropertyInfo left, PropertyInfo right)
            {
                ...
                return ReferenceEquals(left.DeclaringType, right.DeclaringType) && left.Name.Equals(right.Name, StringComparison.InvariantCulture);
            }
        ...
        private sealed class ExpressionPresenceVisitor : DataServiceLinqExpressionVisitor
        {
            ...
            internal override Expression Visit(Expression exp)
            {
                ...
                if (found || ReferenceEquals(target, exp))
                {
                    found = true;
                    result = exp;
                }
