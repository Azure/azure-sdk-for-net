// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.NeonPostgres
{
    /// <summary></summary>
    internal partial class NeonBranchOperationSource : IOperationSource<NeonBranchResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal NeonBranchOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        NeonBranchResource IOperationSource<NeonBranchResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            NeonBranchData data = NeonBranchData.DeserializeNeonBranchData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new NeonBranchResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<NeonBranchResource> IOperationSource<NeonBranchResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            NeonBranchData data = NeonBranchData.DeserializeNeonBranchData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new NeonBranchResource(_client, data);
        }
    }
}
