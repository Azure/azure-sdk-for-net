...
{
    public partial class HealthErrorModel : IUtf8JsonSerializable, IJsonModel<HealthErrorModel>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<HealthErrorModel>)this).Write(writer, ModelSerializationExtensions.WireOptions);

        void IJsonModel<HealthErrorModel>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
        ...
        {
            var format = options.Format == "W" ? ((IPersistableModel<HealthErrorModel>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(HealthErrorModel)} does not support writing '{format}' format.");
            }
        ...
        HealthErrorModel IJsonModel<HealthErrorModel>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<HealthErrorModel>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(HealthErrorModel)} does not support reading '{format}' format.");
            }

            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeHealthErrorModel(document.RootElement, options);
        }

        internal static HealthErrorModel DeserializeHealthErrorModel(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            IReadOnlyList<InnerHealthErrorModel> childErrors = default;
            string code = default;
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("childErrors"u8))
                {
                    ...
                    List<InnerHealthErrorModel> array = new List<InnerHealthErrorModel>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(InnerHealthErrorModel.DeserializeInnerHealthErrorModel(item, options));
                    }
                    childErrors = array;
                    continue;
                }
        ...
        BinaryData IPersistableModel<HealthErrorModel>.Write(ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<HealthErrorModel>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(HealthErrorModel)} does not support writing '{options.Format}' format.");
            }
        }

        HealthErrorModel IPersistableModel<HealthErrorModel>.Create(BinaryData data, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<HealthErrorModel>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(HealthErrorModel)} does not support reading '{options.Format}' format.");
            }
        }

        string IPersistableModel<HealthErrorModel>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
