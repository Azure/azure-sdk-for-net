// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.ComponentModel;

namespace Azure.AI.ContentSafety
{
    /// <summary> The harm category supported in Image content analysis. </summary>
    public readonly partial struct ImageCategory : IEquatable<global::Azure.AI.ContentSafety.ImageCategory>
    {
        private readonly string _value;
        /// <summary> The harm category for Image - Hate. </summary>
        private const string HateValue = "Hate";
        /// <summary> The harm category for Image - SelfHarm. </summary>
        private const string SelfHarmValue = "SelfHarm";
        /// <summary> The harm category for Image - Sexual. </summary>
        private const string SexualValue = "Sexual";
        /// <summary> The harm category for Image - Violence. </summary>
        private const string ViolenceValue = "Violence";

        /// <summary> Initializes a new instance of <see cref="global::Azure.AI.ContentSafety.ImageCategory"/>. </summary>
        /// <param name="value"> The value. </param>
        /// <exception cref="global::System.ArgumentNullException"> <paramref name="value"/> is null. </exception>
        public ImageCategory(string value)
        {
            global::Azure.AI.ContentSafety.Argument.AssertNotNull(value, nameof(value));

            _value = value;
        }

        /// <summary> The harm category for Image - Hate. </summary>
        public static ImageCategory Hate { get; } = new ImageCategory(HateValue);

        /// <summary> The harm category for Image - SelfHarm. </summary>
        public static ImageCategory SelfHarm { get; } = new ImageCategory(SelfHarmValue);

        /// <summary> The harm category for Image - Sexual. </summary>
        public static ImageCategory Sexual { get; } = new ImageCategory(SexualValue);

        /// <summary> The harm category for Image - Violence. </summary>
        public static ImageCategory Violence { get; } = new ImageCategory(ViolenceValue);

        /// <summary> Determines if two <see cref="global::Azure.AI.ContentSafety.ImageCategory"/> values are the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator ==(ImageCategory left, ImageCategory right) => left.Equals(right);

        /// <summary> Determines if two <see cref="global::Azure.AI.ContentSafety.ImageCategory"/> values are not the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator !=(ImageCategory left, ImageCategory right) => !left.Equals(right);

        /// <summary> Converts a string to a <see cref="global::Azure.AI.ContentSafety.ImageCategory"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator ImageCategory(string value) => new ImageCategory(value);

        /// <summary> Converts a string to a <see cref="global::Azure.AI.ContentSafety.ImageCategory"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator ImageCategory?(string value) => (value == null) ? null : new ImageCategory(value);

        /// <inheritdoc/>
        [EditorBrowsableAttribute(global::System.ComponentModel.EditorBrowsableState.Never)]
        public override bool Equals(object obj) => ((obj is ImageCategory other) && this.Equals(other));

        /// <inheritdoc/>
        public bool Equals(ImageCategory other) => string.Equals(_value, other._value, global::System.StringComparison.InvariantCultureIgnoreCase);

        /// <inheritdoc/>
        [EditorBrowsableAttribute(global::System.ComponentModel.EditorBrowsableState.Never)]
        public override int GetHashCode() => (_value != null) ? global::System.StringComparer.InvariantCultureIgnoreCase.GetHashCode(_value) : 0;

        /// <inheritdoc/>
        public override string ToString() => _value;
    }
}
