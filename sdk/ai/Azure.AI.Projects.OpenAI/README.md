...
Trace context propagation allows client-side spans generated by the Projects SDK to be correlated with server-side spans. When enabled, the SDK automatically injects W3C Trace Context headers (`traceparent` and `tracestate`) into outgoing HTTP requests.
...
```bash
set AZURE_TRACING_GEN_AI_ENABLE_TRACE_CONTEXT_PROPAGATION=true
```

You can also enable the feature with the following code (before creating the `AIProjectClient`):
```C#
AppContext.SetSwitch("Azure.Experimental.TraceContextPropagation", true);
```

If neither the environment variable nor the `AppContext` switch is set, trace context propagation defaults to `false` (opt-in).
...
```bash
set AZURE_TRACING_GEN_AI_TRACE_CONTEXT_PROPAGATION_INCLUDE_BAGGAGE=true
```

Or via code:
```C#
AppContext.SetSwitch("Azure.Experimental.TraceContextBaggagePropagation", true);
```

If neither the environment variable nor the `AppContext` switch is set, baggage propagation defaults to `false`.
...
Baggage is automatically propagated through your entire application's call chain, meaning data added in one part of your application will be included in requests to Azure services unless explicitly controlled.

**Important Security Considerations:**

- **Review Baggage Contents**: Before enabling baggage propagation, audit what data your application (and any third-party libraries) adds to `Activity.Baggage`.
- **Sensitive Data Risk**: Baggage is sent to Azure services and may be logged or processed. Never add sensitive information to baggage when baggage propagation is enabled.
