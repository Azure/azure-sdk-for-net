        ...
        public static void AddAzureFinetuningParityPolicy(ClientPipelineOptions options)
        {
            options.AddPolicy(
                new GenericActionPipelinePolicy(
                    ...
                    {
                        string uri = message?.Request?.Uri?.AbsoluteUri ?? "null";
                        bool isFileOperation = uri.Contains("openai/files");
                        bool isFineTuningOperation = uri.Contains("openai/fine_tuning");

                        // Skip this policy for everything except file and fine-tuning operations
                        if (!isFileOperation && !isFineTuningOperation)
                        {
                            Console.Error.WriteLine($"[AzureFinetuningParityPolicy] SKIPPING - not file or fine-tuning operation");
                            return;
                        }
