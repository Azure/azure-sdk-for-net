...
public class Sample_Models : SamplesBase
{
    private void Cleanup(AIProjectClient projectClient, string name, string version)
    {
        try
        {
            Directory.Delete(Path.Combine(Path.GetTempPath(), "sample-model"), recursive: true);
        }
        catch { }
        try
        {
            projectClient.Models.DeleteModelVersion(name: name, version: version);
        }
        catch { }
        try
        {
            projectClient.Datasets.Delete(name: name, version: version);
        }
        catch { }
    }
    ...
    public async Task ModelsAsync()
    {
        ...
        #region Snippet:Sample_UploadData_Models_Async
        ...
        #endregion
        #region Snippet:Sample_UpdateModel_Models_Async
