...
public partial class Sample_Agents_Azure_AI_Search : SamplesBase<AIProjectsTestEnvironment>
{
    ...
    public async Task AzureAISearchExample()
    {
        ...
        ListConnectionsResponse searchConnections = await projectClient.GetConnectionsClient().GetConnectionsAsync(ConnectionType.AzureAISearch);

        if (searchConnections?.Value == null || searchConnections.Value.Count == 0)
        {
           Console.WriteLine("No connections found for the specified parameters.");
           return;
        }

        GetConnectionResponse searchConnection = searchConnections.Value[0];

        var connectionId = searchConnection.Id;
        Console.Write($"search Connection Id: {connectionId}");
