...
```C# Snippet:Sample_CreateClients_EvaluationsFunction
...
2. Define the static method, which will be used as a function and a function tool.

```C# Snippet:Sample_FunctionTool_EvaluationsFunction_Async
...
3. Create a target Agent for evaluation.
...
4. Define the `GetResolvedToolOutput` method to call the function by Agent request.

```C# Snippet:Sample_Resolver_EvaluationsFunction
...
5. Process the response from an Agent and provide function output.

Synchronous sample:
```C# Snippet:Sample_CreateResponse_EvaluationsFunction_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateResponse_EvaluationsFunction_Async
ProjectResponsesClient responseClient = projectClient.OpenAI.GetProjectResponsesClientForAgent(agentVersion.Name);
...
6. Define the evaluation criteria and the data source config. Testing criteria lists all the evaluators. In this example we will use only built-in "tool_call_accuracy" evaluator. We will configure the data source to get data from the Agent responses.

```C# Snippet:Sample_CreateData_EvaluationsFunction
...
8. Use `EvaluationClient` to create the evaluation with provided parameters.

Synchronous sample:
```C# Snippet:Sample_CreateEvaluationObject_EvaluationsFunction_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateEvaluationObject_EvaluationsFunction_Async
using BinaryContent evaluationDataContent = BinaryContent.Create(GetEvaluationConfig(modelDeploymentName));
...
9. Create the `BinaryData` object used for run. It contains name and ID of the evaluation we have created above, and data source, configured to retrieve the data from the Agent response items.


```C# Snippet:Sample_CreateDataSource_EvaluationsFunction
...
Synchronous sample:
```C# Snippet:Sample_CreateRun_EvaluationsFunction_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateRun_EvaluationsFunction_Async
using BinaryContent runDataContent = BinaryContent.Create(GetRunData(agentVersion.Name, response.Id, evaluationId));
...
12. Wait for evaluation run to arrive at the terminal state.

Synchronous sample:
```C# Snippet:Sample_WaitForRun_EvaluationsFunction_Sync
...
Asynchronous sample:
```C# Snippet:Sample_WaitForRun_EvaluationsFunction_Async
while (runStatus != "failed" && runStatus != "completed")
{
...
Synchronous sample:
```C# Snippet:Sample_ParseEvaluations_EvaluationsFunction_Sync
...
Asynchronous sample:
```C# Snippet:Sample_ParseEvaluations_EvaluationsFunction_Async
Console.WriteLine("Evaluation run completed successfully!");
Console.WriteLine($"Result Counts: {GetResultsCounts(run)}");
...
16. Finally, delete evaluation and Agent used in this sample.

Synchronous sample:
```C# Snippet:Sample_Cleanup_EvaluationsFunction_Sync
evaluationClient.DeleteEvaluation(evaluationId, new System.ClientModel.Primitives.RequestOptions());
projectClient.Agents.DeleteAgentVersion(agentName: agentVersion.Name, agentVersion: agentVersion.Version);
```

Asynchronous sample:
```C# Snippet:Sample_Cleanup_EvaluationsFunction_Async
