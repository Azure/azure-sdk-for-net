# Sample of using AI-Assisted Evaluators in Azure.AI.Projects

In this example we demonstrate how to use AI-assisted evaluators (Similarity, ROUGE, METEOR, GLEU, F1, BLEU) to evaluate model responses against ground truth data.
...
3. Create the project client and the `EvaluationClient` for managing evaluations.

```C# Snippet:Sample_CreateClients_EvaluationsAIAssisted
...
4. Define the data source configuration and testing criteria. The data source schema specifies `response` and `ground_truth` fields. The testing criteria uses six AI-assisted evaluators: **Similarity** (requires a model deployment), **ROUGE**, **METEOR**, **GLEU**, **F1**, and **BLEU** scores.

```C# Snippet:Sample_CreateData_EvaluationsAIAssisted
...
Synchronous sample:
```C# Snippet:Sample_CreateEvaluation_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateEvaluation_EvaluationsAIAssisted_Async
using BinaryContent evaluationDataContent = BinaryContent.Create(GetDataConfig(modelDeploymentName));
...
6. Retrieve the evaluation by ID to verify it was created.

Synchronous sample:
```C# Snippet:Sample_GetEvaluation_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_GetEvaluation_EvaluationsAIAssisted_Async
ClientResult evaluationResponse = await evaluationClient.GetEvaluationAsync(evaluationId, new());
Console.WriteLine($"Retrieved evaluation: {evaluationResponse.GetRawResponse().Content}");
```

7. Define the inline JSONL data source with response/ground_truth pairs for the evaluation run.

```C# Snippet:Sample_CreateDataSource_EvaluationsAIAssisted
...
Synchronous sample:
```C# Snippet:Sample_CreateRun_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateRun_EvaluationsAIAssisted_Async
using BinaryContent runDataContent = BinaryContent.Create(GetData());
...
9. Wait for the evaluation run to complete by polling the run status.

Synchronous sample:
```C# Snippet:Sample_WaitForRun_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_WaitForRun_EvaluationsAIAssisted_Async
while (runStatus != "failed" && runStatus != "completed")
{
...
12. Parse and display the evaluation results including result counts and individual output items.

Synchronous sample:
```C# Snippet:Sample_ParseResults_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_ParseResults_EvaluationsAIAssisted_Async
Console.WriteLine("Evaluation run completed successfully!");
Console.WriteLine($"Result Counts: {GetResultsCounts(run)}");
...
Synchronous sample:
```C# Snippet:Sample_Cleanup_EvaluationsAIAssisted_Sync
...
Asynchronous sample:
```C# Snippet:Sample_Cleanup_EvaluationsAIAssisted_Async
