...
```C# Snippet:Sample_HostedAgentDefinition_RoutinesTimerTrigger
...
Synchronous sample:
```C# Snippet:Sample_CreateHostedAgent_RoutinesTimerTrigger_Sync
...
Asynchronous sample:
```C# Snippet:Sample_CreateHostedAgent_RoutinesTimerTrigger_Async
HostedAgentDefinition agentDefinition = GetAgentDefinition(
    dockerImage: dockerImage
);
ProjectsAgentVersionCreationOptions creationOptions = new(agentDefinition);
...
4. Create a routine with a timer-based trigger and an agent invocation action.
