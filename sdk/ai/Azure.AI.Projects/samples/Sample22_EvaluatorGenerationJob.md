...
In this example we will demonstrate how to manage evaluator generation jobs in `Azure.AI.Projects`.
...
2. Create the Agent, which will be for generation of evaluator inputs. We create `EvaluatorGenerationJob` using the Agent, it will generate the new version of built in **coherence** evaluator, which will use the supplied model.

**Note:** In this example the same model is used for both data generation and evaluation just for demonstration purposes. Please use different models in the production scenarios.
