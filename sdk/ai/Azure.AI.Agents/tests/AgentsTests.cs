...
{
    public class AgentsTests : RecordedTestBase<AIAgentsTestEnvironment>
    {
        ...
        private static async Task<Agent> GetAgent(AgentsClient client, string agentName=AGENT_NAME, string instruction= "You are helpful agent.")
        {
        ...
        public void Cleanup()
        {
            ...
            if (!string.IsNullOrEmpty(cli) && string.Compare(cli, "true", StringComparison.OrdinalIgnoreCase) == 0)
            {
                client = new AgentsClient(TestEnvironment.PROJECT_ENDPOINT, new AzureCliCredential());
            }
            else
            {
                client = new AgentsClient(TestEnvironment.PROJECT_ENDPOINT, new DefaultAzureCredential());
            }
