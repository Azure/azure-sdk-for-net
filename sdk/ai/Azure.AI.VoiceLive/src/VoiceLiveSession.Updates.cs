...
namespace Azure.AI.VoiceLive
{
    public partial class VoiceLiveSession
    {
        ...
        /// </remarks>
        public async IAsyncEnumerable<VoiceLiveServerEvent> GetUpdatesAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ThrowIfDisposed();
            ...
            try
            {
                await foreach (BinaryData message in _receiveCollectionResult.WithCancellation(cancellationToken))
                {
                    ...
                    foreach (VoiceLiveServerEvent serverEvent in ProcessMessage(message))
                    {
        ...
        /// </remarks>
        public IEnumerable<VoiceLiveServerEvent> GetUpdates(CancellationToken cancellationToken = default)
        {
        ...
        public async IAsyncEnumerable<T> GetUpdatesAsync<T>([EnumeratorCancellation] CancellationToken cancellationToken = default)
            where T : VoiceLiveServerEvent
        {
            await foreach (VoiceLiveServerEvent serverEvent in GetUpdatesAsync(cancellationToken).ConfigureAwait(false))
            {
        ...
        private IEnumerable<VoiceLiveServerEvent> ProcessMessage(BinaryData message)
        {
            ...
            VoiceLiveServerEvent serverEvent = null;
            try
            {
                ...
                serverEvent = VoiceLiveServerEvent.DeserializeVoiceLiveServerEvent(root, ModelSerializationExtensions.WireOptions);
            }
            catch (JsonException)
            {
