// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Diagnostics.CodeAnalysis;
using OpenAI;

namespace Azure.AI.Projects.Agents
{
    /// <summary> A user message item. `input_text`, `input_audio`, and `input_image` content are valid for user messages. </summary>
    [Experimental("AAIP001")]
    public partial class VoiceUserMessageItem : RealtimeConversationItemMessageUser
    {
        /// <summary> Initializes a new instance of <see cref="VoiceUserMessageItem"/>. </summary>
        /// <param name="content"> The content of the message. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="content"/> is null. </exception>
        public VoiceUserMessageItem(IEnumerable<RealtimeConversationItemMessageUserContent> content) : base(content)
        {
            Argument.AssertNotNull(content, nameof(content));

        }

        /// <summary> Initializes a new instance of <see cref="VoiceUserMessageItem"/>. </summary>
        /// <param name="role"></param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="id"> The unique ID of the item. This may be provided by the client or generated by the server. </param>
        /// <param name="object"> Identifier for the API object being returned - always `realtime.item`. Optional when creating a new item. </param>
        /// <param name="status"> The status of the item. Has no effect on the conversation. </param>
        /// <param name="content"> The content of the message. </param>
        /// <param name="createdAt"> The Unix timestamp (in seconds) for when the item was persisted. </param>
        /// <param name="responseId"> The id of the response that produced this item, when applicable. </param>
        internal VoiceUserMessageItem(RealtimeConversationItemMessageType role, IDictionary<string, BinaryData> additionalBinaryDataProperties, string id, RealtimeConversationItemMessageUserObject? @object, RealtimeConversationItemMessageUserStatus? status, IList<RealtimeConversationItemMessageUserContent> content, DateTimeOffset? createdAt, string responseId) : base(role, additionalBinaryDataProperties, id, @object, status, content)
        {
            CreatedAt = createdAt;
            ResponseId = responseId;
        }

        /// <summary> The Unix timestamp (in seconds) for when the item was persisted. </summary>
        public DateTimeOffset? CreatedAt { get; }

        /// <summary> The id of the response that produced this item, when applicable. </summary>
        public string ResponseId { get; }
    }
}
