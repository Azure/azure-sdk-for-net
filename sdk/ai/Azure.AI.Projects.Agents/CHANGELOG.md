...
- Updated the `OpenAI` package dependency to `2.14.0`, which adds public support for extending the `OpenAI.Realtime.RealtimeClientCommand`/`RealtimeServerUpdate` hierarchies with custom types; this is what makes the typed avatar/RTC realtime commands and server updates above possible. This also renamed `OpenAI.Responses.GlobalMcpToolCallApprovalPolicy` to `DefaultMcpToolCallApprovalPolicy`.
