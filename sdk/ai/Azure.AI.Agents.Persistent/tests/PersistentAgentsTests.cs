...
namespace Azure.AI.Agents.Persistent.Tests
{
    public class PersistentAgentsTests : RecordedTestBase<AIAgentsTestEnvironment>
    {
        ...
        public async Task TestListAgent()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(ids, Does.Not.Contain(agent2.Id));
                Assert.That(count, Is.EqualTo(initialAgentCount + 1));
            });
        ...
        public async Task TestThreadPageable()
        {
            ...
            {
                Assert.That(pg.Values.Count, Is.LessThanOrEqualTo(2));
            }
            ...
            Assert.Multiple(() =>
            {
                Assert.That(sbUndeleted.Length, Is.EqualTo(0), $"The threads{sbUndeleted.ToString()} were not deleted.");
        ...
        public async Task TestCreateRun(ArgumentType argType)
        {
            ...
            Assert.That(stepsList, Is.Not.Empty);
        ...
        public async Task TestSubmitToolOutputs(ArgumentType argType, bool parallelToolCalls, bool CreateThreadAndRun)
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(messagesList, Is.Not.Empty);
        ...
        public async Task TestCreateVectorStore(VecrorStoreTestType testType, bool useFileSource, bool useStreaming)
        {
            ...
            Assert.That(fileSearchRun, Is.Not.Null);
            ...
            Assert.That(getVct, Is.Not.Null);
        ...
        public async Task TestIncludeFileSearchContent(bool useStream, bool includeContent)
        {
            ...
            Assert.That(fileSearchCall.FileSearch.Results, Is.Not.Empty);
            if (includeContent)
            {
                Assert.That(fileSearchCall.FileSearch.Results[0].Content, Is.Not.Empty);
                Assert.Multiple(() =>
                {
        ...
        public async Task TestClientWithPersistentThreadMessages()
        {
            ...
            Assert.That(afterRunMessages.Count(), Is.GreaterThan(1));
        }
                ...
                foreach (MessageContent cont in msg.ContentItems)
                {
                    if (cont is MessageTextContent textCont)
                    {
                        foreach (MessageTextAnnotation annotation in textCont.Annotations)
                        {
                            ...
                            {
                                Assert.That(pathAnnotation.FileId, Is.Not.Null);
                                foundId = true;
                            }
                        }
                    }
        ...
        public async Task TestMcpTool(string trust, bool isPerTool, bool shouldApprove)
        {
            ...
            Assert.Multiple(async () =>
            {
                ...
                Assert.That((await client.Messages.GetMessagesAsync(thread.Id).ToListAsync()).Count, Is.GreaterThan(1));
            });
        ...
        public async Task TestImageAsInput(bool useUploaded, string expectedWord)
        {
            ...
            Assert.That(response, Does.Contain(expectedWord), $"The word {expectedWord} was not found in the response: {response}");
        }
