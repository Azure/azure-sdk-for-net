...
public partial class Sample_PersistentAgents_Telemetry : SamplesBase<AIAgentsTestEnvironment>
{
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentChatWithTracingActivitySourceDisabledAsync()
    {
#if SNIPPET
    ...
    [Test]
    [SyncOnly]
    public void AgentChatWithTracingActivitySourceDisabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentChatWithTracingContentRecordingEnabledAsync()
    {
#if SNIPPET
        var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
        var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentChatWithTracingContentRecordingEnabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentChatWithTracingContentRecordingDisabledAsync()
    {
#if SNIPPET
        var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
        var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentChatWithTracingContentRecordingDisabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
        ...
        var expectedCreateAgentEvents = new List<(string, Dictionary<string, object>)>
        {
            ...
            {
                { "gen_ai.system", "az.ai.agents" },
                { "gen_ai.event.content", "\"\"" } // Content recording disabled
            })
        };
        Assert.IsTrue(_traceVerifier.CheckSpanEvents(createAgentSpan, expectedCreateAgentEvents));
        ...
        var expectedCreateMessageEvents = new List<(string, Dictionary<string, object>)>
        {
            ("gen_ai.user.message", new Dictionary<string, object>
            {
                { "gen_ai.system", "az.ai.agents" },
                { "gen_ai.thread.id", "*" },
                { "gen_ai.event.content", "{\"role\": \"user\"}" } // Content recording disabled
            })
        };
        Assert.IsTrue(_traceVerifier.CheckSpanEvents(createMessageSpan, expectedCreateMessageEvents));
        ...
        var expectedListMessagesEvents = new List<(string, Dictionary<string, object>)>
        {
            ("gen_ai.assistant.message", new Dictionary<string, object>
            {
                ...
                { "gen_ai.thread.run.id", "*" },
                { "gen_ai.message.id", "*" },
                { "gen_ai.event.content", "{\"role\": \"assistant\"}" } // Content recording disabled
            }),
            ("gen_ai.user.message", new Dictionary<string, object>
            {
                { "gen_ai.system", "az.ai.agents" },
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentChatWithFunctionToolTracingContentRecordingEnabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentChatWithFunctionToolTracingContentRecordingEnabledSync()
    {
#if SNIPPET
        var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
        var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
        ...
        // Enumerate messages and run steps to trigger spans
        foreach (var messageInList in client.Messages.GetMessages(thread.Id))
        {
            _ = messageInList;
        }

        foreach (var step in client.Runs.GetRunSteps(thread.Id, run.Id))
        {
            _ = step;
        }
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentChatWithFunctionToolTracingContentRecordingDisabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentChatWithFunctionToolTracingContentRecordingDisabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
        ...
        var expectedSubmitToolOutputsEvents = new List<(string, Dictionary<string, object>)>
        {
            ("gen_ai.tool.message", new Dictionary<string, object>
            {
                { "gen_ai.event.content", "{\"content\":\"\",\"id\":\"*\"}" } // Content recording disabled
            })
        };
        Assert.IsTrue(_traceVerifier.CheckSpanEvents(submitToolOutputsSpan, expectedSubmitToolOutputsEvents));
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentStreamingWithTracingContentRecordingEnabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentStreamingWithTracingContentRecordingEnabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
        ...
        // Start streaming run
        var stream = client.Runs.CreateRunStreaming(thread.Id, agent.Id);
        ...
        // Cleanup
        client.Threads.DeleteThread(threadId: threadId);
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentStreamingWithTracingContentRecordingDisabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentStreamingWithTracingContentRecordingDisabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentStreamingWithFunctionToolTracingContentRecordingEnabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentStreamingWithFunctionToolTracingContentRecordingEnabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
        ...
        ToolOutput GetResolvedToolOutput(string functionName, string toolCallId, string functionArguments)
        {
            if (functionName == getCurrentWeatherAtLocationTool.Name)
            {
                ...
                if (argumentsJson.RootElement.TryGetProperty("unit", out JsonElement unitElement))
                {
                    ...
                    return new ToolOutput(toolCallId, JsonSerializer.Serialize(new { temperature = unitArgument == "f" ? "70f" : "21c" }));
                }
                return new ToolOutput(toolCallId, JsonSerializer.Serialize(new { temperature = "70f" }));
            }
            return null;
    ...
    [Test]
    [AsyncOnly]
    public async Task AgentStreamingWithFunctionToolTracingContentRecordingDisabledAsync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
    ...
    [Test]
    [SyncOnly]
    public void AgentStreamingWithFunctionToolTracingContentRecordingDisabledSync()
    {
#if SNIPPET
    var projectEndpoint = Environment.GetEnvironmentVariable("PROJECT_ENDPOINT");
    var modelDeploymentName = Environment.GetEnvironmentVariable("MODEL_DEPLOYMENT_NAME");
