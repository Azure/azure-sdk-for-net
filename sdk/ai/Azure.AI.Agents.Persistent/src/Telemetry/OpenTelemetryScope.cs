...
namespace Azure.AI.Agents.Persistent.Telemetry
{
    internal class OpenTelemetryScope : IDisposable
    {
        ...
        public void RecordCreateMessageResponse(Response response)
        {
            ...
            {
                _response = new RecordedResponse(s_traceContent);
                var messageResponse = Response.FromValue(ThreadMessage.FromResponse(response), response);
        ...
        public void RecordListRunStepsResponse(Response response)
        {
            ...
            {
                _response = new RecordedResponse(s_traceContent);
                Response<InternalOpenAIPageableListOfRunStep> r1 = Response.FromValue(InternalOpenAIPageableListOfRunStep.FromResponse(response), response);
        ...
        private void End()
        {
            ...
            if (_response.Messages?.Value?.Data != null)
            {
                foreach (ThreadMessage threadMessage in _response.Messages.Value.Data)
                {
                    RecordThreadMessageEventAttributes(threadMessage);
                }
            }

            if (_response.RunSteps?.Value?.Data != null)
            {
                foreach (RunStep runStep in _response.RunSteps.Value.Data)
                {
        ...
        private void RecordThreadMessageEventAttributes(ThreadMessage threadMessage)
        {
