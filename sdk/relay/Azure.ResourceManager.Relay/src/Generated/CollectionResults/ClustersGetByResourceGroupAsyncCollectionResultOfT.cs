...
{
    internal partial class ClustersGetByResourceGroupAsyncCollectionResultOfT : AsyncPageable<RelayClusterData>
    {
        ...
        public ClustersGetByResourceGroupAsyncCollectionResultOfT(Clusters client, string subscriptionId, string resourceGroupName, RequestContext context, string diagnosticScope) : base(context?.CancellationToken ?? default)
        {
            _client = client;
        ...
        public override async IAsyncEnumerable<Page<RelayClusterData>> AsPages(string continuationToken, int? pageSizeHint)
        {
            ...
            while (true)
            {
                ...
                RelayClusterListResult result = RelayClusterListResult.FromResponse(response);
                nextPage = result.NextLink;
                yield return Page<RelayClusterData>.FromValues((IReadOnlyList<RelayClusterData>)result.Value, nextPage?.IsAbsoluteUri == true ? nextPage.AbsoluteUri : nextPage?.OriginalString, response);
                if (nextPage == null)
                {
                    yield break;
                }
