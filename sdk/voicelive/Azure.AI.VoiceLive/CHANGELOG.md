...
### Features Added
- Added `VoiceLiveSessionResponse.ExpiresAt` (`System.DateTimeOffset?`), a server-provided session expiration timestamp (sent as epoch seconds on the wire). This helps clients proactively refresh or recreate sessions before expiry.

### Breaking Changes
- Removed preview-only smart end-of-turn detection support from the stable API surface: `SmartEndOfTurnDetection`.
- Removed preview-only WebRTC signaling/event types from the stable API surface: `ClientEventRtcCallSdpCreate`, `ServerEventRtcCallSdpCreated`, `ServerEventRtcCallError`, `RtcCallErrorDetails`.
- Removed preview-only output audio buffer lifecycle events from the stable API surface: `ServerEventOutputAudioBufferStarted`, `ServerEventOutputAudioBufferStopped`.
