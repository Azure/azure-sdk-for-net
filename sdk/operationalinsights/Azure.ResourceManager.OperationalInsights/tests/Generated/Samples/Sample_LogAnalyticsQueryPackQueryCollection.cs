...
{
    public partial class Sample_LogAnalyticsQueryPackQueryCollection
    {
        // QueryList
        ...
        public async Task GetAll_QueryList()
        {
            ...
            // get the collection of this LogAnalyticsQueryPackQueryResource
            LogAnalyticsQueryPackQueryCollection collection = logAnalyticsQueryPack.GetLogAnalyticsQueryPackQueries();

            // invoke the operation and iterate over the result
            bool? includeBody = true;
            await foreach (LogAnalyticsQueryPackQueryResource item in collection.GetAllAsync(includeBody: includeBody))
            {
                // the variable item is a resource, you could call other operations on this instance as well
                // but just for demo, we get its data from this resource instance
                LogAnalyticsQueryPackQueryData resourceData = item.Data;
        ...
        public async Task Get_QueryGet()
        {
            ...
            LogAnalyticsQueryPackQueryResource result = await collection.GetAsync(id);

            // the variable result is a resource, you could call other operations on this instance as well
            // but just for demo, we get its data from this resource instance
            LogAnalyticsQueryPackQueryData resourceData = result.Data;
            // for demo we just print out the id
            Console.WriteLine($"Succeeded on id: {resourceData.Id}");
        }
        ...
        public async Task CreateOrUpdate_QueryPut()
        {
            ...
            // invoke the operation
            string id = "a449f8af-8e64-4b3a-9b16-5a7165ff98c4";
            LogAnalyticsQueryPackQueryData data = new LogAnalyticsQueryPackQueryData()
            {
                DisplayName = "Exceptions - New in the last 24 hours",
                Description = "my description",
                Body = "let newExceptionsTimeRange = 1d;\nlet timeRangeToCheckBefore = 7d;\nexceptions\n| where timestamp < ago(timeRangeToCheckBefore)\n| summarize count() by problemId\n| join kind= rightanti (\nexceptions\n| where timestamp >= ago(newExceptionsTimeRange)\n| extend stack = tostring(details[0].rawStack)\n| summarize count(), dcount(user_AuthenticatedId), min(timestamp), max(timestamp), any(stack) by problemId  \n) on problemId \n| order by  count_ desc\n",
                Related = new LogAnalyticsQueryPackQueryPropertiesRelated()
                {
                    Categories =
{
"analytics"
...
"label2"
            ...
            ArmOperation<LogAnalyticsQueryPackQueryResource> lro = await collection.CreateOrUpdateAsync(WaitUntil.Completed, id, data);
            LogAnalyticsQueryPackQueryResource result = lro.Value;
