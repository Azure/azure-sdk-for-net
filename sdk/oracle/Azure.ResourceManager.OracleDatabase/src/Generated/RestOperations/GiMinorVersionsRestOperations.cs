...
namespace Azure.ResourceManager.OracleDatabase
{
    internal partial class GiMinorVersionsRestOperations
    {
        ...
        public async Task<Response<GiMinorVersionData>> GetAsync(string subscriptionId, AzureLocation location, string giversionname, string giMinorVersionName, CancellationToken cancellationToken = default)
        {
            Argument.AssertNotNullOrEmpty(subscriptionId, nameof(subscriptionId));
            Argument.AssertNotNullOrEmpty(giversionname, nameof(giversionname));
            ...
            switch (message.Response.Status)
            {
                case 200:
                    {
                        GiMinorVersionData value = default;
                        using var document = await JsonDocument.ParseAsync(message.Response.ContentStream, ModelSerializationExtensions.JsonDocumentOptions, cancellationToken).ConfigureAwait(false);
                        value = GiMinorVersionData.DeserializeGiMinorVersionData(document.RootElement);
                        return Response.FromValue(value, message.Response);
                    }
                case 404:
                    return Response.FromValue((GiMinorVersionData)null, message.Response);
                default:
                    throw new RequestFailedException(message.Response);
            }
        }
        ...
        /// <exception cref="ArgumentNullException"> <paramref name="subscriptionId"/>, <paramref name="giversionname"/> or <paramref name="giMinorVersionName"/> is null. </exception>
        /// <exception cref="ArgumentException"> <paramref name="subscriptionId"/>, <paramref name="giversionname"/> or <paramref name="giMinorVersionName"/> is an empty string, and was expected to be non-empty. </exception>
        public Response<GiMinorVersionData> Get(string subscriptionId, AzureLocation location, string giversionname, string giMinorVersionName, CancellationToken cancellationToken = default)
        {
            Argument.AssertNotNullOrEmpty(subscriptionId, nameof(subscriptionId));
            Argument.AssertNotNullOrEmpty(giversionname, nameof(giversionname));
            Argument.AssertNotNullOrEmpty(giMinorVersionName, nameof(giMinorVersionName));
