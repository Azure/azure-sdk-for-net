...
namespace Azure.ResourceManager.OracleDatabase
{
    ...
    {
        /// <summary> Generate the resource identifier of a <see cref="GiMinorVersionResource"/> instance. </summary>
        ...
        private readonly GiMinorVersionData _data;

        /// <summary> Gets the resource type for the operations. </summary>
        public static readonly ResourceType ResourceType = "Oracle.Database/locations/giVersions/giMinorVersions";

        /// <summary> Initializes a new instance of the <see cref="GiMinorVersionResource"/> class for mocking. </summary>
        protected GiMinorVersionResource()
        {
        }

        /// <summary> Initializes a new instance of the <see cref="GiMinorVersionResource"/> class. </summary>
        /// <param name="client"> The client parameters to use in these operations. </param>
        /// <param name="data"> The resource that is the target of operations. </param>
        internal GiMinorVersionResource(ArmClient client, GiMinorVersionData data) : this(client, data.Id)
        {
            HasData = true;
            _data = data;
        }
        ...
        internal GiMinorVersionResource(ArmClient client, ResourceIdentifier id) : base(client, id)
        {
            _giMinorVersionClientDiagnostics = new ClientDiagnostics("Azure.ResourceManager.OracleDatabase", ResourceType.Namespace, Diagnostics);
            TryGetApiVersion(ResourceType, out string giMinorVersionApiVersion);
        ...
        public virtual GiMinorVersionData Data
        {
            get
            {
                if (!HasData)
        ...
        public virtual async Task<Response<GiMinorVersionResource>> GetAsync(CancellationToken cancellationToken = default)
        {
            using var scope = _giMinorVersionClientDiagnostics.CreateScope("GiMinorVersionResource.Get");
            scope.Start();
            try
            {
                var response = await _giMinorVersionRestClient.GetAsync(Id.SubscriptionId, new AzureLocation(Id.Parent.Parent.Name), Id.Parent.Name, Id.Name, cancellationToken).ConfigureAwait(false);
        ...
        /// </summary>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        public virtual Response<GiMinorVersionResource> Get(CancellationToken cancellationToken = default)
        {
            using var scope = _giMinorVersionClientDiagnostics.CreateScope("GiMinorVersionResource.Get");
            scope.Start();
            try
            {
                var response = _giMinorVersionRestClient.Get(Id.SubscriptionId, new AzureLocation(Id.Parent.Parent.Name), Id.Parent.Name, Id.Name, cancellationToken);
                if (response.Value == null)
                    throw new RequestFailedException(response.GetRawResponse());
                return Response.FromValue(new GiMinorVersionResource(Client, response.Value), response.GetRawResponse());
            }
