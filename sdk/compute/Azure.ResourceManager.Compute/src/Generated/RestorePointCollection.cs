...
namespace Azure.ResourceManager.Compute
{
    ...
    public partial class RestorePointCollection : ArmCollection
    {
        ...
        public virtual async Task<ArmOperation<RestorePointResource>> CreateOrUpdateAsync(WaitUntil waitUntil, string restorePointName, RestorePointData data, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _restorePointRestClient.CreateCreateRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, restorePointName, data);
                var response = await _restorePointRestClient.CreateAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new ComputeArmOperation<RestorePointResource>(new RestorePointOperationSource(Client), _restorePointClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
        ...
        public virtual ArmOperation<RestorePointResource> CreateOrUpdate(WaitUntil waitUntil, string restorePointName, RestorePointData data, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _restorePointRestClient.CreateCreateRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, restorePointName, data);
                var response = _restorePointRestClient.Create(message, cancellationToken);
                var operation = new ComputeArmOperation<RestorePointResource>(new RestorePointOperationSource(Client), _restorePointClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
