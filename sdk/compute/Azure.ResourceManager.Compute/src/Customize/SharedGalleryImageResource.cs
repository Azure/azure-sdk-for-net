...
namespace Azure.ResourceManager.Compute
{
    ...
    public partial class SharedGalleryImageResource : ArmResource
    {
        ...
        {
            var response = await GetSubscriptionResource().GetSharedGalleryImageAsync(new AzureLocation(Id.Parent.Parent.Name), Id.Parent.Name, Id.Name, cancellationToken).ConfigureAwait(false);
        ...
        {
            var response = GetSubscriptionResource().GetSharedGalleryImage(new AzureLocation(Id.Parent.Parent.Name), Id.Parent.Name, Id.Name, cancellationToken);
            response.Value.Id = CreateResourceIdentifier(Id.SubscriptionId, new AzureLocation(Id.Parent.Parent.Name), Id.Parent.Name, Id.Name);
            return Response.FromValue(new SharedGalleryImageResource(Client, response.Value), response.GetRawResponse());
        }
    }
