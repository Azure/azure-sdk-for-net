// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;
using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.HDInsight.Models
{
    public partial class AzureMonitorSelectedConfigurations : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            if (Optional.IsDefined(ConfigurationVersion))
            {
                writer.WritePropertyName("configurationVersion");
                writer.WriteStringValue(ConfigurationVersion);
            }
            if (Optional.IsCollectionDefined(GlobalConfigurations))
            {
                writer.WritePropertyName("globalConfigurations");
                writer.WriteStartObject();
                foreach (var item in GlobalConfigurations)
                {
                    writer.WritePropertyName(item.Key);
                    writer.WriteStringValue(item.Value);
                }
                writer.WriteEndObject();
            }
            if (Optional.IsCollectionDefined(TableList))
            {
                writer.WritePropertyName("tableList");
                writer.WriteStartArray();
                foreach (var item in TableList)
                {
                    writer.WriteObjectValue(item);
                }
                writer.WriteEndArray();
            }
            writer.WriteEndObject();
        }

        internal static AzureMonitorSelectedConfigurations DeserializeAzureMonitorSelectedConfigurations(JsonElement element)
        {
            Optional<string> configurationVersion = default;
            Optional<IDictionary<string, string>> globalConfigurations = default;
            Optional<IList<AzureMonitorTableConfiguration>> tableList = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("configurationVersion"))
                {
                    configurationVersion = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("globalConfigurations"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    Dictionary<string, string> dictionary = new Dictionary<string, string>();
                    foreach (var property0 in property.Value.EnumerateObject())
                    {
                        dictionary.Add(property0.Name, property0.Value.GetString());
                    }
                    globalConfigurations = dictionary;
                    continue;
                }
                if (property.NameEquals("tableList"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    List<AzureMonitorTableConfiguration> array = new List<AzureMonitorTableConfiguration>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(AzureMonitorTableConfiguration.DeserializeAzureMonitorTableConfiguration(item));
                    }
                    tableList = array;
                    continue;
                }
            }
            return new AzureMonitorSelectedConfigurations(configurationVersion.Value, Optional.ToDictionary(globalConfigurations), Optional.ToList(tableList));
        }
    }
}
