...
namespace Azure.ResourceManager.DevTestLabs
{
    ...
    public partial class DevTestLabScheduleResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation> ExecuteAsync(WaitUntil waitUntil, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _devTestLabScheduleSchedulesRestClient.CreateExecuteRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name);
                var response = await _devTestLabScheduleSchedulesRestClient.ExecuteAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new DevTestLabsArmOperation(_devTestLabScheduleSchedulesClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionResponseAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        public virtual ArmOperation Execute(WaitUntil waitUntil, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _devTestLabScheduleSchedulesRestClient.CreateExecuteRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name);
                var response = _devTestLabScheduleSchedulesRestClient.Execute(message, cancellationToken);
                var operation = new DevTestLabsArmOperation(_devTestLabScheduleSchedulesClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletionResponse(cancellationToken);
                return operation;
