// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;
using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.DevTestLabs.Models
{
    public partial class PolicySetResult
    {
        internal static PolicySetResult DeserializePolicySetResult(JsonElement element)
        {
            Optional<bool> hasError = default;
            Optional<IReadOnlyList<PolicyViolation>> policyViolations = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("hasError"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    hasError = property.Value.GetBoolean();
                    continue;
                }
                if (property.NameEquals("policyViolations"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    List<PolicyViolation> array = new List<PolicyViolation>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(PolicyViolation.DeserializePolicyViolation(item));
                    }
                    policyViolations = array;
                    continue;
                }
            }
            return new PolicySetResult(Optional.ToNullable(hasError), Optional.ToList(policyViolations));
        }
    }
}
