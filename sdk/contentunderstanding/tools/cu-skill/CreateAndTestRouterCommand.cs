...
internal static class CreateAndTestRouterCommand
{
    ...
    private static Dictionary<string, string> ParseInnerArg(IReadOnlyList<string> values)
    {
    ...
    private static (JsonObject Patched, List<string> Errors) WireInnerIds(
    ...
    private static string SummarizeRouted(IReadOnlyList<(string DocName, JsonObject Doc)> results)
    {
        ...
        foreach (var (docName, doc) in results)
        {
            ...
            foreach (var content in contents.OfType<JsonObject>())
            {
                ...
                foreach (var (fname, fnode) in fields)
                {
                    ...
                    rows.Add((docName, FieldValue(fobj), GetConfidence(fobj)));
                }
            }
        }
