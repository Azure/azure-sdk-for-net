...
namespace Azure.AI.ContentUnderstanding.Samples
{
    public partial class ContentUnderstandingSamples
    {
        ...
        public async Task CreateAnalyzerWithLabelsAsync()
        {
            ...
            Assert.AreEqual("receipt_schema", result.FieldSchema.Name, "Field schema name should match");
            ...
            Assert.IsTrue(result.FieldSchema.Fields.ContainsKey("MerchantName"), "Should contain MerchantName field");
            var merchantNameDef = result.FieldSchema.Fields["MerchantName"];
            Assert.AreEqual(ContentFieldType.String, merchantNameDef.Type, "MerchantName should be String type");
            Assert.AreEqual(GenerationMethod.Extract, merchantNameDef.Method, "MerchantName should use Extract method");
            Console.WriteLine("  MerchantName field verified (String, Extract)");

            // Verify Items field (array of objects)
            Assert.IsTrue(result.FieldSchema.Fields.ContainsKey("Items"), "Should contain Items field");
            ...
            Assert.AreEqual(GenerationMethod.Generate, itemsDef.Method, "Items should use Generate method");
            Assert.IsNotNull(itemsDef.ItemDefinition, "Items should have item definition");
            Assert.AreEqual(ContentFieldType.Object, itemsDef.ItemDefinition.Type, "Items.ItemDefinition should be Object type");
            Assert.IsNotNull(itemsDef.ItemDefinition.Properties, "Items.ItemDefinition should have properties");
            Assert.AreEqual(3, itemsDef.ItemDefinition.Properties.Count, "Items.ItemDefinition should have 3 properties");
            Assert.IsTrue(itemsDef.ItemDefinition.Properties.ContainsKey("Quantity"), "Items.ItemDefinition should have Quantity property");
            Assert.IsTrue(itemsDef.ItemDefinition.Properties.ContainsKey("Name"), "Items.ItemDefinition should have Name property");
            Assert.IsTrue(itemsDef.ItemDefinition.Properties.ContainsKey("Price"), "Items.ItemDefinition should have Price property");
            Console.WriteLine("  Items field verified (Array of Objects with 3 properties)");

            // Verify TotalPrice field
            Assert.IsTrue(result.FieldSchema.Fields.ContainsKey("TotalPrice"), "Should contain TotalPrice field");
            var totalPriceDef = result.FieldSchema.Fields["TotalPrice"];
            Assert.AreEqual(ContentFieldType.String, totalPriceDef.Type, "TotalPrice should be String type");
            Assert.AreEqual(GenerationMethod.Extract, totalPriceDef.Method, "TotalPrice should use Extract method");
            ...
            Assert.IsInstanceOf<LabeledDataKnowledgeSource>(ks, "Knowledge source should be LabeledDataKnowledgeSource");
            var labeledKs = (LabeledDataKnowledgeSource)ks;
            Assert.IsNotNull(labeledKs.ContainerUrl, "Knowledge source container URL should not be null");
            Assert.IsFalse(string.IsNullOrWhiteSpace(labeledKs.Prefix), "Knowledge source prefix should not be empty");
            Console.WriteLine($"Knowledge source verified: type=LabeledData, prefix={labeledKs.Prefix}");
            ...
            Console.WriteLine("All analyzer creation with labeled data properties validated successfully");
            #endregion
        ...
        private async Task<string> GetOrGenerateTrainingDataSasUrlAsync()
        {
            ...
            var sasToken = sasBuilder.ToSasQueryParameters(userDelegationKey.Value, storageAccount).ToString();
            var sasUrl = $"https://{storageAccount}.blob.core.windows.net/{containerName}?{sasToken}";
