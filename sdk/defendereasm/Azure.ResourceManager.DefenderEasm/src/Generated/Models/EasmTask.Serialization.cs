...
namespace Azure.ResourceManager.DefenderEasm.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<EasmTask>)this).Write(writer, ModelReaderWriterOptions.Wire);

        void IJsonModel<EasmTask>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<EasmTask>)this).GetWireFormat(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<EasmTask>)} interface");
            }
        ...
        string IPersistableModel<EasmTask>.GetWireFormat(ModelReaderWriterOptions options) => "J";
    }
}
