...
namespace Azure.ResourceManager.ChangeAnalysis.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<ChangeList>)this).Write(writer, ModelReaderWriterOptions.Wire);

        void IJsonModel<ChangeList>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<ChangeList>)this).GetWireFormat(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<ChangeList>)} interface");
            }
        ...
        string IPersistableModel<ChangeList>.GetWireFormat(ModelReaderWriterOptions options) => "J";
    }
}
