// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.ComponentModel;
using System.Diagnostics.CodeAnalysis;
using Azure.Core;
using Azure.Provisioning;
using Azure.Provisioning.Primitives;
using Azure.Provisioning.Resources;

namespace Azure.Provisioning.ApiManagement
{
    /// <summary> The developer portal configuration contract. </summary>
    public partial class PortalConfigContract : ProvisionableResource
    {
        private BicepValue<ResourceIdentifier> _id;
        private BicepValue<string> _name;
        private SystemData _systemData;
        private PortalConfigProperties _properties;
        private ResourceReference<ApiManagementService> _parent;

        /// <summary> Creates a new PortalConfigContract. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public PortalConfigContract(string bicepIdentifier, string resourceVersion = null) : base(bicepIdentifier, "Microsoft.ApiManagement/service/portalconfigs", resourceVersion ?? "2025-09-01-preview")
        {
        }

        /// <summary> Gets the Id. </summary>
        public BicepValue<ResourceIdentifier> Id
        {
            get
            {
                Initialize();
                return _id;
            }
        }

        /// <summary> Gets or sets the Name. </summary>
        public BicepValue<string> Name
        {
            get
            {
                Initialize();
                return _name;
            }
            set
            {
                Initialize();
                _name.Assign(value);
            }
        }

        /// <summary> Gets the SystemData. </summary>
        public SystemData SystemData
        {
            get
            {
                Initialize();
                return _systemData;
            }
        }

        /// <summary> Gets or sets the Properties. </summary>
        internal PortalConfigProperties Properties
        {
            get
            {
                Initialize();
                return _properties;
            }
            set
            {
                Initialize();
                AssignOrReplace(ref _properties, value);
            }
        }

        /// <summary> Gets or sets the Parent. </summary>
        public ApiManagementService Parent
        {
            get
            {
                Initialize();
                return _parent.Value;
            }
            set
            {
                Initialize();
                _parent.Value = value;
            }
        }

        /// <summary> Gets or sets the EnableBasicAuth. </summary>
        public BicepValue<bool> EnableBasicAuth
        {
            get
            {
                return Properties is null ? default : Properties.EnableBasicAuth;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.EnableBasicAuth = value;
            }
        }

        /// <summary> Gets or sets the Delegation. </summary>
        public PortalConfigDelegationProperties Delegation
        {
            get
            {
                return Properties is null ? default : Properties.Delegation;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.Delegation = value;
            }
        }

        /// <summary> Gets or sets the Csp. </summary>
        public PortalConfigCspProperties Csp
        {
            get
            {
                return Properties is null ? default : Properties.Csp;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.Csp = value;
            }
        }

        /// <summary> Gets or sets the Require. </summary>
        public BicepValue<bool> SigninRequire
        {
            get
            {
                return Properties is null ? default : Properties.SigninRequire;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.SigninRequire = value;
            }
        }

        /// <summary> Gets or sets the TermsOfService. </summary>
        public PortalConfigTermsOfServiceProperties SignupTermsOfService
        {
            get
            {
                return Properties is null ? default : Properties.SignupTermsOfService;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.SignupTermsOfService = value;
            }
        }

        /// <summary> Gets or sets the AllowedOrigins. </summary>
        public BicepList<string> CorsAllowedOrigins
        {
            get
            {
                return Properties is null ? default : Properties.CorsAllowedOrigins;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new PortalConfigProperties();
                }
                Properties.CorsAllowedOrigins = value;
            }
        }

        /// <summary> Define all the provisionable properties for PortalConfigContract. </summary>
        protected override void DefineProvisionableProperties()
        {
            base.DefineProvisionableProperties();
            _id = DefineProperty<ResourceIdentifier>(nameof(Id), new string[] { "id" }, isOutput: true);
            _name = DefineProperty<string>(nameof(Name), new string[] { "name" }, isRequired: true);
            _systemData = DefineModelProperty<SystemData>(nameof(SystemData), new string[] { "systemData" }, isOutput: true);
            _properties = DefineModelProperty<PortalConfigProperties>(nameof(Properties), new string[] { "properties" });
            _parent = DefineResource<ApiManagementService>(nameof(Parent), new string[] { "parent" }, isRequired: true);
            DefineAdditionalProperties();
        }

        /// <summary> Creates a reference to an existing PortalConfigContract. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public static PortalConfigContract FromExisting(string bicepIdentifier, string resourceVersion = null)
        {
            PortalConfigContract result = new PortalConfigContract(bicepIdentifier, resourceVersion);
            result.IsExistingResource = true;
            return result;
        }

        /// <summary> Define additional provisionable properties for PortalConfigContract that are not part of the generated code. </summary>
        partial void DefineAdditionalProperties();

        /// <summary> Get the requirements for naming this resource. </summary>
        /// <returns> Naming requirements. </returns>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override ResourceNameRequirements GetResourceNameRequirements() => new ResourceNameRequirements(1, 80, ResourceNameCharacters.LowercaseLetters | ResourceNameCharacters.UppercaseLetters | ResourceNameCharacters.Numbers | ResourceNameCharacters.Hyphen | ResourceNameCharacters.Underscore | ResourceNameCharacters.Period | ResourceNameCharacters.Parentheses);

        /// <summary></summary>
        public static partial class ResourceVersions
        {
            /// <summary> API version "2025-09-01-preview". </summary>
            [Experimental("AZPROVISION001")]
            public static readonly string V2025_09_01_PREVIEW = "2025-09-01-preview";
        }
    }
}
