// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.ComponentModel;
using System.Diagnostics.CodeAnalysis;
using Azure.Core;
using Azure.Provisioning;
using Azure.Provisioning.Primitives;
using Azure.Provisioning.Resources;

namespace Azure.Provisioning.ApiManagement
{
    /// <summary> Diagnostic details. </summary>
    public partial class ServiceWorkspaceDiagnostic : ProvisionableResource
    {
        private BicepValue<ResourceIdentifier> _id;
        private BicepValue<string> _name;
        private SystemData _systemData;
        private DiagnosticContractProperties _properties;
        private ResourceReference<WorkspaceContract> _parent;

        /// <summary> Creates a new ServiceWorkspaceDiagnostic. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public ServiceWorkspaceDiagnostic(string bicepIdentifier, string resourceVersion = null) : base(bicepIdentifier, "Microsoft.ApiManagement/service/workspaces/diagnostics", resourceVersion ?? "2025-09-01-preview")
        {
        }

        /// <summary> Gets the Id. </summary>
        public BicepValue<ResourceIdentifier> Id
        {
            get
            {
                Initialize();
                return _id;
            }
        }

        /// <summary> Gets or sets the Name. </summary>
        public BicepValue<string> Name
        {
            get
            {
                Initialize();
                return _name;
            }
            set
            {
                Initialize();
                _name.Assign(value);
            }
        }

        /// <summary> Gets the SystemData. </summary>
        public SystemData SystemData
        {
            get
            {
                Initialize();
                return _systemData;
            }
        }

        /// <summary> Gets or sets the Properties. </summary>
        internal DiagnosticContractProperties Properties
        {
            get
            {
                Initialize();
                return _properties;
            }
            set
            {
                Initialize();
                AssignOrReplace(ref _properties, value);
            }
        }

        /// <summary> Gets or sets the Parent. </summary>
        public WorkspaceContract Parent
        {
            get
            {
                Initialize();
                return _parent.Value;
            }
            set
            {
                Initialize();
                _parent.Value = value;
            }
        }

        /// <summary> Gets or sets the AlwaysLog. </summary>
        public BicepValue<AlwaysLog> AlwaysLog
        {
            get
            {
                return Properties is null ? default : Properties.AlwaysLog;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.AlwaysLog = value;
            }
        }

        /// <summary> Gets or sets the LoggerId. </summary>
        public BicepValue<string> LoggerId
        {
            get
            {
                return Properties is null ? default : Properties.LoggerId;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.LoggerId = value;
            }
        }

        /// <summary> Gets or sets the Sampling. </summary>
        public SamplingSettings Sampling
        {
            get
            {
                return Properties is null ? default : Properties.Sampling;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.Sampling = value;
            }
        }

        /// <summary> Gets or sets the Frontend. </summary>
        public PipelineDiagnosticSettings Frontend
        {
            get
            {
                return Properties is null ? default : Properties.Frontend;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.Frontend = value;
            }
        }

        /// <summary> Gets or sets the Backend. </summary>
        public PipelineDiagnosticSettings Backend
        {
            get
            {
                return Properties is null ? default : Properties.Backend;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.Backend = value;
            }
        }

        /// <summary> Gets or sets the LargeLanguageModel. </summary>
        public LLMDiagnosticSettings LargeLanguageModel
        {
            get
            {
                return Properties is null ? default : Properties.LargeLanguageModel;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.LargeLanguageModel = value;
            }
        }

        /// <summary> Gets or sets the IsLogClientIPEnabled. </summary>
        public BicepValue<bool> IsLogClientIPEnabled
        {
            get
            {
                return Properties is null ? default : Properties.IsLogClientIPEnabled;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.IsLogClientIPEnabled = value;
            }
        }

        /// <summary> Gets or sets the HttpCorrelationProtocol. </summary>
        public BicepValue<HttpCorrelationProtocol> HttpCorrelationProtocol
        {
            get
            {
                return Properties is null ? default : Properties.HttpCorrelationProtocol;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.HttpCorrelationProtocol = value;
            }
        }

        /// <summary> Gets or sets the Verbosity. </summary>
        public BicepValue<TraceVerbosityLevel> Verbosity
        {
            get
            {
                return Properties is null ? default : Properties.Verbosity;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.Verbosity = value;
            }
        }

        /// <summary> Gets or sets the OperationNameFormat. </summary>
        public BicepValue<OperationNameFormat> OperationNameFormat
        {
            get
            {
                return Properties is null ? default : Properties.OperationNameFormat;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.OperationNameFormat = value;
            }
        }

        /// <summary> Gets or sets the Metrics. </summary>
        public BicepValue<bool> Metrics
        {
            get
            {
                return Properties is null ? default : Properties.Metrics;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DiagnosticContractProperties();
                }
                Properties.Metrics = value;
            }
        }

        /// <summary> Define all the provisionable properties for ServiceWorkspaceDiagnostic. </summary>
        protected override void DefineProvisionableProperties()
        {
            base.DefineProvisionableProperties();
            _id = DefineProperty<ResourceIdentifier>(nameof(Id), new string[] { "id" }, isOutput: true);
            _name = DefineProperty<string>(nameof(Name), new string[] { "name" }, isRequired: true);
            _systemData = DefineModelProperty<SystemData>(nameof(SystemData), new string[] { "systemData" }, isOutput: true);
            _properties = DefineModelProperty<DiagnosticContractProperties>(nameof(Properties), new string[] { "properties" });
            _parent = DefineResource<WorkspaceContract>(nameof(Parent), new string[] { "parent" }, isRequired: true);
            DefineAdditionalProperties();
        }

        /// <summary> Creates a reference to an existing ServiceWorkspaceDiagnostic. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public static ServiceWorkspaceDiagnostic FromExisting(string bicepIdentifier, string resourceVersion = null)
        {
            ServiceWorkspaceDiagnostic result = new ServiceWorkspaceDiagnostic(bicepIdentifier, resourceVersion);
            result.IsExistingResource = true;
            return result;
        }

        /// <summary> Define additional provisionable properties for ServiceWorkspaceDiagnostic that are not part of the generated code. </summary>
        partial void DefineAdditionalProperties();

        /// <summary> Get the requirements for naming this resource. </summary>
        /// <returns> Naming requirements. </returns>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override ResourceNameRequirements GetResourceNameRequirements() => new ResourceNameRequirements(1, 80, ResourceNameCharacters.LowercaseLetters | ResourceNameCharacters.UppercaseLetters | ResourceNameCharacters.Numbers | ResourceNameCharacters.Hyphen | ResourceNameCharacters.Underscore | ResourceNameCharacters.Period | ResourceNameCharacters.Parentheses);

        /// <summary></summary>
        public static partial class ResourceVersions
        {
            /// <summary> API version "2025-09-01-preview". </summary>
            [Experimental("AZPROVISION001")]
            public static readonly string V2025_09_01_PREVIEW = "2025-09-01-preview";
        }
    }
}
