...
{
    // HEAD/entity-tag/existence-check backward compatibility.
    //
    // The MPG generator intentionally omits CheckExistence (HEAD) operations from the
    // resource class public API (see https://github.com/Azure/azure-sdk-for-net/issues/56996).
    // The old AutoRest SDK exposed these as GetEntityTag/GetEntityTagAsync returning Response<bool>
    // on each resource, plus cross-resource existence checks (e.g., GetGatewayApiEntityTag,
    // CheckGroupUserEntityExists). These custom methods preserve the old surface.
    //
    // All 65+ resource classes are kept in one file rather than split per-type because:
    // 1. Every entry follows the exact same mechanical pattern (delegate to collection.Exists
    //    or call a Raw method and convert via ToExistsResponse)
    // 2. Splitting would create 65+ tiny files with no independent logic
    // 3. The entire file can be removed once issue #56996 is resolved
