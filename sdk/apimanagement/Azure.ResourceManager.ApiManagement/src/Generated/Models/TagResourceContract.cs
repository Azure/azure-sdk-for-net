// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;

namespace Azure.ResourceManager.ApiManagement.Models
{
    /// <summary> TagResource contract properties. </summary>
    public partial class TagResourceContract
    {
        /// <summary> Initializes a new instance of TagResourceContract. </summary>
        /// <param name="tag"> Tag associated with the resource. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="tag"/> is null. </exception>
        internal TagResourceContract(TagResourceContractProperties tag)
        {
            if (tag == null)
            {
                throw new ArgumentNullException(nameof(tag));
            }

            Tag = tag;
        }

        /// <summary> Initializes a new instance of TagResourceContract. </summary>
        /// <param name="tag"> Tag associated with the resource. </param>
        /// <param name="api"> API associated with the tag. </param>
        /// <param name="operation"> Operation associated with the tag. </param>
        /// <param name="product"> Product associated with the tag. </param>
        internal TagResourceContract(TagResourceContractProperties tag, ApiTagResourceContractProperties api, OperationTagResourceContractProperties operation, ProductTagResourceContractProperties product)
        {
            Tag = tag;
            Api = api;
            Operation = operation;
            Product = product;
        }

        /// <summary> Tag associated with the resource. </summary>
        public TagResourceContractProperties Tag { get; }
        /// <summary> API associated with the tag. </summary>
        public ApiTagResourceContractProperties Api { get; }
        /// <summary> Operation associated with the tag. </summary>
        public OperationTagResourceContractProperties Operation { get; }
        /// <summary> Product associated with the tag. </summary>
        public ProductTagResourceContractProperties Product { get; }
    }
}
