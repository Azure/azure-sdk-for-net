// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.ApiManagement.Models
{
    /// <summary> OAuth2 authorization code with federated identity credentials grant type parameters. </summary>
    public partial class AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType"/>. </summary>
        public AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType()
        {
        }

        /// <summary> Initializes a new instance of <see cref="AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType"/>. </summary>
        /// <param name="clientId"> Client ID of the application in the identity provider. </param>
        /// <param name="clientAssertionType"> The type of client assertion used for federated identity credentials. </param>
        /// <param name="resourceUri"> The resource URI for the target service. </param>
        /// <param name="tenantId"> Tenant ID of the identity provider. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal AuthorizationProviderOAuth2FederatedIdentityCredentialsGrantType(string clientId, string clientAssertionType, string resourceUri, string tenantId, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            ClientId = clientId;
            ClientAssertionType = clientAssertionType;
            ResourceUri = resourceUri;
            TenantId = tenantId;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Client ID of the application in the identity provider. </summary>
        public string ClientId { get; set; }

        /// <summary> The type of client assertion used for federated identity credentials. </summary>
        public string ClientAssertionType { get; }

        /// <summary> The resource URI for the target service. </summary>
        public string ResourceUri { get; set; }

        /// <summary> Tenant ID of the identity provider. </summary>
        public string TenantId { get; set; }
    }
}
