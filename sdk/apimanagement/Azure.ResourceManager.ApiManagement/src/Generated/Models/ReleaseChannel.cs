// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.ComponentModel;
using Azure.ResourceManager.ApiManagement;

namespace Azure.ResourceManager.ApiManagement.Models
{
    /// <summary> Release Channel of this API Management service. </summary>
    public readonly partial struct ReleaseChannel : IEquatable<ReleaseChannel>
    {
        private readonly string _value;
        /// <summary> Preview Channel of the service. </summary>
        private const string PreviewValue = "Preview";
        /// <summary> Default Channel of the service. </summary>
        private const string DefaultValue = "Default";
        /// <summary> Stable Channel of the service. </summary>
        private const string StableValue = "Stable";

        /// <summary> Initializes a new instance of <see cref="ReleaseChannel"/>. </summary>
        /// <param name="value"> The value. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="value"/> is null. </exception>
        public ReleaseChannel(string value)
        {
            Argument.AssertNotNull(value, nameof(value));

            _value = value;
        }

        /// <summary> Preview Channel of the service. </summary>
        public static ReleaseChannel Preview { get; } = new ReleaseChannel(PreviewValue);

        /// <summary> Default Channel of the service. </summary>
        public static ReleaseChannel Default { get; } = new ReleaseChannel(DefaultValue);

        /// <summary> Stable Channel of the service. </summary>
        public static ReleaseChannel Stable { get; } = new ReleaseChannel(StableValue);

        /// <summary> Determines if two <see cref="ReleaseChannel"/> values are the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator ==(ReleaseChannel left, ReleaseChannel right) => left.Equals(right);

        /// <summary> Determines if two <see cref="ReleaseChannel"/> values are not the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator !=(ReleaseChannel left, ReleaseChannel right) => !left.Equals(right);

        /// <summary> Converts a string to a <see cref="ReleaseChannel"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator ReleaseChannel(string value) => new ReleaseChannel(value);

        /// <summary> Converts a string to a <see cref="ReleaseChannel"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator ReleaseChannel?(string value) => value == null ? null : new ReleaseChannel(value);

        /// <inheritdoc/>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override bool Equals(object obj) => obj is ReleaseChannel other && Equals(other);

        /// <inheritdoc/>
        public bool Equals(ReleaseChannel other) => string.Equals(_value, other._value, StringComparison.InvariantCultureIgnoreCase);

        /// <inheritdoc/>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public override int GetHashCode() => _value != null ? StringComparer.InvariantCultureIgnoreCase.GetHashCode(_value) : 0;

        /// <inheritdoc/>
        public override string ToString() => _value;
    }
}
