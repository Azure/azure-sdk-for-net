// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.ApiManagement;

namespace Azure.ResourceManager.ApiManagement.Models
{
    /// <summary> GraphQL API Resolver Entity Base Contract details. </summary>
    internal partial class ResolverEntityBaseContract
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ResolverEntityBaseContract"/>. </summary>
        public ResolverEntityBaseContract()
        {
        }

        /// <summary> Initializes a new instance of <see cref="ResolverEntityBaseContract"/>. </summary>
        /// <param name="displayName"> Resolver Name. </param>
        /// <param name="path"> Path is type/field being resolved. </param>
        /// <param name="description"> Description of the resolver. May include HTML formatting tags. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ResolverEntityBaseContract(string displayName, string path, string description, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            DisplayName = displayName;
            Path = path;
            Description = description;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Resolver Name. </summary>
        [WirePath("displayName")]
        public string DisplayName { get; set; }

        /// <summary> Path is type/field being resolved. </summary>
        [WirePath("path")]
        public string Path { get; set; }

        /// <summary> Description of the resolver. May include HTML formatting tags. </summary>
        [WirePath("description")]
        public string Description { get; set; }
    }
}
