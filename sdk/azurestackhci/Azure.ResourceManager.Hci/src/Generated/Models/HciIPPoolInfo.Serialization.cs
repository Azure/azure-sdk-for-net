// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.Hci.Models
{
    public partial class HciIPPoolInfo : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            writer.WriteEndObject();
        }

        internal static HciIPPoolInfo DeserializeHciIPPoolInfo(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            Optional<string> used = default;
            Optional<string> available = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("used"u8))
                {
                    used = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("available"u8))
                {
                    available = property.Value.GetString();
                    continue;
                }
            }
            return new HciIPPoolInfo(used.Value, available.Value);
        }
    }
}
