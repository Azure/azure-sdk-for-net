// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.MigrationDiscovery.Models
{
    /// <summary> BizTalkServer in the guest virtual machine. </summary>
    public partial class BizTalkServer
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="BizTalkServer"/>. </summary>
        internal BizTalkServer()
        {
        }

        /// <summary> Initializes a new instance of <see cref="BizTalkServer"/>. </summary>
        /// <param name="productName"> Gets or sets ProductName of the BizTalkServer. </param>
        /// <param name="status"> Gets or sets Status of the BizTalkServer. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal BizTalkServer(string productName, string status, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            ProductName = productName;
            Status = status;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Gets or sets ProductName of the BizTalkServer. </summary>
        public string ProductName { get; }

        /// <summary> Gets or sets Status of the BizTalkServer. </summary>
        public string Status { get; }
    }
}
