// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.Databricks.Models
{
    /// <summary> The value which should be used for this field. </summary>
    public partial class WorkspaceCustomBooleanParameterValue
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="WorkspaceCustomBooleanParameterValue"/>. </summary>
        /// <param name="isEnabled"> The value which should be used for this field. </param>
        public WorkspaceCustomBooleanParameterValue(bool isEnabled)
        {
            IsEnabled = isEnabled;
        }

        /// <summary> Initializes a new instance of <see cref="WorkspaceCustomBooleanParameterValue"/>. </summary>
        /// <param name="type"> The type of variable that this is. </param>
        /// <param name="isEnabled"> The value which should be used for this field. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal WorkspaceCustomBooleanParameterValue(DatabricksCustomParameterType? @type, bool isEnabled, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Type = @type;
            IsEnabled = isEnabled;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The type of variable that this is. </summary>
        public DatabricksCustomParameterType? Type { get; set; }

        /// <summary> The value which should be used for this field. </summary>
        public bool IsEnabled { get; set; }
    }
}
