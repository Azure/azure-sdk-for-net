...
{
    public partial class ProjectMetadata : IUtf8JsonSerializable, IJsonModel<ProjectMetadata>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<ProjectMetadata>)this).Write(writer, ModelSerializationExtensions.WireOptions);

        void IJsonModel<ProjectMetadata>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
        ...
        {
            var format = options.Format == "W" ? ((IPersistableModel<ProjectMetadata>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        ProjectMetadata IJsonModel<ProjectMetadata>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ProjectMetadata>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(ProjectMetadata)} does not support reading '{format}' format.");
            }

            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeProjectMetadata(document.RootElement, options);
        }

        internal static ProjectMetadata DeserializeProjectMetadata(JsonElement element, ModelReaderWriterOptions options = null)
        {
        ...
        BinaryData IPersistableModel<ProjectMetadata>.Write(ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ProjectMetadata>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(ProjectMetadata)} does not support writing '{options.Format}' format.");
            }
        }

        ProjectMetadata IPersistableModel<ProjectMetadata>.Create(BinaryData data, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ProjectMetadata>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(ProjectMetadata)} does not support reading '{options.Format}' format.");
            }
        }

        string IPersistableModel<ProjectMetadata>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";

        /// <summary> Deserializes the model from a raw response. </summary>
        /// <param name="response"> The response to deserialize the model from. </param>
        internal static ProjectMetadata FromResponse(Response response)
        {
            using var document = JsonDocument.Parse(response.Content);
            return DeserializeProjectMetadata(document.RootElement);
        }
