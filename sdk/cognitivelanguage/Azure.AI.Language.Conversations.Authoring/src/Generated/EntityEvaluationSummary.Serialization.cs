...
{
    public partial class EntityEvaluationSummary : IUtf8JsonSerializable, IJsonModel<EntityEvaluationSummary>
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<EntityEvaluationSummary>)this).Write(writer, ModelSerializationExtensions.WireOptions);
        ...
        EntityEvaluationSummary IJsonModel<EntityEvaluationSummary>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            ...
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(EntityEvaluationSummary)} does not support reading '{format}' format.");
            }

            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeEntityEvaluationSummary(document.RootElement, options);
        }

        internal static EntityEvaluationSummary DeserializeEntityEvaluationSummary(JsonElement element, ModelReaderWriterOptions options = null)
        {
        ...
        BinaryData IPersistableModel<EntityEvaluationSummary>.Write(ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<EntityEvaluationSummary>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(EntityEvaluationSummary)} does not support writing '{options.Format}' format.");
            }
        }

        EntityEvaluationSummary IPersistableModel<EntityEvaluationSummary>.Create(BinaryData data, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<EntityEvaluationSummary>)this).GetFormatFromOptions(options) : options.Format;
            ...
            switch (format)
            {
                ...
                default:
                    throw new FormatException($"The model {nameof(EntityEvaluationSummary)} does not support reading '{options.Format}' format.");
            }
        }

        string IPersistableModel<EntityEvaluationSummary>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";

        /// <summary> Deserializes the model from a raw response. </summary>
        /// <param name="response"> The response to deserialize the model from. </param>
        internal static EntityEvaluationSummary FromResponse(Response response)
        {
            using var document = JsonDocument.Parse(response.Content);
            return DeserializeEntityEvaluationSummary(document.RootElement);
        }
