...
### Features Added
- Added `QuestionAnsweringProject.ConfigureSemanticRanking` property to enable configuring semantic ranking behavior for authoring projects.
- Added support for preview service version `2025-05-15-preview`(`QuestionAnsweringClientOptions.ServiceVersion.V2025_05_15_Preview`).

### Migration Guidance
- Inference only: add the NuGet package `Azure.AI.Language.QuestionAnswering.inference`.
- Authoring (it transitively brings inference): add the NuGet package `Azure.AI.Language.QuestionAnswering`.
