...
To summarize one or more text documents using extractive summarization, call `AnalyzeTextOperation` on the `TextClient` by passing the documents as `MultiLanguageTextInput` parameter and a `AnalyzeTextOperationAction` with a `ExtractiveSummarizationOperationAction` action. This returns a `Response<AnalyzeTextOperationState>` which you can extract the `ExtractiveSummarizationOperationResult`.

```C# Snippet:Sample11_AnalyzeTextOperation_ExtractiveSummarizationOperationAction
string documentA =
