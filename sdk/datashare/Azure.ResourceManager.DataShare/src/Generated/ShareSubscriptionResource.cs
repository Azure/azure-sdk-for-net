...
namespace Azure.ResourceManager.DataShare
{
    ...
    public partial class ShareSubscriptionResource : ArmResource
    {
        ...
        internal ShareSubscriptionResource(ArmClient client, ShareSubscriptionData data, ResourceIdentifier id) : this(client, id)
        {
            HasData = true;
            _data = data;
        }
        ...
        public virtual async Task<Response<ShareSubscriptionResource>> GetAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                return Response.FromValue(new ShareSubscriptionResource(Client, response.Value, response.Value.Id), response.GetRawResponse());
            }
            catch (Exception e)
            {
                scope.Failed(e);
        ...
        public virtual async Task<ArmOperation<ShareSubscriptionResource>> UpdateAsync(WaitUntil waitUntil, ShareSubscriptionData data, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var operation = new DataShareArmOperation<ShareSubscriptionResource>(Response.FromValue(new ShareSubscriptionResource(Client, response.Value, response.Value.Id), response.GetRawResponse()));
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
