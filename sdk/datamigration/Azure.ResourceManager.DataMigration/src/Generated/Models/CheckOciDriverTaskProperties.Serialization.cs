...
{
    public partial class CheckOciDriverTaskProperties : IUtf8JsonSerializable
    {
        ...
        internal static CheckOciDriverTaskProperties DeserializeCheckOciDriverTaskProperties(JsonElement element)
        {
            ...
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("input"u8))
                {
                    ...
                    input = CheckOciDriverTaskInput.DeserializeCheckOciDriverTaskInput(property.Value);
                    continue;
                }
                if (property.NameEquals("output"u8))
                {
                    ...
                    List<CheckOciDriverTaskOutput> array = new List<CheckOciDriverTaskOutput>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(CheckOciDriverTaskOutput.DeserializeCheckOciDriverTaskOutput(item));
                    }
                    output = array;
                    continue;
                }
            ...
            return new CheckOciDriverTaskProperties(taskType, Optional.ToList(errors), Optional.ToNullable(state), Optional.ToList(commands), Optional.ToDictionary(clientData), input.Value, Optional.ToList(output));
        }
    }
}
