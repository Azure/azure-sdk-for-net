...
#### via ILogger Compile-Time Source Generation (recommended)

Starting with .NET 6, [compile-time logging source generation](https://learn.microsoft.com/dotnet/core/extensions/logger-message-generator)
was introduced introduced as a high performant logging option.

In this example, the message template is defined in a `LoggerMessage` attribute.
The parameter names must match the names in the message template.
However, C# doesn't support parameter names with special characters.
For this approach we must use an additional package `Microsoft.Extensions.Telemetry`
and the `TagName` attribute to specify which method parameter matches the `"microsoft.custom_event.name"` attribute in the message template.

```csharp
[LoggerMessage(level: LogLevel.Information, Message = "{microsoft.custom_event.name} {key1} {key2}")]
public static partial void WriteCustomEvent(this ILogger logger, [TagName("microsoft.custom_event.name")] string customEventName, string key1, string key2);
```
...
logger.WriteCustomEvent("MyCustomEventName", "value1", "value2");
```
