...
namespace Azure.Monitor.OpenTelemetry.Exporter.Demo.Logs
{
    ...
    /// exercise multi-endpoint log routing end to end. Mirrors <see cref="MultiEndpointTraceDemo"/> but
    /// for <see cref="LogRecord"/>s.
    /// </summary>
    /// <remarks>
    /// The switch this depends on is read once into a static, so
    /// <see cref="MultiEndpointTraceDemo.EnableMultiEndpointRouting"/> has to run before any exporter type
    /// is touched.
    /// </remarks>
    internal sealed class MultiEndpointLogDemo : IDisposable
    {
        ...
        /// the routing tags in its OnEnd.
        /// </summary>
        public void GenerateLogs(int count)
        {
            var logger = _loggerFactory.CreateLogger<MultiEndpointLogDemo>();
            ...
            for (int i = 0; i < count; i++)
            {
                switch (i % 4)
                {
                    case 0:
                        ...
                        logger.LogInformation("MultiEndpointTrace-{iteration} from {source}.", i, "demo");
                        break;
                    ...
                    case 1:
                        ...
                        {
                            logger.LogError(ex, "MultiEndpointException-{iteration} failed.", i);
                        }
                        break;
                    ...
                    case 2:
                        ...
                        Emit(
                            ...
                            $"MultiEndpointEvent-{i}",
                            exception: null,
                            new KeyValuePair<string, object?>(CustomEventNameAttribute, "MultiEndpointDemoEvent"),
                    ...
                    default:
                        ...
                        Emit(
                            logger,
                            LogLevel.Information,
                            $"MultiEndpointAvailability-{i}",
                            exception: null,
                            new KeyValuePair<string, object?>(AvailabilityIdAttribute, Guid.NewGuid().ToString("N")),
                            new KeyValuePair<string, object?>(AvailabilityNameAttribute, "MultiEndpointDemoTest"),
        ...
        /// <summary>
        /// Stamps each <see cref="LogRecord"/> with a randomly chosen route's routing attributes, so
        ...
        /// </summary>
        private sealed class EndpointRoutingLogProcessor : BaseProcessor<LogRecord>
        {
            ...
            internal EndpointRoutingLogProcessor(IReadOnlyList<MultiEndpointTraceDemo.EndpointRoute> routes, string runId)
            {
                _routes = routes;
                _runId = runId;
            ...
            {
                MultiEndpointTraceDemo.EndpointRoute route;
                ...
                attributes.Add(new KeyValuePair<string, object?>(CloudRoleAttributeName, route.Name));
                ...
                attributes.Add(new KeyValuePair<string, object?>("demo.route", route.Name));
