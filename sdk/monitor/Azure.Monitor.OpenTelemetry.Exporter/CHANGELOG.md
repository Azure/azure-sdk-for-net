...
- Added multi-endpoint routing for traces, off by default and enabled with the `Azure.Monitor.OpenTelemetry.EnableMultiEndpointRouting` AppContext switch. When enabled, an Activity carrying the `microsoft.instrumentation_key` and `microsoft.ingestion_endpoint` attributes is sent to that endpoint instead of the exporter's own; Activities without both attributes are dropped. Live Metrics is disabled while the switch is on, and sampling defaults to fixed-rate rather than rate-limited because a per-process rate limit would be shared across every destination the process carries.
  ([#62707](https://github.com/Azure/azure-sdk-for-net/pull/62707))

- Extended multi-endpoint routing to logs, off by default and enabled with the same `Azure.Monitor.OpenTelemetry.EnableMultiEndpointRouting` AppContext switch used for traces. When enabled, a `LogRecord` carrying the `microsoft.instrumentation_key` and `microsoft.ingestion_endpoint` attributes is sent to that endpoint instead of the exporter's own; log records without both attributes are dropped. The two routing attributes are consumed for routing and are not emitted as custom properties. Routing reads only `LogRecord.Attributes`, not logging scopes. Live Metrics disablement applies to logs as well, since it is enforced on the shared transmitter.

- Multi-endpoint routing can now be combined with Microsoft Entra ID authentication. Setting `AzureMonitorExporterOptions.Credential` while the switch is on lets telemetry request the exporter's token by carrying the `microsoft.use_aad_auth` attribute. The attribute is opt-in: telemetry without it is sent unauthenticated, so a destination that still accepts instrumentation key authentication keeps working. Because the `Authorization` header applies to a whole request, one ingestion endpoint carrying both kinds of telemetry is sent as two requests. The identity must hold the Monitoring Metrics Publisher role on every component that opts in, and those components must be in the cloud the credential's audience belongs to. An opted-in destination is only accepted if it is an Azure Monitor ingestion host or the endpoint from the exporter's own connection string; telemetry addressed anywhere else is dropped, and the token is never attached to a request bound for such a host. Destinations that do not opt in are unrestricted, as they are without a credential.
