...
- Multi-endpoint routing can now be combined with Microsoft Entra ID authentication. Setting `AzureMonitorExporterOptions.Credential` while the switch is on lets telemetry request the exporter's token by carrying the `microsoft.use_aad_auth` attribute. The attribute is opt-in: telemetry without it is sent unauthenticated, so a destination that still accepts instrumentation key authentication keeps working. Because the `Authorization` header applies to a whole request, one ingestion endpoint carrying both kinds of telemetry is sent as two requests. The identity must hold the Monitoring Metrics Publisher role on every component that opts in. An opted-in destination is only accepted if it is an Azure Monitor ingestion host in the cloud the credential's audience belongs to, or the endpoint from the exporter's own connection string on the same port; telemetry addressed anywhere else is dropped, and the token is never attached to a request bound for such a host. Destinations that do not opt in are unrestricted, as they are without a credential.
