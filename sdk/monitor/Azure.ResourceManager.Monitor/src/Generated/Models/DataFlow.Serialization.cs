...
namespace Azure.ResourceManager.Monitor.Models
{
    public partial class DataFlow : IUtf8JsonSerializable, IJsonModel<DataFlow>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(TransformKql))
            {
                builder.Append("  transformKql:");
                if (TransformKql.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{TransformKql}'''");
                }
                else
                {
                    builder.AppendLine($" '{TransformKql}'");
            ...
            if (Optional.IsDefined(OutputStream))
            {
                ...
                if (OutputStream.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{OutputStream}'''");
                }
                else
                {
                    builder.AppendLine($" '{OutputStream}'");
            ...
            if (Optional.IsDefined(BuiltInTransform))
            {
                builder.Append("  builtInTransform:");
                if (BuiltInTransform.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{BuiltInTransform}'''");
                }
                else
                {
                    builder.AppendLine($" '{BuiltInTransform}'");
