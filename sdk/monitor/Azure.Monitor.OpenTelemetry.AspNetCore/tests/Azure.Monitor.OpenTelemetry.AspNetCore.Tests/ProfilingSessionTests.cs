...
namespace Azure.Monitor.OpenTelemetry.AspNetCore.Tests
{
    ...
    public class ProfilingSessionTests : IDisposable
    {
        ...
        [Fact]
        public void ProfilerReceivesActivityEvents()
        {
            const int numActivities = 10;

            int numActivitiesStarted = 0;
            int numActivitiesStopped = 0;
            Exception? eventWrittenException = null;
            ...
            void EventWrittenHandler(object? sender, EventWrittenEventArgs args)
            {
                try
                {
                    if (args.EventSource.Name == ProfilingSessionEventSource.EventSourceName)
                    {
                        switch (args.EventId)
                        {
                            case ProfilingSessionEventSource.EventIds.ActivityStart:
                                numActivitiesStarted++;
                                break;

                            case ProfilingSessionEventSource.EventIds.ActivityStop:
                                numActivitiesStopped++;
                                break;
            ...
            using (TracerProvider tracerProvider = Sdk.CreateTracerProviderBuilder()
                ...
                .Build())
            {
                using IDisposable profilerSession = _profiler.StartProfiling(ProfilingSessionEventSource.Keywords.Activities);
                _profiler.EventWritten += EventWrittenHandler;
                try
                {
                    // The ActivitySource name must begin with "Azure."
            ...
            Assert.Null(eventWrittenException);
            Assert.Equal(numActivities, numActivitiesStarted);
            Assert.Equal(numActivities, numActivitiesStopped);
        }
