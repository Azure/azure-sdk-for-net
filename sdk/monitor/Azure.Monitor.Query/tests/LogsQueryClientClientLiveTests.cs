...
namespace Azure.Monitor.Query.Tests
{
    public class LogsQueryClientClientLiveTests : RecordedTestBase<MonitorQueryClientTestEnvironment>
    {
        ...
        public async Task CanQueryBatch()
        {
            ...
            string id1 = batch.AddQuery(TestEnvironment.WorkspaceId, "Heartbeat", _logsTestData.DataTimeSpan);
            string id2 = batch.AddQuery(TestEnvironment.WorkspaceId, "Heartbeat", _logsTestData.DataTimeSpan);
        ...
        public async Task CanQueryIntoPrimitive()
        {
            ...
            Assert.AreEqual(DateTimeOffset.Parse("2015-12-31 23:59:59.9+00:00"), (await client.QueryAsync<DateTimeOffset>(TestEnvironment.WorkspaceId, $"datatable (DateTime: datetime) [ datetime(2015-12-31 23:59:59.9) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(false, (await client.QueryAsync<bool>(TestEnvironment.WorkspaceId, $"datatable (Bool: bool) [ false ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(Guid.Parse("74be27de-1e4e-49d9-b579-fe0b331d3642"), (await client.QueryAsync<Guid>(TestEnvironment.WorkspaceId, $"datatable (Guid: guid) [ guid(74be27de-1e4e-49d9-b579-fe0b331d3642) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(12345, (await client.QueryAsync<int>(TestEnvironment.WorkspaceId, $"datatable (Int: int) [ 12345 ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(1234567890123, (await client.QueryAsync<long>(TestEnvironment.WorkspaceId, $"datatable (Long: long) [ 1234567890123 ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(12345.6789d, (await client.QueryAsync<double>(TestEnvironment.WorkspaceId, $"datatable (Double: double) [ 12345.6789 ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual("string value", (await client.QueryAsync<string>(TestEnvironment.WorkspaceId, $"datatable (String: string) [ \"string value\" ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(TimeSpan.FromSeconds(10), (await client.QueryAsync<TimeSpan>(TestEnvironment.WorkspaceId, $"datatable (Timespan: timespan) [ 10s ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.AreEqual(0.10101m, (await client.QueryAsync<decimal>(TestEnvironment.WorkspaceId, $"datatable (Decimal: decimal) [ decimal(0.10101) ]", _logsTestData.DataTimeSpan)).Value[0]);
        }
        ...
        public async Task CanQueryIntoNullablePrimitive()
        {
            ...
            Assert.IsNull((await client.QueryAsync<DateTimeOffset?>(TestEnvironment.WorkspaceId, $"datatable (DateTime: datetime) [ datetime(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<bool?>(TestEnvironment.WorkspaceId, $"datatable (Bool: bool) [ bool(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<Guid?>(TestEnvironment.WorkspaceId, $"datatable (Guid: guid) [ guid(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<int?>(TestEnvironment.WorkspaceId, $"datatable (Int: int) [ int(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<long?>(TestEnvironment.WorkspaceId, $"datatable (Long: long) [ long(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<double?>(TestEnvironment.WorkspaceId, $"datatable (Double: double) [ double(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<TimeSpan?>(TestEnvironment.WorkspaceId, $"datatable (Timespan: timespan) [ timespan(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
            Assert.IsNull((await client.QueryAsync<decimal?>(TestEnvironment.WorkspaceId, $"datatable (Decimal: decimal) [ decimal(null) ]", _logsTestData.DataTimeSpan)).Value[0]);
        }
        ...
        public async Task CanQueryIntoNullablePrimitiveNull()
        {
            ...
            var results = await client.QueryAsync<DateTimeOffset?>(TestEnvironment.WorkspaceId, $"datatable (DateTime: datetime) [ datetime(null) ]", _logsTestData.DataTimeSpan);
        ...
        public async Task CanQueryBatchWithTimespan()
        {
            ...
            var client = CreateClient();
            LogsBatchQuery batch = InstrumentClient(client.CreateBatchQuery());
            string id1 = batch.AddQuery(TestEnvironment.WorkspaceId, $"{_logsTestData.TableAName} | project {LogsTestData.TimeGeneratedColumnName}", _logsTestData.DataTimeSpan);
        ...
        {
            var client = CreateClient();
            var exception = Assert.ThrowsAsync<RequestFailedException>(async () => await client.QueryAsync(TestEnvironment.WorkspaceId, "this won't work", _logsTestData.DataTimeSpan));
        ...
        public async Task ThrowsExceptionWhenQueryFailsBatch()
        {
            ...
            LogsBatchQuery batch = InstrumentClient(client.CreateBatchQuery());
            var queryId = batch.AddQuery(TestEnvironment.WorkspaceId, "this won't work", _logsTestData.DataTimeSpan);
        ...
        public async Task ThrowsExceptionWhenBatchQueryNotFound()
        {
            ...
            LogsBatchQuery batch = InstrumentClient(client.CreateBatchQuery());
            batch.AddQuery(TestEnvironment.WorkspaceId, _logsTestData.TableAName, _logsTestData.DataTimeSpan);
            var batchResult = await batch.SubmitAsync();
        ...
        public async Task CanQueryWithStatistics(bool include)
        {
            ...
            var response = await client.QueryAsync(TestEnvironment.WorkspaceId, _logsTestData.TableAName, _logsTestData.DataTimeSpan, options: new LogsQueryOptions()
            {
        ...
        public async Task CanQueryWithStatisticsBatch(bool include)
        {
            ...
            LogsBatchQuery batch = InstrumentClient(client.CreateBatchQuery());
            var queryId = batch.AddQuery(TestEnvironment.WorkspaceId, _logsTestData.TableAName, _logsTestData.DataTimeSpan, options: new LogsQueryOptions()
            {
                IncludeStatistics = include
            });
            var batchResult = await batch.SubmitAsync();
