...
namespace Azure.Monitor.Query.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<AdditionalInfoErrorResponse>)this).Write(writer, new ModelReaderWriterOptions("W"));

        void IJsonModel<AdditionalInfoErrorResponse>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<AdditionalInfoErrorResponse>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new InvalidOperationException($"The model {nameof(AdditionalInfoErrorResponse)} does not support '{format}' format.");
            }
        ...
        BinaryData IPersistableModel<AdditionalInfoErrorResponse>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new InvalidOperationException($"The model {nameof(AdditionalInfoErrorResponse)} does not support '{options.Format}' format.");
            }
        }
        ...
        string IPersistableModel<AdditionalInfoErrorResponse>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
