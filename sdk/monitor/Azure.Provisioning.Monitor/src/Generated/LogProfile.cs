// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.Core;
using Azure.Provisioning;
using Azure.Provisioning.Primitives;
using Azure.Provisioning.Resources;

namespace Azure.Provisioning.Monitor
{
    /// <summary> The log profile resource. </summary>
    public partial class LogProfile : ProvisionableResource
    {
        private BicepValue<ResourceIdentifier> _id;
        private BicepValue<string> _name;
        private SystemData _systemData;
        private BicepDictionary<string> _tags;
        private BicepValue<AzureLocation> _location;
        private LogProfileProperties _properties;

        /// <summary> Creates a new LogProfile. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public LogProfile(string bicepIdentifier, string resourceVersion = null) : base(bicepIdentifier, "Microsoft.Insights/logprofiles", resourceVersion ?? "2016-03-01")
        {
        }

        /// <summary> Gets the Id. </summary>
        public BicepValue<ResourceIdentifier> Id
        {
            get
            {
                Initialize();
                return _id;
            }
        }

        /// <summary> Gets or sets the Name. </summary>
        public BicepValue<string> Name
        {
            get
            {
                Initialize();
                return _name;
            }
            set
            {
                Initialize();
                _name.Assign(value);
            }
        }

        /// <summary> Gets the SystemData. </summary>
        public SystemData SystemData
        {
            get
            {
                Initialize();
                return _systemData;
            }
        }

        /// <summary> Gets or sets the Tags. </summary>
        public BicepDictionary<string> Tags
        {
            get
            {
                Initialize();
                return _tags;
            }
            set
            {
                Initialize();
                _tags.Assign(value);
            }
        }

        /// <summary> Gets or sets the Location. </summary>
        public BicepValue<AzureLocation> Location
        {
            get
            {
                Initialize();
                return _location;
            }
            set
            {
                Initialize();
                _location.Assign(value);
            }
        }

        /// <summary> Gets or sets the Properties. </summary>
        internal LogProfileProperties Properties
        {
            get
            {
                Initialize();
                return _properties;
            }
            set
            {
                Initialize();
                AssignOrReplace(ref _properties, value);
            }
        }

        /// <summary> Gets or sets the StorageAccountId. </summary>
        public BicepValue<ResourceIdentifier> StorageAccountId
        {
            get
            {
                return Properties is null ? default : Properties.StorageAccountId;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new LogProfileProperties();
                }
                Properties.StorageAccountId = value;
            }
        }

        /// <summary> Gets or sets the ServiceBusRuleId. </summary>
        public BicepValue<ResourceIdentifier> ServiceBusRuleId
        {
            get
            {
                return Properties is null ? default : Properties.ServiceBusRuleId;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new LogProfileProperties();
                }
                Properties.ServiceBusRuleId = value;
            }
        }

        /// <summary> Gets or sets the Locations. </summary>
        public BicepList<AzureLocation> Locations
        {
            get
            {
                return Properties is null ? default : Properties.Locations;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new LogProfileProperties();
                }
                Properties.Locations = value;
            }
        }

        /// <summary> Gets or sets the Categories. </summary>
        public BicepList<string> Categories
        {
            get
            {
                return Properties is null ? default : Properties.Categories;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new LogProfileProperties();
                }
                Properties.Categories = value;
            }
        }

        /// <summary> Gets or sets the RetentionPolicy. </summary>
        public RetentionPolicy RetentionPolicy
        {
            get
            {
                return Properties is null ? default : Properties.RetentionPolicy;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new LogProfileProperties();
                }
                Properties.RetentionPolicy = value;
            }
        }

        /// <summary> Define all the provisionable properties for LogProfile. </summary>
        protected override void DefineProvisionableProperties()
        {
            base.DefineProvisionableProperties();
            _id = DefineProperty<ResourceIdentifier>(nameof(Id), new string[] { "id" }, isOutput: true);
            _name = DefineProperty<string>(nameof(Name), new string[] { "name" }, isRequired: true);
            _systemData = DefineModelProperty<SystemData>(nameof(SystemData), new string[] { "systemData" }, isOutput: true);
            _tags = DefineDictionaryProperty<string>(nameof(Tags), new string[] { "tags" });
            _location = DefineProperty<AzureLocation>(nameof(Location), new string[] { "location" }, isRequired: true);
            _properties = DefineModelProperty<LogProfileProperties>(nameof(Properties), new string[] { "properties" }, isRequired: true);
            DefineAdditionalProperties();
        }

        /// <summary> Creates a reference to an existing LogProfile. </summary>
        /// <param name="bicepIdentifier"> The bicep identifier name. </param>
        /// <param name="resourceVersion"> The resource API version. </param>
        public static LogProfile FromExisting(string bicepIdentifier, string resourceVersion = null)
        {
            LogProfile result = new LogProfile(bicepIdentifier, resourceVersion);
            result.IsExistingResource = true;
            return result;
        }

        /// <summary> Define additional provisionable properties for LogProfile that are not part of the generated code. </summary>
        partial void DefineAdditionalProperties();

        /// <summary></summary>
        public static partial class ResourceVersions
        {
            /// <summary> API version "2016-03-01". </summary>
            public static readonly string V2016_03_01 = "2016-03-01";
        }
    }
}
