...
namespace Azure.Monitor.Query.Logs.Tests
{
    ...
    public class LogsTestData
    {
        ...
        public LogsTestData(RecordedTestBase<MonitorQueryLogsTestEnvironment> test)
        {
        ...
        private async Task InitializeData(string workspaceId, string workspaceKey)
        {
            ...
            if (count == 0)
            {
                throw new RequestFailedException("The data initialization failed.");
            }
        }

        private async Task<int> QueryCount(string workspaceId)
        {
            var logsClient = new LogsQueryClient(new Uri(_testEnvironment.GetLogsAudience()), _testEnvironment.Credential);
            try
            {
            ...
            {
                Console.WriteLine($"Error querying logs: {ex}");
                return 0;
            }
        }
    }
