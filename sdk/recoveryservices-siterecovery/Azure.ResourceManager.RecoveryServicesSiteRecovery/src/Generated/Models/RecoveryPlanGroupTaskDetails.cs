// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.RecoveryServicesSiteRecovery.Models
{
    /// <summary> This class represents the recovery plan group task. </summary>
    public partial class RecoveryPlanGroupTaskDetails : SiteRecoveryGroupTaskDetails
    {
        /// <summary> Initializes a new instance of <see cref="RecoveryPlanGroupTaskDetails"/>. </summary>
        internal RecoveryPlanGroupTaskDetails() : base("RecoveryPlanGroupTaskDetails")
        {
        }

        /// <summary> Initializes a new instance of <see cref="RecoveryPlanGroupTaskDetails"/>. </summary>
        /// <param name="instanceType"> The type of task details. </param>
        /// <param name="childTasks"> The child tasks. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="name"> The name. </param>
        /// <param name="groupId"> The group identifier. </param>
        /// <param name="rpGroupType"> The group type. </param>
        internal RecoveryPlanGroupTaskDetails(string instanceType, IReadOnlyList<AsrTask> childTasks, IDictionary<string, BinaryData> additionalBinaryDataProperties, string name, string groupId, string rpGroupType) : base(instanceType, childTasks, additionalBinaryDataProperties)
        {
            Name = name;
            GroupId = groupId;
            RpGroupType = rpGroupType;
        }

        /// <summary> Initializes a new instance of <see cref="RecoveryPlanGroupTaskDetails"/>. </summary>
        /// <param name="instanceType"> The type of task details. </param>
        private protected RecoveryPlanGroupTaskDetails(string instanceType) : base(instanceType)
        {
        }

        /// <summary> The name. </summary>
        public string Name { get; }

        /// <summary> The group identifier. </summary>
        public string GroupId { get; }

        /// <summary> The group type. </summary>
        public string RpGroupType { get; }
    }
}
