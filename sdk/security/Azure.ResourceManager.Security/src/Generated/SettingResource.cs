...
namespace Azure.ResourceManager.Security
{
    ...
    /// If you have a <see cref="Core.ResourceIdentifier"/> you can construct a <see cref="SettingResource"/> from an instance of <see cref="ArmClient"/> using the GetResource method.
    /// Otherwise you can get one from its parent resource <see cref="SubscriptionResource"/> using the GetSettings method.
    /// </summary>
    public partial class SettingResource : ArmResource
    {
        ...
        internal SettingResource(ArmClient client, Core.ResourceIdentifier id) : base(client, id)
        {
            TryGetApiVersion(ResourceType, out string settingApiVersion);
            _settingsClientDiagnostics = new ClientDiagnostics("Azure.ResourceManager.Security", ResourceType.Namespace, Diagnostics);
        ...
        public virtual async Task<ArmOperation<SettingResource>> UpdateAsync(WaitUntil waitUntil, SettingData data, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                HttpMessage message = _settingsRestClient.CreateUpdateRequest(Guid.Parse(Id.SubscriptionId), Id.Name, SettingData.ToRequestContent(data), context);
                Response result = await Pipeline.ProcessMessageAsync(message, context).ConfigureAwait(false);
                Response<SettingData> response = Response.FromValue(SettingData.FromResponse(result), result);
