// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure;
using Azure.Core;
using Azure.ResourceManager.Consumption.Models;
using Azure.ResourceManager.Models;

namespace Azure.ResourceManager.Consumption
{
    /// <summary> A credit summary resource. </summary>
    public partial class ConsumptionCreditSummaryData : ResourceData
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ConsumptionCreditSummaryData"/>. </summary>
        internal ConsumptionCreditSummaryData()
        {
            Tags = new ChangeTrackingDictionary<string, string>();
        }

        /// <summary> Initializes a new instance of <see cref="ConsumptionCreditSummaryData"/>. </summary>
        /// <param name="id"> Fully qualified resource ID for the resource. Ex - /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/{resourceProviderNamespace}/{resourceType}/{resourceName}. </param>
        /// <param name="name"> The name of the resource. </param>
        /// <param name="resourceType"> The type of the resource. E.g. "Microsoft.Compute/virtualMachines" or "Microsoft.Storage/storageAccounts". </param>
        /// <param name="systemData"> Azure Resource Manager metadata containing createdBy and modifiedBy information. </param>
        /// <param name="properties"> The properties of the credit summary. </param>
        /// <param name="eTag"> eTag of the resource. To handle concurrent update scenario, this field will be used to determine whether the user is updating the latest version or not. </param>
        /// <param name="tags"> A list of Tag. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ConsumptionCreditSummaryData(ResourceIdentifier id, string name, ResourceType resourceType, SystemData systemData, CreditSummaryProperties properties, ETag? eTag, IDictionary<string, string> tags, IDictionary<string, BinaryData> additionalBinaryDataProperties) : base(id, name, resourceType, systemData)
        {
            Properties = properties;
            ETag = eTag;
            Tags = tags;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The properties of the credit summary. </summary>
        internal CreditSummaryProperties Properties { get; }

        /// <summary> eTag of the resource. To handle concurrent update scenario, this field will be used to determine whether the user is updating the latest version or not. </summary>
        public ETag? ETag { get; }

        /// <summary> A list of Tag. </summary>
        public IDictionary<string, string> Tags { get; }

        /// <summary> Summary of balances associated with this credit summary. </summary>
        public CreditBalanceSummary BalanceSummary
        {
            get
            {
                return Properties is null ? default : Properties.BalanceSummary;
            }
        }

        /// <summary> Pending credit adjustments. </summary>
        public ConsumptionAmount PendingCreditAdjustments
        {
            get
            {
                return Properties is null ? default : Properties.PendingCreditAdjustments;
            }
        }

        /// <summary> Expired credit. </summary>
        public ConsumptionAmount ExpiredCredit
        {
            get
            {
                return Properties is null ? default : Properties.ExpiredCredit;
            }
        }

        /// <summary> Pending eligible charges. </summary>
        public ConsumptionAmount PendingEligibleCharges
        {
            get
            {
                return Properties is null ? default : Properties.PendingEligibleCharges;
            }
        }

        /// <summary> The credit currency. </summary>
        public string CreditCurrency
        {
            get
            {
                return Properties is null ? default : Properties.CreditCurrency;
            }
        }

        /// <summary> The billing currency. </summary>
        public string BillingCurrency
        {
            get
            {
                return Properties is null ? default : Properties.BillingCurrency;
            }
        }

        /// <summary> Credit's reseller. </summary>
        public ConsumptionReseller Reseller
        {
            get
            {
                return Properties is null ? default : Properties.Reseller;
            }
        }

        /// <summary> If true, the listed details are based on an estimation and it will be subjected to change. </summary>
        public bool? IsEstimatedBalance
        {
            get
            {
                return Properties is null ? default : Properties.IsEstimatedBalance;
            }
        }

        /// <summary> The eTag for the resource. </summary>
        public string PropertiesETag
        {
            get
            {
                return Properties is null ? default : Properties.PropertiesETag;
            }
        }
    }
}
