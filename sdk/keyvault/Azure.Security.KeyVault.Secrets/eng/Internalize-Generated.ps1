...
if ($kvClient) {
    ...
    # SecretClient builds the HttpPipeline from the customer's SecretClientOptions
    ...
    if ($text -ne $original) {
        ...
        Write-Host '  patched: KeyVaultSecretsClient.cs (UpdateSecret arg order + customization hooks)'
    }
}
