...
namespace Azure.Security.KeyVault.Keys.Tests
{
    public class AesCryptographyProviderTests
    {
        ...
        public void EncryptionAlgorithmNotSupported()
        {
            ...
            Assert.That(provider.Encrypt(new EncryptParameters(new EncryptionAlgorithm("invalid"), new byte[] { 0 })), Is.Null);
        ...
        public void DecryptionAlgorithmNotSupported()
        {
            ...
            AesCryptographyProvider provider = new AesCryptographyProvider(key, null, false);
            Assert.That(provider.Decrypt(new DecryptParameters(new EncryptionAlgorithm("invalid"), new byte[] { 0 })), Is.Null);
        ...
        public void EncryptDecryptRoundtrips(EncryptionAlgorithm algorithm)
        {
            ...
            switch (algorithm.ToString())
            {
                ...
                case EncryptionAlgorithm.A128CbcValue:
                    Assert.That(
                        ...
                        Is.EqualTo(new byte[] { 0x63, 0x23, 0x21, 0xaf, 0x94, 0xf9, 0xe1, 0x21, 0xc2, 0xbd, 0xb1, 0x1b, 0x04, 0x89, 0x8c, 0x3a }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;

                case EncryptionAlgorithm.A192CbcValue:
                    Assert.That(
                        encrypted.Ciphertext,
                        Is.EqualTo(new byte[] { 0x95, 0x9d, 0x75, 0x91, 0x09, 0x8b, 0x70, 0x0b, 0x9c, 0xfe, 0xaf, 0xcd, 0x60, 0x1f, 0xaa, 0x79 }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;
                ...
                case EncryptionAlgorithm.A256CbcValue:
                    Assert.That(
                        encrypted.Ciphertext,
                        Is.EqualTo(new byte[] { 0xf4, 0xe8, 0x5a, 0xa4, 0xa8, 0xb3, 0xff, 0xc3, 0x85, 0x89, 0x17, 0x9a, 0x70, 0x09, 0x96, 0x7f }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;
                ...
                case EncryptionAlgorithm.A128CbcPadValue:
                    Assert.That(
                        encrypted.Ciphertext,
                        Is.EqualTo(new byte[] { 0xec, 0xb2, 0x63, 0x4c, 0xe0, 0x04, 0xe0, 0x31, 0x2d, 0x9a, 0x77, 0xb2, 0x11, 0xe5, 0x28, 0x7f }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;
                ...
                case EncryptionAlgorithm.A192CbcPadValue:
                    Assert.That(
                        encrypted.Ciphertext,
                        Is.EqualTo(new byte[] { 0xc3, 0x4e, 0x1b, 0xe7, 0x6e, 0xa1, 0xf1, 0xc3, 0x24, 0xae, 0x05, 0x1b, 0x0e, 0x32, 0xac, 0xb4 }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;
                ...
                case EncryptionAlgorithm.A256CbcPadValue:
                    Assert.That(
                        encrypted.Ciphertext,
                        Is.EqualTo(new byte[] { 0x4e, 0xbd, 0x78, 0xda, 0x90, 0x73, 0xc8, 0x97, 0x67, 0x2b, 0xa1, 0x0a, 0x41, 0x67, 0xf8, 0x99 }).AsCollection);
                    Assert.That(encrypted.Iv, Is.EqualTo(iv).AsCollection);
                    Assert.That(encrypted.AuthenticationTag, Is.Null);
                    Assert.That(encrypted.AdditionalAuthenticatedData, Is.Null);
                    break;
            ...
            Assert.That(Encoding.UTF8.GetString(decrypted.Plaintext), Does.StartWith("plaintext"));
        }
        ...
        public void InitializesIv(EncryptionAlgorithm algorithm)
        {
            ...
            Assert.That(encryptOptions.Iv, Is.Not.Null);
            Assert.That(encrypted.Iv, Is.EqualTo(encryptOptions.Iv).AsCollection);
        ...
        public void AesGcmEncryptNotSupported([EnumValues(nameof(EncryptionAlgorithm.A128Gcm), nameof(EncryptionAlgorithm.A192Gcm), nameof(EncryptionAlgorithm.A256Gcm))] EncryptionAlgorithm algorithm)
        {
            ...
            AesCryptographyProvider provider = new AesCryptographyProvider(key, null, false);
            Assert.That(provider.Encrypt(new EncryptParameters(algorithm, plaintext)), Is.Null);
        }
        ...
        public void AesGcmDecryptNotSupported([EnumValues(nameof(EncryptionAlgorithm.A128Gcm), nameof(EncryptionAlgorithm.A192Gcm), nameof(EncryptionAlgorithm.A256Gcm))] EncryptionAlgorithm algorithm)
        {
            ...
            AesCryptographyProvider provider = new AesCryptographyProvider(key, null, false);
            Assert.That(provider.Decrypt(new DecryptParameters(algorithm, ciphertext, iv, tag, null)), Is.Null);
        }
