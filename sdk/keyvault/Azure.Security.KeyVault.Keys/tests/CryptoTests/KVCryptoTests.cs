...
namespace Azure.Security.KeyVault.Cryptography.Tests
{
    ...
    public class KVCryptoTests: CryptoTestBase
    {
        ...
        public async Task EncryptRsa15Key()
        {
            ...
            EncryptResult encryptedData = await KVCryptoClient.EncryptAsync(CEK, null, null, EncryptionAlgorithmKind.Rsa15, default(CancellationToken)).ConfigureAwait(false);
            Assert.NotNull(encryptedData);
            Assert.AreEqual(encryptedData.Algorithm, EncryptionAlgorithmKind.Rsa15);
        }
        ...
        public async Task DecryptRsa15Key()
        {
            ...
            DecryptResult decryptedData = await KVCryptoClient.DecryptAsync(encryptedData.CipherText, null, null, null, EncryptionAlgorithmKind.Rsa15, default(CancellationToken)).ConfigureAwait(false);
        ...
        public async Task WrapRsa15Key()
        {
            ...
            WrapKeyResult wkr = await KVCryptoClient.WrapKeyAsync(keyData, EncryptionAlgorithmKind.Rsa15, default(CancellationToken)).ConfigureAwait(false);
        }
