...
In order to interact with the Azure Key Vault service, you'll need to create an instance of the [KeyClient][key_client_class] class. You would need a **vault url**, which you may see as "DNS Name" in the portal,
...
* Use the above mentioned Azure Key Vault name to retrieve details of your Azure Key Vault which also contains your Azure Key Vault URL:
    ```PowerShell
...
### List Keys
This example lists all the keys in the specified Azure Key Vault. The value is not returned when listing all keys. You will need to call `KeyClient.GetKey` to retrieve the value.
