...
/// <summary>
/// A long-running operation for <see cref="KeyVaultBackupRestoreClient.StartFullBackup(Uri, string, CancellationToken)"/> or <see cref="KeyVaultBackupRestoreClient.StartFullBackupAsync(Uri, string, CancellationToken)"/>.
/// </summary>
public class FullBackupOperation : Operation<FullBackupDetails>
{
    ...
    /// <param name="response">The <see cref="ResponseWithHeaders{T, THeaders}" /> returned from <see cref="KeyVaultBackupRestoreClient.StartFullBackup(Uri, string, CancellationToken)"/> or <see cref="KeyVaultBackupRestoreClient.StartFullBackupAsync(Uri, string, CancellationToken)"/>.</param>
    internal FullBackupOperation(KeyVaultBackupRestoreClient client, ResponseWithHeaders<FullBackupDetails, ServiceFullBackupHeaders> response)
    {
        _client = client;
        _response = response;
        _retryAfterSeconds = response.Headers.RetryAfter;
        _ = response.Value ?? throw new InvalidOperationException("The response does not contain a value.");
    }
    ...
    /// <param name="client">An instance of <see cref="KeyVaultBackupRestoreClient" />.</param>
    internal FullBackupOperation(FullBackupDetails value, Response response, KeyVaultBackupRestoreClient client)
    {
