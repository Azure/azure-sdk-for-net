...
- `KeyVaultAccessControlClient`, `KeyVaultBackupClient`, `KeyVaultSettingsClient`, and `KeyVaultEkmClient` now implement `IDisposable` to deterministically release the dedicated transport created internally when Proof-of-Possession token binding is enabled and the transport supports being updated in place.
