    ...
    internal partial class AvsAssessedMachinesRestOperations
    {
        ...
        public async Task<Response<MigrateAvsAssessedMachineData>> GetAsync(string subscriptionId, string resourceGroupName, string projectName, string groupName, string assessmentName, string avsAssessedMachineName, CancellationToken cancellationToken = default)
        {
            ...
            switch (message.Response.Status)
            {
                case 200:
                    {
                        MigrateAvsAssessedMachineData value = default;
                        using var document = await JsonDocument.ParseAsync(message.Response.ContentStream, default, cancellationToken).ConfigureAwait(false);
                        value = MigrateAvsAssessedMachineData.DeserializeMigrateAvsAssessedMachineData(document.RootElement);
                        return Response.FromValue(value, message.Response);
                    }
                case 404:
                    return Response.FromValue((MigrateAvsAssessedMachineData)null, message.Response);
                default:
                    throw new RequestFailedException(message.Response);
            }
        }
