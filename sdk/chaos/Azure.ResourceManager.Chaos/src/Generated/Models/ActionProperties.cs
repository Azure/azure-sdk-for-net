// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.Chaos;

namespace Azure.ResourceManager.Chaos.Models
{
    /// <summary> Model that represents the properties of an Action resource. </summary>
    internal partial class ActionProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ActionProperties"/>. </summary>
        internal ActionProperties()
        {
            SupportedTargetTypes = new ChangeTrackingList<ChaosActionSupportedTargetType>();
            RecommendedRoles = new ChangeTrackingList<Guid>();
        }

        /// <summary> Initializes a new instance of <see cref="ActionProperties"/>. </summary>
        /// <param name="canonicalId"> Canonical identifier of the action (e.g., "microsoft-compute-shutdown/1.0"). </param>
        /// <param name="displayName"> Human-readable display name of the action. </param>
        /// <param name="description"> Description of what this action does. </param>
        /// <param name="actionName"> The short name of the action (e.g., "Shutdown"). </param>
        /// <param name="version"> The version of the action (e.g., "1.0.0"). </param>
        /// <param name="actionType"> The type of the action. </param>
        /// <param name="supportedTargetTypes"> List of target types supported by this action. </param>
        /// <param name="parametersSchema"> JSON Schema describing the parameters for this action. </param>
        /// <param name="recommendedRoles"> Recommended Azure RBAC role definition GUIDs for this action. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ActionProperties(string canonicalId, string displayName, string description, string actionName, string version, ChaosActionKind? actionType, IReadOnlyList<ChaosActionSupportedTargetType> supportedTargetTypes, ActionParametersSchema parametersSchema, IReadOnlyList<Guid> recommendedRoles, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            CanonicalId = canonicalId;
            DisplayName = displayName;
            Description = description;
            ActionName = actionName;
            Version = version;
            ActionType = actionType;
            SupportedTargetTypes = supportedTargetTypes;
            ParametersSchema = parametersSchema;
            RecommendedRoles = recommendedRoles;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Canonical identifier of the action (e.g., "microsoft-compute-shutdown/1.0"). </summary>
        public string CanonicalId { get; }

        /// <summary> Human-readable display name of the action. </summary>
        public string DisplayName { get; }

        /// <summary> Description of what this action does. </summary>
        public string Description { get; }

        /// <summary> The short name of the action (e.g., "Shutdown"). </summary>
        public string ActionName { get; }

        /// <summary> The version of the action (e.g., "1.0.0"). </summary>
        public string Version { get; }

        /// <summary> The type of the action. </summary>
        public ChaosActionKind? ActionType { get; }

        /// <summary> List of target types supported by this action. </summary>
        public IReadOnlyList<ChaosActionSupportedTargetType> SupportedTargetTypes { get; } = new ChangeTrackingList<ChaosActionSupportedTargetType>();

        /// <summary> JSON Schema describing the parameters for this action. </summary>
        public ActionParametersSchema ParametersSchema { get; }

        /// <summary> Recommended Azure RBAC role definition GUIDs for this action. </summary>
        public IReadOnlyList<Guid> RecommendedRoles { get; } = new ChangeTrackingList<Guid>();
    }
}
