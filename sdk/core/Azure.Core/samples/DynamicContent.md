...
To use dynamic content with an idiomatic .NET style, pass `NameConversion.CamelCase` to `ToDynamicFromJson()`.  This will allow using PascalCase property names to get camelCase JSON members.
...
JSON members can be set on the dynamic object.  Pass `NameConversion.CamelCase` to `ToDynamicFromJson()` to write JSON members with camelCase names.
...
If you are using the `NameConversion.CamelCase` setting and there is a need to bypass these name mappings, JSON members can be accessed with exact strings using property indexers.
...
using (dynamic widget = response.Content.ToDynamicFromJson(NameConversion.CamelCase))
{
    widget.Name = "New Name";
    client.SetWidget(RequestContent.Create(widget));
}
