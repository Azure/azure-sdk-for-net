...
"";
        ...
        public async Task ParseBatchChangesetResponse(string batchResponse)
        {
            ...
            {
                Assert.That(response.TryGetHeaderValue("Location", out var location));
                Assert.That(location.Contains("RowKey='01'"));

                Assert.That(response.TryGetHeaderValue("Content-Type", out var contentType));
        ...
        public async Task ParseBatchResponse()
        {
            ...
            Assert.That(response.TryGetHeaderValue("x-ms-version", out var version));
            Assert.That(version, Is.EqualTo("2018-11-09"));
            Assert.That(response.TryGetHeaderValue("x-ms-request-id", out _));
