...
namespace Azure.Core.Tests
{
    [TestFixture(true)]
    [TestFixture(false)]
    public class SimulateRequestRetryAttributeTests : SyncAsyncTestBase
    {
        public SimulateRequestRetryAttributeTests(bool isAsync) : base(isAsync) { }
        ...
        private class TestRetryAttribute : SimulateRequestRetryAttribute
        {
            public override bool CanRetry(HttpMessage message)
            {
                return message.Request.ContainsHeader("test-header");
            }

            public override void Retry(HttpMessage message)
            {
                message.Response = new MockResponse(499);
            }
        }
