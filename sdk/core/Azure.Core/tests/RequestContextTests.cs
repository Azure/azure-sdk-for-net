...
namespace Azure.Core.Tests
{
    public class RequestContextTests
    {
        ...
        [Test]
        public async Task PerRequestClassifierTakesPrecedenceOverRetryClassifier()
        {
            // pipeline setup
            var response = new MockResponse(429);
            var mockTransport = new MockTransport(response);
            var retryResponseClassifier = new ResponseClassifier();
            var clientOptions = new TestOptions()
            {
                Transport = mockTransport,
                RetryPolicy = new RetryPolicy(new RetryPolicyOptions { ResponseClassifier = retryResponseClassifier })
            };
            var pipeline = HttpPipelineBuilder.Build(clientOptions);
            var context = new RequestContext();
            var message = pipeline.CreateMessage(context, new StatusCodeClassifier(stackalloc ushort[] { 429 }));

            await pipeline.SendAsync(message, context.CancellationToken);

            // should not be retried since we specify 429 as a success code
            Assert.AreEqual(0, message.ProcessingContext.RetryNumber);
        }

        [Test]
        public async Task RetryResponseClassifierTakesPrecedenceOverHttpPipelineOptionsClassifier()
        {
            // pipeline setup
            var response = new MockResponse(429);
            var mockTransport = new MockTransport(response);
            var retryResponseClassifier = new StatusCodeClassifier(stackalloc ushort[] { 429 });
            var clientOptions = new TestOptions()
            {
                Transport = mockTransport,
                RetryPolicy = new RetryPolicy(new RetryPolicyOptions { ResponseClassifier = retryResponseClassifier })
            };
            var httpPipelineOptions = new HttpPipelineOptions(clientOptions)
            {
                ResponseClassifier = new ResponseClassifier()
            };
            var pipeline = HttpPipelineBuilder.Build(httpPipelineOptions);
            var context = new RequestContext();
            var message = pipeline.CreateMessage(context);
        ...
        [Test]
        public async Task HttpPipelineOptionsClassifierTakesPrecedenceOverDefaultResponseClassifier()
        {
            // pipeline setup
            var response = new MockResponse(429);
            var mockTransport = new MockTransport(response);
            var responseClassifier = new StatusCodeClassifier(stackalloc ushort[] { 429 });
            var clientOptions = new TestOptions()
            {
                Transport = mockTransport,
            };
