    ...
    internal class EnvelopeTests
    {
        ...
        public void CanRoundTripFutureVersionWithoutLoss(string format)
        {
            ...
            if (format == ModelSerializerFormat.Wire)
            {
                ...
                options.GenericTypeSerializerCreator = type => type.Equals(typeof(ModelC)) ? new NewtonsoftJsonObjectSerializer(settings) : null;
            }
            else
            {
                options.GenericTypeSerializerCreator = type => type.Equals(typeof(ModelC)) ? new NewtonsoftJsonObjectSerializer() : null;
            }
