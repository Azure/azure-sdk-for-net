...
## Note on sharing fields between Generated and Customized code (such as _clientDiagnostics)
...
```csharp
    ...
    {
        using var scope = ClientDiagnostics.CreateScope("SparkSessionClient.GetSparkSession");
```
...
The correct solution is the use a virtual method, so the testing framework stubs it out:

```csharp
    internal virtual ClientDiagnostics ClientDiagnostics => _clientDiagnostics;
```

This may require an InternalsVisisbleTo addition to the test AssemblyInfo.cs:
