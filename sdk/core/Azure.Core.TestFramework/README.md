...
## Note on private/non-virtual fields in your clients (such as _clientDiagnostics)
...
The correct solution is the use a virtual property, so the testing framework stubs it out:

```csharp
    internal virtual ClientDiagnostics ClientDiagnostics => _clientDiagnostics;
```

Then update your client code to use the new virtual property.  (Pro tip: you can do a VS rename on the field from `_clientDiagnostics` to `ClientDiagnostics` and then change just the field/property definitions back to `_clientDiagnostics` to make this quicker.)

If it's an `internal` property, it will also require an `InternalsVisisbleTo` addition in your AssemblyInfo.cs:
