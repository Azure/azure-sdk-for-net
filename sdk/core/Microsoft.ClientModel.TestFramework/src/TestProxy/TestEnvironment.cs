...
public abstract class TestEnvironment
{
    ...
    protected TestEnvironment()
    {
        ...
        _type = GetType();
        _loggerFactory = new NullLoggerFactory(); // TODO
...
#pragma warning restore CA1416
                var json = JsonDocument.Parse(File.ReadAllBytes(testEnvironmentFile)); // TODO

                foreach (var property in json.RootElement.EnumerateObject())
                {
                    _environmentFile[property.Name] = property.Value.GetString()!; // TODO
                }
    ...
    /// TODO.
    /// </summary>
    public virtual AuthenticationToken Token
    {
        get
        {
            if (_credential != null)
    ...
    private async Task WaitForEnvironmentInternalAsync()
    {
        if (GlobalIsRunningInCI)
        {
            if (Mode == RecordedTestMode.Live)
            {
                ...
                {
                    var isReady = await IsEnvironmentReadyAsync().ConfigureAwait(false);
                    if (isReady)
                    {
                        return;
                    }
        ...
        {
            await ExtendResourceGroupExpirationAsync().ConfigureAwait(false);
        }
    }
