...
public class RetryOnErrorAttributeTests
{
    ...
    {
        var method = typeof(TestClassWithRetryOnError).GetMethod(nameof(TestClassWithRetryOnError.RetryableMethod));
        var attribute = method.GetCustomAttribute<TestRetryOnErrorAttribute>();
    ...
    {
        [TestRetryOnError(3, context => true)]
        public void RetryableMethod()
        {
            // Method with retry on error attribute
        }
    }

    public class TestRetryOnError : RetryOnErrorAttribute
    {
        public TestRetryOnError(int tryCount, Func<TestExecutionContext, bool> shouldRetry)
            : base(tryCount, shouldRetry)
        {
        }
    }
