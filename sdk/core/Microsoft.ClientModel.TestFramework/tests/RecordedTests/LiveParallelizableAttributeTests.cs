...
public class LiveParallelizableAttributeTests
{
    ...
    [Test]
    public void Constructor_WithAllParallelScopeValues_DoesNotThrow()
    {
        var scopeValues = Enum.GetValues<ParallelScope>();
        
        foreach (var scope in scopeValues)
        {
            Assert.DoesNotThrow(() => new LiveParallelizableAttribute(scope));
        }
    }
