...
{
    [NonParallelizable]
    public class ClientEventSourceListenerTests
    {
        [Test]
        public void CallsCallbackWhenMessageIsLogged()
        {
            var invocations = new List<(EventWrittenEventArgs, string)>();
            using var _ = new ClientEventSourceListener(
                (args, s) =>
                {
                    invocations.Add((args, s));
                }, EventLevel.Verbose);

            ClientModelEventSource.Singleton.Request("id", "GET", "http", "header", "Test-SDK");
        ...
        public void FiltersMessagesUsingLevel()
        {
            ...
            using var verboseListener = new ClientEventSourceListener((args, s) => verboseInvocations.Add((args, s)), EventLevel.Verbose);
            using var warningListener = new ClientEventSourceListener((args, s) => warningInvocations.Add((args, s)), EventLevel.Warning);

            ClientModelEventSource.Singleton.ErrorResponse("id", 500, "GET", "http", 5);
            ClientModelEventSource.Singleton.Request("id", "GET", "http", "header", "Test-SDK");
        ...
        private class TestSource : EventSource
        {
            ...
            private TestSource() : base("Test-source", EventSourceSettings.Default, "ClientEventSource", "true")
            {
            }
