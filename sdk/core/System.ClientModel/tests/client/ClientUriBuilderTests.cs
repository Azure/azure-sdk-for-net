...
{
    [Test]
    public void CanAppendPath()
    {
        string endpoint = "http://localhost:3000";
        string path = "/authentication/http/valid";

        ClientUriBuilder uriBuilder = new();

        uriBuilder.Reset(new Uri(endpoint));
        uriBuilder.AppendPath(path, false);

        Assert.AreEqual(endpoint + path, uriBuilder.ToUri().ToString());
    }

    [Test]
    public void CanAppendMultiplePathSegments()
    {
        string endpoint = "http://localhost:3000";
        string path1 = "/authentication/";
        string path2 = "/http/";
        string path3 = "/valid";
        ...
        uriBuilder.Reset(new Uri(endpoint));
        uriBuilder.AppendPath(path1, false);
        uriBuilder.AppendPath(path2, false);
        uriBuilder.AppendPath(path3, false);

        Assert.AreEqual(endpoint + "/authentication/http/valid", uriBuilder.ToUri().ToString());
    }

    [Test]
    public void CanAppendQuery()
    {
        string endpoint = "http://localhost:3000";
        ClientUriBuilder uriBuilder = new();

        uriBuilder.Reset(new Uri(endpoint));
        uriBuilder.AppendQuery("id", "ABC", false);

        Assert.AreEqual(endpoint + "/?id=ABC", uriBuilder.ToUri().ToString());
    }

    [Test]
    public void CanAppendMultipleQueryParameters()
    {
        string endpoint = "http://localhost:3000";
        ClientUriBuilder uriBuilder = new();
        ...
        uriBuilder.Reset(new Uri(endpoint));
        uriBuilder.AppendQuery("id", "ABC", false);
        uriBuilder.AppendQuery("name", "Alice", false);

        Assert.AreEqual(endpoint + "/?id=ABC&name=Alice", uriBuilder.ToUri().ToString());
    }
