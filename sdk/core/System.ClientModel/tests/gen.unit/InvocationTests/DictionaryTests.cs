    ...
    internal class DictionaryTests : InvocationTestBase
    {
        ...
        private static void AssertDictionary(ModelExpectation expectation, bool invocationDuped, Dictionary<string, TypeBuilderSpec> dict)
        {
            ...
            if (invocationDuped)
            {
                ...
                Assert.AreEqual($"Dictionary<string, {expectation.TypeName}_0>", dictionaryType.Type.Alias);
                Assert.AreEqual($"Dictionary<string, {expectation.TypeName}_1>", dupedDictionaryType.Type.Alias);
            }
            else
            {
