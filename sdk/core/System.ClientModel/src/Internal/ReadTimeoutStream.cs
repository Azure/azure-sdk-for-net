...
internal class ReadTimeoutStream : Stream
{
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        {
            PipelineTransport.ThrowIfCancellationRequestedOrTimeout(default, source.Token, ex, _readTimeout);
            throw;
        }
        // We dispose stream on timeout so catch and check if cancellation token was cancelled
...
#pragma warning restore // ReadAsync(Memory<>) overload is not available in all targets
        ...
        {
            PipelineTransport.ThrowIfCancellationRequestedOrTimeout(cancellationToken, source.Token, ex, _readTimeout);
            throw;
        }
        // We dispose stream on timeout so catch and check if cancellation token was cancelled
        catch (ObjectDisposedException ex)
