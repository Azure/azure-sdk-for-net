...
public sealed partial class DynamicJsonData : IDisposable
{
    private static readonly MethodInfo GetPropertyMethod = typeof(DynamicJsonData).GetMethod(nameof(GetProperty), BindingFlags.NonPublic | BindingFlags.Instance)!;
    private static readonly MethodInfo SetPropertyMethod = typeof(DynamicJsonData).GetMethod(nameof(SetProperty), BindingFlags.NonPublic | BindingFlags.Instance)!;
    private static readonly MethodInfo GetEnumerableMethod = typeof(DynamicJsonData).GetMethod(nameof(GetEnumerable), BindingFlags.NonPublic | BindingFlags.Instance)!;
    private static readonly MethodInfo GetViaIndexerMethod = typeof(DynamicJsonData).GetMethod(nameof(GetViaIndexer), BindingFlags.NonPublic | BindingFlags.Instance)!;
    private static readonly MethodInfo SetViaIndexerMethod = typeof(DynamicJsonData).GetMethod(nameof(SetViaIndexer), BindingFlags.NonPublic | BindingFlags.Instance)!;
    ...
    internal DynamicJsonData(MutableJsonElement element, DynamicJsonDataOptions options)
    {
        _element = element;
        _options = options;
        _serializerOptions = DynamicJsonDataOptions.ToSerializerOptions(options);
    }
    ...
    private object? GetProperty(string name)
    {
        ...
        if (_element.TryGetProperty(name, out MutableJsonElement element))
        {
            ...
            return new DynamicJsonData(element, _options);
        }
    ...
    private object? GetViaIndexer(object index)
    {
        switch (index)
        {
            ...
            case int arrayIndex:
                ...
                return new DynamicJsonData(arrayElement, _options);
        }
    ...
    public override bool Equals(object? obj)
    {
        ...
        {
            return obj is DynamicJsonData d && Equals(d);
        }
        ...
        return obj switch
        {
            ...
            DynamicJsonData data => Equals(data),

            _ => base.Equals(obj),
        };
    }

    internal bool Equals(DynamicJsonData other)
    {
        if (other is null)
        {
            return _element.ValueKind == JsonValueKind.Null;
    ...
    private bool NumberEqual(DynamicJsonData other)
    {
    ...
    private class DynamicDataJsonConverter : JsonConverter<DynamicJsonData>
    {
        ...
        public override DynamicJsonData Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            MutableJsonDocument mdoc = MutableJsonDocument.Parse(ref reader);
            return new DynamicJsonData(mdoc.RootElement, DynamicJsonDataOptions.FromSerializerOptions(options));
        }

        public override void Write(Utf8JsonWriter writer, DynamicJsonData value, JsonSerializerOptions options)
        {
