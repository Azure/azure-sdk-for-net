...
public partial class MultipartContent : BinaryContent
{
    ...
    private static int GetLength(ReadOnlySpan<byte> headerName, ReadOnlySpan<byte> headerValue)
        => headerName.Length + headerValue.Length + 4; // ": " + "\r\n"

    private static int GetLength(string headerName, string headerValue)
        => headerName.Length + headerValue.Length + 4; // ": " + "\r\n"
...
internal readonly struct Part
{
    ...
    public void WriteTo(Stream stream, CancellationToken cancellationToken)
    {
        ...
        _data.WriteTo(stream, cancellationToken); // TODO: how are we going to validate that the boundary is not contained in data?
    }
}
