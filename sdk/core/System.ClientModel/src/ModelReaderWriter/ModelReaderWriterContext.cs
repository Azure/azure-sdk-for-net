...
public abstract class ModelReaderWriterContext
{
    ...
    {
        if (!TryGetModelBuilder(type, out ModelBuilder? modelBuilder))
        {
    ...
    /// </summary>
    /// <param name="type">The type to get info for.</param>
    /// <param name="modelBuilder">The <see cref="ModelBuilder"/> if found.</param>
    /// <returns>True if the corresponding <see cref="ModelBuilder"/> if defined in the context otherwise false.</returns>
    public virtual bool TryGetModelBuilder(Type type, [NotNullWhen(true)] out ModelBuilder? modelBuilder)
    {
        modelBuilder = null;
        return false;
    }
