...
namespace Azure.ResourceManager.StorageCache.Models
{
    public partial class StorageCacheUpgradeStatus : IUtf8JsonSerializable, IJsonModel<StorageCacheUpgradeStatus>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            {
                builder.Append("  currentFirmwareVersion:");
                if (CurrentFirmwareVersion.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{CurrentFirmwareVersion}'''");
                }
                else
                {
                    builder.AppendLine($" '{CurrentFirmwareVersion}'");
            ...
            {
                builder.Append("  firmwareUpdateStatus:");
                builder.AppendLine($" '{FirmwareUpdateStatus.Value.ToString()}'");
            }

            if (Optional.IsDefined(FirmwareUpdateDeadline))
            {
                builder.Append("  firmwareUpdateDeadline:");
                var formattedDateTimeString = TypeFormatters.ToString(FirmwareUpdateDeadline.Value, "o");
                builder.AppendLine($" '{formattedDateTimeString}'");
            }

            if (Optional.IsDefined(LastFirmwareUpdate))
            {
                builder.Append("  lastFirmwareUpdate:");
                var formattedDateTimeString = TypeFormatters.ToString(LastFirmwareUpdate.Value, "o");
                builder.AppendLine($" '{formattedDateTimeString}'");
            }

            if (Optional.IsDefined(PendingFirmwareVersion))
            {
                builder.Append("  pendingFirmwareVersion:");
                if (PendingFirmwareVersion.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{PendingFirmwareVersion}'''");
                }
                else
                {
                    builder.AppendLine($" '{PendingFirmwareVersion}'");
