// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.WeightsAndBiases
{
    /// <summary></summary>
    internal partial class WeightsAndBiasesInstanceResourceOperationSource : IOperationSource<WeightsAndBiasesInstanceResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal WeightsAndBiasesInstanceResourceOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        WeightsAndBiasesInstanceResource IOperationSource<WeightsAndBiasesInstanceResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            WeightsAndBiasesInstanceData data = WeightsAndBiasesInstanceData.DeserializeWeightsAndBiasesInstanceData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new WeightsAndBiasesInstanceResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<WeightsAndBiasesInstanceResource> IOperationSource<WeightsAndBiasesInstanceResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            WeightsAndBiasesInstanceData data = WeightsAndBiasesInstanceData.DeserializeWeightsAndBiasesInstanceData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new WeightsAndBiasesInstanceResource(_client, data);
        }
    }
}
