...
namespace Azure.ResourceManager.ManagedNetworkFabric.Models
{
    ...
    public partial class NetworkFabricIPPrefixPatch
    {
        ...
        private static NetworkFabricIPPrefixPatch Deserialize(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkFabricIPPrefixPatch>)new NetworkFabricIPPrefixPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        private static NetworkFabricIPPrefixPatch Deserialize(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkFabricIPPrefixPatch>)new NetworkFabricIPPrefixPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
