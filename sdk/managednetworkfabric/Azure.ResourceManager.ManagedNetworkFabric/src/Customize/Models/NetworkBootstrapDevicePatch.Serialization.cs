...
namespace Azure.ResourceManager.ManagedNetworkFabric.Models
{
    ...
    public partial class NetworkBootstrapDevicePatch
    {
        ...
        private static NetworkBootstrapDevicePatch Deserialize(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkBootstrapDevicePatch>)new NetworkBootstrapDevicePatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        private static NetworkBootstrapDevicePatch Deserialize(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkBootstrapDevicePatch>)new NetworkBootstrapDevicePatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
