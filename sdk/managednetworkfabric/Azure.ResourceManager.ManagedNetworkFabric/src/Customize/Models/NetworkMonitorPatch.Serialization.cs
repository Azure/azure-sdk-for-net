...
namespace Azure.ResourceManager.ManagedNetworkFabric.Models
{
    ...
    public partial class NetworkMonitorPatch
    {
        ...
        private static NetworkMonitorPatch Deserialize(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkMonitorPatch>)new NetworkMonitorPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        private static NetworkMonitorPatch Deserialize(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkMonitorPatch>)new NetworkMonitorPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
