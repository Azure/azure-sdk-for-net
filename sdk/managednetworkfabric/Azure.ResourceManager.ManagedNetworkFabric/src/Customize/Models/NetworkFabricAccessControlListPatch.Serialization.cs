...
namespace Azure.ResourceManager.ManagedNetworkFabric.Models
{
    ...
    public partial class NetworkFabricAccessControlListPatch
    {
        ...
        private static NetworkFabricAccessControlListPatch Deserialize(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkFabricAccessControlListPatch>)new NetworkFabricAccessControlListPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        private static NetworkFabricAccessControlListPatch Deserialize(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkFabricAccessControlListPatch>)new NetworkFabricAccessControlListPatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
