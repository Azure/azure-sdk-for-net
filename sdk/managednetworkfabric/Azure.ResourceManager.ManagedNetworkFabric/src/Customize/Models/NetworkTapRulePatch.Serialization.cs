...
namespace Azure.ResourceManager.ManagedNetworkFabric.Models
{
    ...
    public partial class NetworkTapRulePatch
    {
        ...
        private static NetworkTapRulePatch Deserialize(BinaryData data, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkTapRulePatch>)new NetworkTapRulePatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
        ...
        private static NetworkTapRulePatch Deserialize(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<NetworkTapRulePatch>)new NetworkTapRulePatch()).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
