    ...
    public class ContainerRegistryContentClientTests : ClientTestBase
    {
        ...
        public async Task CanParseMultipleErrors()
        {
            ...
            catch (RequestFailedException ex) when (ex.Status == 404 && ex.ErrorCode == "FIRST_ERROR_CODE")
            {
                ...
                caught = true;
                Assert.That(ex.Message, Does.Contain("first error message"), "Exception message does not contain first error message.");
                Assert.That(ex.Message, Does.Contain("second error message"), "Exception message does not contain second error message.");
            }
        ...
        public async Task UploadBlobExceptionHasTsgLink()
        {
            ...
            catch (RequestFailedException ex)
            {
                ...
                caught = true;
                Assert.That(ex.Message, Does.Contain("https://aka.ms/azsdk/net/containerregistry/uploadblob/troubleshoot"), "Exception message does not contain troubleshooting guide link.");
            }
