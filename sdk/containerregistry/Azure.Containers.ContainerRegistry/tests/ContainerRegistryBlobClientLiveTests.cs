...
namespace Azure.Containers.ContainerRegistry.Tests
{
    ...
    public class ContainerRegistryBlobClientLiveTests : ContainerRegistryRecordedTestBase
    {
        ...
        public async Task CanPullHelloWorld()
        {
            ...
            // Download the manifest list to get the list of manifests per architecture
            DownloadManifestOptions options = new(tag)
            {
                MediaType = "application/vnd.docker.distribution.manifest.list.v2+json"
            };
            DownloadManifestResult listResult = await blobClient.DownloadManifestAsync(options);

            // Write it out
            await WriteFileAsync(Path.Combine(path, "manifest-list.json"), listResult.Content);

            OciIndex manifestList = listResult.AsOciIndex();
            foreach (var manifestInfo in manifestList.Manifests)
            {
                DownloadManifestResult manifestResult = await blobClient.DownloadManifestAsync(new(manifestInfo.Digest));
                string subPath = Path.Combine(path, GetPlatformString(manifestInfo.Platform));
                Directory.CreateDirectory(subPath);

                await WriteFileAsync(Path.Combine(subPath, "manifest.json"), manifestResult.Content);

                OciManifest manifest = manifestResult.AsOciManifest();

                // Download and write out the config
                DownloadBlobResult configResult = await blobClient.DownloadBlobAsync(manifest.Config.Digest);

                await WriteFileAsync(Path.Combine(subPath, "config.json"), configResult.Content);

                // Download and write out the layers
                foreach (var layerFile in manifest.Layers)
                {
                    DownloadBlobResult layerResult = await blobClient.DownloadBlobAsync(layerFile.Digest);

                    await WriteFileAsync(Path.Combine(subPath, TrimSha(layerFile.Digest)), layerResult.Content);
                }
            }
        }

        private string GetPlatformString(ManifestPlatform platform)
        {
            StringBuilder sb = new();
            sb.Append(platform.Architecture);

            if (!string.IsNullOrEmpty(platform.OperatingSystem.ToString()))
            {
                sb.Append("-");
                sb.Append(platform.OperatingSystem);
            }

            if (!string.IsNullOrEmpty(platform.Variant))
            {
                sb.Append("-");
                sb.Append(platform.Variant);
            }

            if (!string.IsNullOrEmpty(platform.OsVersion))
            {
                sb.Append("-");
                sb.Append(platform.OsVersion);
            }

            return sb.ToString();
        }

        private async Task WriteFileAsync(string path, BinaryData content)
        {
