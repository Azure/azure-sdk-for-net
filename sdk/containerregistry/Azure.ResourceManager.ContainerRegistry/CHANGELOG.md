...
- Added new resource types `ArchiveResource` and `ArchiveVersionResource` for managing OCI artifact archives within a registry.
- Added new resource types `ExportPipelineResource`, `ImportPipelineResource`, and `PipelineRunResource` for data transfer pipeline management.
- Added new models: `ArchivePackageSourceProperties`, `GarbageCollectionProperties`, `RegistrySyncResult`, `SoftDeletePolicy`, `PipelineRunResponse`.
- Added new extensible enum types: `EndpointProtocol`, `MetadataSearch`, `PackageSourceType`, `PipelineOption`, `PipelineRunSourceType`, `PipelineRunTargetType`, `PipelineSourceType`, `RegionalEndpoint`, `StorageAccessMode`, `SyncState`, `SyncTrigger`.
- Added new properties to `ContainerRegistryData`: `RegionalEndpoints`, `RegionalEndpointHostNames`, `EndpointProtocol`, `MetadataSearch`, `AutoGeneratedDomainNameLabelScope`, `RoleAssignmentMode`.
