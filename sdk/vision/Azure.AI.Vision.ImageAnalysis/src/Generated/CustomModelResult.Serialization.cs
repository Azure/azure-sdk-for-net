// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure;

namespace Azure.AI.Vision.ImageAnalysis
{
    public partial class CustomModelResult
    {
        internal static CustomModelResult DeserializeCustomModelResult(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            ObjectsResult objectsResult = default;
            TagsResult tagsResult = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("objectsResult"u8))
                {
                    objectsResult = ObjectsResult.DeserializeObjectsResult(property.Value);
                    continue;
                }
                if (property.NameEquals("tagsResult"u8))
                {
                    tagsResult = TagsResult.DeserializeTagsResult(property.Value);
                    continue;
                }
            }
            return new CustomModelResult(objectsResult, tagsResult);
        }

        /// <summary> Deserializes the model from a raw response. </summary>
        /// <param name="response"> The response to deserialize the model from. </param>
        internal static CustomModelResult FromResponse(Response response)
        {
            using var document = JsonDocument.Parse(response.Content);
            return DeserializeCustomModelResult(document.RootElement);
        }
    }
}
