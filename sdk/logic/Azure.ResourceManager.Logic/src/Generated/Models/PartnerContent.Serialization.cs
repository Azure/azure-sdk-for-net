// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.Logic.Models
{
    public partial class PartnerContent : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            if (Optional.IsDefined(B2B))
            {
                writer.WritePropertyName("b2b");
                writer.WriteObjectValue(B2B);
            }
            writer.WriteEndObject();
        }

        internal static PartnerContent DeserializePartnerContent(JsonElement element)
        {
            Optional<B2BPartnerContent> b2b = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("b2b"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    b2b = B2BPartnerContent.DeserializeB2BPartnerContent(property.Value);
                    continue;
                }
            }
            return new PartnerContent(b2b.Value);
        }
    }
}
