...
namespace Azure.Extensions.AspNetCore.Configuration.Secrets.Tests
{
    public class ConfigurationSecretsFunctionalTests: LiveTestBase<ConfigurationTestEnvironment>
    {
        ...
        public async Task SecretsAreLoadedFromKeyVault()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(configuration["TestSecret1"], Is.EqualTo("1"));
                Assert.That(configuration["TestSecret2"], Is.EqualTo("2"));
                Assert.That(configuration["Nested:TestSecret3"], Is.EqualTo("3"));
            });
            ...
            Assert.Multiple(() =>
            {
                Assert.That(configuration["TestSecret1"], Is.EqualTo("2"));
                Assert.That(configuration["TestSecret2"], Is.EqualTo("2"));
                Assert.That(configuration["Nested:TestSecret3"], Is.EqualTo("3"));
            });
        }
