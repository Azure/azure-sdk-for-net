...
namespace Azure.Core.Extensions.Tests
{
    ...
    internal static class ReflectionHelpers
    {
        ...
        {
            Assert.That(target, Is.Not.Null, $"Cannot get property '{propertyName}' from a null target.");
            Type type = target.GetType();
            PropertyInfo property = type.GetProperty(propertyName, NonPublicInstance);
            Assert.That(property, Is.Not.Null, $"Non-public instance property '{propertyName}' was not found on type '{type.FullName}'.");
        ...
        public static object GetNonPublicPropertyValue(Type declaringType, object target, string propertyName)
        {
            ...
            Assert.That(property, Is.Not.Null, $"Non-public instance property '{propertyName}' was not found on type '{declaringType.FullName}'.");
            return property.GetValue(target);
        }
        ...
        {
            Assert.That(target, Is.Not.Null, $"Cannot get field '{fieldName}' from a null target.");
            Type type = target.GetType();
            FieldInfo field = type.GetField(fieldName, NonPublicInstance);
            Assert.That(field, Is.Not.Null, $"Non-public instance field '{fieldName}' was not found on type '{type.FullName}'.");
        ...
        public static object GetNonPublicFieldValue(Type declaringType, object target, string fieldName)
        {
            ...
            Assert.That(field, Is.Not.Null, $"Non-public instance field '{fieldName}' was not found on type '{declaringType.FullName}'.");
            return field.GetValue(target);
        }
        ...
        {
            Assert.That(target, Is.Not.Null, $"Cannot get field ending with '{fieldNameSuffix}' from a null target.");
            FieldInfo field = declaringType
                .GetFields(NonPublicInstance)
                .FirstOrDefault(f => f.Name.EndsWith(fieldNameSuffix));
            Assert.That(field, Is.Not.Null, $"Non-public instance field ending with '{fieldNameSuffix}' was not found on type '{declaringType.FullName}'.");
            return field.GetValue(target);
        }
    }
}
