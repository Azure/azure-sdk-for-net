// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.Core;
using Azure.ResourceManager;
using Azure.ResourceManager.Automanage;

namespace Azure.ResourceManager.Automanage.Mock
{
    /// <summary> A class to add extension methods to TenantResource. </summary>
    public partial class AutomanageTenantResourceExtension : ArmResource
    {
        /// <summary> Initializes a new instance of the <see cref="AutomanageTenantResourceExtension"/> class for mocking. </summary>
        protected AutomanageTenantResourceExtension()
        {
        }

        /// <summary> Initializes a new instance of the <see cref="AutomanageTenantResourceExtension"/> class. </summary>
        /// <param name="client"> The client parameters to use in these operations. </param>
        /// <param name="id"> The identifier of the resource that is the target of operations. </param>
        internal AutomanageTenantResourceExtension(ArmClient client, ResourceIdentifier id) : base(client, id)
        {
        }

        private string GetApiVersionOrNull(ResourceType resourceType)
        {
            TryGetApiVersion(resourceType, out string apiVersion);
            return apiVersion;
        }

        /// <summary> Gets a collection of AutomanageBestPracticeResources in the TenantResource. </summary>
        /// <returns> An object representing collection of AutomanageBestPracticeResources and their operations over a AutomanageBestPracticeResource. </returns>
        public virtual AutomanageBestPracticeCollection GetAutomanageBestPractices()
        {
            return GetCachedClient(Client => new AutomanageBestPracticeCollection(Client, Id));
        }
    }
}
