...
namespace Azure.ResourceManager.DevOpsInfrastructure.Mocking
{
    ...
    public partial class MockableDevOpsInfrastructureResourceGroupResource : ArmResource
    {
        ...
        /// <returns> An async collection of <see cref="ImageVersion"/> that may take multiple service requests to iterate over. </returns>
        public virtual AsyncPageable<ImageVersion> GetImageVersionsByImageAsync(string imageName, CancellationToken cancellationToken = default)
        {
            ...
            return GeneratorPageableHelpers.CreateAsyncPageable(FirstPageRequest, NextPageRequest, e => ImageVersion.DeserializeImageVersion(e), ImageVersionsClientDiagnostics, Pipeline, "MockableDevOpsInfrastructureResourceGroupResource.GetImageVersionsByImage", "value", "nextLink", cancellationToken);
        }
        ...
        /// <exception cref="ArgumentException"> <paramref name="imageName"/> is an empty string, and was expected to be non-empty. </exception>
        /// <exception cref="ArgumentNullException"> <paramref name="imageName"/> is null. </exception>
        /// <returns> A collection of <see cref="ImageVersion"/> that may take multiple service requests to iterate over. </returns>
        public virtual Pageable<ImageVersion> GetImageVersionsByImage(string imageName, CancellationToken cancellationToken = default)
        {
            Argument.AssertNotNullOrEmpty(imageName, nameof(imageName));

            HttpMessage FirstPageRequest(int? pageSizeHint) => ImageVersionsRestClient.CreateListByImageRequest(Id.SubscriptionId, Id.ResourceGroupName, imageName);
            HttpMessage NextPageRequest(int? pageSizeHint, string nextLink) => ImageVersionsRestClient.CreateListByImageNextPageRequest(nextLink, Id.SubscriptionId, Id.ResourceGroupName, imageName);
            return GeneratorPageableHelpers.CreatePageable(FirstPageRequest, NextPageRequest, e => ImageVersion.DeserializeImageVersion(e), ImageVersionsClientDiagnostics, Pipeline, "MockableDevOpsInfrastructureResourceGroupResource.GetImageVersionsByImage", "value", "nextLink", cancellationToken);
        }
    }
}
