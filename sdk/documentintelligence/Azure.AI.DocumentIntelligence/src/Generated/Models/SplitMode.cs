// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.ComponentModel;

namespace Azure.AI.DocumentIntelligence
{
    /// <summary> Document splitting mode. </summary>
    public readonly partial struct SplitMode : IEquatable<global::Azure.AI.DocumentIntelligence.SplitMode>
    {
        private readonly string _value;
        /// <summary> Automatically split file into documents. </summary>
        private const string AutoValue = "auto";
        /// <summary> Treat the entire file as a single document. </summary>
        private const string NoneValue = "none";
        /// <summary> Treat each page in the file as a separate document. </summary>
        private const string PerPageValue = "perPage";

        /// <summary> Initializes a new instance of <see cref="global::Azure.AI.DocumentIntelligence.SplitMode"/>. </summary>
        /// <param name="value"> The value. </param>
        /// <exception cref="global::System.ArgumentNullException"> <paramref name="value"/> is null. </exception>
        public SplitMode(string value)
        {
            global::Azure.AI.DocumentIntelligence.Argument.AssertNotNull(value, nameof(value));

            _value = value;
        }

        /// <summary> Automatically split file into documents. </summary>
        public static SplitMode Auto { get; } = new SplitMode(AutoValue);

        /// <summary> Treat the entire file as a single document. </summary>
        public static SplitMode None { get; } = new SplitMode(NoneValue);

        /// <summary> Treat each page in the file as a separate document. </summary>
        public static SplitMode PerPage { get; } = new SplitMode(PerPageValue);

        /// <summary> Determines if two <see cref="global::Azure.AI.DocumentIntelligence.SplitMode"/> values are the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator ==(SplitMode left, SplitMode right) => left.Equals(right);

        /// <summary> Determines if two <see cref="global::Azure.AI.DocumentIntelligence.SplitMode"/> values are not the same. </summary>
        /// <param name="left"> The left value to compare. </param>
        /// <param name="right"> The right value to compare. </param>
        public static bool operator !=(SplitMode left, SplitMode right) => !left.Equals(right);

        /// <summary> Converts a string to a <see cref="global::Azure.AI.DocumentIntelligence.SplitMode"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator SplitMode(string value) => new SplitMode(value);

        /// <summary> Converts a string to a <see cref="global::Azure.AI.DocumentIntelligence.SplitMode"/>. </summary>
        /// <param name="value"> The value. </param>
        public static implicit operator SplitMode?(string value) => (value == null) ? null : new SplitMode(value);

        /// <inheritdoc/>
        [EditorBrowsableAttribute(global::System.ComponentModel.EditorBrowsableState.Never)]
        public override bool Equals(object obj) => ((obj is SplitMode other) && this.Equals(other));

        /// <inheritdoc/>
        public bool Equals(SplitMode other) => string.Equals(_value, other._value, global::System.StringComparison.InvariantCultureIgnoreCase);

        /// <inheritdoc/>
        [EditorBrowsableAttribute(global::System.ComponentModel.EditorBrowsableState.Never)]
        public override int GetHashCode() => (_value != null) ? global::System.StringComparer.InvariantCultureIgnoreCase.GetHashCode(_value) : 0;

        /// <inheritdoc/>
        public override string ToString() => _value;
    }
}
