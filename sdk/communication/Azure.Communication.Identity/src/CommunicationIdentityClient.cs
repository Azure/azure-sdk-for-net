...
namespace Azure.Communication.Identity
{
    ...
    public class CommunicationIdentityClient
    {
        ...
        public virtual Response<CommunicationIdentityResult> GetUser(CommunicationUserIdentifier communicationUser, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(CommunicationIdentityClient)}.{nameof(GetUser)}");
            scope.Start();
            try
            {
                Response<CommunicationIdentityResult> response = RestClient.Get(communicationUser.Id, cancellationToken: cancellationToken);
                ...
                return Response.FromValue(new CommunicationIdentityResult(responseId, externalId, lastTokenIssuedAt), response.GetRawResponse());
            }
            catch (Exception ex)
            {
                scope.Failed(ex);
        ...
        public virtual async Task<Response<CommunicationIdentityResult>> GetUserAsync(CommunicationUserIdentifier communicationUser, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                Response<CommunicationIdentityResult> response = await RestClient.GetAsync(communicationUser.Id, cancellationToken: cancellationToken).ConfigureAwait(false);
                var externalId = response.Value.ExternalId;
                var responseId = response.Value.Id;
                var lastTokenIssuedAt = response.Value.LastTokenIssuedAt;
        ...
        public virtual Response<CommunicationUserIdentifier> CreateUser(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                Response<CommunicationUserIdentifierAndToken> response = RestClient.Create(default, Array.Empty<CommunicationTokenScope>(), cancellationToken: cancellationToken);
                var id = response.Value.Identity.Id;
                return Response.FromValue(new CommunicationUserIdentifier(id), response.GetRawResponse());
            }
            catch (Exception ex)
