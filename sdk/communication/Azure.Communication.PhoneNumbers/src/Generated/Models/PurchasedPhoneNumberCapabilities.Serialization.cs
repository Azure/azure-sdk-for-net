// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.Communication.PhoneNumbers
{
    public partial class PurchasedPhoneNumberCapabilities
    {
        internal static PurchasedPhoneNumberCapabilities DeserializePurchasedPhoneNumberCapabilities(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            PhoneNumberCapabilityType calling = default;
            PhoneNumberCapabilityType sms = default;
            Optional<string> tenDLCCampaignBriefId = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("calling"u8))
                {
                    calling = new PhoneNumberCapabilityType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("sms"u8))
                {
                    sms = new PhoneNumberCapabilityType(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("tenDLCCampaignBriefId"u8))
                {
                    tenDLCCampaignBriefId = property.Value.GetString();
                    continue;
                }
            }
            return new PurchasedPhoneNumberCapabilities(calling, sms, tenDLCCampaignBriefId.Value);
        }
    }
}
