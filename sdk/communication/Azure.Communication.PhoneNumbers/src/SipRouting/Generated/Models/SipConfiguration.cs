// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;

namespace Azure.Communication.PhoneNumbers.SipRouting
{
    /// <summary>
    /// Represents a SIP configuration.
    /// When a call is being routed the routes are applied in the same order as in the routes list.
    /// A route is matched by its number pattern.
    /// Call is then directed into route's first available trunk, based on the order in the route's trunks list. The configuration can be expanded with additional data.
    /// </summary>
    internal partial class SipConfiguration
    {
        /// <summary> Initializes a new instance of <see cref="SipConfiguration"/>. </summary>
        /// <param name="domains">
        /// Validated Domains.
        /// Map key is domain.
        /// </param>
        /// <param name="trunks">
        /// SIP trunks for routing calls.
        /// Map key is trunk's FQDN (1-249 characters).
        /// </param>
        /// <param name="routes"> Trunk routes for routing calls. </param>
        internal SipConfiguration(IReadOnlyDictionary<string, SipDomain> domains, IReadOnlyDictionary<string, SipTrunk> trunks, IReadOnlyList<SipTrunkRoute> routes)
        {
            Domains = domains;
            Trunks = trunks;
            Routes = routes;
        }
    }
}
