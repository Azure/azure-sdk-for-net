// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Text.Json;
using Azure.Core;

namespace Azure.Communication.JobRouter.Models
{
    public partial class UpsertDistributionPolicyResponse
    {
        internal static UpsertDistributionPolicyResponse DeserializeUpsertDistributionPolicyResponse(JsonElement element)
        {
            string id = default;
            Optional<string> name = default;
            TimeSpan offerTTL = default;
            DistributionMode mode = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("id"))
                {
                    id = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("name"))
                {
                    name = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("offerTTL"))
                {
                    offerTTL = property.Value.GetTimeSpan();
                    continue;
                }
                if (property.NameEquals("mode"))
                {
                    mode = DistributionMode.DeserializeDistributionMode(property.Value);
                    continue;
                }
            }
            return new UpsertDistributionPolicyResponse(id, name.Value, offerTTL, mode);
        }
    }
}
