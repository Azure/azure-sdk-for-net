// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.Communication.JobRouter
{
    public partial class StaticWorkerSelector : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            writer.WritePropertyName("labelSelector");
            writer.WriteObjectValue(LabelSelector);
            writer.WritePropertyName("kind");
            writer.WriteStringValue(Kind);
            writer.WriteEndObject();
        }

        internal static StaticWorkerSelector DeserializeStaticWorkerSelector(JsonElement element)
        {
            WorkerSelector labelSelector = default;
            string kind = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("labelSelector"))
                {
                    labelSelector = WorkerSelector.DeserializeWorkerSelector(property.Value);
                    continue;
                }
                if (property.NameEquals("kind"))
                {
                    kind = property.Value.GetString();
                    continue;
                }
            }
            return new StaticWorkerSelector(kind, labelSelector);
        }
    }
}
