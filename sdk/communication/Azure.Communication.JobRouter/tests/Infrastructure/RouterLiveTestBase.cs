...
namespace Azure.Communication.JobRouter.Tests.Infrastructure
{
    public class RouterLiveTestBase : RecordedTestBase<RouterTestEnvironment>
    {
        ...
        protected async Task<Response<ClassificationPolicy>> CreateQueueSelectionCPAsync(string? uniqueIdentifier = default)
        {
            ...
            var createClassificationPolicyResponse = await routerClient.CreateClassificationPolicyAsync(
                new CreateClassificationPolicyOptions(classificationPolicyId)
                {
                    ...
                    QueueSelectors =
                    {
                        // TODO: don't new a new Value
                        new StaticQueueSelectorAttachment(new RouterQueueSelector("Id", LabelOperator.Equal, new Value(createQueueResponse.Value.Id)))
                    }
                });
        ...
        protected async Task<Response<RouterQueue>> CreateQueueAsync(string? uniqueIdentifier = default)
        {
            ...
            var queueLabels = new Dictionary<string, Value> { ["Label_1"] = new("Value_1") };
        ...
        protected void AssertQueueResponseIsEqual(Response<Models.RouterQueue> upsertQueueResponse, string queueId, string distributionPolicyId, string? queueName = default, IDictionary<string, Value>? queueLabels = default, string? exceptionPolicyId = default)
        {
            ...
            if (queueLabels != default)
            {
                ...
                {
                    labelsWithID.Add("Id", new Value(queueId));
                }

                Assert.AreEqual(labelsWithID.ToDictionary(x => x.Key, x => x.Value), response.Labels.ToDictionary(x => x.Key, x => x.Value));
            }
        ...
        protected void AssertRegisteredWorkerIsValid(Response<RouterWorker> routerWorkerResponse, string workerId,
            ...
            IDictionary<string, Value>? workerLabels = default,
            IDictionary<string, ChannelConfiguration?>? channelConfigList = default,
            IDictionary<string, Value>? workerTags = default)
        {
            ...
            if (workerLabels != default)
            {
                ...
                labelsWithID.Add("Id", new Value(workerId));
