...
`SmsClient` provides the functionality to send SMS messages, manage opt-out preferences, and retrieve delivery reports:
- Send SMS to one or multiple recipients
- `SmsClient.OptOuts` - manage opt-out preferences
- `SmsClient.GetDeliveryReport[Async]` - retrieve message delivery status
...
To get delivery report for a specific outgoing message, call the `GetDeliveryReport` or `GetDeliveryReportAsync` function from the `SmsClient`.
...
To send SMS messages through Messaging Connect partners, provide partner-specific configuration as a dictionary. This approach automatically supports any messaging partner without requiring SDK changes.

```csharp
// Create partner-specific parameters
