...
namespace Azure.Communication.CallAutomation
{
    ...
    public class CallMedia
    {
        ...
        /// </summary>
        /// <param name="options">An optional object containing interrupt audio and announce options and configurations.</param>
        /// <param name="cancellationToken">An optional CancellationToken to cancel the request.</param>
        public virtual async Task<Response> InterruptAudioAndAnnounceAsync(InterruptAudioAndAnnounceOptions options = default, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(CallMedia)}.{nameof(InterruptAudioAndAnnounce)}");
            scope.Start();
            try
            {
                InterruptAudioAndAnnounceRequestInternal request = CreateInterruptAudioAndAnnounceRequest(options);
        ...
        /// <param name="options">An optional object containing interrupt audio and announce options and configurations.</param>
        /// <param name="cancellationToken">An optional CancellationToken to cancel the request.</param>
        public virtual Response InterruptAudioAndAnnounce(InterruptAudioAndAnnounceOptions options = default, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(CallMedia)}.{nameof(InterruptAudioAndAnnounce)}");
            scope.Start();
            try
        ...
        private static InterruptAudioAndAnnounceRequestInternal CreateInterruptAudioAndAnnounceRequest(InterruptAudioAndAnnounceOptions options)
        {
            InterruptAudioAndAnnounceRequestInternal request = new InterruptAudioAndAnnounceRequestInternal(options.PlaySources.Select(t => TranslatePlaySourceToInternal(t)).ToList(), options.PlayTo.Select(t => CommunicationIdentifierSerializer.Serialize(t)).ToList().FirstOrDefault());
            if (options != null)
            {
                request.OperationContext = options.OperationContext;
            }
