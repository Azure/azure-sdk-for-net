...
namespace Azure.Communication.CallAutomation
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<UnmuteParticipantsResult>)this).Write(writer, new ModelReaderWriterOptions("W"));

        void IJsonModel<UnmuteParticipantsResult>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<UnmuteParticipantsResult>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new InvalidOperationException($"The model {nameof(UnmuteParticipantsResult)} does not support '{format}' format.");
            }
        ...
        BinaryData IPersistableModel<UnmuteParticipantsResult>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new InvalidOperationException($"The model {nameof(UnmuteParticipantsResult)} does not support '{options.Format}' format.");
            }
        }
        ...
        string IPersistableModel<UnmuteParticipantsResult>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
