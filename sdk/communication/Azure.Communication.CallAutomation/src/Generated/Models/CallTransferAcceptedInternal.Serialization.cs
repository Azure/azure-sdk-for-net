...
namespace Azure.Communication.CallAutomation
{
    internal partial class CallTransferAcceptedInternal : IUtf8JsonSerializable, IJsonModel<CallTransferAcceptedInternal>
    {
        ...
        {
            var format = options.Format == "W" ? ((IPersistableModel<CallTransferAcceptedInternal>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new InvalidOperationException($"The model {nameof(CallTransferAcceptedInternal)} does not support '{format}' format.");
            }
            ...
            if (options.Format != "W" && Optional.IsDefined(TransferTarget))
            {
                writer.WritePropertyName("transferTarget"u8);
                writer.WriteObjectValue(TransferTarget);
            }
            if (options.Format != "W" && Optional.IsDefined(Transferee))
            {
        ...
        internal static CallTransferAcceptedInternal DeserializeCallTransferAcceptedInternal(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            return new CallTransferAcceptedInternal(operationContext.Value, resultInformation.Value, transferTarget.Value, transferee.Value, callConnectionId.Value, serverCallId.Value, correlationId.Value, serializedAdditionalRawData);
        }
        ...
        BinaryData IPersistableModel<CallTransferAcceptedInternal>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new InvalidOperationException($"The model {nameof(CallTransferAcceptedInternal)} does not support '{options.Format}' format.");
            }
        }
