// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

namespace Azure.Communication.CallingServer
{
    /// <summary> The response payload for start hold music operation. </summary>
    public partial class StartHoldMusicResult
    {
        /// <summary> Initializes a new instance of StartHoldMusicResult. </summary>
        /// <param name="status"> The status of the operation. </param>
        internal StartHoldMusicResult(OperationStatus status)
        {
            Status = status;
        }

        /// <summary> Initializes a new instance of StartHoldMusicResult. </summary>
        /// <param name="operationId"> The operation id. </param>
        /// <param name="status"> The status of the operation. </param>
        /// <param name="operationContext"> The operation context provided by client. </param>
        /// <param name="resultInfo"> The result info for the operation. </param>
        internal StartHoldMusicResult(string operationId, OperationStatus status, string operationContext, ResultInfo resultInfo)
        {
            OperationId = operationId;
            Status = status;
            OperationContext = operationContext;
            ResultInfo = resultInfo;
        }

        /// <summary> The operation id. </summary>
        public string OperationId { get; }
        /// <summary> The status of the operation. </summary>
        public OperationStatus Status { get; }
        /// <summary> The operation context provided by client. </summary>
        public string OperationContext { get; }
        /// <summary> The result info for the operation. </summary>
        public ResultInfo ResultInfo { get; }
    }
}
