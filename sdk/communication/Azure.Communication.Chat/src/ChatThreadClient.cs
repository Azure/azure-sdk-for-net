...
namespace Azure.Communication.Chat
{
    ...
    public class ChatThreadClient
    {
        ...
        /// <summary> Posts a typing event to a thread, on behalf of a user asynchronously. </summary>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <param name="sendTypingNotificationOptions"> Typing notification options. </param>
        /// <exception cref="RequestFailedException">The server returned an error. See <see cref="Exception.Message"/> for details returned from the server.</exception>
        public virtual async Task<Response> SendTypingNotificationAsync(SendTypingNotificationOptions sendTypingNotificationOptions, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(ChatThreadClient)}.{nameof(SendTypingNotification)}");
            scope.Start();
            try
            {
                return await _chatThreadRestClient.SendTypingNotificationAsync(Id, sendTypingNotificationOptions.SenderDisplayName, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                scope.Failed(ex);
        ...
        /// <param name="sendTypingNotificationOptions"> Typing notification options. </param>
        /// <exception cref="RequestFailedException">The server returned an error. See <see cref="Exception.Message"/> for details returned from the server.</exception>
        public virtual Response SendTypingNotification(SendTypingNotificationOptions sendTypingNotificationOptions, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(ChatThreadClient)}.{nameof(SendTypingNotification)}");
            scope.Start();
            try
            {
                return _chatThreadRestClient.SendTypingNotification(Id, sendTypingNotificationOptions.SenderDisplayName, cancellationToken);
            }
            catch (Exception ex)
            {
                scope.Failed(ex);
