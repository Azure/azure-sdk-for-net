...
namespace Azure.ResourceManager.DevCenter
{
    public partial class DevCenterPoolData : IUtf8JsonSerializable, IJsonModel<DevCenterPoolData>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(DevBoxDefinitionName))
            {
                builder.Append("    devBoxDefinitionName:");
                if (DevBoxDefinitionName.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{DevBoxDefinitionName}'''");
                }
                else
                {
                    builder.AppendLine($" '{DevBoxDefinitionName}'");
            ...
            if (Optional.IsDefined(NetworkConnectionName))
            {
                builder.Append("    networkConnectionName:");
                if (NetworkConnectionName.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{NetworkConnectionName}'''");
                }
                else
                {
                    builder.AppendLine($" '{NetworkConnectionName}'");
            ...
            if (Optional.IsDefined(LocalAdministrator))
            {
                builder.Append("    localAdministrator:");
                builder.AppendLine($" '{LocalAdministrator.Value.ToString()}'");
            }

            if (Optional.IsDefined(StopOnDisconnect))
            {
                builder.Append("    stopOnDisconnect:");
                AppendChildObject(builder, StopOnDisconnect, options, 4, false);
            }
            ...
            {
                if (HealthStatusDetails.Any())
                {
                    builder.Append("    healthStatusDetails:");
                    builder.AppendLine(" [");
                    foreach (var item in HealthStatusDetails)
                    {
