...
#pragma warning restore SA1649 // File name should match first type name
    {
        ...
        internal DnsResolverArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            ...
            _operation = new OperationInternal(clientDiagnostics, nextLinkOperation, response, "DnsResolverArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }

        internal DnsResolverArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, string id)
        {
            var nextLinkOperation = NextLinkOperationImplementation.Create(pipeline, id, out string finalResponse);
            _operation = OperationInternal.Create(clientDiagnostics, nextLinkOperation, finalResponse, "DnsResolverArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }
