...
namespace Azure.ResourceManager.DnsResolver
{
    ...
    public partial class InboundEndpointResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation<InboundEndpointResource>> UpdateAsync(WaitUntil waitUntil, InboundEndpointPatch patch, string ifMatch = null, CancellationToken cancellationToken = default)
        {
            Argument.AssertNotNull(patch, nameof(patch));

            using var scope = _inboundEndpointClientDiagnostics.CreateScope("InboundEndpointResource.Update");
            scope.Start();
            try
            {
                var response = await _inboundEndpointRestClient.UpdateAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, patch, ifMatch, cancellationToken).ConfigureAwait(false);
                var operation = new DnsResolverArmOperation<InboundEndpointResource>(new InboundEndpointOperationSource(Client), _inboundEndpointClientDiagnostics, Pipeline, _inboundEndpointRestClient.CreateUpdateRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, patch, ifMatch).Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
        ...
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="patch"/> is null. </exception>
        public virtual ArmOperation<InboundEndpointResource> Update(WaitUntil waitUntil, InboundEndpointPatch patch, string ifMatch = null, CancellationToken cancellationToken = default)
        {
            Argument.AssertNotNull(patch, nameof(patch));
            ...
            try
            {
                var response = _inboundEndpointRestClient.Update(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, patch, ifMatch, cancellationToken);
                var operation = new DnsResolverArmOperation<InboundEndpointResource>(new InboundEndpointOperationSource(Client), _inboundEndpointClientDiagnostics, Pipeline, _inboundEndpointRestClient.CreateUpdateRequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, Id.Name, patch, ifMatch).Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
