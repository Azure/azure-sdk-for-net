    ...
    public partial class ConnectorMappingProperties : IUtf8JsonSerializable, IJsonModel<ConnectorMappingProperties>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(FileFilter))
            {
                ...
                if (FileFilter.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{FileFilter}'''");
                }
                else
                {
                    builder.AppendLine($" '{FileFilter}'");
            ...
            if (Optional.IsDefined(HasHeader))
            {
                ...
                var boolValue = HasHeader.Value == true ? "true" : "false";
                builder.AppendLine($" {boolValue}");
            }

            if (Optional.IsDefined(ErrorManagement))
            {
                builder.Append("  errorManagement:");
                AppendChildObject(builder, ErrorManagement, options, 2, false);
            }

            if (Optional.IsDefined(Format))
            {
                builder.Append("  format:");
                AppendChildObject(builder, Format, options, 2, false);
            }

            if (Optional.IsDefined(Availability))
            {
                builder.Append("  availability:");
                AppendChildObject(builder, Availability, options, 2, false);
            }

            if (Optional.IsCollectionDefined(Structure))
            {
                if (Structure.Any())
                {
            ...
            if (Optional.IsDefined(CompleteOperation))
            {
                builder.Append("  completeOperation:");
                AppendChildObject(builder, CompleteOperation, options, 2, false);
            }
