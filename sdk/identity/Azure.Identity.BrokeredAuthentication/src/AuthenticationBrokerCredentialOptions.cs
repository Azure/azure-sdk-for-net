...
{
    /// <summary>
    /// Options to configure the <see cref="AuthenticationBrokerCredential"/>.
    /// </summary>
    public class AuthenticationBrokerCredentialOptions : TokenCredentialOptions, ITokenCacheOptions
    {
        private string _tenantId;

        /// <summary>
        /// Prevents the <see cref="AuthenticationBrokerCredential"/> from automatically prompting the user. If automatic authentication is disabled a AuthenticationRequiredException will be thrown from <see cref="AuthenticationBrokerCredential.GetToken"/> and <see cref="AuthenticationBrokerCredential.GetTokenAsync"/> in the case that
        /// user interaction is necessary. The application is responsible for handling this exception, and calling <see cref="AuthenticationBrokerCredential.Authenticate(CancellationToken)"/> or <see cref="AuthenticationBrokerCredential.AuthenticateAsync(CancellationToken)"/> to authenticate the user interactively.
        /// </summary>
        ...
        internal CredentialPipeline CredentialPipeline { get; set; }
