    ...
    public class KubernetesProxyHttpHandlerTests
    {
        ...
        public async Task SendAsync_PreservesQueryParameters()
        {
            ...
            Assert.Multiple(() =>
            {
                // Assert
                Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));
                Assert.That(request.Uri.Query, Does.Contain("api-version=2.0"), $"Expected query to contain api-version=2.0 but got: {request.Uri.Query}");
            });
            Assert.That(request.Uri.Query, Does.Contain("scope=test"));
        }
        ...
        public async Task SendAsync_ConcurrentRequestsDuringCaFileReloadAllSucceed()
        {
            ...
            foreach (var response in responses)
            {
                ...
                Assert.That(content, Does.Contain("request"), "Response should contain request counter");
            }
        ...
        public void CertificateValidationCallback_ConfiguresCorrectly()
        {
            ...
            if (innerHandlerWithCa?.ServerCertificateCustomValidationCallback != null)
            {
                ...
                using (var chain = new X509Chain())
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        Assert.That(chain.ChainPolicy.ExtraStore.Count, Is.GreaterThan(0), "CA should be added to ExtraStore");
