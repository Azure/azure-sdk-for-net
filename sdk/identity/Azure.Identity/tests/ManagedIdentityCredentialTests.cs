...
namespace Azure.Identity.Tests
{
    ...
    public class ManagedIdentityCredentialTests : ClientTestBase
    {
        ...
        public async Task VerifyAuthenticationFailedExceptionsAreDeferredToGetToken(Dictionary<string, string> environmentVariables)
        {
            ...
            // Provide a mock transport to prevent real network calls to IMDS
            var mockTransport = new MockTransport(req => throw new RequestFailedException("Invalid URI"));
            var options = new TokenCredentialOptions { Transport = mockTransport };
            options.Retry.MaxDelay = TimeSpan.Zero;
            var pipeline = CredentialPipeline.GetInstance(options, IsManagedIdentityCredential: true);

            var credential = InstrumentClient(new ManagedIdentityCredential(new ManagedIdentityClient(new ManagedIdentityClientOptions { IsForceRefreshEnabled = true, Pipeline = pipeline, Options = options })));
