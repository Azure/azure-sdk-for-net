...
{
    internal abstract class ConfigurableCredentialTestBase<TCred, TCredOptions> : CredentialTestBase<TCredOptions>
        where TCred : TokenCredential
        where TCredOptions : TokenCredentialOptions
    {
        protected ConfigurableCredentialTestBase(bool isAsync) : base(isAsync)
        {
        }
        ...
        public override TokenCredential GetTokenCredential(TokenCredentialOptions options)
        {
            var defaultOptions = new DefaultAzureCredentialOptions
            {
                ...
                // If we don't do this the env var kicks in.
                // This behavior matches the non configurable cli credential tests.
                TenantId = null
            };
            ConfigurableCredential credential = new(defaultOptions);

            return InstrumentCredential(credential);
        }

        protected ConfigurableCredential InstrumentCredential(ConfigurableCredential credential, IProcessService processService = null)
        {
            var (_, _, processOutput) = CredentialTestHelpers.CreateTokenForAzureCli();
            TestProcess testProcess = new() { Output = processOutput };
            IProcessService testProcessService = processService ?? new TestProcessService(testProcess, true);

            TCred azureCliSource = GetUnderlyingCredential(credential);

            azureCliSource
                .GetType()
                .GetField("_processService", BindingFlags.NonPublic | BindingFlags.Instance)
                .SetValue(azureCliSource, testProcessService);

            azureCliSource
                .GetType()
                .GetField("_pipeline", BindingFlags.NonPublic | BindingFlags.Instance)
                .SetValue(azureCliSource, CredentialPipeline.GetInstance(null));
        ...
        private IConfiguration GetConfigurationFromCommonCredentialTestConfig(CommonCredentialTestConfig config)
        {
            IConfiguration configuration = GetConfiguration();

            configuration["TenantId"] = config.TenantId;
        ...
        protected IConfiguration GetConfiguration()
        {
            IConfiguration config = new ConfigurationBuilder()
                ...
                .Build();
            return GetConfigurationCore(config);
        }

        protected abstract IConfiguration GetConfigurationCore(IConfiguration config);

        private static TCred GetUnderlyingCredential(ConfigurableCredential credential)
        {
        ...
        private static ConfigurableCredential InstrumentConfigurableCredential(ConfigurableCredential credential)
        {
            var (_, _, processOutput) = CredentialTestHelpers.CreateTokenForAzureCli();
            TestProcess testProcess = new() { Output = processOutput };
            IProcessService testProcessService = new TestProcessService(testProcess, true);
        ...
        protected static ConfigurableCredential GetCredentialFromConfig(IConfiguration config)
        {
