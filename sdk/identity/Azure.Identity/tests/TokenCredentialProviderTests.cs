...
namespace Azure.Identity.Tests
{
    public class TokenCredentialProviderTests
    {
        ...
        public class SimpleMockTokenCredential : TokenCredential
        {
            ...
            {
                return _scope == options.Scopes[0] ? new AccessToken(_token, DateTimeOffset.MaxValue) : default;
            }

            public override async Task<AccessToken> GetTokenAsync(TokenOptions options, CancellationToken cancellationToken)
            {
        ...
        public async Task CredentialSequenceValid()
        {
            ...
            Assert.AreEqual("tokenA", (await provider.GetTokenAsync(new TokenOptions(new string[] { "scopeA" }))).Token);
            Assert.AreEqual("tokenB", (await provider.GetTokenAsync(new TokenOptions(new string[] { "scopeB" }))).Token);
            Assert.AreEqual("tokenC", (await provider.GetTokenAsync(new TokenOptions(new string[] { "scopeC" }))).Token);
            Assert.IsNull((await provider.GetTokenAsync(new TokenOptions(new string[] { "scopeD" }))).Token);
        }
        ...
        public async Task CredentialThrows()
        {
            ...
            Assert.AreEqual("tokenA", (await provider.GetTokenAsync(new TokenOptions(new string[] { "scopeA" }))).Token);
            Assert.ThrowsAsync<MockException>(async () => await provider.GetTokenAsync(new TokenOptions(new string[] { "ScopeB" })));
            Assert.ThrowsAsync<MockException>(async () => await provider.GetTokenAsync(new TokenOptions(new string[] { "ScopeC" })));
        }
    }
}
