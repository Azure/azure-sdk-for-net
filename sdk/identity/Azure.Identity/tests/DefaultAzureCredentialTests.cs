...
namespace Azure.Identity.Tests
{
    public class DefaultAzureCredentialTests : ClientTestBase
    {
        ...
        public void ValidateCtorNoOptions()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(sources[0], Is.InstanceOf(typeof(EnvironmentCredential)));
                Assert.That(sources[1], Is.InstanceOf(typeof(WorkloadIdentityCredential)));
                Assert.That(sources[2], Is.InstanceOf(typeof(ManagedIdentityCredential)));
                Assert.That(sources[3], Is.InstanceOf(typeof(VisualStudioCredential)));
                Assert.That(sources[4], Is.InstanceOf(typeof(VisualStudioCodeCredential)));
                Assert.That(sources[5], Is.InstanceOf(typeof(AzureCliCredential)));
                Assert.That(sources[6], Is.InstanceOf(typeof(AzurePowerShellCredential)));
                Assert.That(sources[7], Is.InstanceOf(typeof(AzureDeveloperCliCredential)));
            });
        }
        ...
        public void ValidateCtorIncludedInteractiveParam([Values(true, false)] bool includeInteractive)
        {
            ...
            Assert.That(sources, Is.Not.Null);
            Assert.That(sources, Has.Length.EqualTo(includeInteractive ? 10 : 9));
            ...
            {
                Assert.That(sources[8], Is.InstanceOf(typeof(InteractiveBrowserCredential)));
            }
        }
                                           ...
                                           bool excludeInteractiveBrowserCredential)
        {
            ...
            {
                Assert.That(ex.Message, Does.Contain("EnvironmentCredential Unavailable"));
            }
            if (!excludeWorkloadIdentityCredential)
            {
                Assert.That(ex.Message, Does.Contain("WorkloadIdentityCredential Unavailable"));
            }
            if (!excludeManagedIdentityCredential)
            {
                Assert.That(ex.Message, Does.Contain("ManagedIdentityCredential Unavailable"));
            }
            if (!excludeDeveloperCliCredential)
            {
                Assert.That(ex.Message, Does.Contain("DeveloperCliCredential Unavailable"));
            }
            if (!excludeSharedTokenCacheCredential)
            {
                Assert.That(ex.Message, Does.Contain("SharedTokenCacheCredential Unavailable"));
            }
            if (!excludeCliCredential)
            {
                Assert.That(ex.Message, Does.Contain("CliCredential Unavailable"));
            }
            if (!excludePowerShellCredential)
            {
                Assert.That(ex.Message, Does.Contain("PowerShellCredential Unavailable"));
            }
            if (!excludeInteractiveBrowserCredential)
            {
                Assert.That(ex.Message, Does.Contain("InteractiveBrowserCredential Unavailable"));
            }
            if (!excludeVisualStudioCredential)
            {
                Assert.That(ex.Message, Does.Contain("VisualStudioCredential Unavailable"));
            }
            if (!excludeVisualStudioCodeCredential)
            {
                Assert.That(ex.Message, Does.Contain("VisualStudioCodeCredential Unavailable"));
            }
        }
        ...
        public void DisableMetadataDiscoveryOptionIsHonoredWhenTrue(Type availableCredential)
        {
                    ...
                    { "AZURE_FEDERATED_TOKEN_FILE", "/temp/token" } }))
            {
                ...
                Assert.That(credential._sources, Has.Length.EqualTo(1));
        ...
        public void AdditionallyAllowedTenantsOptionIsHonored(Type availableCredential)
        {
            ...
            }))
            {
                ...
                Assert.That(additionallyAllowedTenantIds, Has.Member(additionalTenant));
            }
        }
        ...
        public void ExcludeWorkloadIdentityCredential_Disables_TokenExchangeManagedIdentitySource()
        {
            ...
            }))
            {
                ...
                Assert.That(targetCred, Is.InstanceOf<ManagedIdentityCredential>());
                ManagedIdentityCredential miCred = targetCred as ManagedIdentityCredential;
                var source = miCred.Client._identitySource.Value;
                Assert.That(source, Is.Not.InstanceOf<TokenExchangeManagedIdentitySource>());
            }
        }
        ...
        public void ValidateCustomEnvironmentVariableConstructorWithInvalidVariableName()
        {
            ...
            Assert.That(ex.Message, Does.Contain("Invalid environment variable name: 'INVALID-VAR-NAME'. Only letters, digits, and underscores are allowed."));
        }
        ...
        public void ValidateCustomEnvironmentVariableConstructorWithUnsetVariable()
        {
            using (new TestEnvVar("CUSTOM_TOKEN_CREDENTIALS", null))
            {
                ...
                Assert.That(ex.Message, Does.Contain("Environment variable 'CUSTOM_TOKEN_CREDENTIALS' is not set or is empty"));
            }
        }
        ...
        public void ValidateCustomEnvironmentVariableConstructorWithInvalidValue()
        {
            ...
            {
                var ex = Assert.Throws<InvalidOperationException>(() => new DefaultAzureCredential("CUSTOM_TOKEN_CREDENTIALS"));
                Assert.That(ex.Message, Does.Contain("Invalid value for environment variable CUSTOM_TOKEN_CREDENTIALS: invalid_credential_type"));
            }
        }
        ...
        public void ValidateCustomEnvironmentVariableConstructorWithValidDevCredentials()
        {
            using (new TestEnvVar("CUSTOM_TOKEN_CREDENTIALS", "dev"))
            {
                ...
                Assert.Multiple(() =>
                {
                    Assert.That(sources[0], Is.InstanceOf(typeof(VisualStudioCredential)));
                    Assert.That(sources[1], Is.InstanceOf(typeof(VisualStudioCodeCredential)));
                    Assert.That(sources[2], Is.InstanceOf(typeof(AzureCliCredential)));
                    Assert.That(sources[3], Is.InstanceOf(typeof(AzurePowerShellCredential)));
                    Assert.That(sources[4], Is.InstanceOf(typeof(AzureDeveloperCliCredential)));
                    Assert.That(sources[5], Is.InstanceOf(typeof(BrokerCredential)));
                });
            }
        }
        ...
        public void ValidateCustomEnvironmentVariableConstructorWithSpecificCredential()
        {
            using (new TestEnvVar("CUSTOM_TOKEN_CREDENTIALS", "azureclicredential"))
            {
                ...
                Assert.That(sources[0], Is.InstanceOf(typeof(AzureCliCredential)));
            }
        }
