...
namespace Azure.Identity.Tests
{
    public class DefaultAzureCredentialTests : ClientTestBase
    {
        ...
        public async Task ValidateSelectedCredentialCaching(Type availableCredential)
        {
            ...
            var credFactory = GetMockDefaultAzureCredentialFactory(availableCredential, calledCredentials);
            ...
            Assert.That(actToken.Token, Does.Contain(availableCredential.GetType().Name));
        ...
        [Test]
        [TestCaseSource(nameof(AllCredentialTypes))]
        public void CredentialTypeStashedInException(Type availableCredential)
        {
            List<Type> calledCredentials = new();
            var credFactory = GetMockDefaultAzureCredentialFactory(availableCredential, calledCredentials, true);
            ...
            var ex = Assert.ThrowsAsync<AuthenticationFailedException>(async () => await cred.GetTokenAsync(new TokenRequestContext(MockScopes.Default)));
            Assert.Contains("SelectedCredential", ex.Data.Keys);
            Assert.That(ex.Data["SelectedCredential"], Does.Contain(availableCredential.Name));
        }

        internal MockDefaultAzureCredentialFactory GetMockDefaultAzureCredentialFactory(Type availableCredential, List<Type> calledCredentials, bool throwsOnGetToken = false)
        {
            var credFactory = new MockDefaultAzureCredentialFactory(CredentialPipeline.GetInstance(null));

            void SetupMockForException<T>(Mock<T> mock) where T : TokenCredential
            {
                var setup = mock.Setup(m => m.GetTokenAsync(It.IsAny<TokenRequestContext>(), It.IsAny<CancellationToken>()));
                if (availableCredential == typeof(T))
                {
                    if (throwsOnGetToken)
                    {
                        setup.Throws(new RequestFailedException("GetTokeFailed"));
                    }
                    else
                    {
                        setup.Returns(() => new ValueTask<AccessToken>(new AccessToken(availableCredential.GetType().FullName, DateTimeOffset.MaxValue)))
                            .Callback(() => calledCredentials.Add(availableCredential));
                    }
                }
                else
                {
                    setup.Throws(new CredentialUnavailableException("Unavailable"));
                }
            }
