...
### Other Changes
- All developer credentials in the `DefaultAzureCredential` token chain will fall through to the next credential in the chain on any failure. Previously, some exceptions would throw `AuthenticationFailedException`, which stops further progress in the chain.
