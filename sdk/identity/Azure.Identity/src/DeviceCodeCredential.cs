        ...
        public override AccessToken GetToken(TokenOptions options, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                if (_account != null)
                {
                    try
                    {
                        AuthenticationResult result = _pubApp.AcquireTokenSilent(options.Scopes, _account).ExecuteAsync(cancellationToken).GetAwaiter().GetResult();
                    ...
                    catch (MsalUiRequiredException)
                    {
                        ...
                        return GetTokenViaDeviceCodeAsync(options.Scopes, cancellationToken).GetAwaiter().GetResult();
                    }
                }
                else
                {
        ...
        public override async Task<AccessToken> GetTokenAsync(TokenOptions options, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                if (_account != null)
                {
                    try
                    {
                        AuthenticationResult result = await _pubApp.AcquireTokenSilent(options.Scopes, _account).ExecuteAsync(cancellationToken).ConfigureAwait(false);
                    ...
                    {
                        // TODO: logging for exception here?
                        return await GetTokenViaDeviceCodeAsync(options.Scopes, cancellationToken).ConfigureAwait(false);
                    }
                }
                else
                {
