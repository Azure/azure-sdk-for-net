    ...
    public class AuthorizationCodeCredential : TokenCredential
    {
        ...
        public override async Task<AccessToken> GetTokenAsync(TokenOptions options, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (_account is null)
                {
                    AuthenticationResult result = await _confidentialClient.AcquireTokenByAuthorizationCode(options.Scopes, _authCode).ExecuteAsync().ConfigureAwait(false);
                ...
                {
                    AuthenticationResult result = await _confidentialClient.AcquireTokenSilent(options.Scopes, _account).ExecuteAsync().ConfigureAwait(false);
