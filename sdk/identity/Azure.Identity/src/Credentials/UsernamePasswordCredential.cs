        ...
        private async Task<AuthenticationResult> AuthenticateImplAsync(bool async, TokenRequestContext requestContext, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                AuthenticationResult result = await Client
                    .AcquireTokenByUsernamePasswordAsync(requestContext.Scopes, requestContext.Claims, _username, _password, tenantId, requestContext.EnableCae, async, cancellationToken)
        ...
        private async Task<AccessToken> GetTokenImplAsync(bool async, TokenRequestContext requestContext, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (_record != null)
                {
                    ...
                    try
                    {
                        result = await Client.AcquireTokenSilentAsync(requestContext.Scopes, requestContext.Claims, _record, tenantId, requestContext.EnableCae, async, cancellationToken)
                            .ConfigureAwait(false);
