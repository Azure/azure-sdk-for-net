...
namespace Microsoft.Azure.WebJobs.Extensions.AuthenticationEvents
{
    ...
    public class AuthEventResponseHandler : IValueBinder
    {
        ...
        public Task SetValueAsync(object result, CancellationToken cancellationToken)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (response == null)
                        throw new MissingFieldException(AuthEventResource.Ex_Missing_Request_Response);
        ...
        internal AuthEventResponse GetActionResult(object result, AuthEventResponse response)
        {
            ...
            if (Request.RequestStatus != AuthEventConvertStatusType.Successful)
            {
            ...
            if (convertedResponse != null && !string.IsNullOrEmpty(response.Body))
            {
                ...
                if (string.IsNullOrEmpty(convertedResponse.Schema))
                    convertedResponse.Schema = response.Schema;
        ...
        internal static AuthEventJsonElement GetJsonObjectFromString(string result)
        {
            if (!Helpers.IsJson(result))
                throw new InvalidCastException(AuthEventResource.Ex_Invalid_Return);
            return new AuthEventJsonElement(result);
        }
