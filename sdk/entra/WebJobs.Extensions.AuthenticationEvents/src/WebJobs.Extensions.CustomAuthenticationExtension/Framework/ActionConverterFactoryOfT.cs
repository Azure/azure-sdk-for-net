    ...
    internal class ActionConverterFactoryOfT : JsonConverterFactory
    {
        public override bool CanConvert(Type typeToConvert)
        {
            if (typeToConvert.IsGenericType)
                return typeof(AuthEventAction).IsAssignableFrom(typeToConvert.GenericTypeArguments[0]);
        ...
        internal class ActionConverter<T> : JsonConverter<List<T>> where T : AuthEventAction
        {
            ...
            {
                List<T> result = new();
                AuthEventJsonElement actions = new(ref reader);
                ...
                if (result.Count == 0)
                    throw new Exception(AuthEventResource.Ex_No_Action);
