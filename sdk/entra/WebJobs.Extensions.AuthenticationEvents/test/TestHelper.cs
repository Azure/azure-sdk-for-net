...
namespace WebJobs.Extensions.AuthenticationEvents.Tests
{
    ...
    public static partial class TestHelper
    {
        ...
        public static TokenIssuanceStartResponse CreateTokenIssuanceStartResponse()
        {
            ...
            return new TokenIssuanceStartResponse()
            {
                Schema = GetJsonValidationSchema(TestSchemaType.Response, EventDefinition.TokenIssuanceStart),
                Body = jBody.ToString()
            };
        }

        /// <summary>Helper method to load the json validation schema based on schema full name.</summary>
        /// <returns>Requested json validation schema.</returns>
        internal static string GetJsonValidationSchema(TestSchemaType type, EventDefinition version)
        {
            string schemaFullName = GetJsonValidationSchemaPaths(type, version);
            return ReadResource(MainAssembly, $"{DefaultNamespace}.Schema.{schemaFullName}");
        }

        /// <summary>Gets the JSON schema name based on schema type and version.</summary>
        /// <returns>The JSON validation schema name.</returns>
        private static string GetJsonValidationSchemaPaths(TestSchemaType type, EventDefinition version)
        {
            var versioningAttribute = version.GetAttribute<AuthEventMetadataAttribute>();
            return type == TestSchemaType.Request
                ? string.Join('.', versioningAttribute.EventNamespace, versioningAttribute.RequestSchema)
                : string.Join('.', versioningAttribute.EventNamespace, versioningAttribute.ResponseSchema);
        }
