// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.Core;
using Azure.ResourceManager.EventGrid;

namespace Azure.ResourceManager.EventGrid.Models
{
    /// <summary> The properties that represent the Service Bus Topic destination of an event subscription. </summary>
    internal partial class ServiceBusTopicEventSubscriptionDestinationProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ServiceBusTopicEventSubscriptionDestinationProperties"/>. </summary>
        public ServiceBusTopicEventSubscriptionDestinationProperties()
        {
            DeliveryAttributeMappings = new ChangeTrackingList<DeliveryAttributeMapping>();
        }

        /// <summary> Initializes a new instance of <see cref="ServiceBusTopicEventSubscriptionDestinationProperties"/>. </summary>
        /// <param name="resourceId"> The Azure Resource Id that represents the endpoint of the Service Bus Topic destination of an event subscription. </param>
        /// <param name="deliveryAttributeMappings"> Delivery attribute details. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ServiceBusTopicEventSubscriptionDestinationProperties(ResourceIdentifier resourceId, IList<DeliveryAttributeMapping> deliveryAttributeMappings, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            ResourceId = resourceId;
            DeliveryAttributeMappings = deliveryAttributeMappings;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The Azure Resource Id that represents the endpoint of the Service Bus Topic destination of an event subscription. </summary>
        public ResourceIdentifier ResourceId { get; set; }

        /// <summary> Delivery attribute details. </summary>
        public IList<DeliveryAttributeMapping> DeliveryAttributeMappings { get; } = new ChangeTrackingList<DeliveryAttributeMapping>();
    }
}
