...
namespace Azure.Messaging.EventGrid.Models
{
    ...
    public partial class CloudEvent
    {
        ...
        /// <summary>
        /// Adds or overwrites an extension attribute to the event envelope. Attribute must not have the same name as standard CloudEvent
        /// properties (like 'id' and 'source'), and must follow the same naming convention (lower-case letters and/or numbers).
        /// </summary>
        /// <param name="key">Attribute name.</param>
        /// <param name="value">Attribute value.</param>
        public void AddExtensionAttribute(string key, object value)
        {
            if (ExtensionAttributes.ContainsKey(key))
            {
                ExtensionAttributes.Remove(key);
            }
            ExtensionAttributes.Add(key, value);
        }
        /// <summary>
        /// Removes an extension attribute from the event.
        /// </summary>
        /// <param name="key">Name of the attribute to remove.</param>
        /// <returns>True if the element is successfully found and removed; otherwise, false.</returns>
        public bool RemoveExtensionAttribute(string key)
        {
            return ExtensionAttributes.Remove(key);
        }
    }
}
