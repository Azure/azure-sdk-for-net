...
namespace EventGridSourceGenerator
{
    ...
    internal class EventGridSourceGenerator : ISourceGenerator
    {
        ...
        {
            // using the context, get a list of syntax trees in the compilation
            var syntaxTrees = context.Compilation.WithAssemblyName("Azure.Messaging.EventGrid").SyntaxTrees;
            _visitor = new SourceVisitor();

            foreach (CSharpSyntaxTree tree in syntaxTrees)
            {
                _visitor.Visit(tree.GetCompilationUnitRoot());
            }

            context.AddSource("SystemEventNames", SourceText.From(ConstructSystemEventNames(), Encoding.UTF8));
            context.AddSource("SystemEventExtensions", SourceText.From(ConstructSystemEventExtensions(), Encoding.UTF8));
        }
