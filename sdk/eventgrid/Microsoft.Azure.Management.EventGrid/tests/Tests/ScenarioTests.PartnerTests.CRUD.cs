...
namespace EventGrid.Tests.ScenarioTests
{
    public partial class ScenarioTests
    {
            ...
            using (MockContext context = MockContext.Start(this.GetType()))
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    PartnerTopic activatePartnerTopicResponse = this.EventGridManagementClient.PartnerTopics.ActivateAsync(resourceGroup, partnerTopicName).Result;
                    Assert.NotNull(activatePartnerTopicResponse);
                    ...
                    if (eventSubscriptionsPage != null)
                    {
                        ...
                        string nextLink = eventSubscriptionsPage.NextPageLink;
                        while (nextLink != null)
                        {
        ...
        [Fact]
        public void PartnerChannelCreateGetUpdateDelete()
        {
            using (MockContext context = MockContext.Start(this.GetType()))
            {
                this.InitializeClients(context);
                ...
                bool partnerRegistrationCreated = false;
                bool partnerNamespaceCreated = false;
                bool partnerConfigurationCreated = false;
                bool channelCreated = false;
                bool partnerTopicCreated = false;
                bool eventSubscriptionCreated = false;
                var partnerNamespaceName = TestUtilities.GenerateName(EventGridManagementHelper.PartnerNamespacePrefix);
                var channelName = TestUtilities.GenerateName(EventGridManagementHelper.ChannelPrefix);
                var partnerTopicName = TestUtilities.GenerateName(EventGridManagementHelper.PartnerTopicPrefix);
                var eventSubscriptionName = TestUtilities.GenerateName(EventGridManagementHelper.EventSubscriptionPrefix);
                ...
                try
                {
                    ...
                    Partner authorizePartnerInstance = new Partner
                    {
                        PartnerRegistrationImmutableId = getPartnerRegistrationResponse.PartnerRegistrationImmutableId.Value,
                        AuthorizationExpirationTimeInUtc = DateTime.UtcNow.AddDays(3)
                    };

                    PartnerConfiguration partnerConfigurationInstance = new PartnerConfiguration()
                    {
                        Location = "global",
                        ...
                        {
                            DefaultMaximumExpirationTimeInDays = 20,
                            AuthorizedPartnersList = new List<Partner>
                            {
                                {
                                    authorizePartnerInstance
                                }
                            }
                        }
                    };

                    PartnerConfiguration createdPartnerConfigurationResponse = this.EventGridManagementClient.PartnerConfigurations.CreateOrUpdateAsync(resourceGroup, partnerConfigurationInstance).Result;
                    partnerConfigurationCreated = true;

                    Assert.NotNull(createdPartnerConfigurationResponse);
                    TestUtilities.Wait(TimeSpan.FromSeconds(5));

                    // Get the created partnerConfiguration
                    var getPartnerConfigurationResponse = this.EventGridManagementClient.PartnerConfigurations.Get(resourceGroup);
                    if (string.Compare(getPartnerConfigurationResponse.ProvisioningState, "Succeeded", true) != 0)
                    {
                        TestUtilities.Wait(TimeSpan.FromSeconds(5));
                    }

                    getPartnerConfigurationResponse = this.EventGridManagementClient.PartnerConfigurations.Get(resourceGroup);
                    Assert.NotNull(getPartnerConfigurationResponse);
                    Assert.Equal("Succeeded", getPartnerConfigurationResponse.ProvisioningState, StringComparer.CurrentCultureIgnoreCase);

                    // Authorize the partner
                    PartnerConfiguration authorizedPartnerConfigurationResponse = this.EventGridManagementClient.PartnerConfigurations.AuthorizePartnerAsync(resourceGroup, authorizePartnerInstance).Result;
                    Assert.NotNull(authorizedPartnerConfigurationResponse);
                    Assert.Equal("Succeeded", authorizedPartnerConfigurationResponse.ProvisioningState, StringComparer.CurrentCultureIgnoreCase);
                    Assert.Single(authorizedPartnerConfigurationResponse.PartnerAuthorization.AuthorizedPartnersList);
                    ...
                    eventTypeInfo.Kind = EventDefinitionKind.Inline;
                    eventTypeInfo.InlineEventTypes = new Dictionary<string, InlineEventProperties>();
                    int numberOfEventTypes = 5;
                    for (int i = 0; i < numberOfEventTypes; i++)
                    {
                        eventTypeInfo.InlineEventTypes.Add(eventType + "_" + i, new InlineEventProperties
                        {
                            DataSchemaUrl = Guid.NewGuid().ToString(),
                            Description = Guid.NewGuid().ToString(),
                            DocumentationUrl = Guid.NewGuid().ToString()
                        });
                    }
                    ...
                    {
                        ChannelType = ChannelType.PartnerTopic,
                    ...
                    Assert.NotNull(createChannelResponse);
                    Assert.Equal(createChannelResponse.Name, channelName);

                    TestUtilities.Wait(TimeSpan.FromSeconds(5));

                    // Get the created channel
                    var getChannelResponse = this.EventGridManagementClient.Channels.Get(resourceGroup, partnerNamespaceName, channelName);
                    if (string.Compare(getChannelResponse.ProvisioningState, "Succeeded", true) != 0)
                    {
                        TestUtilities.Wait(TimeSpan.FromSeconds(5));
                    }

                    getChannelResponse = this.EventGridManagementClient.Channels.Get(resourceGroup, partnerNamespaceName, channelName);
                    Assert.NotNull(getChannelResponse);
                    Assert.Equal("Succeeded", getChannelResponse.ProvisioningState, StringComparer.CurrentCultureIgnoreCase);
                    Assert.Equal(numberOfEventTypes, getChannelResponse.PartnerTopicInfo.EventTypeInfo.InlineEventTypes.Count());

                    // Get all channels created within a partnerNamespace
                    IPage<Channel> channelsInPartnerNamespacePage = this.EventGridManagementClient.Channels.ListByPartnerNamespaceAsync(resourceGroup, partnerNamespaceName).Result;
                    var channelsInPartnerNamespaceList = new List<Channel>();
                    if (channelsInPartnerNamespacePage.Any())
                    {
                        channelsInPartnerNamespaceList.AddRange(channelsInPartnerNamespacePage);
                        var nextLink = channelsInPartnerNamespacePage.NextPageLink;
                        while (nextLink != null)
                        {
                            channelsInPartnerNamespacePage = this.EventGridManagementClient.Channels.ListByPartnerNamespaceNextAsync(nextLink).Result;
                            channelsInPartnerNamespaceList.AddRange(channelsInPartnerNamespacePage);
                            nextLink = channelsInPartnerNamespacePage.NextPageLink;
                        }
                    }

                    Assert.NotNull(channelsInPartnerNamespaceList);
                    Assert.True(channelsInPartnerNamespaceList.Count() >= 1);
                    Assert.Contains(channelsInPartnerNamespaceList, t => t.Name == channelName);
                    Assert.True(channelsInPartnerNamespaceList.All(ns => ns.Id.Contains(resourceGroup)));
                    ...
                    // Test GET APIs for verifiedPartners
                    var verifiedPartnersPage = this.EventGridManagementClient.VerifiedPartners.ListAsync().Result;
                    Assert.NotNull(verifiedPartnersPage);

                    List<VerifiedPartner> verifiedPartnersList = new List<VerifiedPartner>();
                    verifiedPartnersList.AddRange(verifiedPartnersPage);
                    Assert.Single(verifiedPartnersList);

                    var verifiedPartner = this.EventGridManagementClient.VerifiedPartners.GetAsync(verifiedPartnersList[0].Name).Result;
                    Assert.NotNull(verifiedPartner);
                }
                finally
                {
                    if (eventSubscriptionCreated)
                    ...
                    if (partnerConfigurationCreated)
                    {
                        // Delete partnerConfiguration
                        this.EventGridManagementClient.PartnerConfigurations.DeleteAsync(resourceGroup).Wait();
                    }
