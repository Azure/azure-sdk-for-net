...
{
    internal partial class ScriptCmdletsGetAllAsyncCollectionResultOfT : AsyncPageable<ScriptCmdletData>
    {
        private readonly ScriptCmdlets _client;
        ...
        /// <returns> The pages of ScriptCmdletsGetAllAsyncCollectionResultOfT as an enumerable collection. </returns>
        public override async IAsyncEnumerable<Page<ScriptCmdletData>> AsPages(string continuationToken, int? pageSizeHint)
        {
            ...
            while (true)
            {
                ...
                ScriptCmdletsList result = ScriptCmdletsList.FromResponse(response);
                yield return Page<ScriptCmdletData>.FromValues((IReadOnlyList<ScriptCmdletData>)result.Value, nextPage?.IsAbsoluteUri == true ? nextPage.AbsoluteUri : nextPage?.OriginalString, response);
