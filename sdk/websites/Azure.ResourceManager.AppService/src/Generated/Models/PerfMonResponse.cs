// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Microsoft.Web.Models
{
    /// <summary> Performance monitor API response. </summary>
    public partial class PerfMonResponse
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="PerfMonResponse"/>. </summary>
        internal PerfMonResponse()
        {
        }

        /// <summary> Initializes a new instance of <see cref="PerfMonResponse"/>. </summary>
        /// <param name="code"> The response code. </param>
        /// <param name="message"> The message. </param>
        /// <param name="data"> The performance monitor counters. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal PerfMonResponse(string code, string message, PerfMonSet data, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Code = code;
            Message = message;
            Data = data;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The response code. </summary>
        public string Code { get; }

        /// <summary> The message. </summary>
        public string Message { get; }

        /// <summary> The performance monitor counters. </summary>
        public PerfMonSet Data { get; }
    }
}
