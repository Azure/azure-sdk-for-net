...
{
    public partial class SitePatchInfo : Core.IUtf8JsonSerializable
    {
        ...
        internal static SitePatchInfo DeserializeSitePatchInfo(JsonElement element)
        {
            ...
            return new SitePatchInfo(id, name, type, systemData.Value, identity, state.Value, Core.Optional.ToList(hostNames), repositorySiteName.Value, Core.Optional.ToNullable(usageState), Core.Optional.ToNullable(enabled), Core.Optional.ToList(enabledHostNames), Core.Optional.ToNullable(availabilityState), Core.Optional.ToList(hostNameSslStates), serverFarmId.Value, Core.Optional.ToNullable(reserved), Core.Optional.ToNullable(isXenon), Core.Optional.ToNullable(hyperV), Core.Optional.ToNullable(lastModifiedTimeUtc), siteConfig.Value, Core.Optional.ToList(trafficManagerHostNames), Core.Optional.ToNullable(scmSiteAlsoStopped), targetSwapSlot.Value, hostingEnvironmentProfile.Value, Core.Optional.ToNullable(clientAffinityEnabled), Core.Optional.ToNullable(clientCertEnabled), Core.Optional.ToNullable(clientCertMode), clientCertExclusionPaths.Value, Core.Optional.ToNullable(hostNamesDisabled), customDomainVerificationId.Value, outboundIPAddresses.Value, possibleOutboundIPAddresses.Value, Core.Optional.ToNullable(containerSize), Core.Optional.ToNullable(dailyMemoryTimeQuota), Core.Optional.ToNullable(suspendedTill), Core.Optional.ToNullable(maxNumberOfWorkers), cloningInfo.Value, resourceGroup.Value, Core.Optional.ToNullable(isDefaultContainer), defaultHostName.Value, slotSwapStatus.Value, Core.Optional.ToNullable(httpsOnly), Core.Optional.ToNullable(redundancyMode), Core.Optional.ToNullable(inProgressOperationId), Core.Optional.ToNullable(storageAccountRequired), keyVaultReferenceIdentity.Value, virtualNetworkSubnetId.Value, kind.Value);
        }
    }
}
