...
namespace Azure.ResourceManager.AppService
{
    public partial class StaticSiteARMResourceData : IUtf8JsonSerializable
    {
        ...
        internal static StaticSiteARMResourceData DeserializeStaticSiteARMResourceData(JsonElement element)
        {
            ...
            return new StaticSiteARMResourceData(id, name, type, systemData, tags, location, kind.Value, sku.Value, identity, defaultHostname.Value, repositoryUrl.Value, branch.Value, Optional.ToList(customDomains), repositoryToken.Value, buildProperties.Value, Optional.ToList(privateEndpointConnections), Optional.ToNullable(stagingEnvironmentPolicy), Optional.ToNullable(allowConfigFileUpdates), templateProperties.Value, contentDistributionEndpoint.Value, keyVaultReferenceIdentity.Value, Optional.ToList(userProvidedFunctionApps), Optional.ToList(linkedBackends), provider.Value, Optional.ToNullable(enterpriseGradeCdnStatus), publicNetworkAccess.Value);
        }
    }
}
