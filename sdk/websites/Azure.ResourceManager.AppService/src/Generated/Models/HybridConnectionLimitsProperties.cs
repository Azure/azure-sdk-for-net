// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.AppService;

namespace Azure.ResourceManager.AppService.Models
{
    /// <summary> HybridConnectionLimits resource specific properties. </summary>
    internal partial class HybridConnectionLimitsProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="HybridConnectionLimitsProperties"/>. </summary>
        internal HybridConnectionLimitsProperties()
        {
        }

        /// <summary> Initializes a new instance of <see cref="HybridConnectionLimitsProperties"/>. </summary>
        /// <param name="current"> The current number of Hybrid Connections. </param>
        /// <param name="maximum"> The maximum number of Hybrid Connections allowed. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal HybridConnectionLimitsProperties(int? current, int? maximum, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Current = current;
            Maximum = maximum;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The current number of Hybrid Connections. </summary>
        [WirePath("current")]
        public int? Current { get; }

        /// <summary> The maximum number of Hybrid Connections allowed. </summary>
        [WirePath("maximum")]
        public int? Maximum { get; }
    }
}
