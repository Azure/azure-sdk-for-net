...
namespace Azure.ResourceManager.AppService
{
    ...
    public partial class HybridConnectionLimits : ArmResource
    {
        ...
        /// <summary> Initializes a new instance of the <see cref="HybridConnectionLimits"/> class for mocking. </summary>
        ...
        internal HybridConnectionLimits(ArmResource options, HybridConnectionLimitsData data) : base(options, data.Id)
        {
            HasData = true;
            _data = data;
            Parent = options;
        ...
        /// <param name="id"> The identifier of the resource that is the target of operations. </param>
        internal HybridConnectionLimits(ArmClientOptions clientOptions, TokenCredential credential, Uri uri, HttpPipeline pipeline, ResourceIdentifier id) : base(clientOptions, credential, uri, pipeline, id)
        {
            _clientDiagnostics = new ClientDiagnostics(ClientOptions);
            ClientOptions.TryGetApiVersion(ResourceType, out string apiVersion);
            _appServicePlansRestClient = new AppServicePlansRestOperations(_clientDiagnostics, Pipeline, ClientOptions, BaseUri, apiVersion);
        ...
        {
            using var scope = _clientDiagnostics.CreateScope("HybridConnectionLimits.Get");
            scope.Start();
            try
            {
                var response = await _appServicePlansRestClient.GetHybridConnectionPlanLimitAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, cancellationToken).ConfigureAwait(false);
                if (response.Value == null)
                    throw await _clientDiagnostics.CreateRequestFailedExceptionAsync(response.GetRawResponse()).ConfigureAwait(false);
                return Response.FromValue(new HybridConnectionLimits(this, response.Value), response.GetRawResponse());
            }
            catch (Exception e)
            {
                scope.Failed(e);
        ...
        public virtual Response<HybridConnectionLimits> Get(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var response = _appServicePlansRestClient.GetHybridConnectionPlanLimit(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Name, cancellationToken);
                if (response.Value == null)
        ...
        {
            using var scope = _clientDiagnostics.CreateScope("HybridConnectionLimits.GetAvailableLocations");
            scope.Start();
            try
            {
