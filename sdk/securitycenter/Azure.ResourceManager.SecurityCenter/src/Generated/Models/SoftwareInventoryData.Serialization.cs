    ...
    public partial class SoftwareInventoryData : IUtf8JsonSerializable, IJsonModel<SoftwareInventoryData>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(OSPlatform))
            {
                ...
                if (OSPlatform.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{OSPlatform}'''");
                }
                else
                {
                    builder.AppendLine($" '{OSPlatform}'");
            ...
            if (Optional.IsDefined(SoftwareName))
            {
                ...
                if (SoftwareName.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{SoftwareName}'''");
                }
                else
                {
                    builder.AppendLine($" '{SoftwareName}'");
            ...
            if (Optional.IsDefined(EndOfSupportStatus))
            {
                builder.Append("    endOfSupportStatus:");
                builder.AppendLine($" '{EndOfSupportStatus.Value.ToString()}'");
            }

            if (Optional.IsDefined(EndOfSupportDate))
            {
                builder.Append("    endOfSupportDate:");
                if (EndOfSupportDate.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{EndOfSupportDate}'''");
                }
                else
                {
                    builder.AppendLine($" '{EndOfSupportDate}'");
            ...
            if (Optional.IsDefined(NumberOfKnownVulnerabilities))
            {
                builder.Append("    numberOfKnownVulnerabilities:");
                builder.AppendLine($" {NumberOfKnownVulnerabilities.Value}");
            }

            if (Optional.IsDefined(FirstSeenOn))
            {
                builder.Append("    firstSeenAt:");
                var formattedDateTimeString = TypeFormatters.ToString(FirstSeenOn.Value, "o");
