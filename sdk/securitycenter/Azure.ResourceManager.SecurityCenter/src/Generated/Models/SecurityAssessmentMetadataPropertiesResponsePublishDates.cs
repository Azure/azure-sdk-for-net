// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.SecurityCenter;

namespace Azure.ResourceManager.SecurityCenter.Models
{
    /// <summary> The SecurityAssessmentMetadataPropertiesResponsePublishDates. </summary>
    public partial class SecurityAssessmentMetadataPropertiesResponsePublishDates
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="SecurityAssessmentMetadataPropertiesResponsePublishDates"/>. </summary>
        /// <param name="public"></param>
        /// <exception cref="ArgumentNullException"> <paramref name="public"/> is null. </exception>
        public SecurityAssessmentMetadataPropertiesResponsePublishDates(string @public)
        {
            Argument.AssertNotNull(@public, nameof(@public));

            Public = @public;
        }

        /// <summary> Initializes a new instance of <see cref="SecurityAssessmentMetadataPropertiesResponsePublishDates"/>. </summary>
        /// <param name="ga"></param>
        /// <param name="public"></param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal SecurityAssessmentMetadataPropertiesResponsePublishDates(string ga, string @public, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            GA = ga;
            Public = @public;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Gets or sets the GA. </summary>
        public string GA { get; set; }

        /// <summary> Gets or sets the Public. </summary>
        public string Public { get; set; }
    }
}
