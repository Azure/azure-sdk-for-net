...
{
    internal sealed class SecurityCenterCompatibilityAsyncPageable<T> : AsyncPageable<T>
    {
        ...
        public SecurityCenterCompatibilityAsyncPageable(
        ...
        public override async IAsyncEnumerable<Page<T>> AsPages(string continuationToken, int? pageSizeHint)
        {
        ...
        private async ValueTask<Response> GetNextResponseAsync(Uri nextLink)
        {
            ...
            try
            {
                return await _pipeline.ProcessMessageAsync(message, _context).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                scope.Failed(e);
