...
namespace ServiceBus.Tests.ScenarioTests
{
    ...
    {
        [Fact]
        public void NamespaceAZCreateGetUpdateDelete()
        {
            using (MockContext context = MockContext.Start(this.GetType()))
            {
                InitializeClients(context);

                var location = "East US 2";// this.ResourceManagementClient.GetLocationFromProvider();
                ...
                Assert.True(createNamespaceResponse.ZoneRedundant);
