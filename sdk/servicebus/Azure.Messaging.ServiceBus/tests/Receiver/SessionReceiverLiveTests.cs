    ...
    public class SessionReceiverLiveTests : ServiceBusLiveTestBase
    {
        ...
        public async Task LockSameSessionShouldThrow()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                ...
                ServiceBusSessionReceiver receiver1 = client.CreateSessionReceiver(scope.QueueName);
                await receiver1.AcceptSessionAsync(sessionId);

                var receiver2 = GetNoRetryClient().CreateSessionReceiver(scope.QueueName);
                Assert.That(
                    async () => await receiver2.AcceptSessionAsync(sessionId),
        ...
        public async Task AcceptSession()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                await using var client = new ServiceBusClient(TestEnvironment.ServiceBusConnectionString);
                ServiceBusSender sender = client.CreateSender(scope.QueueName);
                List<string> sessions = new List<string>() { "1"};
                // send the messages
                foreach (string session in sessions)
                {
                    using ServiceBusMessageBatch batch = await sender.CreateMessageBatchAsync();
                    ServiceBusMessageBatch messageBatch = AddMessages(batch, 5, session);
                    await sender.SendMessagesAsync(messageBatch);
                }
                var receiver = client.CreateSessionReceiver(scope.QueueName);
                await receiver.AcceptSessionAsync();
                var msgs = await receiver.PeekMessagesAsync(maxMessages:15);
                var msg = await receiver.ReceiveMessageAsync();
                Assert.IsNotNull(msg);

                Assert.That(
                    async () => await receiver.AcceptSessionAsync("1"),
                    Throws.InstanceOf<InvalidOperationException>());
                await receiver.CloseAsync();
                var receiver2 = client.CreateSessionReceiver(scope.QueueName);
                await receiver2.AcceptSessionAsync("1");
        ...
        public async Task ReceiveMessagesInReceiveAndDeleteMode()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                ...
                ServiceBusSessionReceiver receiver = client.CreateSessionReceiver(
                    scope.QueueName,
                    clientOptions);
        ...
        public async Task CompleteMessages(bool useSpecificSession)
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                ...
                ServiceBusSessionReceiver receiver = client.CreateSessionReceiver(scope.QueueName);
                await receiver.AcceptSessionAsync(useSpecificSession ? sessionId : null);
        ...
        public async Task DeadLetterMessagesSubscription(bool useSpecificSession)
        {
            await using (var scope = await ServiceBusScope.CreateWithTopic(enablePartitioning: false, enableSession: true))
            {
                ...
                var receiver = client.CreateSessionReceiver(
                    topicName: topicName,
        ...
        public async Task RenewSessionLock(bool isSessionSpecified)
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                ...
                ServiceBusSessionReceiver receiver = client.CreateSessionReceiver(scope.QueueName);
                await receiver.AcceptSessionAsync(isSessionSpecified ? sessionId1: null);
        ...
        public async Task ClientThrowsSessionCannotBeLockedWhenSessionLocked()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: true))
            {
                ...
                var receiver2 = client.CreateSessionReceiver(scope.QueueName);
                await receiver2.AcceptSessionAsync("sessionId2");


                // for some reason, using Throws.InstanceOf and Assert.ThrowsAsync always end up coming back
                // as a timeout exception.
                var receiver3 = client.CreateSessionReceiver(scope.QueueName);
                try
                {
                    await receiver3.AcceptSessionAsync(sessionId);
                }
