    ...
    public class ReceiverLiveTests : ServiceBusLiveTestBase
    {
        ...
        public async Task PeekWithZeroTimeout()
        {
                ...
                await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(durationWithPrefetchModeInSecs < 1, Is.True);
            }
        }
        ...
        public async Task DeferMessagesList()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(deferredMessages.Count, Is.EqualTo(messageList.Count));
        ...
        public async Task ReceiveMessagesReactsToClosingTheClient()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(firstReceivedBatch.Count, Is.EqualTo(halfMessageCount), "The first batch of messages should have the correct count.");
        ...
        public async Task ReceiveDeferredMessagesReactsToClosingTheClient()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(deferredMessages.Count, Is.EqualTo(halfMessageCount), "The batch of deferred messages should have the correct count.");
        ...
        public async Task CompleteMessageReactsToClosingTheClient()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(receivedMessages.Count, Is.EqualTo(receiveCount), "An incorrect number of messages were received.");
        ...
        public async Task NullableAmqpPropertiesRoundTripCorrectly()
        {
            await using (var scope = await ServiceBusScope.CreateWithQueue(enablePartitioning: false, enableSession: false))
            {
                ...
                Assert.That(message.ReplyTo, Is.Empty);
                Assert.That(message.To, Is.Empty);
                Assert.That(message.CorrelationId, Is.Empty);

                await sender.SendMessageAsync(message);

                receivedMessage = await receiver.ReceiveMessageAsync();
                Assert.That(receivedMessage.ReplyTo, Is.Empty);
                Assert.That(receivedMessage.To, Is.Empty);
                Assert.That(receivedMessage.CorrelationId, Is.Empty);
            }
        }
