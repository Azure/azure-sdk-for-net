        ...
        private async Task DeadLetterFireEvents()
        {
            await ServiceBusScope.UsingQueueAsync(partitioned: false, sessionEnabled: false, async queueName =>
            {
                ...
                using (var subscription = this.SubscribeToEvents(listener))
                {
                    ...
                    {
                        await using var deadLetterQueueClient = new QueueClient(TestUtility.NamespaceConnectionString, EntityNameHelper.FormatDeadLetterPath(queueClient.QueueName));
