    ...
    public sealed class QueueSessionTests
    {
        ...
        public async Task ReceiveDeferredMessageForSessionTest(bool partitioned, bool sessionEnabled)
        {
            await ServiceBusScope.UsingQueueAsync(partitioned, sessionEnabled, async queueName =>
            {
                ...
                await using var sender = new MessageSender(TestUtility.NamespaceConnectionString, queueName);
                await using var sessionClient = new SessionClient(TestUtility.NamespaceConnectionString, queueName);
                await using var messageSession = await sessionClient.AcceptMessageSessionAsync(sessionId);
        ...
        public async Task AcceptSessionThrowsSessionCannotBeLockedException()
        {
            await ServiceBusScope.UsingQueueAsync(partitioned: true, sessionEnabled: true, async queueName =>
            {
                ...
                await using MessageSession session = await sessionClient.AcceptMessageSessionAsync(someSessionId);
                await Assert.ThrowsAsync<SessionCannotBeLockedException>(async () => await sessionClient.AcceptMessageSessionAsync(someSessionId));
            });
        }

        private async Task AcceptAndCompleteSessionsAsync(SessionClient sessionClient, string sessionId, string messageId)
        {
            await using var sessionReceiver = await sessionClient.AcceptMessageSessionAsync(sessionId);
            if (sessionId != null)
            {
