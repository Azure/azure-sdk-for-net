...
namespace Azure.Messaging.ServiceBus.Tests
{
    ...
    public sealed class LiveResourceManager
    {
        ...
        {
            using (var client = new ResourceManagementClient(new Uri(ServiceBusTestEnvironment.Instance.ResourceManagerUrl), new TokenCredentials(accessToken)) { SubscriptionId = subscriptionId })
            {
        ...
        public async Task<string> AcquireManagementTokenAsync()
        {
            ...
            if ((token == null) || (token.ExpiresOn <= DateTimeOffset.UtcNow.Add(CredentialRefreshBuffer)))
            {
                ...
                var result = await context.AcquireTokenAsync(ServiceBusTestEnvironment.Instance.ServiceManagementUrl, credential).ConfigureAwait(false);
