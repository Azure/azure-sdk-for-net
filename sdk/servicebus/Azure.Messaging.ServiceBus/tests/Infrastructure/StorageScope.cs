...
namespace Azure.Messaging.ServiceBus.Tests
{
    ...
    public sealed class StorageScope : IAsyncDisposable
    {
        ...
        private static readonly Uri AzureResourceManagerUri = new Uri(ServiceBusTestEnvironment.Instance.ResourceManagerUrl);
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            var client = new StorageManagementClient(AzureResourceManagerUri, new TokenCredentials(token)) { SubscriptionId = ServiceBusTestEnvironment.Instance.SubscriptionId };
        ...
        public static async Task<StorageScope> CreateAsync([CallerMemberName] string caller = "")
        {
            ...
            using (var client = new StorageManagementClient(AzureResourceManagerUri, new TokenCredentials(token)) { SubscriptionId = ServiceBusTestEnvironment.Instance.SubscriptionId })
            {
        ...
        {
            var subscription = ServiceBusTestEnvironment.Instance.SubscriptionId;
            var resourceGroup = ServiceBusTestEnvironment.Instance.ResourceGroup;
            var token = await ResourceManager.AcquireManagementTokenAsync().ConfigureAwait(false);
