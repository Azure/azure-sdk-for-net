...
<#
  .SYNOPSIS
    Performs the tasks needed to setup an Azure subscription for use with the Service Bus client
    library test suite.

  .DESCRIPTION
    This script handles creation and configuration of needed resources within an Azure subscription
    for use with the Service Bus client library's Live test suite.
...
function DisplayHelp
{
  ...
  Write-Host "`n"
  Write-Host "Service Bus Live Test Environment Setup"
  Write-Host ""
  Write-Host "$($indent)This script handles creation and configuration of needed resources within an Azure subscription"
  Write-Host "$($indent)for use with the Service Bus client library Live test suite."
  Write-Host ""
...
try
{
    ...
    # The "Azure Service Bus Data Owner" role is needed to test the Azure Identity samples.
    ...
    Write-Host ""
    Write-Host ""
    Write-Host "SERVICE_BUS_RESOURCEGROUP=$($resourceGroupName)"
    Write-Host ""
    Write-Host "SERVICE_BUS_SUBSCRIPTION=$($subscription.SubscriptionId)"
    Write-Host ""
    Write-Host "SERVICE_BUS_TENANT=$($subscription.TenantId)"
    Write-Host ""
    Write-Host "SERVICE_BUS_CLIENT=$($principal.ApplicationId)"
    Write-Host ""
    Write-Host "SERVICE_BUS_SECRET=$($credentials.Password)"
    Write-Host ""
}
catch
{
