...
function ValidateAzureRegion()
{
  <#
    .SYNOPSIS
      Checks if a region provides Azure Service Bus.

    .DESCRIPTION
      Lists all the regions that provide Azure Service Bus
      and looks for the one passed in as a parameter. It returns
      true if found or false otherwise. It outputs an error message listing
      all the available regions if the one chosen could not be found.
  #>
  ...
  # Verify the location is valid for a Service Bus namespace.

  $validLocations = @{ }

  Get-AzLocation | where { $_.Providers.Contains("Microsoft.ServiceBus") } | ForEach { $validLocations[$_.Location] = $_.Location }
