...
- Fixed unnecessary credit drain on empty receives for session processors accepting any available session. A null entry in `SessionIds` now also releases its accepted session on idle. Direct session receivers and processors configured for specific sessions retain partial-receive drain. ([#63077](https://github.com/Azure/azure-sdk-for-net/pull/63077))
