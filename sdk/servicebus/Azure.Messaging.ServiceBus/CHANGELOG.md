...
- Fixed a bug where session processors accepting any available session unnecessarily drained link credits on empty receives after the service assigned a session ID. Direct session receivers and processors configured for specific sessions continue to drain partial receives to preserve message ordering.
