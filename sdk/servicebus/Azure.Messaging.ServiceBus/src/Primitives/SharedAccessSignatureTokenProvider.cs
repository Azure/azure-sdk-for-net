...
namespace Azure.Messaging.ServiceBus.Primitives
{
    ...
    public class SharedAccessSignatureTokenProvider : TokenProvider
    {
        ...
        protected SharedAccessSignatureTokenProvider(string keyName, string sharedAccessKey, Func<string, byte[]> customKeyEncoder, TimeSpan tokenTimeToLive, TokenScope tokenScope)
        {
            ...
            this._encodedSharedAccessKey = customKeyEncoder != null ?
        ...
        {
            return string.IsNullOrWhiteSpace(this._sharedAccessSignature)
                ? SharedAccessSignatureBuilder.BuildSignature(
                    this._keyName,
                    this._encodedSharedAccessKey,
                    targetUri,
                    this._tokenTimeToLive)
                : this._sharedAccessSignature;
        }

        private string NormalizeAppliesTo(string appliesTo)
        {
            return ServiceBusUriHelper.NormalizeUri(appliesTo, "https", true, stripPath: this._tokenScope == TokenScope.Namespace, ensureTrailingSlash: true);
        }
