// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.MachineLearning;

namespace Azure.ResourceManager.MachineLearning.Models
{
    /// <summary> The List Aml user feature operation response. </summary>
    internal partial class ListAmlUserFeatureResult
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ListAmlUserFeatureResult"/>. </summary>
        internal ListAmlUserFeatureResult()
        {
            Value = new ChangeTrackingList<MachineLearningUserFeature>();
        }

        /// <summary> Initializes a new instance of <see cref="ListAmlUserFeatureResult"/>. </summary>
        /// <param name="value"> The AmlUserFeature items on this page. </param>
        /// <param name="nextLink"> The link to the next page of items. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ListAmlUserFeatureResult(IReadOnlyList<MachineLearningUserFeature> value, Uri nextLink, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Value = value;
            NextLink = nextLink;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The AmlUserFeature items on this page. </summary>
        [WirePath("value")]
        public IReadOnlyList<MachineLearningUserFeature> Value { get; }

        /// <summary> The link to the next page of items. </summary>
        [WirePath("nextLink")]
        public Uri NextLink { get; }
    }
}
