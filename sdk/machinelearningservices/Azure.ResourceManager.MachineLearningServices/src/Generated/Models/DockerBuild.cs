// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;

namespace Azure.ResourceManager.MachineLearningServices
{
    /// <summary> Class to represent configuration settings for Docker Build. </summary>
    public partial class DockerBuild : DockerSpecification
    {
        /// <summary> Initializes a new instance of DockerBuild. </summary>
        /// <param name="dockerfile">
        /// Docker command line instructions to assemble an image.
        /// 
        /// &lt;seealso href=&quot;https://repo2docker.readthedocs.io/en/latest/config_files.html#dockerfile-advanced-environments&quot; /&gt;.
        /// </param>
        /// <exception cref="ArgumentNullException"> <paramref name="dockerfile"/> is null. </exception>
        public DockerBuild(string dockerfile)
        {
            if (dockerfile == null)
            {
                throw new ArgumentNullException(nameof(dockerfile));
            }

            Dockerfile = dockerfile;
            DockerSpecificationType = DockerSpecificationType.Build;
        }

        /// <summary> Initializes a new instance of DockerBuild. </summary>
        /// <param name="dockerSpecificationType"> Docker specification must be either Build or Image. </param>
        /// <param name="platform"> The platform information of the docker image. </param>
        /// <param name="dockerfile">
        /// Docker command line instructions to assemble an image.
        /// 
        /// &lt;seealso href=&quot;https://repo2docker.readthedocs.io/en/latest/config_files.html#dockerfile-advanced-environments&quot; /&gt;.
        /// </param>
        /// <param name="context">
        /// Path to a snapshot of the Docker Context. This property is only valid if Dockerfile is specified.
        /// 
        /// The path is relative to the asset path which must contain a single Blob URI value.
        /// 
        /// &lt;seealso href=&quot;https://docs.docker.com/engine/context/working-with-contexts/&quot; /&gt;.
        /// </param>
        internal DockerBuild(DockerSpecificationType dockerSpecificationType, DockerImagePlatform platform, string dockerfile, string context) : base(dockerSpecificationType, platform)
        {
            Dockerfile = dockerfile;
            Context = context;
            DockerSpecificationType = dockerSpecificationType;
        }

        /// <summary>
        /// Docker command line instructions to assemble an image.
        /// 
        /// &lt;seealso href=&quot;https://repo2docker.readthedocs.io/en/latest/config_files.html#dockerfile-advanced-environments&quot; /&gt;.
        /// </summary>
        public string Dockerfile { get; set; }
        /// <summary>
        /// Path to a snapshot of the Docker Context. This property is only valid if Dockerfile is specified.
        /// 
        /// The path is relative to the asset path which must contain a single Blob URI value.
        /// 
        /// &lt;seealso href=&quot;https://docs.docker.com/engine/context/working-with-contexts/&quot; /&gt;.
        /// </summary>
        public string Context { get; set; }
    }
}
