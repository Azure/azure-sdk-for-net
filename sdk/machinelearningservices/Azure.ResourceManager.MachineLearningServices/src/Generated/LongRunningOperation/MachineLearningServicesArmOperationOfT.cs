...
#pragma warning disable SA1649 // File name should match first type name
    internal class MachineLearningServicesArmOperation<T> : ArmOperation<T>
#pragma warning restore SA1649 // File name should match first type name
    {
        private readonly OperationOrResponseInternals<T> _operation;

        /// <summary> Initializes a new instance of MachineLearningServicesArmOperation for mocking. </summary>
        protected MachineLearningServicesArmOperation()
        {
        }

        internal MachineLearningServicesArmOperation(Response<T> response)
        {
            _operation = new OperationOrResponseInternals<T>(response);
        }

        internal MachineLearningServicesArmOperation(IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            _operation = new OperationOrResponseInternals<T>(source, clientDiagnostics, pipeline, request, response, finalStateVia, "MachineLearningServicesArmOperation");
        }
