...
namespace Azure.ResourceManager.CosmosDB.Tests
{
    public class SqlContainerTests : CosmosDBManagementClientBase
    {
        ...
        public async Task SqlContainerCreateAndUpdate()
        {
            ...
            SqlContainerCreateUpdateParameters updateParameters = new SqlContainerCreateUpdateParameters(container.Id, _containerName, container.Data.Type,
                new Dictionary<string, string>(),// TODO: use original tags see defect: https://github.com/Azure/autorest.csharp/issues/1590
                Resources.Models.Location.WestUS2, container.Data.Resource, new CreateUpdateOptions { Throughput = TestThroughput2 });

            container = await (await SqlContainerCollection.CreateOrUpdateAsync(_containerName, updateParameters)).WaitForCompletionAsync();
            Assert.AreEqual(_containerName, container.Data.Resource.Id);
        ...
        {
            var container = await CreateSqlContainer(null);
            ThroughputSettings throughput = await container.GetSqlContainerThroughputAsync();

            Assert.AreEqual(TestThroughput1, throughput.Resource.Throughput);

            ThroughputSettings throughput2 = await container.UpdateSqlContainerThroughput(new ThroughputSettingsUpdateParameters(Resources.Models.Location.WestUS2,
        ...
        public async Task SqlContainerMigrateToAutoscale()
        {
            ...
            throughput = await container.MigrateSqlContainerToAutoscale().WaitForCompletionAsync();
        ...
        public async Task SqlContainerMigrateToManual()
        {
            ...
            throughput = await container.MigrateSqlContainerToManualThroughput().WaitForCompletionAsync();
            AssertManualThroughput(throughput);
        }
        ...
        public async Task SqlContainerRetrieveContinuousBackupInformation()
        {
            ...
            BackupInformation backupInfo =  await container.RetrieveContinuousBackupInformation(new ContinuousBackupRestoreLocation{ Location  = Resources.Models.Location.WestUS2}).WaitForCompletionAsync();
            ...
            container = await (await SqlContainerCollection.CreateOrUpdateAsync(_containerName, updateParameters)).WaitForCompletionAsync();
            backupInfo =  await container.RetrieveContinuousBackupInformation(new ContinuousBackupRestoreLocation{ Location  = Resources.Models.Location.WestUS2}).WaitForCompletionAsync();
        ...
        {
            SqlContainerCreateUpdateParameters sqlDatabaseCreateUpdateParameters = new SqlContainerCreateUpdateParameters(Resources.Models.Location.WestUS2,
            ...
            var sqlContainerLro = await sqlContainerCollection.CreateOrUpdateAsync(name, sqlDatabaseCreateUpdateParameters);
