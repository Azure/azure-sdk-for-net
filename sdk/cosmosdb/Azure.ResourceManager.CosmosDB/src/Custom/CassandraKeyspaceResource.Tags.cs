...
namespace Azure.ResourceManager.CosmosDB
{
    ...
    public partial class CassandraKeyspaceResource
    {
        private const string TagFallbackUnsupportedMessage =
            "AddTag/SetTags/RemoveTag fall back to the resource PUT operation when the subscription does not have tag-resource privileges, but the underlying PUT body type for CassandraKeyspaceResource is a different model from its data type, so the fallback is not supported. See https://github.com/Azure/azure-sdk-for-net/issues/58747.";

        internal Task<ArmOperation<CassandraKeyspaceResource>> UpdateAsync(WaitUntil waitUntil, CassandraKeyspaceData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);

        internal ArmOperation<CassandraKeyspaceResource> Update(WaitUntil waitUntil, CassandraKeyspaceData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);
    }
}
