...
namespace Azure.ResourceManager.CosmosDB
{
    ...
    public partial class CosmosDBSqlTriggerResource
    {
        private const string TagFallbackUnsupportedMessage =
            "AddTag/SetTags/RemoveTag fall back to the resource PUT operation when the subscription does not have tag-resource privileges, but the underlying PUT body type for CosmosDBSqlTriggerResource is a different model from its data type, so the fallback is not supported. See https://github.com/Azure/azure-sdk-for-net/issues/58747.";

        internal Task<ArmOperation<CosmosDBSqlTriggerResource>> UpdateAsync(WaitUntil waitUntil, CosmosDBSqlTriggerData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);

        internal ArmOperation<CosmosDBSqlTriggerResource> Update(WaitUntil waitUntil, CosmosDBSqlTriggerData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);
    }
}
