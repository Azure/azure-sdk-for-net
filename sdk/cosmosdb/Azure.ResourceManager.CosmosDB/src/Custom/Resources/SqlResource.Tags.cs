...
namespace Azure.ResourceManager.CosmosDB
{
    ...
    public partial class SqlResource
    {
        private const string TagFallbackUnsupportedMessage =
            "AddTag/SetTags/RemoveTag fall back to the resource PUT operation when the subscription does not have tag-resource privileges, but the underlying PUT body type for SqlResource is a different model from its data type, so the fallback is not supported. See https://github.com/Azure/azure-sdk-for-net/issues/58747.";

        internal Task<ArmOperation<SqlResource>> UpdateAsync(WaitUntil waitUntil, ThroughputSettingsData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);

        internal ArmOperation<SqlResource> Update(WaitUntil waitUntil, ThroughputSettingsData data, CancellationToken cancellationToken = default) =>
            throw new NotSupportedException(TagFallbackUnsupportedMessage);
    }
}
