...
namespace Azure.ResourceManager.CosmosDB
{
    ...
    public partial class CassandraDataCenterCollection : ArmCollection, IEnumerable<CassandraDataCenterResource>, IAsyncEnumerable<CassandraDataCenterResource>
    {
        ...
        {
            async Task<Page<CassandraDataCenterResource>> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _cassandraDataCenterClientDiagnostics.CreateScope("CassandraDataCenterCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _cassandraDataCenterRestClient.ListAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new CassandraDataCenterResource(Client, value)), null, response.GetRawResponse());
                }
                catch (Exception e)
                {
                    scope.Failed(e);
        ...
        {
            Page<CassandraDataCenterResource> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _cassandraDataCenterClientDiagnostics.CreateScope("CassandraDataCenterCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _cassandraDataCenterRestClient.List(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new CassandraDataCenterResource(Client, value)), null, response.GetRawResponse());
                }
                catch (Exception e)
                {
