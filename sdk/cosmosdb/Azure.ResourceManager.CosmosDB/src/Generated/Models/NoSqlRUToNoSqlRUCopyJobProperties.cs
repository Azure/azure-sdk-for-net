// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure.ResourceManager.CosmosDB;

namespace Azure.ResourceManager.CosmosDB.Models
{
    /// <summary> Source SQL to Destination SQL copy job properties. </summary>
    public partial class NoSqlRUToNoSqlRUCopyJobProperties : BaseCopyJobProperties
    {
        /// <summary> Initializes a new instance of <see cref="NoSqlRUToNoSqlRUCopyJobProperties"/>. </summary>
        /// <param name="tasks"> Copy Job tasks. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="tasks"/> is null. </exception>
        public NoSqlRUToNoSqlRUCopyJobProperties(IEnumerable<NoSqlRUToNoSqlRUCopyJobTask> tasks) : base(CopyJobType.NoSqlRUToNoSqlRU)
        {
            Argument.AssertNotNull(tasks, nameof(tasks));

            Tasks = tasks.ToList();
        }

        /// <summary> Initializes a new instance of <see cref="NoSqlRUToNoSqlRUCopyJobProperties"/>. </summary>
        /// <param name="jobType"> Copy Job Type. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="sourceDetails"> Source SQL DataStore details. </param>
        /// <param name="destinationDetails"> Destination SQL DataStore details. </param>
        /// <param name="tasks"> Copy Job tasks. </param>
        internal NoSqlRUToNoSqlRUCopyJobProperties(CopyJobType jobType, IDictionary<string, BinaryData> additionalBinaryDataProperties, CosmosDBSourceSinkDetails sourceDetails, CosmosDBSourceSinkDetails destinationDetails, IList<NoSqlRUToNoSqlRUCopyJobTask> tasks) : base(jobType, additionalBinaryDataProperties)
        {
            SourceDetails = sourceDetails;
            DestinationDetails = destinationDetails;
            Tasks = tasks;
        }

        /// <summary> Source SQL DataStore details. </summary>
        [WirePath("sourceDetails")]
        internal CosmosDBSourceSinkDetails SourceDetails { get; set; }

        /// <summary> Destination SQL DataStore details. </summary>
        [WirePath("destinationDetails")]
        internal CosmosDBSourceSinkDetails DestinationDetails { get; set; }

        /// <summary> Copy Job tasks. </summary>
        [WirePath("tasks")]
        public IList<NoSqlRUToNoSqlRUCopyJobTask> Tasks { get; }

        /// <summary> Name of remote account in case of cross-account data transfer. </summary>
        [WirePath("sourceDetails.remoteAccountName")]
        public string SourceDetailsRemoteAccountName
        {
            get
            {
                return SourceDetails is null ? default : SourceDetails.RemoteAccountName;
            }
            set
            {
                if (SourceDetails is null)
                {
                    SourceDetails = new CosmosDBSourceSinkDetails();
                }
                SourceDetails.RemoteAccountName = value;
            }
        }

        /// <summary> Name of remote account in case of cross-account data transfer. </summary>
        [WirePath("destinationDetails.remoteAccountName")]
        public string DestinationDetailsRemoteAccountName
        {
            get
            {
                return DestinationDetails is null ? default : DestinationDetails.RemoteAccountName;
            }
            set
            {
                if (DestinationDetails is null)
                {
                    DestinationDetails = new CosmosDBSourceSinkDetails();
                }
                DestinationDetails.RemoteAccountName = value;
            }
        }
    }
}
