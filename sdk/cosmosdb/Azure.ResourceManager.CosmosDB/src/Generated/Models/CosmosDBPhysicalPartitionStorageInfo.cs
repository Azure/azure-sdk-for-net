// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.CosmosDB;

namespace Azure.ResourceManager.CosmosDB.Models
{
    /// <summary> The storage of a physical partition. </summary>
    public partial class CosmosDBPhysicalPartitionStorageInfo
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="CosmosDBPhysicalPartitionStorageInfo"/>. </summary>
        internal CosmosDBPhysicalPartitionStorageInfo()
        {
        }

        /// <summary> Initializes a new instance of <see cref="CosmosDBPhysicalPartitionStorageInfo"/>. </summary>
        /// <param name="id"> The unique identifier of the partition. </param>
        /// <param name="storageInKB"> The storage in KB for the physical partition. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal CosmosDBPhysicalPartitionStorageInfo(string id, double? storageInKB, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Id = id;
            StorageInKB = storageInKB;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The unique identifier of the partition. </summary>
        [WirePath("id")]
        public string Id { get; }

        /// <summary> The storage in KB for the physical partition. </summary>
        [WirePath("storageInKB")]
        public double? StorageInKB { get; }
    }
}
