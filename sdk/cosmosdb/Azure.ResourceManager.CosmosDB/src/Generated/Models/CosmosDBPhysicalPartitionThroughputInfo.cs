// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.CosmosDB;

namespace Azure.ResourceManager.CosmosDB.Models
{
    /// <summary> PhysicalPartitionThroughputInfo object. </summary>
    public partial class CosmosDBPhysicalPartitionThroughputInfo
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="CosmosDBPhysicalPartitionThroughputInfo"/>. </summary>
        /// <param name="id"> Id of a physical partition. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="id"/> is null. </exception>
        public CosmosDBPhysicalPartitionThroughputInfo(string id)
        {
            Argument.AssertNotNull(id, nameof(id));

            Id = id;
        }

        /// <summary> Initializes a new instance of <see cref="CosmosDBPhysicalPartitionThroughputInfo"/>. </summary>
        /// <param name="id"> Id of a physical partition. </param>
        /// <param name="throughput"> Throughput of a physical partition. </param>
        /// <param name="targetThroughput"> Target throughput of a physical partition. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal CosmosDBPhysicalPartitionThroughputInfo(string id, double? throughput, double? targetThroughput, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Id = id;
            Throughput = throughput;
            TargetThroughput = targetThroughput;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Id of a physical partition. </summary>
        [WirePath("id")]
        public string Id { get; set; }

        /// <summary> Throughput of a physical partition. </summary>
        [WirePath("throughput")]
        public double? Throughput { get; set; }

        /// <summary> Target throughput of a physical partition. </summary>
        [WirePath("targetThroughput")]
        public double? TargetThroughput { get; set; }
    }
}
