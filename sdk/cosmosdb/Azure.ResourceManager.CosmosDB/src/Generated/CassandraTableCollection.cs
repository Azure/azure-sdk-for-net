...
{
    /// <summary> A class representing collection of CassandraTable and their operations over a CassandraKeyspace. </summary>
    public partial class CassandraTableCollection : ArmCollection, IEnumerable<CassandraTable>, IAsyncEnumerable<CassandraTable>
    {
        ...
        public virtual CassandraTableCreateUpdateOperation CreateOrUpdate(string tableName, CassandraTableCreateUpdateParameters createUpdateCassandraTableParameters, bool waitForCompletion = true, CancellationToken cancellationToken = default)
        {
            if (tableName == null)
            {
            ...
            try
            {
                var response = _restClient.CreateUpdate(Id.ResourceGroupName, Id.Parent.Name, Id.Name, tableName, createUpdateCassandraTableParameters, cancellationToken);
                var operation = new CassandraTableCreateUpdateOperation(Parent, _clientDiagnostics, Pipeline, _restClient.CreateCreateUpdateRequest(Id.ResourceGroupName, Id.Parent.Name, Id.Name, tableName, createUpdateCassandraTableParameters).Request, response);
        ...
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="tableName"/> or <paramref name="createUpdateCassandraTableParameters"/> is null. </exception>
        public async virtual Task<CassandraTableCreateUpdateOperation> CreateOrUpdateAsync(string tableName, CassandraTableCreateUpdateParameters createUpdateCassandraTableParameters, bool waitForCompletion = true, CancellationToken cancellationToken = default)
        {
            if (tableName == null)
            {
                throw new ArgumentNullException(nameof(tableName));
            ...
            try
            {
                var response = await _restClient.CreateUpdateAsync(Id.ResourceGroupName, Id.Parent.Name, Id.Name, tableName, createUpdateCassandraTableParameters, cancellationToken).ConfigureAwait(false);
                var operation = new CassandraTableCreateUpdateOperation(Parent, _clientDiagnostics, Pipeline, _restClient.CreateCreateUpdateRequest(Id.ResourceGroupName, Id.Parent.Name, Id.Name, tableName, createUpdateCassandraTableParameters).Request, response);
                if (waitForCompletion)
        ...
        // Builders.
        // public ArmBuilder<ResourceIdentifier, CassandraTable, CassandraTableData> Construct() { }
    }
}
