...
namespace Azure.ResourceManager.SecretsStoreExtension.Tests
{
        ...
        SecretsStoreExtensionManagementTestEnvironment te)
    {
        ...
        private SecretSyncCollection _ssc;

        // Create, view, update, tag, and delete a SecretSync.
        internal async Task DoTestSecretSyncCrudAsync(Func<Func<Task>, Task> spcHarness)
        {
            async Task act()
            {
                _ssc = rg.GetSecretSyncs();
        ...
        private async Task CreateAsync()
        {
            ...
            await _ssc.CreateOrUpdateAsync(WaitUntil.Completed, SsName, ssData);
        }
        ...
        private async Task TestExistsAndGetAsync(bool shouldExist)
        {
            ...
            var exists = (await _ssc.ExistsAsync(SsName)).Value;
            Assert.AreEqual(exists, shouldExist);

            // SecretSyncCollection.GetAsync
            if (shouldExist)
            {
                SecretSyncResource ss = (await _ssc.GetAsync(SsName)).Value;
                CheckContents(ss);
            }

            // SecretSyncCollection.GetIfExistsAsync
            NullableResponse<SecretSyncResource> nr = await _ssc.GetIfExistsAsync(SsName);
            ...
            List<SecretSyncResource> secretSyncs = await _ssc.GetAllAsync().ToEnumerableAsync();
        ...
        {
            await SetSecretMapping(RpSecretName2, RpSecretName2);
            await SetSecretMapping(RpSecretName1, RpSecretName1);
        }
        ...
        private async Task SetSecretMapping(string sourcePath, string targetKey)
        {
            SecretSyncResource ss = await GetSecretSync();
        ...
        private async Task TestExtension()
        {
        ...
        private async Task<SecretSyncResource> GetSecretSync()
        {
