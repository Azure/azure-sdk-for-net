// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure.ResourceManager.Cdn;

namespace Azure.ResourceManager.Cdn.Models
{
    /// <summary> Parameters required for content purge. </summary>
    public partial class FrontDoorPurgeParameters
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="FrontDoorPurgeParameters"/>. </summary>
        /// <param name="contentPaths"> The path to the content to be purged. Can describe a file path or a wild card directory. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="contentPaths"/> is null. </exception>
        public FrontDoorPurgeParameters(IEnumerable<string> contentPaths)
        {
            Argument.AssertNotNull(contentPaths, nameof(contentPaths));

            ContentPaths = contentPaths.ToList();
            Domains = new ChangeTrackingList<string>();
        }

        /// <summary> Initializes a new instance of <see cref="FrontDoorPurgeParameters"/>. </summary>
        /// <param name="contentPaths"> The path to the content to be purged. Can describe a file path or a wild card directory. </param>
        /// <param name="domains"> List of domains. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal FrontDoorPurgeParameters(IList<string> contentPaths, IList<string> domains, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            ContentPaths = contentPaths;
            Domains = domains;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The path to the content to be purged. Can describe a file path or a wild card directory. </summary>
        public IList<string> ContentPaths { get; }

        /// <summary> List of domains. </summary>
        public IList<string> Domains { get; }
    }
}
