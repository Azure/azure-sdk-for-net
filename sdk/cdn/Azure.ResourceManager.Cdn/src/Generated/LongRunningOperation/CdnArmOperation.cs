...
#pragma warning restore SA1649 // File name should match first type name
    {
        ...
        internal CdnArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            ...
            _operation = new OperationInternal(clientDiagnostics, nextLinkOperation, response, "CdnArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }

        internal CdnArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, string id)
        {
            var nextLinkOperation = NextLinkOperationImplementation.Create(pipeline, id, out string finalResponse);
            _operation = OperationInternal.Create(clientDiagnostics, nextLinkOperation, finalResponse, "CdnArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }
