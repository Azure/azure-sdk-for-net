...
namespace Azure.ResourceManager.DataBox.Models
{
    public partial class DataBoxDiskJobDetails : IUtf8JsonSerializable, IJsonModel<DataBoxDiskJobDetails>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsCollectionDefined(PreferredDisks))
            {
                if (PreferredDisks.Any())
                {
                    builder.Append("  preferredDisks:");
                    builder.AppendLine(" {");
                    foreach (var item in PreferredDisks)
                    {
            ...
            {
                if (GranularCopyProgress.Any())
                {
                    builder.Append("  granularCopyProgress:");
                    builder.AppendLine(" [");
                    foreach (var item in GranularCopyProgress)
                    {
                        AppendChildObject(builder, item, options, 4, true);
            ...
            {
                if (GranularCopyLogDetails.Any())
                {
                    builder.Append("  granularCopyLogDetails:");
                    builder.AppendLine(" [");
                    foreach (var item in GranularCopyLogDetails)
                    {
                        AppendChildObject(builder, item, options, 4, true);
            ...
            {
                if (DisksAndSizeDetails.Any())
                {
                    builder.Append("  disksAndSizeDetails:");
                    builder.AppendLine(" {");
                    foreach (var item in DisksAndSizeDetails)
                    {
                        builder.Append($"    {item.Key}:");
            ...
            if (Optional.IsDefined(Passkey))
            {
                ...
                if (Passkey.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
