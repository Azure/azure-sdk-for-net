...
namespace Azure.ResourceManager.Billing
{
    ...
    // `tags` field, then PopulateUpdateMethod() falls back to PUT (PartnerTransfers_Initiate).
    // The tag-helper else-branch calls this.Update(WaitUntil, <Data>, CT) but the actual
    // Update takes PartnerTransferDetailCreateOrUpdateContent (separate Request body),
    // producing CS1503. Suppress the 6 helpers entirely — GA 1.2.2 never exposed them and
    // ARM tags are still reachable via the resource's Tag* extension/TagResource. Remove
    // these suppressions once https://github.com/Azure/azure-sdk-for-net/issues/58747 ships.
