...
namespace Azure.ResourceManager.Billing
{
    ...
    public partial class BillingCustomerCollection
    {
        ...
        {
            return GetAllAsync(expand: options?.Expand, filter: options?.Filter, orderBy: options?.OrderBy, maxCount: options?.Top, skip: options?.Skip, count: options?.Count, search: options?.Search, cancellationToken: cancellationToken);
        }
        ...
        {
            return GetAll(expand: options?.Expand, filter: options?.Filter, orderBy: options?.OrderBy, maxCount: options?.Top, skip: options?.Skip, count: options?.Count, search: options?.Search, cancellationToken: cancellationToken);
        }
    }
}
