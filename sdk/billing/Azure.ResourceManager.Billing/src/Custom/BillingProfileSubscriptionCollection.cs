...
namespace Azure.ResourceManager.Billing
{
    ...
    public partial class BillingProfileSubscriptionCollection
    {
        ...
        {
            return GetAllAsync(includeDeleted: options?.IncludeDeleted, expand: options?.Expand, filter: options?.Filter, orderBy: options?.OrderBy, maxCount: options?.Top, skip: options?.Skip, count: options?.Count, search: options?.Search, cancellationToken: cancellationToken);
        }
        ...
        {
            return GetAll(includeDeleted: options?.IncludeDeleted, expand: options?.Expand, filter: options?.Filter, orderBy: options?.OrderBy, maxCount: options?.Top, skip: options?.Skip, count: options?.Count, search: options?.Search, cancellationToken: cancellationToken);
        }
    }
}
