...
namespace Azure.ResourceManager.Applications.Containers
{
    ...
    /// Otherwise you can get one from its parent resource <see cref="RevisionResource" /> using the GetReplica method.
    /// </summary>
    public partial class ReplicaResource : ArmResource
    {
        ...
        {
            _replicaContainerAppsRevisionReplicasClientDiagnostics = new ClientDiagnostics("Azure.ResourceManager.Applications.Containers", ResourceType.Namespace, Diagnostics);
            TryGetApiVersion(ResourceType, out string replicaContainerAppsRevisionReplicasApiVersion);
        ...
        {
            using var scope = _replicaContainerAppsRevisionReplicasClientDiagnostics.CreateScope("ReplicaResource.Get");
            scope.Start();
            try
            {
                var response = await _replicaContainerAppsRevisionReplicasRestClient.GetReplicaAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Parent.Name, Id.Parent.Name, Id.Name, cancellationToken).ConfigureAwait(false);
        ...
        public virtual Response<ReplicaResource> Get(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var response = _replicaContainerAppsRevisionReplicasRestClient.GetReplica(Id.SubscriptionId, Id.ResourceGroupName, Id.Parent.Parent.Name, Id.Parent.Name, Id.Name, cancellationToken);
                if (response.Value == null)
                    throw new RequestFailedException(response.GetRawResponse());
                return Response.FromValue(new ReplicaResource(Client, response.Value), response.GetRawResponse());
            }
