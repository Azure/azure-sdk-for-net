// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.Applications.Containers.Models
{
    public partial class EnvironmentVar : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            if (Optional.IsDefined(Name))
            {
                writer.WritePropertyName("name");
                writer.WriteStringValue(Name);
            }
            if (Optional.IsDefined(Value))
            {
                writer.WritePropertyName("value");
                writer.WriteStringValue(Value);
            }
            if (Optional.IsDefined(SecretRef))
            {
                writer.WritePropertyName("secretRef");
                writer.WriteStringValue(SecretRef);
            }
            writer.WriteEndObject();
        }

        internal static EnvironmentVar DeserializeEnvironmentVar(JsonElement element)
        {
            Optional<string> name = default;
            Optional<string> value = default;
            Optional<string> secretRef = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("name"))
                {
                    name = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("value"))
                {
                    value = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("secretRef"))
                {
                    secretRef = property.Value.GetString();
                    continue;
                }
            }
            return new EnvironmentVar(name.Value, value.Value, secretRef.Value);
        }
    }
}
