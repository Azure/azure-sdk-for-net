// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using System.Threading;
using System.Threading.Tasks;
using Azure;
using Azure.Core;
using Azure.ResourceManager;

namespace Azure.ResourceManager.AppContainers
{
    /// <summary></summary>
    internal partial class DotNetComponentResourceOperationSource : IOperationSource<DotNetComponentResource>
    {
        private readonly ArmClient _client;

        /// <summary></summary>
        /// <param name="client"></param>
        internal DotNetComponentResourceOperationSource(ArmClient client)
        {
            _client = client;
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        DotNetComponentResource IOperationSource<DotNetComponentResource>.CreateResult(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = JsonDocument.Parse(response.ContentStream);
            DotNetComponentData data = DotNetComponentData.DeserializeDotNetComponentData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new DotNetComponentResource(_client, data);
        }

        /// <param name="response"> The response from the service. </param>
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <returns></returns>
        async ValueTask<DotNetComponentResource> IOperationSource<DotNetComponentResource>.CreateResultAsync(Response response, CancellationToken cancellationToken)
        {
            using JsonDocument document = await JsonDocument.ParseAsync(response.ContentStream, default, cancellationToken).ConfigureAwait(false);
            DotNetComponentData data = DotNetComponentData.DeserializeDotNetComponentData(document.RootElement, ModelSerializationExtensions.WireOptions);
            return new DotNetComponentResource(_client, data);
        }
    }
}
