// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.AppContainers.Models
{
    /// <summary> Dynamic pool configuration. </summary>
    public partial class DynamicPoolConfiguration
    {
        /// <summary>
        /// Keeps track of any properties unknown to the library.
        /// <para>
        /// To assign an object to the value of this property use <see cref="BinaryData.FromObjectAsJson{T}(T, System.Text.Json.JsonSerializerOptions?)"/>.
        /// </para>
        /// <para>
        /// To assign an already formatted json string to this property use <see cref="BinaryData.FromString(string)"/>.
        /// </para>
        /// <para>
        /// Examples:
        /// <list type="bullet">
        /// <item>
        /// <term>BinaryData.FromObjectAsJson("foo")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("\"foo\"")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromObjectAsJson(new { key = "value" })</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("{\"key\": \"value\"}")</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// </list>
        /// </para>
        /// </summary>
        private IDictionary<string, BinaryData> _serializedAdditionalRawData;

        /// <summary> Initializes a new instance of <see cref="DynamicPoolConfiguration"/>. </summary>
        public DynamicPoolConfiguration()
        {
        }

        /// <summary> Initializes a new instance of <see cref="DynamicPoolConfiguration"/>. </summary>
        /// <param name="executionType"> The execution type of the session pool. </param>
        /// <param name="cooldownPeriodInSeconds"> The cooldown period of a session in seconds. </param>
        /// <param name="serializedAdditionalRawData"> Keeps track of any properties unknown to the library. </param>
        internal DynamicPoolConfiguration(ExecutionType? executionType, int? cooldownPeriodInSeconds, IDictionary<string, BinaryData> serializedAdditionalRawData)
        {
            ExecutionType = executionType;
            CooldownPeriodInSeconds = cooldownPeriodInSeconds;
            _serializedAdditionalRawData = serializedAdditionalRawData;
        }

        /// <summary> The execution type of the session pool. </summary>
        [WirePath("executionType")]
        public ExecutionType? ExecutionType { get; set; }
        /// <summary> The cooldown period of a session in seconds. </summary>
        [WirePath("cooldownPeriodInSeconds")]
        public int? CooldownPeriodInSeconds { get; set; }
    }
}
