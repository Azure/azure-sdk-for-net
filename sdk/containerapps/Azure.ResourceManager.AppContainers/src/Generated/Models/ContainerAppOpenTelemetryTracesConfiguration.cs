// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.AppContainers;

namespace Azure.ResourceManager.AppContainers.Models
{
    /// <summary> Configuration of Open Telemetry traces. </summary>
    public partial class ContainerAppOpenTelemetryTracesConfiguration
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ContainerAppOpenTelemetryTracesConfiguration"/>. </summary>
        public ContainerAppOpenTelemetryTracesConfiguration()
        {
            Destinations = new ChangeTrackingList<string>();
        }

        /// <summary> Initializes a new instance of <see cref="ContainerAppOpenTelemetryTracesConfiguration"/>. </summary>
        /// <param name="isDaprIncluded"> Boolean indicating if including dapr traces. </param>
        /// <param name="destinations"> Open telemetry traces destinations. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ContainerAppOpenTelemetryTracesConfiguration(bool? isDaprIncluded, IList<string> destinations, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            IsDaprIncluded = isDaprIncluded;
            Destinations = destinations;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Boolean indicating if including dapr traces. </summary>
        [WirePath("includeDapr")]
        public bool? IsDaprIncluded { get; set; }

        /// <summary> Open telemetry traces destinations. </summary>
        [WirePath("destinations")]
        public IList<string> Destinations { get; }
    }
}
