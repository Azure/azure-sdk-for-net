// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;

namespace Azure.ResourceManager.ComputeSchedule.Models
{
    /// <summary> Contains the IP tag associated with the public IP address. </summary>
    public partial class ComputeScheduleVirtualMachineIpTag
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ComputeScheduleVirtualMachineIpTag"/>. </summary>
        public ComputeScheduleVirtualMachineIpTag()
        {
        }

        /// <summary> Initializes a new instance of <see cref="ComputeScheduleVirtualMachineIpTag"/>. </summary>
        /// <param name="ipTagType"> IP tag type. Example: FirstPartyUsage. </param>
        /// <param name="tag"> IP tag associated with the public IP. Example: SQL, Storage etc. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ComputeScheduleVirtualMachineIpTag(string ipTagType, string tag, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            IpTagType = ipTagType;
            Tag = tag;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> IP tag type. Example: FirstPartyUsage. </summary>
        public string IpTagType { get; set; }

        /// <summary> IP tag associated with the public IP. Example: SQL, Storage etc. </summary>
        public string Tag { get; set; }
    }
}
