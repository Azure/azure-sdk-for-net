...
namespace Azure.Verticals.AgriFood.Farming.Models
{
    public partial class ApplicationData : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            ...
            if (Optional.IsCollectionDefined(ApplicationProductDetails))
            {
                writer.WritePropertyName("applicationProductDetails");
                writer.WriteStartArray();
                foreach (var item in ApplicationProductDetails)
                {
        ...
        {
            Optional<IList<ApplicationProductDetail>> applicationProductDetails = default;
            ...
            {
                if (property.NameEquals("applicationProductDetails"))
                {
                    ...
                    List<ApplicationProductDetail> array = new List<ApplicationProductDetail>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(ApplicationProductDetail.DeserializeApplicationProductDetail(item));
                    }
                    applicationProductDetails = array;
                    continue;
                }
            ...
            return new ApplicationData(Optional.ToList(applicationProductDetails), avgMaterial.Value, totalMaterial.Value, area.Value, source.Value, Optional.ToNullable(operationModifiedDateTime), Optional.ToNullable(operationStartDateTime), Optional.ToNullable(operationEndDateTime), attachmentsLink.Value, associatedBoundaryId.Value, operationBoundaryId.Value, farmerId.Value, id.Value, eTag.Value, status.Value, Optional.ToNullable(createdDateTime), Optional.ToNullable(modifiedDateTime), name.Value, description.Value, Optional.ToDictionary(properties));
        }
    }
}
