// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;
using System.Text.Json;
using Azure.Core;

namespace Azure.Verticals.AgriFood.Farming.Models
{
    internal partial class ApplicationDataListResponse
    {
        internal static ApplicationDataListResponse DeserializeApplicationDataListResponse(JsonElement element)
        {
            Optional<IReadOnlyList<ApplicationData>> value = default;
            Optional<string> skipToken = default;
            Optional<string> nextLink = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("value"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    List<ApplicationData> array = new List<ApplicationData>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(ApplicationData.DeserializeApplicationData(item));
                    }
                    value = array;
                    continue;
                }
                if (property.NameEquals("$skipToken"))
                {
                    skipToken = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("nextLink"))
                {
                    nextLink = property.Value.GetString();
                    continue;
                }
            }
            return new ApplicationDataListResponse(Optional.ToList(value), skipToken.Value, nextLink.Value);
        }
    }
}
