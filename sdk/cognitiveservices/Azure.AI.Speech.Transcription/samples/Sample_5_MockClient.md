...
ClientResult<TranscriptionResult> response = await mockClient.TranscribeAsync(options);
...
ApiKeyCredential credential = new ApiKeyCredential("mock-key");
...
using var audioStream = new MemoryStream(new byte[] { 0x00, 0x01, 0x02 });
TranscriptionOptions transcriptionOptions = new TranscriptionOptions(audioStream);

ClientResult<TranscriptionResult> response = await client.TranscribeAsync(transcriptionOptions);
...
Uri endpoint = new Uri("https://mock.api.cognitive.microsoft.com/");
ApiKeyCredential credential = new ApiKeyCredential("invalid-key");
TranscriptionClient client = new TranscriptionClient(endpoint, credential, options);
...
try
{
    ...
    await client.TranscribeAsync(transcriptionOptions);
...
public class TranscriptionService
{
    ...
    public async Task<string> GetTranscriptionAsync(Stream audioStream)
    {
        ...
        var response = await _client.TranscribeAsync(options);
