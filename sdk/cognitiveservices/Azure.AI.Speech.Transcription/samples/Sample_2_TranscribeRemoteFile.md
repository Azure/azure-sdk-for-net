...
var options = new TranscriptionOptions(stream);
...
To transcribe a remote file asynchronously, create a stream from url of the file and call `TranscribeAsync` on the `TranscriptionClient` clientlet, which returns the transcribed phrases and total duration of the file
