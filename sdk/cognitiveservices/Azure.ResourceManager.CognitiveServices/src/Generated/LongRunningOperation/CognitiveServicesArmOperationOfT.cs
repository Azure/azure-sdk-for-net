...
#pragma warning disable SA1649 // File name should match first type name
    internal class CognitiveServicesArmOperation<T> : ArmOperation<T>
#pragma warning restore SA1649 // File name should match first type name
    {
        private readonly OperationInternal<T> _operation;

        /// <summary> Initializes a new instance of CognitiveServicesArmOperation for mocking. </summary>
        protected CognitiveServicesArmOperation()
        {
        }

        internal CognitiveServicesArmOperation(Response<T> response)
        {
            _operation = OperationInternal<T>.Succeeded(response.GetRawResponse(), response.Value);
        }

        internal CognitiveServicesArmOperation(IOperationSource<T> source, ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia, bool skipApiVersionOverride = false, string apiVersionOverrideValue = null)
        {
            ...
            _operation = new OperationInternal<T>(nextLinkOperation, clientDiagnostics, response, "CognitiveServicesArmOperation", fallbackStrategy: new SequentialDelayStrategy());
        }
