// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.CognitiveServices;

namespace Azure.ResourceManager.CognitiveServices.Models
{
    /// <summary> A compute pool configuration. </summary>
    public partial class Pool
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="Pool"/>. </summary>
        /// <param name="name"> The name of the pool. </param>
        /// <param name="vmPriority"> The VM priority of the pool. </param>
        /// <param name="instanceType"> The instance type (VM SKU) used in the pool. </param>
        /// <param name="nodeCount"> The number of nodes in the pool. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="name"/> or <paramref name="instanceType"/> is null. </exception>
        public Pool(string name, VmPriority vmPriority, string instanceType, int nodeCount)
        {
            Argument.AssertNotNull(name, nameof(name));
            Argument.AssertNotNull(instanceType, nameof(instanceType));

            Name = name;
            VmPriority = vmPriority;
            InstanceType = instanceType;
            NodeCount = nodeCount;
        }

        /// <summary> Initializes a new instance of <see cref="Pool"/>. </summary>
        /// <param name="name"> The name of the pool. </param>
        /// <param name="vmPriority"> The VM priority of the pool. </param>
        /// <param name="instanceType"> The instance type (VM SKU) used in the pool. </param>
        /// <param name="nodeCount"> The number of nodes in the pool. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal Pool(string name, VmPriority vmPriority, string instanceType, int nodeCount, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Name = name;
            VmPriority = vmPriority;
            InstanceType = instanceType;
            NodeCount = nodeCount;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The name of the pool. </summary>
        [WirePath("name")]
        public string Name { get; set; }

        /// <summary> The VM priority of the pool. </summary>
        [WirePath("vmPriority")]
        public VmPriority VmPriority { get; set; }

        /// <summary> The instance type (VM SKU) used in the pool. </summary>
        [WirePath("instanceType")]
        public string InstanceType { get; set; }

        /// <summary> The number of nodes in the pool. </summary>
        [WirePath("nodeCount")]
        public int NodeCount { get; set; }
    }
}
