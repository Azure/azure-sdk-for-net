...
namespace Azure.ResourceManager.Media.Models
{
    public partial class LiveEventInput : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            ...
            if (Optional.IsCollectionDefined(TimedMetadataEndpoints))
            {
                writer.WritePropertyName("timedMetadataEndpoints"u8);
                writer.WriteStartArray();
                foreach (var item in TimedMetadataEndpoints)
                {
                    writer.WriteObjectValue(item);
                }
                writer.WriteEndArray();
        ...
        internal static LiveEventInput DeserializeLiveEventInput(JsonElement element)
        {
            ...
            Optional<IList<LiveEventTimedMetadataEndpoint>> timedMetadataEndpoints = default;
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("timedMetadataEndpoints"u8))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                    ...
                    List<LiveEventTimedMetadataEndpoint> array = new List<LiveEventTimedMetadataEndpoint>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(LiveEventTimedMetadataEndpoint.DeserializeLiveEventTimedMetadataEndpoint(item));
                    }
                    timedMetadataEndpoints = array;
                    continue;
                }
            }
            return new LiveEventInput(streamingProtocol, accessControl.Value, Optional.ToNullable(keyFrameIntervalDuration), accessToken.Value, Optional.ToList(endpoints), Optional.ToList(timedMetadataEndpoints));
        }
    }
}
