// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.ResourceManager.Media.Models
{
    public partial class Deinterlace : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            writer.WriteStartObject();
            if (Optional.IsDefined(Parity))
            {
                writer.WritePropertyName("parity");
                writer.WriteStringValue(Parity.Value.ToString());
            }
            if (Optional.IsDefined(Mode))
            {
                writer.WritePropertyName("mode");
                writer.WriteStringValue(Mode.Value.ToString());
            }
            writer.WriteEndObject();
        }

        internal static Deinterlace DeserializeDeinterlace(JsonElement element)
        {
            Optional<DeinterlaceParity> parity = default;
            Optional<DeinterlaceMode> mode = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("parity"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    parity = new DeinterlaceParity(property.Value.GetString());
                    continue;
                }
                if (property.NameEquals("mode"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    mode = new DeinterlaceMode(property.Value.GetString());
                    continue;
                }
            }
            return new Deinterlace(Optional.ToNullable(parity), Optional.ToNullable(mode));
        }
    }
}
