...
namespace Azure.ResourceManager.Media.Models
{
    ...
    /// Please note <see cref="ImageProperties"/> is the base class. According to the scenario, a derived class of the base class might need to be assigned here, or this property needs to be casted to one of the possible derived classes.
    /// The available derived classes include <see cref="JpgImage"/> and <see cref="PngImage"/>.
    /// </summary>
    public partial class ImageProperties : InputVideoEncodingProperties
    {
        ...
        public ImageProperties(string start)
        {
            if (start == null)
            {
        ...
        internal ImageProperties(string odataType, string label, TimeSpan? keyFrameInterval, StretchMode? stretchMode, VideoSyncMode? syncMode, string start, string step, string range) : base(odataType, label, keyFrameInterval, stretchMode, syncMode)
        {
            Start = start;
            Step = step;
            Range = range;
            OdataType = odataType ?? "#Microsoft.Media.Image";
        }

        /// <summary> The position in the input video from where to start generating thumbnails. The value can be in ISO 8601 format (For example, PT05S to start at 5 seconds), or a frame count (For example, 10 to start at the 10th frame), or a relative value to stream duration (For example, 10% to start at 10% of stream duration). Also supports a macro {Best}, which tells the encoder to select the best thumbnail from the first few seconds of the video and will only produce one thumbnail, no matter what other settings are for Step and Range. The default value is macro {Best}. </summary>
