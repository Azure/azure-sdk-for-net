...
namespace Azure.ResourceManager.WorkloadMonitor
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<HealthMonitorData>)this).Write(writer, ModelReaderWriterOptions.Wire);

        void IJsonModel<HealthMonitorData>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<HealthMonitorData>)this).GetWireFormat(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<HealthMonitorData>)} interface");
            }
        ...
        string IPersistableModel<HealthMonitorData>.GetWireFormat(ModelReaderWriterOptions options) => "J";
    }
}
