...
namespace Azure.Analytics.Purview.DataMap
{
    public partial class AtlasGlossaryTerm : IUtf8JsonSerializable
    {
        ...
        internal static AtlasGlossaryTerm DeserializeAtlasGlossaryTerm(JsonElement element)
        {
            ...
            return new AtlasGlossaryTerm(guid.Value, Optional.ToList(classifications), longDescription.Value, name.Value, qualifiedName.Value, shortDescription.Value, lastModifiedTS.Value, Optional.ToNullable(createTime), createdBy.Value, Optional.ToNullable(updateTime), updatedBy.Value, abbreviation.Value, Optional.ToList(templateName), anchor.Value, Optional.ToList(antonyms), Optional.ToNullable(status), nickName.Value, Optional.ToList(hierarchyInfo), Optional.ToList(resources), Optional.ToDictionary(contacts), Optional.ToDictionary(attributes), Optional.ToList(assignedEntities), Optional.ToList(categories), Optional.ToList(classifies), Optional.ToList(examples), Optional.ToList(isA), Optional.ToList(preferredTerms), Optional.ToList(preferredToTerms), Optional.ToList(replacedBy), Optional.ToList(replacementTerms), Optional.ToList(seeAlso), Optional.ToList(synonyms), Optional.ToList(translatedTerms), Optional.ToList(translationTerms), usage.Value, Optional.ToList(validValues), Optional.ToList(validValuesFor));
        }
