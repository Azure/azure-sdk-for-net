...
namespace Azure.ResourceManager.Reservations
{
    ...
    public partial class ReservationOrderResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation<ReservationOrderResource>> UpdateAsync(WaitUntil waitUntil, ReservationPurchaseContent content, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _reservationOrderRestClient.CreatePurchaseRequest(Guid.Parse(Id.Name), content);
        ...
        public virtual async Task<ArmOperation<IList<ReservationDetailData>>> SplitReservationAsync(WaitUntil waitUntil, SplitContent content, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _reservationDetailReservationRestClient.CreateSplitRequest(Guid.Parse(Id.Name), content);
                var response = await _reservationDetailReservationRestClient.SplitAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new ReservationsArmOperation<IList<ReservationDetailData>>(new IListOperationSource(), _reservationDetailReservationClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        public virtual ArmOperation<IList<ReservationDetailData>> SplitReservation(WaitUntil waitUntil, SplitContent content, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _reservationDetailReservationRestClient.CreateSplitRequest(Guid.Parse(Id.Name), content);
                var response = _reservationDetailReservationRestClient.Split(message, cancellationToken);
                var operation = new ReservationsArmOperation<IList<ReservationDetailData>>(new IListOperationSource(), _reservationDetailReservationClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletion(cancellationToken);
                return operation;
        ...
        public virtual async Task<ArmOperation<IList<ReservationDetailData>>> MergeReservationAsync(WaitUntil waitUntil, MergeContent content, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                using var message = _reservationDetailReservationRestClient.CreateMergeRequest(Guid.Parse(Id.Name), content);
                var response = await _reservationDetailReservationRestClient.MergeAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new ReservationsArmOperation<IList<ReservationDetailData>>(new IListOperationSource(), _reservationDetailReservationClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionAsync(cancellationToken).ConfigureAwait(false);
                return operation;
        ...
        public virtual ArmOperation<IList<ReservationDetailData>> MergeReservation(WaitUntil waitUntil, MergeContent content, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _reservationDetailReservationRestClient.CreateMergeRequest(Guid.Parse(Id.Name), content);
                var response = _reservationDetailReservationRestClient.Merge(message, cancellationToken);
                var operation = new ReservationsArmOperation<IList<ReservationDetailData>>(new IListOperationSource(), _reservationDetailReservationClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletion(cancellationToken);
                return operation;
