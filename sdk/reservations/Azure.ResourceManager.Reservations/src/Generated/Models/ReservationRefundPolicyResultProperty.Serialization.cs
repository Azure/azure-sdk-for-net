...
namespace Azure.ResourceManager.Reservations.Models
{
    public partial class ReservationRefundPolicyResultProperty : IUtf8JsonSerializable, IModelJsonSerializable<ReservationRefundPolicyResultProperty>
    {
        ...
        {
            Core.ModelSerializerHelper.ValidateFormat<ReservationRefundPolicyResultProperty>(this, options.Format);

            writer.WriteStartObject();
            if (Optional.IsDefined(ConsumedRefundsTotal))
            {
                writer.WritePropertyName("consumedRefundsTotal"u8);
                if (ConsumedRefundsTotal is null)
                {
                    writer.WriteNullValue();
                }
                else
                {
                    ((IModelJsonSerializable<PurchasePrice>)ConsumedRefundsTotal).Serialize(writer, options);
                }
            }
            if (Optional.IsDefined(MaxRefundLimit))
            {
                writer.WritePropertyName("maxRefundLimit"u8);
                if (MaxRefundLimit is null)
                {
                    writer.WriteNullValue();
                }
                else
                {
                    ((IModelJsonSerializable<PurchasePrice>)MaxRefundLimit).Serialize(writer, options);
                }
            }
            if (Optional.IsCollectionDefined(PolicyErrors))
            {
                ...
                foreach (var item in PolicyErrors)
                {
                    ...
                    {
                        ((IModelJsonSerializable<ReservationRefundPolicyError>)item).Serialize(writer, options);
                    }
                }
        ...
        /// <summary> Converts a <see cref="ReservationRefundPolicyResultProperty"/> into a <see cref="RequestContent"/>. </summary>
        /// <param name="model"> The <see cref="ReservationRefundPolicyResultProperty"/> to convert. </param>
        ...
        /// <summary> Converts a <see cref="Response"/> into a <see cref="ReservationRefundPolicyResultProperty"/>. </summary>
