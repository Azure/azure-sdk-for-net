...
namespace Azure.ResourceManager.ManagedNetwork.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<Scope>)this).Write(writer, ModelReaderWriterOptions.Wire);

        void IJsonModel<Scope>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            if ((options.Format != "W" || ((IPersistableModel<Scope>)this).GetWireFormat(options) != "J") && options.Format != "J")
            {
                throw new InvalidOperationException($"Must use 'J' format when calling the {nameof(IJsonModel<Scope>)} interface");
            }
        ...
        string IPersistableModel<Scope>.GetWireFormat(ModelReaderWriterOptions options) => "J";
    }
}
