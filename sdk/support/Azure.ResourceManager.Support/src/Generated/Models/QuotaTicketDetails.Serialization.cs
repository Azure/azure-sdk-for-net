...
namespace Azure.ResourceManager.Support.Models
{
    public partial class QuotaTicketDetails : IUtf8JsonSerializable, IJsonModel<QuotaTicketDetails>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(QuotaChangeRequestSubType))
            {
                builder.Append("  quotaChangeRequestSubType:");
                if (QuotaChangeRequestSubType.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{QuotaChangeRequestSubType}'''");
                }
                else
                {
                    builder.AppendLine($" '{QuotaChangeRequestSubType}'");
            ...
            if (Optional.IsDefined(QuotaChangeRequestVersion))
            {
                builder.Append("  quotaChangeRequestVersion:");
                if (QuotaChangeRequestVersion.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{QuotaChangeRequestVersion}'''");
                }
                else
                {
                    builder.AppendLine($" '{QuotaChangeRequestVersion}'");
            ...
            if (Optional.IsCollectionDefined(QuotaChangeRequests))
            {
                if (QuotaChangeRequests.Any())
                {
                    builder.Append("  quotaChangeRequests:");
                    builder.AppendLine(" [");
                    foreach (var item in QuotaChangeRequests)
                    {
