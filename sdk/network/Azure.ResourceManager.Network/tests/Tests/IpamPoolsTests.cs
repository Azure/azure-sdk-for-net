...
namespace Azure.ResourceManager.Network.Tests
{
    public class IpamPoolsTests : NetworkServiceClientTestBase
    {
        ...
        {
            string staticCidrName = SessionRecording.GenerateAssetName("staticCidr-");
            var staticCidr = await _ipamPool.CreateStaticCidrAsync(staticCidrName, new List<string>() { "10.0.0.0/24" });
            Assert.IsNotNull(staticCidr.Data);
            Assert.AreEqual(staticCidrName, staticCidr.Data.Name);

            await _ipamPool.DeleteStaticCidrAsync(staticCidr);
