...
namespace Azure.ResourceManager.Network.Tests
{
    internal class BastionHostCollectionTests : NetworkServiceClientTestBase
    {
        ...
        public async Task GlobalSetUp()
        {
            ...
            var subnetLro = await vnet.GetSubnets().CreateOrUpdateAsync(WaitUntil.Completed, "AzureBastionSubnet", subnetData, System.Threading.CancellationToken.None);
            ...
            var ipLro = await rg.GetPublicIPAddresses().CreateOrUpdateAsync(WaitUntil.Completed, SessionRecording.GenerateAssetName("ip-"), ipData, cancellationToken: System.Threading.CancellationToken.None);
        ...
        public async Task TestTearDown()
        {
            if (await _resourceGroup.GetBastionHosts().ExistsAsync(_bastionName))
            {
                ...
                await bastion.DeleteAsync(WaitUntil.Completed, System.Threading.CancellationToken.None);
            }
        }
        ...
        {
            BastionHostResource bastionHost = await CreateBastionHost(_bastionName);
            await bastionHost.DeleteAsync(WaitUntil.Completed, System.Threading.CancellationToken.None);
            List<BastionHostResource> bastionList = await _resourceGroup.GetBastionHosts().GetAllAsync().ToEnumerableAsync();
