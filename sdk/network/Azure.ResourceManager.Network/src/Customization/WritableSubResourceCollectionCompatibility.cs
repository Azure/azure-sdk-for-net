...
namespace Azure.ResourceManager.Network
{
    internal static class WritableSubResourceCollectionCompatibility
    {
        ...
        public static IList<WritableSubResource> AsList(IList<SubscriptionId> source) => source is null ? default : new SubscriptionIdList(source);
        ...
        public static string FormatUri(Uri value) => value?.AbsoluteUri;
        public static BinaryData ParseBinaryData(string value) => value is null ? default : BinaryData.FromString(value);
        public static string FormatBinaryData(BinaryData value) => value?.ToString();
        ...
        private static WritableSubResource ToWritable(SubscriptionId value) => value is null ? default : new WritableSubResource { Id = value.Id };
        private static SubscriptionId ToSubscription(WritableSubResource value) => value is null ? default : new SubscriptionId { Id = value.Id };
        ...
        private sealed class SubscriptionIdList : IList<WritableSubResource>
        {
            private readonly IList<SubscriptionId> _source;
            public SubscriptionIdList(IList<SubscriptionId> source) => _source = source;
            public int Count => _source.Count;
            public bool IsReadOnly => _source.IsReadOnly;
            public WritableSubResource this[int index] { get => ToWritable(_source[index]); set => _source[index] = ToSubscription(value); }
            public void Add(WritableSubResource item) => _source.Add(ToSubscription(item));
            public void Clear() => _source.Clear();
            public bool Contains(WritableSubResource item) => IndexOf(item) >= 0;
            public void CopyTo(WritableSubResource[] array, int arrayIndex)
            {
            ...
            public IEnumerator<WritableSubResource> GetEnumerator()
            {
                foreach (SubscriptionId item in _source)
                {
                    yield return ToWritable(item);
                }
            }
            ...
            public void Insert(int index, WritableSubResource item) => _source.Insert(index, ToSubscription(item));
            public bool Remove(WritableSubResource item)
            {
                int index = IndexOf(item);
                if (index < 0)
