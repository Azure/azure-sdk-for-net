// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.Network;

namespace Azure.ResourceManager.Network.Models
{
    /// <summary> Properties of the rule collection group draft. </summary>
    internal partial class FirewallPolicyRuleCollectionGroupDraftProperties
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="FirewallPolicyRuleCollectionGroupDraftProperties"/>. </summary>
        public FirewallPolicyRuleCollectionGroupDraftProperties()
        {
            RuleCollections = new ChangeTrackingList<FirewallPolicyRuleCollectionInfo>();
        }

        /// <summary> Initializes a new instance of <see cref="FirewallPolicyRuleCollectionGroupDraftProperties"/>. </summary>
        /// <param name="size"> A read-only string that represents the size of the FirewallPolicyRuleCollectionGroupProperties in MB. (ex 1.2MB). </param>
        /// <param name="priority"> Priority of the Firewall Policy Rule Collection Group resource. </param>
        /// <param name="ruleCollections"> Group of Firewall Policy rule collections. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal FirewallPolicyRuleCollectionGroupDraftProperties(string size, int? priority, IList<FirewallPolicyRuleCollectionInfo> ruleCollections, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Size = size;
            Priority = priority;
            RuleCollections = ruleCollections;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> A read-only string that represents the size of the FirewallPolicyRuleCollectionGroupProperties in MB. (ex 1.2MB). </summary>
        [WirePath("size")]
        public string Size { get; }

        /// <summary> Priority of the Firewall Policy Rule Collection Group resource. </summary>
        [WirePath("priority")]
        public int? Priority { get; set; }

        /// <summary> Group of Firewall Policy rule collections. </summary>
        [WirePath("ruleCollections")]
        public IList<FirewallPolicyRuleCollectionInfo> RuleCollections { get; } = new ChangeTrackingList<FirewallPolicyRuleCollectionInfo>();
    }
}
