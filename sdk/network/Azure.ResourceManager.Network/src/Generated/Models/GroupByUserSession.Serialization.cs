...
namespace Azure.ResourceManager.Network.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<GroupByUserSession>)this).Write(writer, ModelSerializationExtensions.WireOptions);
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(GroupByVariables), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("  groupByVariables: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsCollectionDefined(GroupByVariables))
                {
                    if (GroupByVariables.Any())
                    {
                        builder.Append("  groupByVariables: ");
                        builder.AppendLine("[");
                        foreach (var item in GroupByVariables)
                        {
                            BicepSerializationHelpers.AppendChildObject(builder, item, options, 4, true, "  groupByVariables: ");
                        }
                        builder.AppendLine("  ]");
                    }
                }
