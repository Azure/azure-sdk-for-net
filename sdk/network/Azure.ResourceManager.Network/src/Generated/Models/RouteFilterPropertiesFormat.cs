// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.Network;

namespace Azure.ResourceManager.Network.Models
{
    /// <summary> Route Filter Resource. </summary>
    internal partial class RouteFilterPropertiesFormat
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="RouteFilterPropertiesFormat"/>. </summary>
        public RouteFilterPropertiesFormat()
        {
            Rules = new ChangeTrackingList<RouteFilterRule>();
            Peerings = new ChangeTrackingList<ExpressRouteCircuitPeeringData>();
            Ipv6Peerings = new ChangeTrackingList<ExpressRouteCircuitPeeringData>();
        }

        /// <summary> Initializes a new instance of <see cref="RouteFilterPropertiesFormat"/>. </summary>
        /// <param name="rules"> Collection of RouteFilterRules contained within a route filter. </param>
        /// <param name="peerings"> A collection of references to express route circuit peerings. </param>
        /// <param name="ipv6Peerings"> A collection of references to express route circuit ipv6 peerings. </param>
        /// <param name="provisioningState"> The provisioning state of the route filter resource. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal RouteFilterPropertiesFormat(IList<RouteFilterRule> rules, IReadOnlyList<ExpressRouteCircuitPeeringData> peerings, IReadOnlyList<ExpressRouteCircuitPeeringData> ipv6Peerings, NetworkProvisioningState? provisioningState, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Rules = rules;
            Peerings = peerings;
            Ipv6Peerings = ipv6Peerings;
            ProvisioningState = provisioningState;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Collection of RouteFilterRules contained within a route filter. </summary>
        public IList<RouteFilterRule> Rules { get; } = new ChangeTrackingList<RouteFilterRule>();

        /// <summary> A collection of references to express route circuit peerings. </summary>
        public IReadOnlyList<ExpressRouteCircuitPeeringData> Peerings { get; } = new ChangeTrackingList<ExpressRouteCircuitPeeringData>();

        /// <summary> A collection of references to express route circuit ipv6 peerings. </summary>
        public IReadOnlyList<ExpressRouteCircuitPeeringData> Ipv6Peerings { get; } = new ChangeTrackingList<ExpressRouteCircuitPeeringData>();

        /// <summary> The provisioning state of the route filter resource. </summary>
        public NetworkProvisioningState? ProvisioningState { get; }
    }
}
