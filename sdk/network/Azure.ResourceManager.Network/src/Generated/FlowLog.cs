// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.ResourceManager.Core;

namespace Azure.ResourceManager.Network
{
    /// <summary> A Class representing a FlowLog along with the instance operations that can be performed on it. </summary>
    public class FlowLog : FlowLogOperations
    {
        /// <summary> Initializes a new instance of the <see cref = "FlowLog"/> class for mocking. </summary>
        protected FlowLog() : base()
        {
        }

        /// <summary> Initializes a new instance of the <see cref = "FlowLog"/> class. </summary>
        /// <param name="options"> The client parameters to use in these operations. </param>
        /// <param name="resource"> The resource that is the target of operations. </param>
        internal FlowLog(ResourceOperations options, FlowLogData resource) : base(options, resource.Id)
        {
            Data = resource;
        }

        /// <summary> Gets or sets the FlowLogData. </summary>
        public virtual FlowLogData Data { get; private set; }
    }
}
