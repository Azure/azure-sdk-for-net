// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.Core;
using Azure.ResourceManager.Network.Models;

namespace Azure.ResourceManager.Network
{
    /// <summary> Pool of backend IP addresses. </summary>
    public partial class BackendAddressPoolData : SubResourceModel
    {
        /// <summary> Initializes a new instance of <see cref="BackendAddressPoolData"/>. </summary>
        public BackendAddressPoolData()
        {
        }

        /// <summary> Initializes a new instance of <see cref="BackendAddressPoolData"/>. </summary>
        /// <param name="id"> Resource ID. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="name"> Name of the resource. </param>
        /// <param name="type"> Resource type. </param>
        /// <param name="properties"> Properties of load balancer backend address pool. </param>
        /// <param name="eTag"> A unique read-only string that changes whenever the resource is updated. </param>
        internal BackendAddressPoolData(ResourceIdentifier id, IDictionary<string, BinaryData> additionalBinaryDataProperties, string name, string @type, BackendAddressPoolPropertiesFormat properties, string eTag) : base(id, additionalBinaryDataProperties, name, @type)
        {
            Properties = properties;
            ETag = eTag;
        }

        /// <summary> Properties of load balancer backend address pool. </summary>
        internal BackendAddressPoolPropertiesFormat Properties { get; set; }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public string ETag { get; }

        /// <summary> The location of the backend address pool. </summary>
        public string Location
        {
            get
            {
                return Properties is null ? default : Properties.Location;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                Properties.Location = value;
            }
        }

        /// <summary> An array of gateway load balancer tunnel interfaces. </summary>
        public IList<GatewayLoadBalancerTunnelInterface> TunnelInterfaces
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.TunnelInterfaces;
            }
        }

        /// <summary> An array of backend addresses. </summary>
        public IList<LoadBalancerBackendAddress> LoadBalancerBackendAddresses
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.LoadBalancerBackendAddresses;
            }
        }

        /// <summary> An array of references to IP addresses defined in network interfaces. </summary>
        public IReadOnlyList<NetworkInterfaceIPConfigurationData> BackendIPConfigurations
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.BackendIPConfigurations;
            }
        }

        /// <summary> An array of references to load balancing rules that use this backend address pool. </summary>
        public IReadOnlyList<NetworkSubResource> LoadBalancingRules
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.LoadBalancingRules;
            }
        }

        /// <summary> An array of references to outbound rules that use this backend address pool. </summary>
        public IReadOnlyList<NetworkSubResource> OutboundRules
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.OutboundRules;
            }
        }

        /// <summary> An array of references to inbound NAT rules that use this backend address pool. </summary>
        public IReadOnlyList<NetworkSubResource> InboundNatRules
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                return Properties.InboundNatRules;
            }
        }

        /// <summary> The provisioning state of the backend address pool resource. </summary>
        public NetworkProvisioningState? ProvisioningState
        {
            get
            {
                return Properties is null ? default : Properties.ProvisioningState;
            }
        }

        /// <summary> Amount of seconds Load Balancer waits for before sending RESET to client and backend address. </summary>
        public int? DrainPeriodInSeconds
        {
            get
            {
                return Properties is null ? default : Properties.DrainPeriodInSeconds;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                Properties.DrainPeriodInSeconds = value;
            }
        }

        /// <summary> Backend address synchronous mode for the backend pool. </summary>
        public SyncMode? SyncMode
        {
            get
            {
                return Properties is null ? default : Properties.SyncMode;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                Properties.SyncMode = value;
            }
        }

        /// <summary> Resource ID. </summary>
        public ResourceIdentifier OutboundRuleId
        {
            get
            {
                return Properties is null ? default : Properties.OutboundRuleId;
            }
        }

        /// <summary> Resource ID. </summary>
        public ResourceIdentifier VirtualNetworkId
        {
            get
            {
                return Properties is null ? default : Properties.VirtualNetworkId;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new BackendAddressPoolPropertiesFormat();
                }
                Properties.VirtualNetworkId = value;
            }
        }
    }
}
