// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.ResourceManager.Core;

namespace Azure.ResourceManager.Network
{
    /// <summary> A Class representing a RouteTable along with the instance operations that can be performed on it. </summary>
    public class RouteTable : RouteTableOperations
    {
        /// <summary> Initializes a new instance of the <see cref = "RouteTable"/> class for mocking. </summary>
        protected RouteTable() : base()
        {
        }

        /// <summary> Initializes a new instance of the <see cref = "RouteTable"/> class. </summary>
        /// <param name="options"> The client parameters to use in these operations. </param>
        /// <param name="resource"> The resource that is the target of operations. </param>
        internal RouteTable(ResourceOperations options, RouteTableData resource) : base(options, resource.Id)
        {
            Data = resource;
        }

        /// <summary> Gets or sets the RouteTableData. </summary>
        public virtual RouteTableData Data { get; private set; }
    }
}
