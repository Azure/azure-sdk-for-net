// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.Network.Models;

namespace Azure.ResourceManager.Network
{
    /// <summary> The IpGroups resource information. </summary>
    public partial class IpGroupData : Resource
    {
        /// <summary> Initializes a new instance of <see cref="IpGroupData"/>. </summary>
        public IpGroupData()
        {
        }

        /// <summary> Initializes a new instance of <see cref="IpGroupData"/>. </summary>
        /// <param name="id"> Resource ID. </param>
        /// <param name="name"> Resource name. </param>
        /// <param name="type"> Resource type. </param>
        /// <param name="location"> Resource location. </param>
        /// <param name="tags"> Resource tags. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="properties"> Properties of the IpGroups. </param>
        /// <param name="eTag"> A unique read-only string that changes whenever the resource is updated. </param>
        internal IpGroupData(string id, string name, string @type, string location, IDictionary<string, string> tags, IDictionary<string, BinaryData> additionalBinaryDataProperties, IpGroupPropertiesFormat properties, string eTag) : base(id, name, @type, location, tags, additionalBinaryDataProperties)
        {
            Properties = properties;
            ETag = eTag;
        }

        /// <summary> Properties of the IpGroups. </summary>
        internal IpGroupPropertiesFormat Properties { get; set; }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public string ETag { get; }

        /// <summary> The provisioning state of the IpGroups resource. </summary>
        public NetworkProvisioningState? ProvisioningState
        {
            get
            {
                return Properties is null ? default : Properties.ProvisioningState;
            }
        }

        /// <summary> IpAddresses/IpAddressPrefixes in the IpGroups resource. </summary>
        public IList<string> IpAddresses
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new IpGroupPropertiesFormat();
                }
                return Properties.IpAddresses;
            }
        }

        /// <summary> List of references to Firewall resources that this IpGroups is associated with. </summary>
        public IReadOnlyList<SubResource> Firewalls
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new IpGroupPropertiesFormat();
                }
                return Properties.Firewalls;
            }
        }

        /// <summary> List of references to Firewall Policies resources that this IpGroups is associated with. </summary>
        public IReadOnlyList<SubResource> FirewallPolicies
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new IpGroupPropertiesFormat();
                }
                return Properties.FirewallPolicies;
            }
        }
    }
}
