// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.Core;
using Azure.ResourceManager.Network.Models;

namespace Azure.ResourceManager.Network
{
    /// <summary> Differentiated Services Code Point configuration for any given network interface. </summary>
    public partial class DscpConfigurationData : NetworkTrackedResourceData
    {
        /// <summary> Initializes a new instance of <see cref="DscpConfigurationData"/>. </summary>
        public DscpConfigurationData()
        {
        }

        /// <summary> Initializes a new instance of <see cref="DscpConfigurationData"/>. </summary>
        /// <param name="id"> Resource ID. </param>
        /// <param name="name"> Resource name. </param>
        /// <param name="type"> Resource type. </param>
        /// <param name="location"> Resource location. </param>
        /// <param name="tags"> Resource tags. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="properties"> Properties of the network interface. </param>
        /// <param name="eTag"> A unique read-only string that changes whenever the resource is updated. </param>
        internal DscpConfigurationData(ResourceIdentifier id, string name, string @type, AzureLocation? location, IDictionary<string, string> tags, IDictionary<string, BinaryData> additionalBinaryDataProperties, DscpConfigurationPropertiesFormat properties, string eTag) : base(id, name, @type, location, tags, additionalBinaryDataProperties)
        {
            Properties = properties;
            ETag = eTag;
        }

        /// <summary> Properties of the network interface. </summary>
        internal DscpConfigurationPropertiesFormat Properties { get; set; }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public string ETag { get; }

        /// <summary> List of markings to be used in the configuration. </summary>
        public IList<int> Markings
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.Markings;
            }
        }

        /// <summary> Source IP ranges. </summary>
        public IList<QosIpRange> SourceIpRanges
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.SourceIpRanges;
            }
        }

        /// <summary> Destination IP ranges. </summary>
        public IList<QosIpRange> DestinationIpRanges
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.DestinationIpRanges;
            }
        }

        /// <summary> Sources port ranges. </summary>
        public IList<QosPortRange> SourcePortRanges
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.SourcePortRanges;
            }
        }

        /// <summary> Destination port ranges. </summary>
        public IList<QosPortRange> DestinationPortRanges
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.DestinationPortRanges;
            }
        }

        /// <summary> RNM supported protocol types. </summary>
        public ProtocolType? Protocol
        {
            get
            {
                return Properties is null ? default : Properties.Protocol;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                Properties.Protocol = value;
            }
        }

        /// <summary> QoS object definitions. </summary>
        public IList<QosDefinition> QosDefinitionCollection
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.QosDefinitionCollection;
            }
        }

        /// <summary> Qos Collection ID generated by RNM. </summary>
        public string QosCollectionId
        {
            get
            {
                return Properties is null ? default : Properties.QosCollectionId;
            }
        }

        /// <summary> Associated Network Interfaces to the DSCP Configuration. </summary>
        public IReadOnlyList<NetworkInterfaceData> AssociatedNetworkInterfaces
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new DscpConfigurationPropertiesFormat();
                }
                return Properties.AssociatedNetworkInterfaces;
            }
        }

        /// <summary> The resource GUID property of the DSCP Configuration resource. </summary>
        public string ResourceGuid
        {
            get
            {
                return Properties is null ? default : Properties.ResourceGuid;
            }
        }

        /// <summary> The provisioning state of the DSCP Configuration resource. </summary>
        public NetworkProvisioningState? ProvisioningState
        {
            get
            {
                return Properties is null ? default : Properties.ProvisioningState;
            }
        }
    }
}
