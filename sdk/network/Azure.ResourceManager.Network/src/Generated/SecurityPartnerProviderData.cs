// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure;
using Azure.ResourceManager.Network.Models;

namespace Azure.ResourceManager.Network
{
    /// <summary> Security Partner Provider resource. </summary>
    public partial class SecurityPartnerProviderData : Resource
    {
        /// <summary> Initializes a new instance of <see cref="SecurityPartnerProviderData"/>. </summary>
        public SecurityPartnerProviderData()
        {
        }

        /// <summary> Initializes a new instance of <see cref="SecurityPartnerProviderData"/>. </summary>
        /// <param name="id"> Resource ID. </param>
        /// <param name="name"> Resource name. </param>
        /// <param name="type"> Resource type. </param>
        /// <param name="location"> Resource location. </param>
        /// <param name="tags"> Resource tags. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="properties"> Properties of the Security Partner Provider. </param>
        /// <param name="eTag"> A unique read-only string that changes whenever the resource is updated. </param>
        internal SecurityPartnerProviderData(string id, string name, string @type, string location, IDictionary<string, string> tags, IDictionary<string, BinaryData> additionalBinaryDataProperties, SecurityPartnerProviderPropertiesFormat properties, ETag? eTag) : base(id, name, @type, location, tags, additionalBinaryDataProperties)
        {
            Properties = properties;
            ETag = eTag;
        }

        /// <summary> Properties of the Security Partner Provider. </summary>
        internal SecurityPartnerProviderPropertiesFormat Properties { get; set; }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public ETag? ETag { get; }

        /// <summary> The provisioning state of the Security Partner Provider resource. </summary>
        public NetworkProvisioningState? ProvisioningState
        {
            get
            {
                return Properties is null ? default : Properties.ProvisioningState;
            }
        }

        /// <summary> The security provider name. </summary>
        public SecurityProviderName? SecurityProviderName
        {
            get
            {
                return Properties is null ? default : Properties.SecurityProviderName;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new SecurityPartnerProviderPropertiesFormat();
                }
                Properties.SecurityProviderName = value;
            }
        }

        /// <summary> The connection status with the Security Partner Provider. </summary>
        public SecurityPartnerProviderConnectionStatus? ConnectionStatus
        {
            get
            {
                return Properties is null ? default : Properties.ConnectionStatus;
            }
        }

        /// <summary> Resource ID. </summary>
        public string VirtualHubId
        {
            get
            {
                return Properties is null ? default : Properties.VirtualHubId;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new SecurityPartnerProviderPropertiesFormat();
                }
                Properties.VirtualHubId = value;
            }
        }
    }
}
