...
#pragma warning restore SA1649 // File name should match first type name
    {
        ...
        internal NetworkArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, Request request, Response response, OperationFinalStateVia finalStateVia)
        {
            ...
            _operation = new OperationInternal(clientDiagnostics, nextLinkOperation, response, "NetworkArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }

        internal NetworkArmOperation(ClientDiagnostics clientDiagnostics, HttpPipeline pipeline, string id)
        {
            var nextLinkOperation = NextLinkOperationImplementation.Create(pipeline, id, out string finalResponse);
            _operation = OperationInternal.Create(clientDiagnostics, nextLinkOperation, finalResponse, "NetworkArmOperation", fallbackStrategy: new ExponentialDelayStrategy());
        }
