// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure;
using Azure.ResourceManager.Network.Models;

namespace Azure.ResourceManager.Network
{
    /// <summary> VpnServerConfiguration Resource. </summary>
    public partial class VpnServerConfigurationData : TrackedResourceWithSettableName
    {
        /// <summary> Initializes a new instance of <see cref="VpnServerConfigurationData"/>. </summary>
        public VpnServerConfigurationData()
        {
        }

        /// <summary> Initializes a new instance of <see cref="VpnServerConfigurationData"/>. </summary>
        /// <param name="id"> Resource ID. </param>
        /// <param name="name"> Resource name. </param>
        /// <param name="type"> Resource type. </param>
        /// <param name="location"> Resource location. </param>
        /// <param name="tags"> Resource tags. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        /// <param name="properties"> Properties of the P2SVpnServer configuration. </param>
        /// <param name="eTag"> A unique read-only string that changes whenever the resource is updated. </param>
        internal VpnServerConfigurationData(string id, string name, string @type, string location, IDictionary<string, string> tags, IDictionary<string, BinaryData> additionalBinaryDataProperties, VpnServerConfigurationProperties properties, ETag? eTag) : base(id, name, @type, location, tags, additionalBinaryDataProperties)
        {
            Properties = properties;
            ETag = eTag;
        }

        /// <summary> Properties of the P2SVpnServer configuration. </summary>
        internal VpnServerConfigurationProperties Properties { get; set; }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public ETag? ETag { get; }

        /// <summary> The name of the VpnServerConfiguration that is unique within a resource group. </summary>
        public string Name
        {
            get
            {
                return Properties is null ? default : Properties.Name;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                Properties.Name = value;
            }
        }

        /// <summary> VPN protocols for the VpnServerConfiguration. </summary>
        public IList<VpnGatewayTunnelingProtocol> VpnProtocols
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.VpnProtocols;
            }
        }

        /// <summary> VPN authentication types for the VpnServerConfiguration. </summary>
        public IList<VpnAuthenticationType> VpnAuthenticationTypes
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.VpnAuthenticationTypes;
            }
        }

        /// <summary> VPN client root certificate of VpnServerConfiguration. </summary>
        public IList<VpnServerConfigVpnClientRootCertificate> VpnClientRootCertificates
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.VpnClientRootCertificates;
            }
        }

        /// <summary> VPN client revoked certificate of VpnServerConfiguration. </summary>
        public IList<VpnServerConfigVpnClientRevokedCertificate> VpnClientRevokedCertificates
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.VpnClientRevokedCertificates;
            }
        }

        /// <summary> Radius Server root certificate of VpnServerConfiguration. </summary>
        public IList<VpnServerConfigRadiusServerRootCertificate> RadiusServerRootCertificates
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.RadiusServerRootCertificates;
            }
        }

        /// <summary> Radius client root certificate of VpnServerConfiguration. </summary>
        public IList<VpnServerConfigRadiusClientRootCertificate> RadiusClientRootCertificates
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.RadiusClientRootCertificates;
            }
        }

        /// <summary> VpnClientIpsecPolicies for VpnServerConfiguration. </summary>
        public IList<IPsecPolicy> VpnClientIpsecPolicies
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.VpnClientIpsecPolicies;
            }
        }

        /// <summary> The radius server address property of the VpnServerConfiguration resource for point to site client connection. </summary>
        public string RadiusServerAddress
        {
            get
            {
                return Properties is null ? default : Properties.RadiusServerAddress;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                Properties.RadiusServerAddress = value;
            }
        }

        /// <summary> The radius secret property of the VpnServerConfiguration resource for point to site client connection. We will no longer return radiusServerSecret in VpnServerConfiguration Create/Update/Get/List/UpdateTags APIs response. Please use VpnServerConfiguration ListRadiusSecrets API to fetch radius server secrets. </summary>
        public string RadiusServerSecret
        {
            get
            {
                return Properties is null ? default : Properties.RadiusServerSecret;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                Properties.RadiusServerSecret = value;
            }
        }

        /// <summary> Multiple Radius Server configuration for VpnServerConfiguration. </summary>
        public IList<RadiusServer> RadiusServers
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.RadiusServers;
            }
        }

        /// <summary> The set of aad vpn authentication parameters. </summary>
        public AadAuthenticationParameters AadAuthenticationParameters
        {
            get
            {
                return Properties is null ? default : Properties.AadAuthenticationParameters;
            }
            set
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                Properties.AadAuthenticationParameters = value;
            }
        }

        /// <summary> The provisioning state of the VpnServerConfiguration resource. Possible values are: 'Updating', 'Deleting', and 'Failed'. </summary>
        public string ProvisioningState
        {
            get
            {
                return Properties is null ? default : Properties.ProvisioningState;
            }
        }

        /// <summary> List of references to P2SVpnGateways. </summary>
        public IReadOnlyList<P2SVpnGatewayData> P2SVpnGateways
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.P2SVpnGateways;
            }
        }

        /// <summary> List of all VpnServerConfigurationPolicyGroups. </summary>
        public IList<VpnServerConfigurationPolicyGroupData> ConfigurationPolicyGroups
        {
            get
            {
                if (Properties is null)
                {
                    Properties = new VpnServerConfigurationProperties();
                }
                return Properties.ConfigurationPolicyGroups;
            }
        }

        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public string ETag
        {
            get
            {
                return Properties is null ? default : Properties.ETag;
            }
        }
    }
}
