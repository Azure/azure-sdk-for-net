// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure;

namespace Azure.Messaging.WebPubSub.Chat
{
    /// <summary>
    /// Represents a chat role.
    /// A role name must start with 'user.' or 'room.' prefix.
    /// A role must contain either user permissions or room permissions, but not both.
    /// </summary>
    public partial class ChatRole
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ChatRole"/>. </summary>
        /// <param name="permissions"> Permissions associated with the role. Do not mix user permissions and room permissions in one role. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="permissions"/> is null. </exception>
        public ChatRole(IEnumerable<string> permissions)
        {
            Argument.AssertNotNull(permissions, nameof(permissions));

            Permissions = permissions.ToList();
        }

        /// <summary> Initializes a new instance of <see cref="ChatRole"/>. </summary>
        /// <param name="name"> Role name. Must start with 'user.' or 'room.' prefix. </param>
        /// <param name="permissions"> Permissions associated with the role. Do not mix user permissions and room permissions in one role. </param>
        /// <param name="etag"> The entity tag for this resource. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ChatRole(string name, IList<string> permissions, ETag etag, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Name = name;
            Permissions = permissions;
            Etag = etag;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> Role name. Must start with 'user.' or 'room.' prefix. </summary>
        public string Name { get; }

        /// <summary> Permissions associated with the role. Do not mix user permissions and room permissions in one role. </summary>
        public IList<string> Permissions { get; }

        /// <summary> The entity tag for this resource. </summary>
        public ETag Etag { get; }
    }
}
