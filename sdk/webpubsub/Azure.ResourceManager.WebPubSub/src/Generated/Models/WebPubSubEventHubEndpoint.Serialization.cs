...
namespace Azure.ResourceManager.WebPubSub.Models
{
    public partial class WebPubSubEventHubEndpoint : IUtf8JsonSerializable, IJsonModel<WebPubSubEventHubEndpoint>
    {
        ...
        internal static WebPubSubEventHubEndpoint DeserializeWebPubSubEventHubEndpoint(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            {
                if (property.NameEquals("fullyQualifiedNamespace"u8))
                {
                    fullyQualifiedNamespace = property.Value.GetString();
                    continue;
                }
                ...
                {
                    type = new EventListenerEndpointDiscriminator(property.Value.GetString());
                    continue;
                }
                if (options.Format != "W")
                {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            hasPropertyOverride = hasObjectOverride && propertyOverrides.TryGetValue(nameof(FullyQualifiedNamespace), out propertyOverride);
            if (hasPropertyOverride)
            {
                builder.Append("  fullyQualifiedNamespace: ");
                builder.AppendLine(propertyOverride);
            }
            else
            {
                if (Optional.IsDefined(FullyQualifiedNamespace))
                {
                    builder.Append("  fullyQualifiedNamespace: ");
                    if (FullyQualifiedNamespace.Contains(Environment.NewLine))
                    {
                        builder.AppendLine("'''");
                        builder.AppendLine($"{FullyQualifiedNamespace}'''");
                    }
                    else
                    {
                        builder.AppendLine($"'{FullyQualifiedNamespace}'");
