...
namespace Microsoft.Azure.WebJobs.Extensions.WebPubSub.Tests
{
    public class JObjectTests
    {
        ...
        public void TestWebPubSubContext_UserEventStates_AllJsonElement()
        {
            ...
            StringAssert.Contains("JsonElement", ex.Message);
        }
        ...
        public void TestWebPubSubContext_UserEventStates_CollectionsInSync()
        {
            ...
            Assert.AreEqual(ctx.ConnectionStates["aKey"].GetRawText(), ctx.States["aKey"]);
            Assert.AreEqual(ctx.ConnectionStates["bKey"].GetRawText(), ctx.States["bKey"]);
            Assert.AreEqual(ctx.ConnectionStates["cKey"].GetRawText(), ctx.States["cKey"]);
        }
        ...
        private static HttpResponseMessage BuildResponse(string input, RequestType requestType, bool hasTestStates = false)
        {
            ...
            if (hasTestStates)
            {
                states =
                    ...
                    .EncodeConnectionStates().DecodeConnectionStates(); // Required now that we enforce JsonElement
            }
