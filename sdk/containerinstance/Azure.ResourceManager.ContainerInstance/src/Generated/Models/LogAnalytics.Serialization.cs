...
namespace Azure.ResourceManager.ContainerInstance.Models
{
    public partial class LogAnalytics : IUtf8JsonSerializable
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer)
        {
            ...
            if (Optional.IsCollectionDefined(WorkspaceResourceId))
            {
                writer.WritePropertyName("workspaceResourceId");
                writer.WriteStartObject();
                foreach (var item in WorkspaceResourceId)
                {
                    writer.WritePropertyName(item.Key);
                    writer.WriteStringValue(item.Value);
                }
        ...
        internal static LogAnalytics DeserializeLogAnalytics(JsonElement element)
        {
            ...
            Optional<IDictionary<string, string>> workspaceResourceId = default;
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("workspaceResourceId"))
                {
                    ...
                    workspaceResourceId = dictionary;
                    continue;
                }
            }
            return new LogAnalytics(workspaceId, workspaceKey, Optional.ToNullable(logType), Optional.ToDictionary(metadata), Optional.ToDictionary(workspaceResourceId));
        }
    }
}
