    ...
    public partial class ManagedClusterUpgradePolicy : IUtf8JsonSerializable, IJsonModel<ManagedClusterUpgradePolicy>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(HealthPolicy))
            {
                ...
                AppendChildObject(builder, HealthPolicy, options, 2, false);
            }

            if (Optional.IsDefined(DeltaHealthPolicy))
            {
                builder.Append("  deltaHealthPolicy:");
                AppendChildObject(builder, DeltaHealthPolicy, options, 2, false);
            }

            if (Optional.IsDefined(MonitoringPolicy))
            {
                builder.Append("  monitoringPolicy:");
                AppendChildObject(builder, MonitoringPolicy, options, 2, false);
            }

            if (Optional.IsDefined(UpgradeReplicaSetCheckTimeout))
            {
                builder.Append("  upgradeReplicaSetCheckTimeout:");
                if (UpgradeReplicaSetCheckTimeout.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{UpgradeReplicaSetCheckTimeout}'''");
                }
                else
                {
                    builder.AppendLine($" '{UpgradeReplicaSetCheckTimeout}'");
