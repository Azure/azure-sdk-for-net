...
#pragma warning restore AZC0012 // Avoid single word type names
    {
        ...
        private protected Output AddOutput(string name, string expression, bool isLiteral = false, bool isSecure = false, string? formatString = default, BicepKind kind = BicepKind.String)
        {
            string? formatted = formatString != null ? string.Format(formatString, $"{Name}.{expression}") : $"{Name}.{expression}";
            var result = new Output(name, formatted, Scope, this, isLiteral, isSecure, kind);
