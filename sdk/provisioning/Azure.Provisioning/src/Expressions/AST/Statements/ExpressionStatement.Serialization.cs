...
public partial class ExpressionStatement : IJsonModel<BicepStatement>
{
    void IJsonModel<BicepStatement>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
    {
        writer.WriteStartObject();
        writer.WriteString("kind", "expression");
        writer.WritePropertyName("value");
        ((IJsonModel<BicepExpression>)Expression).Write(writer, ModelReaderWriterOptions.Json);
    ...
    BinaryData IPersistableModel<BicepStatement>.Write(ModelReaderWriterOptions options)
    {
        ...
        switch (format)
        {
            ...
            default:
                throw new FormatException($"The model {nameof(ExpressionStatement)} does not support writing '{format}' format.");
        }
    }
