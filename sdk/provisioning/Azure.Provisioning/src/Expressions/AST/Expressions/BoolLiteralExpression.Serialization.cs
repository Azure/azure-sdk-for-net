...
public partial class BoolLiteralExpression : IJsonModel<BicepExpression>
{
    ...
    BicepExpression IJsonModel<BicepExpression>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
    {
        ...
        return DeserializeBoolLiteralExpression(doc.RootElement);
    }
    ...
    BinaryData IPersistableModel<BicepExpression>.Write(ModelReaderWriterOptions options)
    {
        ...
        switch (format)
        {
            ...
            default:
                throw new FormatException($"The model {nameof(BoolLiteralExpression)} does not support writing '{format}' format.");
        }
    }
    ...
    /// <inheritdoc/>
    public override bool Equals(BicepExpression? other) => other is BoolLiteralExpression b && Value == b.Value;
    /// <inheritdoc/>
    public override int GetHashCode() => typeof(BoolLiteralExpression).GetHashCode() ^ Value.GetHashCode();

    internal static BoolLiteralExpression DeserializeBoolLiteralExpression(JsonElement element)
    {
        return new BoolLiteralExpression(element.GetProperty("value").GetBoolean());
    }
}
