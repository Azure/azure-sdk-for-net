...
public partial class IdentifierExpression : IJsonModel<BicepExpression>
{
    ...
    BicepExpression IJsonModel<BicepExpression>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
    {
        ...
        return DeserializeIdentifierExpression(doc.RootElement);
    }
    ...
    BinaryData IPersistableModel<BicepExpression>.Write(ModelReaderWriterOptions options)
    {
        ...
        switch (format)
        {
            ...
            default:
                throw new FormatException($"The model {nameof(IdentifierExpression)} does not support writing '{format}' format.");
        }
    }
    ...
    {
        return new IdentifierExpression(element.GetProperty("id").GetString()!);
    }
}
