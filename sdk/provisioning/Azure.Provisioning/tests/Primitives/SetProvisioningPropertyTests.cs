...
namespace Azure.Provisioning.Tests.Primitives
{
    public class SetProvisioningPropertyTests
    {
        ...
        [Test]
        public async Task SetProvisioningProperty_WithListItem_ShouldNotThrow()
        {
            await using var test = new Trycep();
            ...
            test.Define(
                ctx =>
                {
                    ...
                    StorageAccount storageAccount = new("sa")
                    {
                        Kind = StorageKind.StorageV2,
                    ...
                    // This is the key part - iterating over list items and calling SetProvisioningProperty
                    // The test verifies that this doesn't throw an exception
                    foreach (KeyValuePair<string, IBicepValue> binding in blobService.ProvisionableProperties)
                    {
                        if (binding.Value is IList<IBicepValue> list)
                        {
                            foreach (IBicepValue item in list)
                            {
                                BicepValue<object> newValue = parameter2;
                                blobService.SetProvisioningProperty(item, newValue);
                            }
                        }
                    }
        ...
        [Test]
        public async Task SetProvisioningProperty_WithListItem_MultipleItems_ShouldNotThrow()
        {
            await using var test = new Trycep();
                ...
                ctx =>
                {
                    ...
                    BlobService blobService = new("blobService")
                    {
                        ...
                        [
                            new StorageCorsRule
                            {
                                AllowedOrigins = ["value1"]
                            },
                            new StorageCorsRule
                            {
                                AllowedOrigins = ["value2"]
                            }
                        ]
                    };
                    ...
                    // Update specific list items - the test verifies that this doesn't throw
                    foreach (KeyValuePair<string, IBicepValue> binding in blobService.ProvisionableProperties)
                    {
                        if (binding.Value is IList<IBicepValue> list)
                        {
                            ...
                            {
                                BicepValue<object> newValue1 = parameter1;
                                blobService.SetProvisioningProperty(list[0], newValue1);
                            }
                            if (list.Count > 1)
                            {
                                BicepValue<object> newValue2 = parameter2;
                                blobService.SetProvisioningProperty(list[1], newValue2);
                            }
                        }
                    }
