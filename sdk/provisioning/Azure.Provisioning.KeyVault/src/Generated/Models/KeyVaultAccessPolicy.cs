// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

using Azure.Provisioning;
using Azure.Provisioning.Primitives;
using System;

namespace Azure.Provisioning.KeyVault;

/// <summary>
/// An identity that have access to the key vault. All identities in the array
/// must use the same tenant ID as the key vault&apos;s tenant ID.
/// </summary>
public partial class KeyVaultAccessPolicy : ProvisioningConstruct
{
    /// <summary>
    /// The Azure Active Directory tenant ID that should be used for
    /// authenticating requests to the key vault.
    /// </summary>
    public BicepValue<Guid> TenantId { get => _tenantId; set => _tenantId.Assign(value); }
    private readonly BicepValue<Guid> _tenantId;

    /// <summary>
    /// The object ID of a user, service principal or security group in the
    /// Azure Active Directory tenant for the vault. The object ID must be
    /// unique for the list of access policies.
    /// </summary>
    public BicepValue<string> ObjectId { get => _objectId; set => _objectId.Assign(value); }
    private readonly BicepValue<string> _objectId;

    /// <summary>
    /// Application ID of the client making request on behalf of a principal.
    /// </summary>
    public BicepValue<Guid> ApplicationId { get => _applicationId; set => _applicationId.Assign(value); }
    private readonly BicepValue<Guid> _applicationId;

    /// <summary>
    /// Permissions the identity has for keys, secrets and certificates.
    /// </summary>
    public BicepValue<IdentityAccessPermissions> Permissions { get => _permissions; set => _permissions.Assign(value); }
    private readonly BicepValue<IdentityAccessPermissions> _permissions;

    /// <summary>
    /// Creates a new KeyVaultAccessPolicy.
    /// </summary>
    public KeyVaultAccessPolicy()
    {
        _tenantId = BicepValue<Guid>.DefineProperty(this, "TenantId", ["tenantId"]);
        _objectId = BicepValue<string>.DefineProperty(this, "ObjectId", ["objectId"]);
        _applicationId = BicepValue<Guid>.DefineProperty(this, "ApplicationId", ["applicationId"]);
        _permissions = BicepValue<IdentityAccessPermissions>.DefineProperty(this, "Permissions", ["permissions"]);
    }
}
