    ...
    public class RouteDirectionsTests : RouteClientLiveTestsBase
    {
        ...
        public async Task CanGetRouteDirection()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(result.Value.Routes, Has.Count.EqualTo(1));
            });
            Assert.Multiple(() =>
            {
                ...
                Assert.That(result.Value.Routes[0].Sections, Has.Count.EqualTo(1));
        ...
        public async Task CanGetDirectionsImmediateBatch()
        {
            ...
            Assert.That(result.Value.Results, Has.Count.EqualTo(2));
            Assert.Multiple(() =>
            {
                Assert.That(result.Value.Results[0].Routes, Has.Count.EqualTo(1));
                Assert.That(result.Value.Results[1].Routes, Has.Count.EqualTo(1));
                ...
                Assert.That(result.Value.Results[0].Routes[0].Legs, Has.Count.EqualTo(2));
            });
        }
