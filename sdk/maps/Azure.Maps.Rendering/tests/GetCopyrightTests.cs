    ...
    public class GetCopyrightTests : RenderingClientLiveTestsBase
    {
        ...
        public async Task CanGetCopyrightFromBoundingBox()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(copyright.Value.GeneralCopyrights, Has.Count.EqualTo(3));
                Assert.That(copyright.Value.RegionalCopyrights, Has.Count.EqualTo(2));
            });
            Assert.Multiple(() =>
            {
                ...
                Assert.That(copyright.Value.RegionalCopyrights[0].Copyrights, Has.Count.EqualTo(9));
                Assert.That(copyright.Value.RegionalCopyrights[1].Country.Iso3, Is.EqualTo("ONL"));
                Assert.That(copyright.Value.RegionalCopyrights[1].Country.Label, Is.EqualTo(string.Empty));
                Assert.That(copyright.Value.RegionalCopyrights[1].Copyrights, Has.Count.EqualTo(4));
            });
        }
        ...
        public async Task CanGetCopyrightForTile()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(copyright.Value.RegionalCopyrights[0].Copyrights, Has.Count.EqualTo(4));
