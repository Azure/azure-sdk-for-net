// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.Maps.Search.Models
{
    public partial class AddressRanges
    {
        internal static AddressRanges DeserializeAddressRanges(JsonElement element)
        {
            if (element.ValueKind == JsonValueKind.Null)
            {
                return null;
            }
            Core.Optional<string> rangeLeft = default;
            Core.Optional<string> rangeRight = default;
            Core.Optional<LatLongPairAbbreviated> @from = default;
            Core.Optional<LatLongPairAbbreviated> to = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("rangeLeft"u8))
                {
                    rangeLeft = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("rangeRight"u8))
                {
                    rangeRight = property.Value.GetString();
                    continue;
                }
                if (property.NameEquals("from"u8))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        continue;
                    }
                    @from = LatLongPairAbbreviated.DeserializeLatLongPairAbbreviated(property.Value);
                    continue;
                }
                if (property.NameEquals("to"u8))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        continue;
                    }
                    to = LatLongPairAbbreviated.DeserializeLatLongPairAbbreviated(property.Value);
                    continue;
                }
            }
            return new AddressRanges(rangeLeft.Value, rangeRight.Value, @from.Value, to.Value);
        }
    }
}
