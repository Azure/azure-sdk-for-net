// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Text.Json;
using Azure.Core;

namespace Azure.Maps.Search.Models
{
    public partial class CoordinatesPairAbbreviated
    {
        internal static CoordinatesPairAbbreviated DeserializeCoordinatesPairAbbreviated(JsonElement element)
        {
            Optional<double> lat = default;
            Optional<double> lon = default;
            foreach (var property in element.EnumerateObject())
            {
                if (property.NameEquals("lat"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    lat = property.Value.GetDouble();
                    continue;
                }
                if (property.NameEquals("lon"))
                {
                    if (property.Value.ValueKind == JsonValueKind.Null)
                    {
                        property.ThrowNonNullablePropertyIsNull();
                        continue;
                    }
                    lon = property.Value.GetDouble();
                    continue;
                }
            }
            return new CoordinatesPairAbbreviated(Optional.ToNullable(lat), Optional.ToNullable(lon));
        }
    }
}
