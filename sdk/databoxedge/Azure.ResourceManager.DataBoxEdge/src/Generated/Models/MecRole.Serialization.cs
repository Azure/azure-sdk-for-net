    ...
    public partial class MecRole : IUtf8JsonSerializable, IJsonModel<MecRole>
    {
        ...
        private BinaryData SerializeBicep(ModelReaderWriterOptions options)
        {
            ...
            if (Optional.IsDefined(ConnectionString))
            {
                ...
                AppendChildObject(builder, ConnectionString, options, 4, false);
            }

            if (Optional.IsDefined(ControllerEndpoint))
            {
                builder.Append("    controllerEndpoint:");
                if (ControllerEndpoint.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{ControllerEndpoint}'''");
                }
                else
                {
                    builder.AppendLine($" '{ControllerEndpoint}'");
            ...
            if (Optional.IsDefined(ResourceUniqueId))
            {
                builder.Append("    resourceUniqueId:");
                if (ResourceUniqueId.Contains(Environment.NewLine))
                {
                    builder.AppendLine(" '''");
                    builder.AppendLine($"{ResourceUniqueId}'''");
                }
                else
                {
                    builder.AppendLine($" '{ResourceUniqueId}'");
