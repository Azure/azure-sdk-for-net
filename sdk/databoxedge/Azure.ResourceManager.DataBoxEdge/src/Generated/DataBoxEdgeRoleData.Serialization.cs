...
namespace Azure.ResourceManager.DataBoxEdge
{
    ...
    public partial class DataBoxEdgeRoleData : ResourceData, IJsonModel<DataBoxEdgeRoleData>
    {
        ...
        /// <param name="dataBoxEdgeRoleData"> The <see cref="DataBoxEdgeRoleData"/> to serialize into <see cref="RequestContent"/>. </param>
        internal static RequestContent ToRequestContent(DataBoxEdgeRoleData dataBoxEdgeRoleData)
        {
            if (dataBoxEdgeRoleData == null)
            {
                return null;
            }
            return RequestContent.Create(dataBoxEdgeRoleData, ModelSerializationExtensions.WireOptions);
        }

        /// <param name="response"> The <see cref="Response"/> to deserialize the <see cref="DataBoxEdgeRoleData"/> from. </param>
        internal static DataBoxEdgeRoleData FromResponse(Response response)
        {
            using JsonDocument document = JsonDocument.Parse(response.Content, ModelSerializationExtensions.JsonDocumentOptions);
            return DeserializeDataBoxEdgeRoleData(document.RootElement, ModelSerializationExtensions.WireOptions);
        }
        ...
        /// <param name="options"> The client options for reading and writing models. </param>
        internal static DataBoxEdgeRoleData DeserializeDataBoxEdgeRoleData(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            if (element.TryGetProperty("kind"u8, out JsonElement discriminator))
            {
                switch (discriminator.GetString())
                {
                    case "CloudEdgeManagement":
                        return CloudEdgeManagementRole.DeserializeCloudEdgeManagementRole(element, options);
                    case "IOT":
                        return EdgeIotRole.DeserializeEdgeIotRole(element, options);
                    case "Kubernetes":
                        return EdgeKubernetesRole.DeserializeEdgeKubernetesRole(element, options);
                    case "MEC":
                        return MecRole.DeserializeMecRole(element, options);
                }
            }
            return UnknownDataBoxEdgeRole.DeserializeUnknownDataBoxEdgeRole(element, options);
        }
    }
}
