...
namespace Azure.AI.TextAnalytics.Samples
{
    ...
    public partial class TextAnalyticsSamples: SamplesBase<TextAnalyticsTestEnvironment>
    {
        ...
        public async Task HealthcareAsyncPagination()
        {
            ...
            HealthcareOperation healthOperation = await client.StartHealthcareBatchAsync(
                new List<string>() {
                    document,
                    document,
                    document,
            ...
            Debug.WriteLine($"Results of Azure Text Analytics \"Healthcare Async\"");
            Debug.WriteLine("");

            await foreach (DocumentHealthcareResult result in results)
            {
                Debug.WriteLine($"    Recognized the following {result.Entities.Count} healthcare entities:");

                foreach (HealthcareEntity entity in result.Entities)
                {
                    Debug.WriteLine($"    Entity: {entity.Text}");
                    Debug.WriteLine($"    Subcategory: {entity.Subcategory}");
                    Debug.WriteLine($"    Offset: {entity.Offset}");
                    Debug.WriteLine($"    Length: {entity.Length}");
                    Debug.WriteLine($"    IsNegated: {entity.IsNegated}");
                    Debug.WriteLine($"    Links:");

                    foreach (HealthcareEntityLink healthcareEntityLink in entity.Links)
                    {
                        Debug.WriteLine($"        ID: {healthcareEntityLink.Id}");
                        Debug.WriteLine($"        DataSource: {healthcareEntityLink.DataSource}");
                    }
                }
                Debug.WriteLine("");
            }
