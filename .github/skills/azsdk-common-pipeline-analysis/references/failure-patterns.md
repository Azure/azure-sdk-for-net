...
| Recording file path does not exist       | Missing test recording for a new/modified test     | Best effort: use the `az` CLI to check auth (try `az login`); if authenticated, run tests in record mode; else skip and report re-recording is required |
| Playback failure / ResourceNotFoundError | Recorded responses don't match current API calls   | Best effort: use the `az` CLI to check auth (try `az login`); if authenticated, re-record the session; else skip and report re-recording is required    |
