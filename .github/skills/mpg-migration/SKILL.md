...
7. **Verify resource-hierarchy parity with the previous GA SDK** before entering the build-fix loop. The previous GA DLL is restored automatically by ApiCompat from `<ApiCompatVersion>` in the .csproj.
   ...
   - Exit `2` → **class-name renames only**, structural hierarchy is intact. Record the renames in the migration status; defer the fix until Phase 2/3 via spec-side `@@clientName(<resource model>, "<NewName>", "csharp")` (preferred) or SDK custom code (`[CodeGenType("OldName")]` / partial classes). Re-run the comparator after each fix until exit `0`.
