...
**Out of scope (use the listed skill instead)**
- Authoring or modifying `.tsp` files → `azure-typespec-author`
- Migrating a library from Swagger to TypeSpec → `sdk-migration`
- Release / publish / changelog finalization → `check-package-readiness`
- Breaking-change review or `ApiCompatBaseline.txt` edits → never done here
...
5. **Never** edit `.tsp` files from this skill — defer to `azure-typespec-author`.
...
- `check-package-readiness` — Pre-release validation
