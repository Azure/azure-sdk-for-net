...
| `eng/centralpackagemanagement/Directory.Packages.props` | `<AzureGeneratorVersion>` | Generator NuGet dependency (used by `Azure.Generator.Management.csproj` via `<PackageReference Include="Azure.Generator" />`) |
...
2. **`eng/centralpackagemanagement/Directory.Packages.props`**: Change the `<AzureGeneratorVersion>` value.
...
This runs `dotnet build ./generator`. The `Azure.Generator.Management` project references the `Azure.Generator` NuGet package, whose version is controlled by `AzureGeneratorVersion` in `eng/centralpackagemanagement/Directory.Packages.props`.
...
- Ensure `package.json`, `package-lock.json`, `Directory.Packages.props`, and any regenerated test project files are included
...
| `eng/centralpackagemanagement/Directory.Packages.props` | NuGet version properties (AzureGeneratorVersion) |
