...
>
> **One exception**: `CannotChangeAttribute` errors for **WirePath path changes** (where the JSON path changed in the new API version, e.g. `properties.x` → `properties.nested.x`) may be placed in `ApiCompatBaseline.txt`. These reflect correct generator behavior for the new spec — the WirePath value must match the actual serialization path, and overriding it with custom code would produce incorrect serialization. All other error types must still be mitigated with custom code.
