...
| **CS1729/CS0029/CS1503** (type mismatch) | Sub-resource using `Read<>` template causes wrong REST client | Change `Read<>` to `ActionSync<>` with `@get` in spec. OR update custom code constructor calls to match new signature |
...
3. Not fixable with `@@clientName`/`@@access`/`@@markAsPageable`
