...
2. Run the trusted naming scanner:
   ...
   `OPTPARAM001` is automatically suppressed for *ambiguity-forced* shims: a binary-compatibility overload whose parameters would all be optional after restoring the baseline defaults, and which has a sibling in the same invocation group with at least one bindable parameter, all optional. Invocation groups keep instance and static methods separate. Extension receiver signatures are canonicalized for attributes, nullable annotations, receiver passing modifiers, formatting, and method generic-parameter names before grouping. Restoring the defaults makes the argument-less call applicable to both overloads with no tie-breaker under C# 12.6.4.3, so it fails with `CS0121`. A true zero-parameter sibling is not ambiguous and does not trigger suppression. Keeping the parameters required is the only variant that compiles, so do not ask for defaults to be restored on the suppressed shape. Neither `[Obsolete]` nor `[EditorBrowsable]` changes overload resolution, and `[OverloadResolutionPriority]` only helps consumers compiling with C# 13 or later.
