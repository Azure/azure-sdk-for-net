...
2. Run the trusted naming scanner:
   ...
   `OPTPARAM001` is automatically suppressed for *ambiguity-forced* shims: after restoring the baseline defaults, a call supplying only the member's shared required prefix would be applicable to both the shim and a longer sibling whose remaining parameters are optional. Invocation groups keep instance and static methods separate; same-namespace extension methods can group across static container classes. Extension receiver signatures are canonicalized for attributes, known reference-type nullable annotations (including arrays, constructed types, and reference-constrained type parameters), receiver passing modifiers, nested tuple element names, formatting, and method generic-parameter names. Nullable value types and differing generic constraints remain distinct. If an applicable sibling has exactly the required-prefix arity, it wins overload resolution and prevents suppression; otherwise two candidates requiring default substitution have no tie-breaker under C# 12.6.4.3 and the call fails with `CS0121`. Keeping the affected defaults removed is then the only variant that compiles. Neither `[Obsolete]` nor `[EditorBrowsable]` changes overload resolution, and `[OverloadResolutionPriority]` only helps consumers compiling with C# 13 or later.
