...
2. Run the trusted naming scanner:
   ...
   `OPTPARAM001` has one narrow, fail-closed suppression for an exact ambiguity shape: within the same declaring type and invocation kind (or the same namespace for extension methods), the receiver, generic `where` constraints, and required-prefix parameter types must match the API listing exactly; a longer sibling must accept that prefix with only optional or `params` parameters remaining and use the same normal or expanded applicability form as the restored member. Two expanded-form candidates must also have the same parameter count. Constructor initializers are ignored. Parameter type matching ignores only the `params` modifier for exact-arity normal-form comparison. An applicable exact-arity sibling prevents positional ambiguity, but does not prevent suppression when differing parameter names leave a named call ambiguous. Complex or textually different shapes remain findings for manual review rather than being guessed by this scanner. Neither `[Obsolete]` nor `[EditorBrowsable]` changes overload resolution, and `[OverloadResolutionPriority]` only helps consumers compiling with C# 13 or later.
