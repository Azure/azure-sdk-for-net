...
2. Run the trusted naming scanner:
   ...
   `OPTPARAM001` has one narrow, fail-closed suppression for an exact ambiguity shape: within the same declaring type and invocation kind, the receiver, generic constraints, and required-prefix parameter types must match the API listing exactly; a longer sibling must accept that prefix with only optional parameters remaining. An applicable exact-arity sibling prevents suppression because it wins overload resolution. Complex or textually different shapes remain findings for manual review rather than being guessed by this scanner. Neither `[Obsolete]` nor `[EditorBrowsable]` changes overload resolution, and `[OverloadResolutionPriority]` only helps consumers compiling with C# 13 or later.
