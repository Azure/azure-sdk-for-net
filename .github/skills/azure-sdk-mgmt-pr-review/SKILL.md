...
2. Run the trusted naming scanner:
   ...
   `OPTPARAM001` has one narrow, fail-closed suppression for an exact ambiguity shape: within the same declaring type and invocation kind (or the same namespace for extension methods), the receiver, generic `where` constraints, and required-prefix parameter types must match the API listing exactly; a longer sibling must accept that prefix in normal form with only optional parameters remaining. Constructor initializers are ignored. Parameter type matching ignores only the `params` modifier for exact-arity normal-form comparison; a candidate requiring expanded `params` form does not trigger suppression. An applicable exact-arity sibling prevents suppression because it wins overload resolution. Complex or textually different shapes remain findings for manual review rather than being guessed by this scanner. Neither `[Obsolete]` nor `[EditorBrowsable]` changes overload resolution, and `[OverloadResolutionPriority]` only helps consumers compiling with C# 13 or later.
