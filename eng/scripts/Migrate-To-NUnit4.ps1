...
Write-Host "Optimization: Processing all diagnostics in a single pass per project" -ForegroundColor Yellow
Write-Host ""
...
# Process all projects in parallel, applying ALL diagnostics at once
$results = $projectsToFormat | ForEach-Object -ThrottleLimit 8 -Parallel {
    ...
    $currentIdx = $using:currentProject
    
    # Progress reporting
    $percent = [math]::Round(($currentIdx / $totalProjects) * 100)
    Write-Host "  [$percent%] Processing: $relativePath" -ForegroundColor Gray
    ...
    try {
        ...
        # Add ALL diagnostic severities at once
        foreach ($diagnostic in $diagnostics) {
            "`ndotnet_diagnostic.$diagnostic.severity = warning" | Add-Content -Path $editorConfigPath
        }
        
        # Run dotnet format with ALL diagnostics at once
        $diagnosticsList = $diagnostics -join ','
        $output = & dotnet format analyzers "$csprojPath" --diagnostics $diagnosticsList --severity info --no-restore --verbosity quiet 2>&1
...
$stopwatch.Stop()
$projectsProcessed = ($results | Where-Object { $_ -eq $true }).Count
$projectsFailed = ($results | Where-Object { $_ -eq $false }).Count
