...
function Build-Emitter {
    param (
        ...
        if ($UnitTests) {
            # test the emitter
            Invoke-LoggedCommand "pnpm run prettier" -GroupOutput -ErrorAction Continue
            if ($LastExitCode) {
                $errors += "Prettier failed"
            }

            Invoke-LoggedCommand "pnpm run lint" -GroupOutput -ErrorAction Continue
            if ($LastExitCode) {
                $errors += "Lint failed"
            }

            Invoke-LoggedCommand "pnpm install @types/node --save-dev" -GroupOutput
