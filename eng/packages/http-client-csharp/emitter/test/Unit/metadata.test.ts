...
describe("Metadata generation tests", async () => {
  ...
  it("Reports diagnostic when metadata.json write fails", async () => {
    const runner = await createEmitterTestHost();
    const program = await typeSpecCompile(
      `
      @service(#{
    ...
    // writeFile should not have been called since mkdirp failed
    const metadataCalls = writeFileMock.mock.calls.filter((call: any) =>
      call[0].includes("metadata.json")
    );
    strictEqual(
      ...
      0,
      "writeFile should not be called when mkdirp fails"
    );

    // Should have reported a diagnostic instead of throwing
    const diagnostics = program.diagnostics.filter(
      (d: any) =>
        d.code ===
        "@azure-typespec/http-client-csharp/metadata-file-write-error"
    );
    strictEqual(
      diagnostics.length,
      1,
      "should report a metadata-file-write-error diagnostic"
    );
    ok(
      diagnostics[0].message.includes("Permission denied"),
      "diagnostic message should include the underlying error"
    );
  });
});
