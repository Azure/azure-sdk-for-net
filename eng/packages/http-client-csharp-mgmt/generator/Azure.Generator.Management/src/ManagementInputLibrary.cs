...
namespace Azure.Generator.Management
{
    ...
    public class ManagementInputLibrary : InputLibrary
    {
        ...
        private HashSet<string>? _customResourceRootModelIds;
        private HashSet<string>? _customResourceIntermediateModelIds;

        /// <summary>
        /// Gets the set of model IDs that are root models in custom resource hierarchies.
        /// These are the topmost base models (with no BaseModel) in the inheritance chain
        /// of custom resources, and they should inherit from ResourceData.
        /// </summary>
        private HashSet<string> CustomResourceRootModelIds => _customResourceRootModelIds ??= BuildCustomResourceRootModelIds();

        /// <summary>
        /// Gets the set of model IDs that are intermediate models in custom resource hierarchies.
        /// These models only have standard ARM properties and should be replaced by ResourceData/TrackedResourceData.
        /// </summary>
        private HashSet<string> CustomResourceIntermediateModelIds => _customResourceIntermediateModelIds ??= BuildCustomResourceIntermediateModelIds();

        private HashSet<string> BuildCustomResourceRootModelIds()
        {
            var result = new HashSet<string>();

            // For each custom resource, walk up the base model hierarchy to find the root
            foreach (var resource in ArmProviderSchema.Resources)
            {
                if (resource.IsCustomResource && resource.ResourceModel != null)
                {
                    // Walk up to find the root model (the one with no BaseModel)
                    var current = resource.ResourceModel;
                    while (current.BaseModel != null)
                    {
        ...
        private HashSet<string> BuildCustomResourceIntermediateModelIds()
        {
            var result = new HashSet<string>();

            // For each custom resource, collect all intermediate models in the hierarchy
            foreach (var resource in ArmProviderSchema.Resources)
            {
                if (resource.IsCustomResource && resource.ResourceModel != null)
                {
                    // Walk up the hierarchy, collecting intermediate models
                    var current = resource.ResourceModel.BaseModel;
                    while (current != null)
                    {
        ...
        /// <summary>
        /// Checks if a model is an intermediate model in a custom resource hierarchy
        /// that should be replaced by ResourceData or TrackedResourceData.
        /// </summary>
        internal bool TryGetCustomResourceReplacementType(InputModelType model, [MaybeNullWhen(false)] out Type replacementType)
        {
            replacementType = null;

            if (!CustomResourceIntermediateModelIds.Contains(model.CrossLanguageDefinitionId))
            {
                return false;
            }
        ...
        /// <summary>
        /// Checks if a model (that directly extends a replaced intermediate model) should inherit from ResourceData.
        /// </summary>
        internal bool TryGetCustomResourceBaseType(InputModelType model, [MaybeNullWhen(false)] out Type baseType)
        {
            baseType = null;

            // Check if the model's base is an intermediate model that will be replaced
            if (model.BaseModel == null)
            {
                return false;
            }

            if (!CustomResourceIntermediateModelIds.Contains(model.BaseModel.CrossLanguageDefinitionId))
            {
                // Also check if the base is a root model (which may be optimized away)
                if (!CustomResourceRootModelIds.Contains(model.BaseModel.CrossLanguageDefinitionId))
                {
                    return false;
                }
            }
