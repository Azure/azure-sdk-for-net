...
namespace Azure.Generator.Mgmt.Tests
{
    public class OperationContextTests
    {
        ...
        [TestCase]
        public void PopulateArguments_DuplicateSerializedName_PrefersExactParameterName()
        {
            var mapping = new ParameterContextMapping("providers", null);
            var registry = new ParameterContextRegistry(new List<ParameterContextMapping> { mapping });

            var requestParam = new ParameterProvider("providerNamespace", $"", typeof(string));
            requestParam.Update(wireInfo: new WireInformation(default, "providers"), location: ParameterLocation.Path);

            var providerNamespace = new ParameterProvider("providerNamespace", $"", typeof(string));
            providerNamespace.Update(wireInfo: new WireInformation(default, "providers"), location: ParameterLocation.Path);

            var provider = new ParameterProvider("provider", $"", typeof(string));
            provider.Update(wireInfo: new WireInformation(default, "providers"), location: ParameterLocation.Path);
            ...
            var arguments = registry.PopulateArguments(
                ...
                new List<ParameterProvider> { requestParam },
                contextVariable,
                new List<ParameterProvider> { providerNamespace, provider });

            Assert.AreEqual(1, arguments.Count);
            Assert.That(arguments[0].ToDisplayString(), Does.Contain("providerNamespace"));
        }

        [TestCase]
        public void PopulateArguments_DuplicateSerializedName_PrefersMatchingLocation()
        {
            var mapping = new ParameterContextMapping("scope", null);
            var registry = new ParameterContextRegistry(new List<ParameterContextMapping> { mapping });

            var requestParam = new ParameterProvider("scope", $"", typeof(string));
            requestParam.Update(wireInfo: new WireInformation(default, "scope"), location: ParameterLocation.Query);

            var syntheticScope = new ParameterProvider("scope", $"", typeof(ResourceIdentifier));
            syntheticScope.Update(wireInfo: new WireInformation(default, "scope"), location: ParameterLocation.Path);

            var queryScope = new ParameterProvider("scope", $"", CreateFixedEnumCSharpType("ScopeFilter", "TestNs", isNullable: false));
            queryScope.Update(wireInfo: new WireInformation(default, "scope"), location: ParameterLocation.Query);
            ...
            var arguments = registry.PopulateArguments(
                ...
                new List<ParameterProvider> { requestParam },
                contextVariable,
                new List<ParameterProvider> { syntheticScope, queryScope });
        ...
        [TestCase]
        public void PopulateArguments_UnmatchedSerializedName_ReturnsDefault()
        {
            var registry = new ParameterContextRegistry(new List<ParameterContextMapping>());

            var requestParam = new ParameterProvider("tagName", $"", typeof(string));
            requestParam.Update(wireInfo: new WireInformation(default, "tagName"), location: ParameterLocation.Query);

            var unrelatedParam = new ParameterProvider("key", $"", typeof(string));
            unrelatedParam.Update(wireInfo: new WireInformation(default, "key"), location: ParameterLocation.Query);
            ...
            var arguments = registry.PopulateArguments(
                ...
                new List<ParameterProvider> { requestParam },
                contextVariable,
                new List<ParameterProvider> { unrelatedParam });
        ...
        [TestCase]
        public void PopulateArguments_EmptySerializedName_ReturnsDefault()
        {
            var registry = new ParameterContextRegistry(new List<ParameterContextMapping>());

            var requestParam = new ParameterProvider("tags", $"", typeof(string));
            requestParam.Update(wireInfo: new WireInformation(default, string.Empty), location: ParameterLocation.Query);

            var unrelatedParam = new ParameterProvider("tags", $"", typeof(string));
            unrelatedParam.Update(wireInfo: new WireInformation(default, "tags"), location: ParameterLocation.Query);
