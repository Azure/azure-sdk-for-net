...
if ($null -eq $filter -or $filter -eq "Mgmt-TypeSpec-MultiService") {
    ...
    $mgmtTypespecMultiServiceTestProject = Join-Path $testProjectsLocalDir "Mgmt-TypeSpec-MultiService"

    Invoke (Get-Mgmt-TspCommand "$mgmtTypespecMultiServiceTestProject/client.tsp" $mgmtTypespecMultiServiceTestProject -debug:$Debug)
