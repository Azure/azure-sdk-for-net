// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using System.Linq;
using Azure.Generator.MgmtTypeSpec;
using Azure.ResourceManager.Resources.Models;

namespace Azure.Generator.MgmtTypeSpec.Models
{
    /// <summary> Paged collection of ZooAddress items. </summary>
    public partial class ZooAddressListListResult
    {
        /// <summary> Keeps track of any properties unknown to the library. </summary>
        private protected readonly IDictionary<string, BinaryData> _additionalBinaryDataProperties;

        /// <summary> Initializes a new instance of <see cref="ZooAddressListListResult"/>. </summary>
        /// <param name="value"> The ZooAddress items on this page. </param>
        internal ZooAddressListListResult(IEnumerable<SubResource> value)
        {
            Value = value.ToList();
        }

        /// <summary> Initializes a new instance of <see cref="ZooAddressListListResult"/>. </summary>
        /// <param name="value"> The ZooAddress items on this page. </param>
        /// <param name="nextLink"> The link to the next page of items. </param>
        /// <param name="additionalBinaryDataProperties"> Keeps track of any properties unknown to the library. </param>
        internal ZooAddressListListResult(IList<SubResource> value, Uri nextLink, IDictionary<string, BinaryData> additionalBinaryDataProperties)
        {
            Value = value;
            NextLink = nextLink;
            _additionalBinaryDataProperties = additionalBinaryDataProperties;
        }

        /// <summary> The ZooAddress items on this page. </summary>
        [WirePath("value")]
        public IList<SubResource> Value { get; }

        /// <summary> The link to the next page of items. </summary>
        [WirePath("nextLink")]
        public Uri NextLink { get; }
    }
}
