...
`,
  ...
  it("should assign non-resource list methods to resource by resourceModelId", () => {
    ...
    // (matched by resourceModelId). Non-resource methods can only be relocated as
    // Actions: any operation upstream classified as a real List would already be in
    // the resource's `lists` bucket and would never appear here.
  ...
  it("should assign non-resource list methods by type segment when prefix matching fails due to structural path length mismatch", () => {
    ...
    // as an Action: operation path ends with "configurationAssignments" which matches
    // the resource's type segment (second-to-last segment of the resource ID pattern).
    // Relocated non-resource methods are always Actions on the matched resource.
    const configMethods = resources[0].metadata.methods.filter(
      (m) => m.kind === ResourceOperationKind.Action
    );
    strictEqual(
