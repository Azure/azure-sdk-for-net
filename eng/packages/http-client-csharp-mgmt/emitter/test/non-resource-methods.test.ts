...
`,
  ...
  it("should detect non-resource methods on resource group scope", async () => {
    const program = await typeSpecCompile(
      `
/** Custom operations on resource group scope using ARM templates */
interface ResourceGroupOperations {
  /**
   * Analyzes resources in a resource group.
   */
  @autoRoute
  analyzeResourceGroup is ArmProviderActionSync<
    Request = AnalysisRequest,
    Response = AnalysisResponse,
    Scope = ResourceGroupActionScope
  >;
  
  /**
   * Exports resource group configuration.
   */
  @autoRoute
  exportResourceGroupConfig is ArmProviderActionSync<
    Request = void,
...
`,
    ...
    // Check first method (analyze)
    const analyzeMethod = nonResourceMethods.find((m: any) => 
      m.operationPath === "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ContosoProviderHub/analyze"
    );
    ok(analyzeMethod, "Should find analyze method");
    strictEqual(analyzeMethod.operationScope, ResourceScope.ResourceGroup);
    
    // Check second method (export)
    const exportMethod = nonResourceMethods.find((m: any) => 
      m.operationPath === "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ContosoProviderHub/export"
    );
    ok(exportMethod, "Should find export method");
    strictEqual(exportMethod.operationScope, ResourceScope.ResourceGroup);
  });
