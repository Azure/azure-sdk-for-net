...
`,
  ...
  it("validation emits no error diagnostics for well-formed resources", async () => {
    const program = await typeSpecCompile(
      `
/** Employee properties */
model EmployeeProperties {
...
`,
    ...
    // Verify no error-level diagnostics from our validations
    const validationDiagnostics = program.diagnostics.filter(
      (d) =>
        d.code ===
    ...
    strictEqual(
      ...
      0,
      "Well-formed resources should not produce any validation diagnostics"
    );
  });

  it("validation emits no error diagnostics for parent-child resources", async () => {
    const program = await typeSpecCompile(
      `
/** Parent properties */
model ParentProperties {
...
`,
    ...
    const resolvedSchema = resolveArmResources(program, sdkContext);
    ok(resolvedSchema);
    strictEqual(resolvedSchema.resources.length, 2);

    // Verify no validation diagnostics
    const validationDiagnostics = program.diagnostics.filter(
      (d) =>
        d.code ===
          "@azure-typespec/http-client-csharp-mgmt/invalid-resource-read-response" ||
    ...
    strictEqual(
      validationDiagnostics.length,
      0,
      "Parent-child resources should not produce any validation diagnostics"
    );
  });

  it("validation emits no error diagnostics for multi-scope resources", async () => {
    const program = await typeSpecCompile(
      `
/** Widget properties */
model WidgetProperties {
...
`,
    ...
    strictEqual(
      validationDiagnostics.length,
      0,
      "Multi-scope resources should not produce any validation diagnostics"
    );
  });
});
