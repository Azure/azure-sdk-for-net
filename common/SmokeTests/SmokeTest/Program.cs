...
void LogError(string message, ConsoleColor? color = default) => Console.Error.WriteLine(message);
...
void ProcessType(Type type)
{
    ...
    // constructor is found during the scan, take note for later use.
    ...
    try
    {
        ...
        if ((!type.ContainsGenericParameters) && (!type.IsAbstract) && (hasParameterlessConstructor))
        {
            try
            {
            ...
            {
                // Expected; this indicates that a type is not available on a platform.  For example,
                // Windows Principal-related types on Linux.
