...
using Microsoft.WindowsAzure.Services.ServiceBus.UnitTests.HttpTests;
...
namespace Microsoft.WindowsAzure.Services.ServiceBus.UnitTests.ServiceBusTests
{
    ...
    public class ManagementTests
    {
        ...
        public void CreateSubscriptionDuplicateName()
        {
            using (UniqueSubscription subscription = new UniqueSubscription())
            {
                Task<SubscriptionDescription> task = Service.CreateSubscriptionAsync(subscription.TopicName, subscription.SubscriptionName).AsTask();
        ...
        {
            Assert.ThrowsException<ArgumentNullException>(() => new ServiceBusClient(null, "user", "password"));
            Assert.ThrowsException<ArgumentException>(() => new ServiceBusClient("", "user", "password"));
            Assert.ThrowsException<ArgumentException>(() => new ServiceBusClient(" ", "user", "password"));
            Assert.ThrowsException<ArgumentNullException>(() => new ServiceBusClient("namespace", null, "password"));
