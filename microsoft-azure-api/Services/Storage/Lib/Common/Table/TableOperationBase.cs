// -----------------------------------------------------------------------------------------
// <copyright file="TableOperationBase.cs" company="Microsoft">
...
#if !COMMON       
        private EntityResolver retrieveResolver = DynamicEntityResolver;

        internal EntityResolver RetrieveResolver
        {
        ...
        internal Uri GenerateRequestURI(Uri baseUri, string tableName)
        {
            ...
            else
            {
                ...
                if (this.isTableEntity)
                {
                    ...
                    identity = string.Format("'{0}'", this.Entity.WriteEntity(null /* OperationContext  */)[TableConstants.TableName].StringValue);
                }
                else if (this.OperationType == TableOperationType.Retrieve)
                {
                    identity = string.Format("{0}='{1}',{2}='{3}'", TableConstants.PartitionKey, this.RetrievePartitionKey, TableConstants.RowKey, this.RetrieveRowKey);
                }
                else
                {
                    identity = string.Format("{0}='{1}',{2}='{3}'", TableConstants.PartitionKey, this.Entity.PartitionKey, TableConstants.RowKey, this.Entity.RowKey);
                }

                return NavigationHelper.AppendPathToUri(baseUri, string.Format("{0}({1})", tableName, identity));
            }
        }
    }
}
