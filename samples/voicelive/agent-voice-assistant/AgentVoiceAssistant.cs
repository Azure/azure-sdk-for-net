...
/// <summary>
/// Voice assistant that connects to an Azure AI Foundry agent using the VoiceLive Agent V2 API.
/// Requires DefaultAzureCredential — API key authentication is not supported for agent sessions.
/// </summary>
public class AgentVoiceAssistant : IDisposable
{
    private readonly VoiceLiveClient _client;
    private readonly AgentSessionConfig _agentConfig;
    private readonly string _voice;
    private readonly ILogger<AgentVoiceAssistant> _logger;
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        _logger = loggerFactory.CreateLogger<AgentVoiceAssistant>();
    }
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Connecting to agent '{AgentName}' in project '{ProjectName}'",
                _agentConfig.AgentName, _agentConfig.ProjectName);

            _session = await _client.StartSessionAsync(_agentConfig, cancellationToken).ConfigureAwait(false);
            ...
            Console.WriteLine("AGENT VOICE ASSISTANT READY");
            Console.WriteLine($"Agent: {_agentConfig.AgentName} | Project: {_agentConfig.ProjectName}");
    ...
    private async Task HandleUpdateAsync(SessionUpdate update, CancellationToken cancellationToken)
    {
        ...
        switch (update)
        {
            ...
            case SessionUpdateResponseCreated:
                _logger.LogInformation("Agent response started");
                break;
            ...
            case SessionUpdateResponseTextDone textDone:
                if (!string.IsNullOrEmpty(textDone.Text))
                {
                    Console.WriteLine();
                    Console.WriteLine($"[Agent]: {textDone.Text}");
                }
                break;

            case SessionUpdateResponseAudioTranscriptDelta transcriptDelta:
                _logger.LogDebug("Audio transcript delta: {Delta}", transcriptDelta.Delta);
                break;

            case SessionUpdateResponseAudioTranscriptDone transcriptDone:
                if (!string.IsNullOrEmpty(transcriptDone.Transcript))
                {
                    Console.WriteLine($"[Agent audio]: {transcriptDone.Transcript}");
                    _logger.LogInformation("Agent: {Transcript}", transcriptDone.Transcript);
                }
                break;
            ...
            case SessionUpdateResponseAudioDone:
                _logger.LogInformation("Agent finished speaking");
