...
namespace Microsoft.WindowsAzure.Management.Compute
{
    ...
    internal partial class VirtualMachineOperations : IServiceOperations<ComputeManagementClient>, IVirtualMachineOperations
    {
        ...
        /// request ID.
        /// </returns>
        public async Task<AzureOperationResponse> BeginInitiatingMaintenanceAsync(string serviceName, string deploymentName, string virtualMachineName, CancellationToken cancellationToken)
        {
            // Validate
            if (serviceName == null)
            {
            ...
            if (shouldTrace)
            {
                ...
                tracingParameters.Add("deploymentName", deploymentName);
                tracingParameters.Add("virtualMachineName", virtualMachineName);
                TracingAdapter.Enter(invocationId, this, "BeginInitiatingMaintenanceAsync", tracingParameters);
            }
            ...
            try
            {
                ...
                // Serialize Request
                string requestContent = "<PerformMaintenanceOnRoleOperation xmlns=\"http://schemas.microsoft.com/windowsazure\"><OperationType>PerformMaintenanceOnRoleOperation</OperationType></PerformMaintenanceOnRoleOperation>";
                httpRequest.Content = new StringContent(requestContent, Encoding.UTF8);
                httpRequest.Content.Headers.ContentType = MediaTypeHeaderValue.Parse("application/xml");
        ...
        /// the failure.
        /// </returns>
        public async Task<OperationStatusResponse> InitiateMaintenanceAsync(string serviceName, string deploymentName, string virtualMachineName, CancellationToken cancellationToken)
        {
            ComputeManagementClient client = this.Client;
            bool shouldTrace = TracingAdapter.IsEnabled;
            string invocationId = null;
            if (shouldTrace)
            {
                ...
                tracingParameters.Add("deploymentName", deploymentName);
                tracingParameters.Add("virtualMachineName", virtualMachineName);
                TracingAdapter.Enter(invocationId, this, "InitiateMaintenanceAsync", tracingParameters);
            }
            
            cancellationToken.ThrowIfCancellationRequested();
            AzureOperationResponse response = await client.VirtualMachines.BeginInitiatingMaintenanceAsync(serviceName, deploymentName, virtualMachineName, cancellationToken).ConfigureAwait(false);
            cancellationToken.ThrowIfCancellationRequested();
            OperationStatusResponse result = await client.GetOperationStatusAsync(response.RequestId, cancellationToken).ConfigureAwait(false);
            int delayInSeconds = 30;
            if (client.LongRunningOperationInitialTimeout >= 0)
