    ...
    public class WebKeyRsaValidationTest
    {
        ...
        public void PublicKeyMustWork()
        {
            foreach (var ordinary in GetOrdinaryTestKeys().Values)
            {
                ...
                KeysMustBeCompatible(publicKey, privateKey, false);
            }
        }
        ...
        private static void KeyMustWork(JsonWebKey key, bool includePrivateParameters = false)
        {
            ...
            if (includePrivateParameters)
            {
                ...
                if (!plainText2.SequenceEqual(plainText))
                    throw new Exception("Encrypt-decrypt sequence failed.");
            }
        }

        private static void KeysMustBeCompatible(JsonWebKey publicKey, JsonWebKey privateKey, bool doInvertedTest = true)
        {
            try
            {
                var rsa1 = publicKey.ToRSA(); // Only public key needed.
                var rsa2 = privateKey.ToRSA(true); // Private key needed for decrypt.

                KeysMustBeCompatible(rsa1, rsa2);

                if (doInvertedTest)
                    KeysMustBeCompatible(privateKey, publicKey, false);
            }
            catch (CryptographicException e)
            {
                throw new Exception("Key compatibility tests failed", e);
            }
        }
        ...
        [Fact(Skip = "Enable this test if you want to generate new test data.")]
