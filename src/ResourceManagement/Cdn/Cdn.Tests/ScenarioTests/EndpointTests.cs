    ...
    public class EndpointTests
    {
        ...
        public void EndpointDeleteTest()
        {
            ...
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                CdnTestUtilities.WaitIfNotInPlaybackMode(5);
        ...
        public void EndpointPurgeLoadTest()
        {
            ...
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                cdnMgmtClient.Endpoints.PurgeContent(resourceGroupName, profileName, endpointName, purgeContentPaths);

                // Purge content on non-existing endpoint should fail
                Assert.Throws<ErrorResponseException>(() => {
                    cdnMgmtClient.Endpoints.PurgeContent(resourceGroupName, profileName, "fakeEndpoint", purgeContentPaths);
                });

                // Purge content on endpoint with invalid content paths should fail
                var invalidPurgeContentPaths = new List<string> { "invalidpath!" };
                Assert.Throws<ErrorResponseException>(() => {
                    cdnMgmtClient.Endpoints.PurgeContent(resourceGroupName, profileName, endpointName, invalidPurgeContentPaths); });
                ...
                cdnMgmtClient.Endpoints.LoadContent(resourceGroupName, profileName, endpointName, loadContentPaths);

                // Load content on non-existing endpoint should fail
                Assert.Throws<ErrorResponseException>(() => {
                    cdnMgmtClient.Endpoints.LoadContent(resourceGroupName, profileName, "fakeEndpoint", loadContentPaths);
                });

                // Load content on endpoint with invalid content paths should fail
                var invalidLoadContentPaths = new List<string> { "/movies/*" };
                Assert.Throws<ErrorResponseException>(() => {
                    cdnMgmtClient.Endpoints.LoadContent(resourceGroupName, profileName, endpointName, invalidLoadContentPaths);
                });
        ...
        public void ValidateCustomDomainTest()
        {
            ...
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                // Validate existing custom domain should return true
                var output = cdnMgmtClient.Endpoints.ValidateCustomDomain(resourceGroupName, profileName, endpointName, "customdomain31.azureedge-test.net");
                Assert.Equal(true, output.CustomDomainValidated);

                // Validate non-existing custom domain should return false
                output = cdnMgmtClient.Endpoints.ValidateCustomDomain(resourceGroupName, profileName, endpointName, "customdomain4.hello.com");
                Assert.Equal(false, output.CustomDomainValidated);

                // Validate invalid custom domain should fail
                Assert.ThrowsAny<ErrorResponseException>(() => {
                    cdnMgmtClient.Endpoints.ValidateCustomDomain(resourceGroupName, profileName, endpointName, "invalid\\custom/domain"); });
