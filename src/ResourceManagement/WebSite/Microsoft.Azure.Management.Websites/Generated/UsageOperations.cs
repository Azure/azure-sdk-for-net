...
namespace Microsoft.Azure.Management.WebSites
{
    ...
    internal partial class UsageOperations : IServiceOperations<WebSiteManagementClient>, IUsageOperations
    {
        ...
        public async Task<AzureOperationResponse<object>> GetUsageWithHttpMessagesAsync(string resourceGroupName, string environmentName, string lastId, int? batchSize, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web.Admin/environments/{environmentName}/usage").ToString();
            _url = _url.Replace("{resourceGroupName}", Uri.EscapeDataString(resourceGroupName));
            _url = _url.Replace("{environmentName}", Uri.EscapeDataString(environmentName));
            _url = _url.Replace("{subscriptionId}", Uri.EscapeDataString(this.Client.SubscriptionId));
            List<string> _queryParameters = new List<string>();
            if (lastId != null)
            {
                _queryParameters.Add(string.Format("lastId={0}", Uri.EscapeDataString(lastId)));
            }
            if (batchSize != null)
            {
                _queryParameters.Add(string.Format("batchSize={0}", Uri.EscapeDataString(SafeJsonConvert.SerializeObject(batchSize, this.Client.SerializationSettings).Trim('"'))));
            }
            if (this.Client.ApiVersion != null)
            {
