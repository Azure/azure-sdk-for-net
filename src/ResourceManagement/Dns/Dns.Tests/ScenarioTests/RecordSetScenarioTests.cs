...
namespace Microsoft.Azure.Management.Dns.Testing
{
    ...
    public class RecordSetScenarioTests
    {
        public class SingleRecordSetTestContext
        {
            ...
            public RecordSet TestRecordSkeleton => this.GetNewTestRecordSkeleton(this.RecordSetName);

            public RecordSet GetNewTestRecordSkeleton(string recordSetName, uint ttl = 42)
            {
        ...
        private static SingleRecordSetTestContext SetupSingleRecordSetTest(MockContext context)
        {
            var testContext = new SingleRecordSetTestContext();
            testContext.ResourcesHandler = new RecordedDelegatingHandler { StatusCodeToReturn = HttpStatusCode.OK };
            testContext.DnsHandler = new RecordedDelegatingHandler { StatusCodeToReturn = HttpStatusCode.OK };
            testContext.DnsClient = ResourceGroupHelper.GetDnsClient(context, testContext.DnsHandler);
            var resourceManagementClient = ResourceGroupHelper.GetResourcesClient(context, testContext.ResourcesHandler);
            ...
            testContext.Location = ResourceGroupHelper.GetResourceLocation(resourceManagementClient, "microsoft.network/dnszones");
            testContext.ResourceGroup = ResourceGroupHelper.CreateResourceGroup(resourceManagementClient);
        ...
        public void CrudRecordSetFullCycle()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                recordSetToBeCreated.ARecords = new List<ARecord> { new ARecord { Ipv4Address = "123.32.1.0" } };
                recordSetToBeCreated.TTL = 60;

                // Create the records clean, verify response
                var createResponse = testContext.DnsClient.RecordSets.CreateOrUpdate(
               ...
               Assert.True(
                    TestHelpers.AreEqual(recordSetToBeCreated, createResponse, ignoreEtag: true),
                ...
                Assert.True(
                    TestHelpers.AreEqual(createResponse, getresponse, ignoreEtag: false),
                ...
                createdRecordSet.Metadata = new Dictionary<string, string> { { "tag1", "value1" }, { "tag2", "value2" } };
                ...
                var updateResponse = testContext.DnsClient.RecordSets.CreateOrUpdate(
                    testContext.ResourceGroup.Name,
                    testContext.ZoneName,
                    testContext.RecordSetName,
                    RecordType.A,
                ...
                Assert.True(
                    TestHelpers.AreEqual(createdRecordSet, updateResponse, ignoreEtag: true),
                ...
                Assert.True(
                    TestHelpers.AreEqual(retrievedRecordSet, updateResponse, ignoreEtag: true),
                    "Response body of Update does not match expectations");
                Assert.False(string.IsNullOrWhiteSpace(updateResponse.Etag));
        ...
        public void UpdateSoa()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                Assert.True(
                    TestHelpers.AreEqual(soaResource, updateResponse, ignoreEtag: true),
                    "Response body of Update does not match expectations");
            ...
            string methodName = "testframework_failed")
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName, methodName))
            {
                ...
                if (isCrossType)
                {
                    var listresponse = testContext.DnsClient.RecordSets.ListAllInResourceGroup(
                        testContext.ResourceGroup.Name,
                        testContext.ZoneName);
                    ...
                    Assert.True(
                        listresponse.Any(recordSetReturned => string.Equals(recordSetNames[0], recordSetReturned.Name))
                        && listresponse.Any(recordSetReturned => string.Equals(recordSetNames[1], recordSetReturned.Name))
                        && listresponse.Any(recordSetReturned => string.Equals(recordSetNames[2], recordSetReturned.Name)),
                        "The returned records do not meet expectations");
                }
                else
                {
                    var listresponse = testContext.DnsClient.RecordSets.ListByType(
                        testContext.ResourceGroup.Name,
                        testContext.ZoneName,
                        RecordType.TXT);
            ...
            string methodName = "testframework_failed")
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName, methodName))
            {
                ...
                var recordSetNames = new[] { TestUtilities.GenerateName("hydratestrec") + ".com", TestUtilities.GenerateName("hydratestrec") + ".com", TestUtilities.GenerateName("hydratestrec") + ".com" };
                ...
                if (isCrossType)
                {
                    ...
                    listResponse = testContext.DnsClient.RecordSets.ListAllInResourceGroup(
                        testContext.ResourceGroup.Name,
                        testContext.ZoneName,
                        "3");
        ...
        public void UpdateRecordSetPreconditionFailed()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                createParameters.CnameRecord = new CnameRecord { Cname = "www.contoso.example.com" };
            ...
            string methodName= "testframework_failed")
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName, methodName))
            {
                ...
                Assert.True(
                    TestHelpers.AreEqual(createParameters, createResponse, ignoreEtag: true),
                    "Response body of Create does not match expectations");
                Assert.False(string.IsNullOrWhiteSpace(createResponse.Etag));
        ...
        {
            var createParameters1 = testContext.GetNewTestRecordSkeleton(recordSetNames[0]);
            createParameters1.TxtRecords = new List<TxtRecord> { new TxtRecord { Value = new[] { "text1" }.ToList() } };
            var createParameters2 = testContext.GetNewTestRecordSkeleton(recordSetNames[1]);
            createParameters2.TxtRecords = new List<TxtRecord> { new TxtRecord { Value = new[] { "text1" }.ToList() } };
            var createParameters3 = testContext.GetNewTestRecordSkeleton(recordSetNames[2]);
            createParameters3.AaaaRecords = new List<AaaaRecord> { new AaaaRecord { Ipv6Address = "123::45" } };
