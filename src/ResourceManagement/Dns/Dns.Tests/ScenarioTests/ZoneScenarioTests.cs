    ...
    public class ZoneScenarioTests
    {
        ...
        public void CrudZoneFullCycle()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                var retrievedZone = dnsClient.Zones.Get(resourceGroup.Name, zoneName);
                ...
                var updateResponse = dnsClient.Zones.CreateOrUpdate(resourceGroup.Name, zoneName, ifMatch: null, ifNoneMatch: null, parameters: createdZone );

                assertZoneInvariants(updateResponse);
                Assert.Equal(2, updateResponse.Tags.Count);

                // Retrieve the zone after create, verify response
                retrievedZone = dnsClient.Zones.Get(resourceGroup.Name, zoneName);
                ...
                updateResponse = dnsClient.Zones.CreateOrUpdate(resourceGroup.Name, zoneName, ifMatch: null, ifNoneMatch: null, parameters: retrievedZone );
        ...
        public void ListZones()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                var listresponse = dnsClient.Zones.ListInResourceGroup(resourceGroup.Name);
        ...
        public void ListZonesWithTopParameter()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                var listresponse = dnsClient.Zones.ListInResourceGroup(resourceGroup.Name, "1");
        ...
        public void ListZonesWithListNext()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                listresponse = dnsClient.Zones.ListInResourceGroupNext((listresponse.NextPageLink));
        ...
        public void UpdateZonePreconditionFailed()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                ResourceGroup resourceGroup = ResourceGroupHelper.CreateResourceGroup(resourceManagementClient);
                var createdZone = ResourceGroupHelper.CreateZone(dnsClient, zoneName, location, resourceGroup);
            ...
            ResourceManagementClient resourcesClient)
        {
            ...
            string location = ResourceGroupHelper.GetResourceLocation(resourcesClient, "microsoft.network/dnszones");
        ...
        public static void DeleteZones(DnsManagementClient dnsClient, ResourceGroup resourceGroup, string[] zoneNames)
        {
            foreach (string zoneName in zoneNames)
            {
                var response = dnsClient.Zones.Delete(resourceGroup.Name, zoneName);
