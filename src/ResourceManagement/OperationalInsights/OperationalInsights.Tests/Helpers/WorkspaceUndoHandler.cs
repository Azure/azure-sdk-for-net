...
{
    /// <summary>
    /// Undo Handler for Workspace operations
    /// </summary>
    public class WorkspaceUndoHandler : ComplexTypedOperationUndoHandler<IWorkspaceOperations, OperationalInsightsManagementClient>
    {
        /// <summary>
        /// Look up the undo operation for the given Workspace operation
        /// </summary>
        ...
        {
            undoAction = null;
            WorkspaceCreateOrUpdateParameters createParameters;
            string resourceGroupName;
            if (TryAssignParameter<WorkspaceCreateOrUpdateParameters>(TestHelper.ParametersParameter, parameters, out createParameters)
                && createParameters != null
                && createParameters.Workspace != null
                && createParameters.Workspace.Name != null
                && TryAssignParameter<string>(TestHelper.ResourceGroupNameParameter, parameters, out resourceGroupName))
            {
                undoAction = () =>
                {
                    ...
                    {
                        operationalInsights.Workspaces.Delete(resourceGroupName, createParameters.Workspace.Name);
                    }
                };
