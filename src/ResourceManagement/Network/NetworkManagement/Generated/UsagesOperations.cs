namespace Microsoft.Azure.Management.Network
{
    ...
    internal partial class UsagesOperations : IServiceOperations<NetworkResourceProviderClient>, IUsagesOperations
    {
        ...
        /// Cancellation token.
        /// </param>
        public async Task<AzureOperationResponse<IList<Usage>>> ListWithOperationResponseAsync(string location, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (location == null)
            {
            ...
            // Create Result
            var result = new AzureOperationResponse<IList<Usage>>();
            result.Request = httpRequest;
            result.Response = httpResponse;
            // Deserialize Response
            if (statusCode == (HttpStatusCode)Enum.Parse(typeof(HttpStatusCode), "OK"))
            {
                result.Body = JsonConvert.DeserializeObject<IList<Usage>>(responseContent, this.Client.DeserializationSettings);
            }
            if (shouldTrace)
            {
