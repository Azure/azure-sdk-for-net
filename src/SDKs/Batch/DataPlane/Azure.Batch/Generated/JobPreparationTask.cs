...
namespace Microsoft.Azure.Batch
{
    ...
    /// <summary>
    /// A Job Preparation task to run before any tasks of the job on any given compute node. Batch will retry tasks when 
    /// a recovery operation is triggered on a compute node. Examples of recovery operations include (but are not limited 
    /// to) when an unhealthy compute node is rebooted or a compute node disappeared due to host failure. Retries due to 
    /// recovery operations are independent of and are not counted against the <see cref="TaskConstraints.MaxTaskRetryCount" 
