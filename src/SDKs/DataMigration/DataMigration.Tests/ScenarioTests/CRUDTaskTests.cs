...
namespace DataMigration.Tests.ScenarioTests
{
    public class CRUDTaskTests : CRUDDMSTestsBase
    {
        ...
        private ProjectTask CreateDMSTask(MockContext context,
            ...
            string dmsTaskName)
        {
            var taskProps = new ConnectToSourceSqlServerTaskProperties
            {
                Input = new ConnectToSourceSqlServerTaskInput(new SqlConnectionInfo
                {
                    DataSource = "AALAB03-2K12.REDMOND.CORP.MICROSOFT.COM",
                    EncryptConnection = true,
                    TrustServerCertificate = false,
                    UserName = "testlogin1",
                    Password = Cryptography.Encrypt(service.PublicKey, "passwordToUpdate"),
