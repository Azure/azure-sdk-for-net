...
namespace Azure.Base.Http.Pipeline
{
    class FixedPolicy : RetryPolicy {
        ...
        protected override bool ShouldRetry(HttpPipelineContext pipelineContext, int attempted, out TimeSpan delay)
        {
            delay = _delay;
            if (attempted > _maxRetries) return false;
            if(Array.BinarySearch(_retriableCodes, pipelineContext.Response.Status) < 0) return false;
            return true;
        }
    }
}
