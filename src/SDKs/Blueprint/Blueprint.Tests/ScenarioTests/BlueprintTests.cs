        ...
        public async Task BlueprintCRUD()
        {
            ...
            using (var context = MockContext.Start(GetType().FullName))
            {
                using (var testFixture = new BlueprintTestBase(context))
                {
                    ...
                    await testFixture.BlueprintClient.Blueprints.CreateOrUpdateAsync(managementGroupName, blueprintName, blueprint);

                    // get
                    var blueprintGet = await testFixture.BlueprintClient.Blueprints.GetAsync(managementGroupName, blueprintName);
                    ...
                    blueprintGet = await testFixture.BlueprintClient.Blueprints.CreateOrUpdateAsync(managementGroupName, blueprintName, blueprint);
                    Assert.Contains("defaultKeyVaultStore", blueprintGet.Parameters.Keys);

                    await testFixture.BlueprintClient.Blueprints.CreateOrUpdateAsync(managementGroupName, blueprintName2, blueprint);
                    var blueprints = await testFixture.BlueprintClient.Blueprints.ListAsync(managementGroupName);
                    ...
                    // delete
                    await testFixture.BlueprintClient.Blueprints.DeleteAsync(managementGroupName, blueprintName);
                    await testFixture.BlueprintClient.Blueprints.DeleteAsync(managementGroupName, blueprintName2);
                }
            }
        }
        ...
        public async Task ArtifactCRUD()
        {
            ...
            using (var context = MockContext.Start(GetType().FullName))
            {
                using (var testFixture = new BlueprintTestBase(context))
                {
                    ...
                    await testFixture.BlueprintClient.Artifacts.CreateOrUpdateAsync(managementGroupName, blueprintName, templateArtifactName, templateArtifact);
                    // get templateArtifact
                    var templateArtifactGet = await testFixture.BlueprintClient.Artifacts.GetAsync(managementGroupName, blueprintName, templateArtifactName) as TemplateArtifact;
                    ...
                    await testFixture.BlueprintClient.Artifacts.CreateOrUpdateAsync(managementGroupName, blueprintName, policyArtifactName, policyArtifact);
                    // get policy artifact
                    var policyArtifactGet = await testFixture.BlueprintClient.Artifacts.GetAsync(managementGroupName, blueprintName, policyArtifactName) as PolicyAssignmentArtifact;
                    ...
                    await testFixture.BlueprintClient.Artifacts.CreateOrUpdateAsync(managementGroupName, blueprintName, rbacArtifactName, rbacArtifact);
                    // get roleAssignment artifact
                    var rbacArtifactGet = await testFixture.BlueprintClient.Artifacts.GetAsync(managementGroupName, blueprintName, rbacArtifactName) as RoleAssignmentArtifact;
                    ...
                    // list artifacts
                    var artifactList = await testFixture.BlueprintClient.Artifacts.ListAsync(managementGroupName, blueprintName);
                    ...
                    await testFixture.BlueprintClient.Artifacts.DeleteAsync(managementGroupName, blueprintName, templateArtifactName);
                    await testFixture.BlueprintClient.Artifacts.DeleteAsync(managementGroupName, blueprintName, policyArtifactName);
                    await testFixture.BlueprintClient.Artifacts.DeleteAsync(managementGroupName, blueprintName, rbacArtifactName);

                    await testFixture.BlueprintClient.Blueprints.DeleteAsync("AzBlueprint", "fromDotnetSDK");
                }
            }
        }
        ...
        public async Task AssignBlueprint()
        {
            ...
            using (var context = MockContext.Start(GetType().FullName))
            {
                using (var testFixture = new BlueprintTestBase(context))
                {
                    ...
                    // publish
                    await testFixture.BlueprintClient.PublishedBlueprints.CreateAsync(managementGroupName, blueprintName, "v1.0");
                    ...
                    await testFixture.BlueprintClient.PublishedBlueprints.CreateAsync(managementGroupName, blueprintName, "v1.1");
                    var latestSealed = await testFixture.BlueprintClient.PublishedBlueprints.GetAsync(managementGroupName, blueprintName, "v1.1");

                    // list versions
                    var sealedBlueprints = await testFixture.BlueprintClient.PublishedBlueprints.ListAsync(managementGroupName, blueprintName);
                    Assert.Equal(2, sealedBlueprints.Count());

                    // create assignment
                    var assignment = CreateBlueprintAssignment(latestSealed);
                    await testFixture.BlueprintClient.Assignments.CreateOrUpdateAsync(subscriptionId, assignmentName, assignment);
                    var assignmentGet = await testFixture.BlueprintClient.Assignments.GetAsync(subscriptionId, assignmentName);
                    ...
                    // list assignment
                    var assignmentsList = await testFixture.BlueprintClient.Assignments.ListAsync(subscriptionId);
                    ...
                    assignmentGet = await WaitForCondition(
                        () => testFixture.BlueprintClient.Assignments.GetAsync(subscriptionId, assignmentName),
                        (Assignment assign) => assign.IsTerminalState(),
                        waitTillFinish.Token);

                    // cleanup
                    await testFixture.BlueprintClient.Assignments.DeleteAsync(subscriptionId, assignmentName);
                    ...
                    await testFixture.BlueprintClient.PublishedBlueprints.DeleteAsync(managementGroupName, blueprintName, "v1.0");
                    await testFixture.BlueprintClient.PublishedBlueprints.DeleteAsync(managementGroupName, blueprintName, "v1.1");
                    await testFixture.BlueprintClient.Blueprints.DeleteAsync(managementGroupName, blueprintName);
                }
            }
        }
