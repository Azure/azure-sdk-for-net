...
namespace Microsoft.Azure.Batch.Protocol
{
    ...
    public partial interface IPoolOperations
    {
        ...
        /// target a different pool. The tasks can then run on the new pool.
        /// If you want to override the requeue behavior, then you should
        /// call resize pool explicitly to shrink the pool to zero size
        /// before deleting the pool. If you call an Update, Patch or Delete
        /// API on a pool in the deleting state, it will fail with HTTP
        ...
        /// For example, if the pool has a start task associated with it, and
        /// a request does not specify a start task element, then the pool
        /// keeps the existing start task.
        /// </remarks>
        /// <param name='poolId'>
        ...
        /// </summary>
        /// <remarks>
        /// You cannot enable automatic scaling on a pool if a resize
        /// operation is in progress on the pool. If automatic scaling of the
        /// pool is currently disabled, you must specify a valid autoscale
        /// formula as part of the request. If automatic scaling of the pool
        /// is already enabled, you may specify a new autoscale formula
        /// and/or a new evaluation interval. You cannot call this API for
        ...
        /// simply returns the result without applying the formula to the
        /// pool.
        /// </remarks>
        /// <param name='poolId'>
        ...
        /// The formula is validated and its results calculated, but it is
        /// not applied to the pool. To apply the formula to the pool,
        ...
        /// configured for automatic scaling. If you try to do this, the
        /// Batch service returns an error 409. If you resize a pool
        /// downwards, the Batch service chooses which nodes to remove. To
        /// remove specific nodes, use the pool remove nodes API instead.
        /// </remarks>
        /// <param name='poolId'>
        ...
        /// and the pool maintains its current state. A resize operation need
        /// not be an explicit resize pool request; this API can also be used
        /// to halt the initial sizing of the pool when it is created.
        /// </remarks>
        /// <param name='poolId'>
        ...
        /// running on that node are removed from the node and returned to
        /// the queue to be rerun later (or on a different compute node). The
        /// node will be unavailable until the upgrade is complete. This
        /// operation results in temporarily reduced pool capacity as nodes
        /// are taken out of service to be upgraded. Although the Batch
        /// service tries to avoid upgrading all compute nodes at the same
        /// time, it does not guarantee to do this (particularly on small
        /// pools); therefore, the pool may be temporarily unavailable to run
        /// tasks. When this operation runs, the pool state changes to
        /// upgrading. When all compute nodes have finished upgrading, the
        /// pool state returns to active.
        /// </remarks>
        /// <param name='poolId'>
