...
namespace Microsoft.Azure.Batch.Protocol
{
    ...
    public partial interface IJobOperations
    {
        ...
        /// retained on compute nodes, the Batch services deletes those
        /// tasks' working directories and all their contents.
        /// </remarks>
        /// <param name='jobId'>
        ...
        /// For example, if the job has constraints, and a request does not
        /// specify the constraints element, then the job keeps the existing
        /// constraints.
        /// </remarks>
        /// <param name='jobId'>
        ...
        /// </summary>
        /// <remarks>
        /// The Batch Service immediately moves the job to the disabling
        /// state. Batch then uses the disableTasks parameter to determine
        /// what to do with the currently running tasks of the job. The job
        /// remains in the disabling state until the disable operation is
        /// completed and all tasks have been dealt with according to the
        /// disableTasks option; the job then moves to the disabled state. No
        /// new tasks are started under the job until it moves back to active
        /// state. If you try to disable a job that is in any state other
        /// than active, disabling, or disabled, the request fails with
        /// status code 409.
        /// </remarks>
        /// <param name='jobId'>
        ...
        /// values are: requeue – Terminate running tasks and requeue them.
        /// The tasks will run again when the job is enabled. terminate –
        ...
        /// the enabling state. After the this operation is completed, the
        /// job moves to the active state, and scheduling of new tasks under
        /// the job resumes. The Batch service does not allow a task to
        /// remain in the active state for more than 7 days. Therefore, if
        /// you enable a job containing active tasks which were added more
        ...
        /// the job to the terminating state. The Batch service then
        /// terminates any active or running tasks associated with the job,
        /// and runs any required Job Release tasks. The job then moves into
        /// the completed state.
        /// </remarks>
        /// <param name='jobId'>
        ...
        /// Manager task. The Batch service launches this task when it is
        /// ready to start the job. The Job Manager task controls all other
        /// tasks that run under this job, by using the Task APIs. In the
        /// second approach, the user directly controls the execution of
        /// tasks under an active job, by using the Task APIs. Also note:
        /// when naming jobs, avoid including sensitive information such as
        /// user names or secret project names. This information may appear
        ...
        /// on all compute nodes that have run the Job Preparation or Job
        /// Release task. This includes nodes which have since been removed
        /// from the pool.
        /// </remarks>
        /// <param name='jobId'>
        /// The ID of the job.
