...
namespace Microsoft.Azure.Batch.Protocol.Core
{
    ...
    class UriQueryBuilder
    {
        ...
        /// <param name="uri">Original Uri which may contain query parameters already.</param>
        /// <returns>The appended Uri.</returns>
        public Uri InsertToUri(Uri uri)
        {
            // The correct way to add query parameters to a Uri http://msdn.microsoft.com/en-us/library/system.uribuilder.query.aspx
            string queryToInsert = this.ToString();

            if (queryToInsert.Length > 1)
            {
                queryToInsert = queryToInsert.Substring(1);
            }
            ...
            if (baseUri.Query != null && baseUri.Query.Length > 1)
            {
                baseUri.Query = queryToInsert + "&" + baseUri.Query.Substring(1);
            }
            else
            {
                baseUri.Query = queryToInsert;
            }
