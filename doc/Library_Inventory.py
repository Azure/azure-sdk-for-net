...
def identify_generator(path):
    ...
    if os.path.exists(autorest_md_path):
        with open(autorest_md_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if "typespec" in content or "tsp" in content:
                return "TSP"
            return "Swagger"
    
    # Check for tspconfig.yaml
    tsp_config_path = os.path.join(path, "src", "tspconfig.yaml")
    if os.path.exists(tsp_config_path):
        return "TSP"
    
    # Check for tsp directory
    tsp_dir = os.path.join(path, "src", "tsp")
    if os.path.exists(tsp_dir):
        return "TSP"
    
    # If no autorest.md found but Generated folder exists, assume Swagger
...
def generate_markdown_report(libraries):
    ...
    mgmt_tsp = [lib for lib in libraries if lib["type"] == "Management" and lib["generator"] == "TSP"]
    data_swagger = [lib for lib in libraries if lib["type"] == "Data Plane" and lib["generator"] == "Swagger"]
    data_tsp = [lib for lib in libraries if lib["type"] == "Data Plane" and lib["generator"] == "TSP"]
    ...
    report.append(f"- Management Plane (TSP): {len(mgmt_tsp)}")
    report.append(f"- Data Plane (Swagger): {len(data_swagger)}")
    report.append(f"- Data Plane (TSP): {len(data_tsp)}")
...
if __name__ == "__main__":
    ...
    mgmt_tsp = sum(1 for lib in libraries if lib["type"] == "Management" and lib["generator"] == "TSP")
    data_swagger = sum(1 for lib in libraries if lib["type"] == "Data Plane" and lib["generator"] == "Swagger")
    data_tsp = sum(1 for lib in libraries if lib["type"] == "Data Plane" and lib["generator"] == "TSP")
    ...
    print(f"Management Plane (TSP): {mgmt_tsp}")
    print(f"Data Plane (Swagger): {data_swagger}")
    print(f"Data Plane (TSP): {data_tsp}")
